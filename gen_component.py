...
VERSION = 'v1.6.1'   # the only version home in this file
...
def load_standard():
    ...
    # --- §9 numbered marks: PARSED ONLY, NO EMITTER (S97) -------------------
    # The spec is read and asserted so the DOCUMENT stays conformant, but this
    # file deliberately ships no numbered-mark generator. v1.6 had one; it was
    # WITHDRAWN because §9 names two radii and NO SHAPE, and the star inferred
    # from them came out 53% blunter than the book's own mark, with the number
    # riding out of it because dominant-baseline is unreliable through <img>.
    # Bible §18.2 had already ruled the digits must be VECTOR PATHS for exactly
    # that reason. The 16 live stars are recoloured S40 assets: §9's flat fill
    # over §18.2's proven geometry. Do not re-add an emitter until §9 names a
    # shape -- a generator that silently overwrites 16 good files is a landmine.
