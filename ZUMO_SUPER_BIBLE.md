...
1. **VISIBLE banner** (header, and footer where the lesson has one — L02 has header only): **major.minor only**, e.g. `Version 03.2` / `v03.2`. It updates on a MODERATE-or-larger bump and is deliberately LEFT ALONE on a minor/cosmetic bump — so a pill recolor (`v03.2.4 → v03.2.5`) does NOT touch the visible banner.
...
- **Difficulty pill:** rounded (`border-radius: 12px`), white text. **Up to five tiers** (a lesson uses as many as it needs, in order — order carries difficulty, labels assist; no minimum per tier): EASY `#4caf50` · MEDIUM `#2196f3` · TOUGH `#9c27b0` · HARD `#ff9800` · ADVANCED `#f44336`. *(v8.27 — scale recolored/relabeled from the old EASY/MEDIUM/HARD/EXPERT/COMPETITION set; the book-wide pill sweep is COMPLETE as of S59 — verified from files: 73 pills, all conforming to this scale, zero retired EXPERT/COMPETITION labels remaining.)*
...
- `data-difficulty="easy|medium|hard|…"` where a difficulty pill exists.
...
### 21.2 ACCENT GLOW COLORS — TWO VALUE SETS, RECONCILE LATER
Which set to publish is **undecided (DJ, S61: "keep both, figure it out later")**. CANONICAL = the style-guide neon target; SAMPLED = measured from the first uploaded renders (darker/muted).

| Robot | Canonical (neon target) | Sampled (from uploads) |
|---|---|---|
| Zumo 32U4 OLED | `#42F5D7` / `#3FE8D0` (teal/aqua — intentionally cyan, not green) | `#48D4D4` |
| 3Pi+ 32U4 OLED | `#46F56C` / `#49FF70` (bright green) | `#3DAA54` |
| ROMI 32U4 | `#FF4FBF` / `#FF59D4` (magenta/pink) | `#F53997` |
| Balboa 32U4 | `#9A5BFF` / `#A66BFF` (purple) | `#7D2EC9` |
| Zircon (soccer) | `#FF8A00` / `#FF9400` (orange) | `#F16D03` |

### 21.3 BUILD METHOD — FRAME-SWAP, NOT CUT-AND-REBUILD (LOCKED)
**Keep the robot and its glow together; only replace the outer frame.** From a source image that already has the robot at the right size with a glow that does **not** run into its own border: crop just inside the source's border ring (removing it), map that content into the panel, then draw the shared border on top. **NEVER** separate the robot from its glow and regenerate the glow.

### 21.4 WHY SOLID vs OPEN/DARK ROBOTS BEHAVE DIFFERENTLY
The cut-and-rebuild approach only works on **solid, bright-bodied** robots (Zumo tank body, 3Pi+ white disc): the bright body seals its own interior, so it lifts off a black background cleanly. It **fails** on **dark-bodied** (Zircon PCB disc) and **open-frame** (Balboa roll cage) robots — black-on-black is invisible to a brightness threshold so interiors get eaten, open structure leaks, and a regenerated glow loses the outer rim. The frame-swap method (21.3) sidesteps all of this by never cutting the robot out. This is the documented reason the first two icons went smoothly and the next three fought back.
...
A no-frame transparent cutout also exists per robot (the landing-page Textbook tile uses the Zumo mark). Marks cut cleanly only on solid-bodied robots; open/dark robots resist a clean transparent cutout for the same reason as 21.4. **Prefer the bordered icon for buttons.**
