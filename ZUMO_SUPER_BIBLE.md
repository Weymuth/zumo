...
## 18. CHALLENGE-DESIGN CANON (v8.26 — NEW SECTION, S40)
...
### 18.3 CHALLENGE-STARTER PRINCIPLES (LOCKED, DJ rulings S40)
A challenge **starter** is a **minimal skeleton**, not the finished lesson program (a full program overwhelms even a strong builder). Include everything EXCEPT the concept being taught.

- **Includes + the ONE needed hardware object pre-placed** (e.g. the `OLED` object for a display challenge) — nothing more.
- **Empty section headers**; any header not needed for this challenge reads `// (none needed for this challenge)`.
- **MY PLAN block ships BLANK** — writing the plan is the exercise.
- **A marked landing zone:** a clear `// write your code here`.
- **Do NOT re-explain `setup()` / `loop()`** — taught in L01 §5.3 and L02; re-teaching them in every starter is noise.
