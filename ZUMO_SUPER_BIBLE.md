...
**Tagging is NOT yet aligned.** `data-kind` still reads `bonus`/`mystery`; ~28 cards in L04, L10, L12–L16 carry
no `data-kind` or `data-challenge` at all. Aligning the values to the three families touches §4.2's coverage
gate, §4.3's picker and the §20.1/§20.2 tutor pair — **its own batch.**
