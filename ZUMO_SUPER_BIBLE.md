...
**THE HANDOFF'S NUMBER IS THE SESSION THAT READS IT, AND AS OF S113 THAT IS GATED (v8.102).**
...
**It was violated twice anyway, in consecutive sessions, because nothing asserted it.** S112
wrote its outgoing handoff into the INCOMING handoff's filename — `ZUMO_S112_HANDOFF.md` edited
in place across three commits with its STATE block updated each time (`893b8b6` → `4558257` →
`8ae3857`) while its title stayed *"paste at top of Session 112"* — so `ZUMO_S113_HANDOFF.md`
never existed and S113 opened on a file whose two visible homes both named the wrong session.
S113 then read the convention off that one live example instead of off this Bible, and pushed
S114's content as `ZUMO_S113_HANDOFF.md`. **DJ caught it by reading. No instrument could.**
...
#### 24.14 EVERY CALLOUT BLOCK RESOLVES TO A FAMILY (v8.103 — NEW, S113; the rule gate 47 has been enforcing since S112)
...
line says**: adding one callout at S113 produced `assigned 1049 / 1048`, which reads as a count
...
#### 24.15 A DRAFTING MODEL CANNOT COUNT THE BOOK (v8.101 — NEW, S113; written S112 as `GPT_BRIEF_FORMAT.md`)
