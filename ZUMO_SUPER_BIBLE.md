...
- **Downstream use:** these walkthroughs are the intended raw material for the **AI Tutor rebuild** (queued LAST) — the model that worked (isolate one new idea, let the wrong answer happen and correct in place, trace values by hand) is captured here for reuse.
