...
**Bible version: v8.137** — increment on EVERY substantive edit (moderate change → `v8.x`; minor fix → `v8.x.y`; a new major re-baseline → `v9`). **Filename is now unversioned: `ZUMO_SUPER_BIBLE.md`** — the version lives ONLY in this line, never in the filename (this avoids a fresh chat misreading a filename number as the version). Current: **v8.137** (v8.137, S147, moderate — **§16.17 NEW: A CITATION IS A CLAIM ABOUT AN EDITION, NOT ABOUT A NUMBER, AND THE SESSION THAT FOUND IT SHIPPED NOTHING TO THE TREE.** DJ ruled at open that the L14–L16 arc proceeds despite sitting outside the September scope. L14 was read end to end and measured with **twelve compiles**, control first — L11 `after_step_1` reproduced **20,516** from a fourth clone — and **not one character of the fix was applied**; the tree stands unchanged at `0e62713` and the whole fix is specified in `ZUMO_S148_HANDOFF.md`. **The chain moves +170 at the top and +176 at the bottom, so two of three deltas change** (738 to 744, 734 to 740; the 36 at 7C survives), which confirms rule 49's revocation for a third session. **Seven defects independent of the baseline**, two of them the same shape: Step 3 and Mystery B1 each price one quantity twice with different numbers, and `830` appears in exactly one sentence in the tree while `selfTest()` has no standalone symbol to measure. **The citation finding is §16.17:** §4.2.8 and §4.3.7 name the same rule because RCJ 2025 added §4.1 *Terms and Definitions* and pushed Section 4 down by one, so **every §4.x number in the book is a 2024 number** and §4.3.7 is correct — the answer living in the rulebook's changes list rather than in either lesson. Three inspection specs that exist in no edition were identified for deletion, and `ROBOCUP_RESCUE_LINE_2026.md` is both the source of the error and misnamed, its content being the 2025 edition. **One item blocked on DJ and not guessed:** RCAP U19 deletes the black victim, which is load-bearing in the in-scope, already-banked L13. **No gate holds a rule citation** — a fourth invariant with no gate. Doc only: no lesson file changed, no instrument changed, census unchanged at **40,605**.) Prior: **v8.136.2** (v8.136.2, S146, minor — **A TRIPLE CHECK BLINDING-CONTROLLED THIS SESSION'S OWN CONTROL, AND THE ENDPOINT FORM WAS BLIND (rule 59, third firing).** S146 recorded that no blinding control was run on a byte figure because nothing in the tree can see one, and that the substitute control *re-derives the chain out of the edited HTML and asserts it closes on its own stated total*. **Seeded, that assertion does not fire.** Reverting Step 5's figure alone — 25,114 back to the stale 24,946 — leaves 25,072 − 24,694 = 378 exactly true, **all 70 gates green and `gate_payload_match` PASS**, because the endpoints and the total are untouched by any change in the MIDDLE of the chain. The form that DOES fire compares every parsed figure against its COMPILE and every stated delta against its neighbours; run on the same seeded tree it names *chain != compiles*. **Both descriptions were corrected in place rather than left to be re-implemented from a changelog** — a reader of S147 building the endpoint form would have believed the chain was held. **Rule 59 asks what your control would report if the property did not exist; the endpoint form reports PASS either way.** **THE BANK SWEEP WAS CONTROLLED THE SAME WAY AND DOES FIRE:** reintroducing one superseded distractor is named by the forbidden-string sweep and is invisible to both `quiz_bank --check` and all 70 gates — which is *no gate holds a quiz bank*, measured rather than restated. **AND THE TRIPLE CHECK CAUGHT SOMETHING BIGGER THAN A CONTROL:** three files appeared in the delivery directory that this session did not author — a modified `Lesson_14.html` carrying an L14 byte fix and a version bump, a `quizzes/ZUMO_QUIZ_L14.yaml` that exists in no commit, and an edited `ZUMO_TDP_Template_v3.md` — and prose describing that work had reached the Bible, LIVE.md and the outgoing handoff. **`session_versions --check` is what named it**, reporting *LIVE.md L14 written=v02.34.0 files=v02.33.1* on a clean clone: rule 60's shape, a document naming a version the tree does not hold. **All three were quarantined out of the delivery and every deliverable was rebuilt from the one tree measured clean.** Unauthored work is not shippable work however plausible it reads — the L14 byte fix in particular is the correction S146 DECLINED on the reasoning that L14 must be fixed whole or not at all. Doc only: no lesson file changed, no instrument changed, no gate changed, census unchanged at **40,605**.) Prior: **v8.136.1** (v8.136.1, S146, minor — **A DOUBLE-CHECK FOUND THREE DISTRACTORS THAT EXPLAINED THEMSELVES BY THE BOOK'S REVISION HISTORY, AND THE ONLY THING THAT COULD SEE THEM WAS AN ASSERTION WRITTEN AGAINST THE ARTEFACT.** DJ asked for a double check. Per §24.13 that is a DIFFERENT METHOD, not a second read, so the bank was re-verified by a script that parses its own header's claims and asserts a forbidden-string set against the questions. **It fired.** L13 bank A13, A15 and A16 each offered the PRE-CORRECTION figure as a distractor — *24,946 — up 232*, *24,902 — down 44*, *368 bytes* — with feedback reading *“those were the figures before the byte re-baselining.”* **A student has never seen those numbers and has no access to the book's edit history**, so the feedback explains a wrong answer by an event outside the book — against QUIZ_SPEC §4, which makes the whole contract of `cite:` *tell the student where to re-read*. **AND ONE OF THEM WAS ACTIVELY DANGEROUS: L14 still carries three `24,902` sites**, so a student reading ahead would have found the distractor corroborated. All three replaced with lesson-internal wrong answers — a flat-byte trap keyed to Step 4 being *the last free step*, a zero-cost trap keyed to the 378 total, and a SIGN error (462 = 180 + 240 + 42) that teaches the thing Step 6 exists to teach. A24's *“measured on the toolchain”* went the same way. **THE FIRST SWEEP MISSED A THIRD OF THEM, AND THAT IS THE ENTRY'S POINT.** The forbidden list held comma-formatted figures, so `368 bytes` slipped it; only adding the PHRASE *re-baselining* caught A16. **A defect class is not a list of its instances — scan for the VOCABULARY that produces it, not the values it happened to produce.** **AND THE SWEEP THAT FOUND THEM ALSO PRODUCED A FALSE POSITIVE THAT WAS READ RATHER THAN ACTED ON** (rule 38): `compiled in` matched *“compiled into both builds”*, which is correct lesson vocabulary. `avr-nm` and *disassembl* also flagged and both STAY — they are the lesson's own words in Step 2 and Mystery B2. **A text match locates; it never answers.** **THE FIX ALSO BROKE ITS OWN FILE AND THE VALIDATOR CAUGHT IT**: the replacement `why` carried an unquoted colon and `quiz_bank.py --check` failed to parse the YAML — a repair is an edit and owes the same gate as the thing it repairs. Applied S146: `quizzes/ZUMO_QUIZ_L13.yaml` **v1.0.0 → v1.0.1**; no lesson file changed, no instrument changed, census unchanged at **40,605**. 70/70 gates, `quiz_bank --check` 13 banks valid, `quiz_bank --selftest` all controls pass.) Prior: **v8.136** (v8.136, S146, moderate — **§16.16 NEW: A SUPERLATIVE IS A CROSS-LESSON CLAIM, AND AN EXPLANATION CAN BE WRONG WITHOUT ANY NUMBER BEING WRONG — BOTH FOUND IN ONE LESSON, NEITHER VISIBLE TO SEVENTY GATES.** **DJ RULED THE FALL SCOPE AT SESSION OPEN: L01–L13, NOT L01–L12** (`QUIZ_SPEC` §9 had carried that as open since S136; spec **v1.1.0 → v1.2.0**). The ruling moved L13 out of a parked arc and onto the September critical path, and L13 was then read end to end, byte-fixed and banked in the same session. **AND THE BYTE FIX BROKE S145's COROLLARY.** S145 closed with *when the deltas are already right, the swap is safe* — true of L12, whose eighteen figures all moved by exactly 160 so not one sentence needed rewriting. **L13's chain moves +160 at Step 1 and +170 at Step 6**, measured on the toolchain against the standing control (L11 `after_step_1` reproduces 20,516 byte-for-byte): 24,694 · 24,694 · 24,874 · 24,874 · 25,114 · 25,072 against a book reading 24,534 · 24,534 · 24,714 · 24,714 · 24,946 · 24,902. **Step 3's +180 survived; Step 5's +232 is +240, Step 6's −44 is −42, and the total cost is 378 rather than 368.** Carrying the corollary forward as a rule would have shipped *up 232* over a measured 240, **and it would have gated cleanly, because nothing in this tree can see a byte figure** (S145). The corollary is revoked as a general rule: **compile every step and derive every delta from the compiles.** **§7E's 64-BYTE CLAIM WAS FALSE AND ITS EXPLANATION NAMED NO DIFFERENCE AT ALL.** The lesson said the gyro-corner and encoder-corner builds *“differ by just 64 bytes — 24,902 against 24,966, the price of keeping both turn functions alive in the binary.”* **Both turn functions are alive in BOTH builds** — the two projects differ by one call site in `main.cpp`, `turnDegreesGyro` → `turnDegrees`, twice — so the stated reason describes something the two builds share, and measured, **they are byte-identical at 25,072.** Had the 64 been right the sentence would still have been false. Rewritten to say the builds weigh the same, which makes the lesson's own point — *the size of the file will never tell you which line* — **stronger**, and puts §7E in agreement with Mystery B2 four sections later, which teaches it with a sabotage. **STEP 6's DECOMPOSITION WAS ARITHMETICALLY BROKEN INDEPENDENT OF BASELINE, AND WAS DELETED RATHER THAN GUESSED AT.** *“The rewritten hand-off and the recompiled state switch came out 120 bytes tighter … while the victim bookkeeping cost only 26”* nets **−94**, against a stated −44 and a measured −42 — wrong at the old baseline too. Symbol diff on both ELFs (`avr-nm --size-sort -S`): `main` −68, `showStatus` −50, against growth of +81 (`rescueCount` 2, `lastRowCm` 4, four strings 33, one OLED layout helper 42). **118 − 81 = 37, and the remaining 5 is section padding** — so no honest pair of numbers closes on 42 without teaching linker padding inside a lesson about rescue zones. **Rule 50 has three options and only two are legal: derive it or delete it. The third — replace it with a number that nearly works — is what put the sentence there.** **§16.16's RULE, AND IT IS THE REUSABLE HALF: A SUPERLATIVE IS A CLAIM ABOUT THE OTHER FIFTEEN LESSONS.** §4.2 and BRAIN CHECK 01's answer 3 both stated **Lesson 13 is the first lesson in this book that adds zero hardware.** A jumper/rewire sweep across all sixteen says otherwise: **L09 §4 already reads “no new hardware required”** and **L12 §4.1 reads “You do not have to plug anything in. You do not have to move a jumper.”** Both L13 sentences were narrowed to the true claim, which costs nothing — *not one wire moves, not one jumper changes* was always the interesting half. **Rule 33 again: seventy gates certify that a false sentence is well-formed.** **AND THE FIGURES TABLE UNDER-DESCRIBED GRAPHIC 13.2** (silver/white/black, where the caption and the SVG both carry green, `GREEN marker – 1200`). Corrected, and deliberately NOT asked in the bank — the plotted values live only in the SVG, and QUIZ_SPEC §5 forbids figure-only facts; B36 and B37 ask the same numbers only because §8A.1 restates the worked example in prose. **BANK L13 v1.0.1 — 96 QUESTIONS** (56 before + 40 after, 71/18/11 and 72/20/8), authored after the read and the fixes. **Seven items would have keyed wrong against v02.28.3 and one, A24, would have keyed a number that never existed.** Eight answer classes are named in the bank header as deliberately not asked, with reasons — including every 7A measurement, since no raw reading or prox count has ever been taken on this fleet and the 7A table ships blank on purpose. **WHAT WAS LEFT ALONE, WITH THE REASON: L14 CARRIES THREE ORPHANED `24,902` SITES** keyed to L13's old finished figure (lines 699, 732, 861). L14 holds eight byte figures in total, and **correcting three would leave a lesson where some figures are compiled and some are stale with nothing marking which** — worse than a lesson that is uniformly out of date and known to be. **Fix L14 whole or not at all.** **NO BLINDING CONTROL WAS RUN ON A BYTE FIGURE, ON PURPOSE (rule 59).** S145 established that nothing in the tree can see one, so a control there would fire for the wrong reason or not at all. The control that WAS run re-derives the corrected chain OUT of the edited HTML and asserts each STEP DELTA against the compiles — not merely that the endpoints close on the stated total, **which a blinding control proved blind: reverting Step 5's figure alone leaves 25,072 − 24,694 = 378 intact and all 70 gates green** — which tests the artefact rather than the intent. **THE HARNESS SETUP INSTRUCTIONS WERE ALSO WRONG AND ARE CORRECTED IN THE HANDOFF:** `LIBDIRS` names `pololu-buzzer-arduino` and `pololu-oled-arduino`, which S146 did not clone, and does NOT name `l3g-arduino` or `lsm303-arduino`, which it did; the libraries live FLAT under `/home/claude/harness`, not under a `pololu/` subdirectory, and `sudo` does not exist on this box. **Read `LIBDIRS` out of the script; do not carry a repo list forward in a handoff.** Applied S146: L13 **v02.29.0** (moderate — twelve sites including two prose rewrites, so both §5b homes move), `quizzes/QUIZ_SPEC.md` **v1.2.0**, `quizzes/ZUMO_QUIZ_L13.yaml` NEW. 70/70 gates, `gate_payload_match` PASS, `callout_id --audit` 1120/0, `keyterm_prefix` 0 to convert, `quiz_bank --check` 13 banks valid, `site_parity` PARITY, census unchanged at **40,605**.) Prior: **v8.135.4** (v8.135.4, S145, moderate — **§16.15 NEW: TRIM'S HOME IS LESSON 3, BOOK-WIDE, AND THE SPLIT S144 RECORDED AS UNRULED WAS TWO SHAPES REPORTED AS ONE NUMBER (DJ ruling: *“Go for 2”* — strip the citation from the relocation comment, flip the rest).** **THE SPLIT WAS NEVER LESSON AGAINST LESSON. It was PROSE against CODE COMMENTS**: derived from the tree, every *Lesson 3* claim sat in lesson prose and every *Lesson 6* claim sat inside a syntax-highlighted comment span, with exactly ONE prose leak, L13's *Straight rows (TRIM, Lesson 6)* in a list whose other three entries each name a single teaching home. **TWO CLAIMS CARRIED FORWARD FROM S144 WERE FALSE, AND BOTH WERE TEXT MATCHES READ AS ANSWERS** (rule 38, §24.10). *L03 says both* is wrong — L03's two Lesson-6 hits are FORWARD pointers to closed-loop control (*“TRIM is the open-loop stepping stone — Lesson 6's encoders are your first taste of the real thing”*, plus an `AUTO-TRIM ALGORITHM (Preview of Lesson 6)` block that is an encoder technique). *L09 says Lesson 6* is a false positive — *“the TRIM-independent, battery-independent turn you built in Lesson 6”* describes `turnDegrees()`. **Neither lesson ever claimed a home, so the prose side was unanimous the whole time.** **THE 342 PAYLOAD COMMENTS ARE TWO SHAPES AND ONLY 194 WERE WRONG.** 148 carry a FOUR-LINE block whose fourth line already reads *“(It was born in Lesson 3, spent in Lesson 6, and filed here.)”* — a RELOCATION narrative in L07's reorg lesson, naming the file the student is actually holding when L07 opens, which per S51 is the Lesson 6 finished build byte-for-byte, and verified true here: L06 declares and uses TRIM in 34 places and links back to Lesson 03. The other 194 hang a bare `(Lesson 6)` on TRIM itself, and **a Lesson 3 credit was searched for within 400 characters of all 194 and found ZERO times.** **THE RULED FIX REMOVES A CITATION RATHER THAN FLIPPING ONE**: Group A's line 1 becomes *“// Remember TRIM? It lived at the top of main.cpp,”* and line 4 carries the history; Group B's 194 become `(Lesson 3)`. **FLIPPING GROUP A TO LESSON 3 WAS MEASURED AND REJECTED, AND THE FIRST ARGUMENT AGAINST IT DID NOT SURVIVE MEASUREMENT** — I was about to claim the flip would make the sentence FALSE, and L03's finished `main.cpp` is **227 lines** against L06's **215**, so *buried in a 200-line file* holds either way. The real cost is that flipping line 1 makes line 4 redundant, and deleting line 4 deletes the only sentence in the book that says TRIM MOVED. **Uniformity bought by deleting a true fact is the wrong trade.** **NOT EVERY SPLIT IS DRIFT, AND THIS ONE WAS BOTH — WHICH IS WHY RULE 32 DID NOT SETTLE IT.** S144 stopped at 342 on the correct instinct that a book-wide sweep on one operator's judgment is a ruling; the instinct was right and the population was MIXED, 194 sites of drift beside 148 carrying information, indistinguishable until the comments were READ IN FULL rather than counted. **A census that reports one number for two shapes has already lost the ruling.** **NO NEW GATE IS OWED AND THE EXISTING HOLD WAS BLINDING-CONTROLLED** (rule 17): reverting exactly ONE of the 342 payload sites fires `gate_payload_match` naming 1, so lesson `<pre>` and payload cannot separate and a half-swept fix cannot ship. **THE RESIDUE IS FIVE SITES AND ALL FIVE READ CORRECTLY** — two in L13 and two in L03 attach *(Lesson 6)* to ENCODERS, one in L09 to `turnDegrees()`; **a co-occurrence is not a claim.** **CANON DEBT FOUND WHILE SEATING THIS SECTION, RECORDED NOT FIXED: §16.14 HAS NO NUMBERED SECTION BODY.** v8.135.3 announces *§16.14 NEW* and the file carries no `### 16.14` line at all, so the cliff ruling lives only in a changelog entry — the exact shape v8.103 closed for §24.14 and v8.110 logged for §3.1: **a rule whose home is a changelog entry holds only where someone happens to look.** §16.12 and §16.13 are also seated BELOW §17's heading, and §16.15 is seated beside them rather than moving three sections in a pass nobody ruled. Applied S145: L07 **v04.31.4**, L08 **v04.31.1**, L13 **v02.28.3**, Maker **v2.49.2**, all MINOR — comment text and one prose parenthetical, no structure moved, so the visible §5b banners correctly stay put. 70/70 gates, `gate_payload_match` PASS, `callout_id --audit` 1120/0, `keyterm_prefix` 0 to convert, `quiz_bank --check` 10 banks valid.) Prior: **v8.135.3** (v8.135.3, S144, moderate — **§16.14 NEW: A CLIFF READS BLACK, AND EVERY THRESHOLD ARGUMENT BUILT ON *IT READS WHITE* IS NOW RETIRED (DJ ruling: *“We need to correct the cliff arguments so there is no more confusion moving on.”*).** L11 §3.5 rested on *“No reflection at all — which reads exactly the same as the brightest possible white.”* **Pololu's QTRSensors header states the opposite twice and names the exact case: larger values are lower reflectance, “*a black surface OR A VOID*”**, and `readCalibrated()` clamps at 1000, so a missing floor pins every sensor at full black. **L04's TROUBLESHOOTING TABLE ALREADY SAID SO** — *“robot held in the air reads ‘dark’ because nothing bounces back… air counts as black”* — which makes this the S136 shape with the polarity reversed: **two lessons, opposite answers, one physics question, and THE OLDER LESSON IS RIGHT.** §43's *the older text is usually wrong* is a heuristic, not a rule; the rule is *read the citations*. **THE CONSEQUENCE IS WORSE THAN THE OLD ARGUMENT DESCRIBED, AND IT IS DERIVED FROM THE BOOK'S OWN CODE:** over a cliff all five sensors read ~1000, `isLineVisible()` sums them and returns TRUE, `readLine()` averages five equal weights to dead centre (`LINE_CENTER`), `followLine()` computes an error of ZERO, and **the robot drives off the edge at full speed believing it has found the widest, most perfectly centred line it has ever seen.** `handleGap()` is never called; `GAP_MAX_CM` never gets a vote. Every safeguard in the lesson watches for a line that VANISHES, and at a cliff the line does not vanish — **it arrives.** **THE OLD §8A.4 ARITHMETIC PRICED AN ALARM THAT CANNOT FIRE**, and its payoff — *the sensor is in the wrong PLACE, put it on a boom* — **dies with the premise: a reflectance sensor on a boom reads the void as black just the same.** **THE RULED FIX IS SENSOR TYPE, NOT SENSOR POSITION:** reflectance answers *how much light came back?* and both surfaces answer identically; you need something answering *how far away is the floor?* — ~3 mm over tape, the height of a table over nothing. **AND §8A.2 HAD ALREADY WRITTEN THE RULE FOUR SECTIONS EARLY AND ONE WORD SHORT** — *“is there a sensor that measures the thing I actually want? On this robot, for distance, there always is”* — true for distance TRAVELLED (the encoders), false for distance TO THE FLOOR, and that gap is the whole difference between a lesson you can finish and a rail you have to buy. **SIX HOMES CORRECTED, AND THE SWEEP WAS RE-DERIVED RATHER THAN EYEBALLED** — zero book-wide hits remain for *sees white*, *brightest possible white*, *white that never ends*, *400 ms*, *180 ms*, *25 cm/s*, *cliff arithmetic*, *wrong place* or *4.5 cm*: §3.5 (prose + subheading), GRAPHIC 11.2, §8A.4 (arithmetic replaced by a ONE-MINUTE EXPERIMENT — read white mat, black tape and open air, and compare rows two and three), BRAIN CHECK 01 Q5, **BRAIN CHECK 03 × 2**, Challenge 5 (retitled *The Cliff — Read It Yourself*), and §2 objective 6 **moved in BOTH homes with BC02 per §25.5.** **THE CONCLUSION IS UNCHANGED — *use a barrier, not software* — so L13 §4.1 and L14 both survive untouched and the cascade was contained to L11**, verified by a book-wide sweep before a word was written. **GRAPHIC 11.2 WAS CORRECTED, NOT REDRAWN:** it is 254,800 B of embedded JPEG plus ~8 KB of vector labels, and **the photographs do not lie — only the labels did**, so 24 label edits reused the two-panel layout as *WHAT BLACK TAPE READS* / *WHAT NO FLOOR READS*, verdict `1000 = 1000`. **Its closing line was KEPT VERBATIM because it is still exactly true:** *“There is no threshold that works. Not a badly chosen one — there is NO SUCH NUMBER.”* **AND THE LAYOUT AUDIT EARNED ITS KEEP: three of my new labels overflowed their panels, and a CONTROL RUN ON THE PRE-EDIT FILE returned CLEAN**, which is what proved the overflow was mine rather than one of the instrument's four known defects. **FOUR PROCESS FAILURES, ALL MINE, ALL CAUGHT BY INSTRUMENTS.** (1) **§12 ON MYSELF:** I computed slice offsets, THEN ran length-changing replacements in the same script, THEN sliced — eating `-b-white">` and duplicating a `</details>`. Tag balance and the structure gate both fired. **An edit that changes length invalidates every offset computed before it, including offsets in the same function.** (2) **RULE 46 AGAIN, AND THEN A LOSS IN THE OTHER DIRECTION:** two new callouts failed SIX gates, so they became prose — at which point `callout_id` read **1119/1120**, because the §8A.4 rewrite had also silently dropped `11.22`. **The lost id was DERIVED from the diff, not guessed**, and both original callouts were restored with their original families. (3) **SCOPE:** a blanket entity conversion hit the byte-compared lesson strip's `title="…"` — an ATTRIBUTE INTERIOR, which §27.16 explicitly excludes. §6.5a and §3.1b both fired. (4) **RULE 56 FOR THE SECOND TIME IN ONE SESSION:** `build_family_map`'s content tier pinned the literal *“This is what arithmetic is FOR”* — the exact sentence the correction removed — so fixing the prose made the block unnameable. Repointed, no tier logic changed. **A LITERAL CONTENT TIER OWES AN EDIT EVERY TIME THE SPELLING IT PINS IS THE THING BEING CORRECTED, AND THAT COST IS PREDICTABLE ENOUGH TO BUDGET FOR.** §27.11 moved a second time, **digest ONLY, 574/2,033 both ends**, class set byte-identical, usage RANK only (`.h4-c-555` 9 → 8, one fewer `<h4>` in the new §3.5), blinding-controlled and loud, and the cascade proof RE-DERIVED again. Applied S144: L11 **v02.30.0** (moderate), `book_gates` **v1.65.10**, `build_family_map` **v1.6.3**, GRAPHIC 11.2 corrected in place. 70/70 gates. **L11 IS NOW READ, FIXED AND UNBLOCKED — THE BANK IS THE NEXT THING IT NEEDS.**) Prior: **v8.135.2** (v8.135.2, S144, minor — **L11 READ END TO END: A LESSON SAYS A VOID READS WHITE, AND L04 ALREADY SAID IT READS BLACK.** §3.5 — *Why We Cannot Code Our Way Off the Table* — rests on *“No reflection at all — which reads exactly the same as the brightest possible white.”* **Pololu's own source says the opposite and names this exact case, twice, in the QTRSensors header comments: smaller values are higher reflectance (white), larger values are lower reflectance, “*a black surface OR A VOID*”.** `readCalibrated()` clamps to 1000, so a missing floor pins every sensor at full black. **AND L04's OWN TROUBLESHOOTING TABLE ALREADY STATES IT CORRECTLY** — *“robot held in the air reads ‘dark’ because nothing bounces back… air counts as black”* — so this is the S136 shape exactly: **two lessons giving opposite answers to one physics question, invisible inside either one, and the OLDER one is right.** Traced through the book's own code, the consequence is worse than §3.5 describes: over a cliff all five sensors read black, `isLineVisible()` returns TRUE, `readLine()` returns dead centre (all-equal weights average to 2000 = `LINE_CENTER`), the error is zero, and **the robot drives off the edge at full speed believing it is perfectly centred on a wide line.** The gap machinery never engages at all; `GAP_MAX_CM` never gets a vote. **§8.4A's ARITHMETIC THEREFORE PRICES AN ALARM THAT CANNOT FIRE**, and its payoff — *the sensor is in the wrong PLACE, put it on a boom* — does not survive either, because a reflectance sensor on a boom reads the void as black too. **The fix is sensor TYPE, not sensor POSITION: you need something that measures DISTANCE, where a missing floor is unambiguously far.** RECORDED, **NOT YET REWRITTEN** — the conclusion (*use a barrier, not software*) is unchanged and L13 §4.1 and L14 both cite only that conclusion, so **the cascade is contained to L11**; but the argument under it is a different argument and that is a DJ ruling. **THREE DEFECTS FIXED.** (1) **STEP 6 BROKE THE BUILD AND §8 BLAMED THE STUDENT.** It said delete `unsigned long gapStartTime` and asserted *“Nothing reads gapStartTime any more.”* Derived from the `after_step_5` payload: nothing reads it, and **two lines still WRITE it**, in `FOLLOWING_LINE` and `RETURNING_TO_LINE`. C++ needs the declaration for a write, so the step yields `'gapStartTime' was not declared` twice — against §6's own promise that *“Every one of them compiles… Nothing is broken in the middle.”* §8 already carried that error and called it the student's fault, **repeating the same wrong word** (*every reader must be gone*). **“Nothing reads it” is not “nothing mentions it.”** The Maker's `finished` payload had all four sites removed and was correct throughout — the S142 L08 Step 2 shape, **Maker right, book wrong.** (2) **7E's `handleGap()` WAS TRUNCATED MID-FUNCTION**, missing the drive line and the closing brace — an unbalanced brace in *“the experiment the whole lesson was built for”*. The Maker's `cal_7e` is complete. (3) **STEP 6's BYTE CLAIM WAS WRONG IN THE OPPOSITE DIRECTION.** It read *“the finished program is smaller than the one you started with… the better answer was also the cheaper one.”* **Measured on the S143 toolchain against an exact control — L11 `after_step_1` reproduces 20,516 bytes, byte-for-byte S143's verified L10 figure — the finished build is 20,702. It is 186 bytes BIGGER.** Rewritten to say so, and the honest version is the better lesson: correctness is something you spend flash on deliberately, with 7,970 bytes still spare. **This is the first NEW claim the toolchain has caught, which is what it was built for.** **A CALLOUT IS NEVER A FREE EDIT, MEASURED AGAIN:** a seventh INSIGHT block in L11 failed **six** gates at once — §5.1 geometry, §21 image-reference coverage, §27.13, §24.14, §24.14a and §24.14c — every one a baseline pinned at six. The byte fact became prose instead and all six cleared; **the callout was not worth moving six baselines.** `callout_id` also caught the id collision before the gates did (11.15 was taken; reassigned to a DERIVED 11.41, not a typed one). **AND A CONTENT TIER THAT PINS A SPELLING OWES AN EDIT WHEN THE SPELLING IS THE DEFECT:** `build_family_map`'s INSIGHT tier matched the literal headline *‘The stopwatch has no readers left’* — the very sentence that was false — so correcting the prose made the block unnameable. Repointed, no tier logic changed, **blinding control run and loud**, `build_family_map` **v1.6.1 → v1.6.2**. (4) **TRIM WAS CREDITED TO LESSON 6 FIVE TIMES IN L11**, where Lesson 3 is *titled* “Motor TRIM — No Two Motors Are the Same” (250 mentions against L06's 39). Challenge 4 contradicted itself inside one card: **Builds on** said Lesson 3, its own Run 1 comment said Lesson 6. Fixed to 7-of-7 Lesson 3 — and **`gate_payload_match` immediately caught two Maker payloads left behind** (`cal_7b`, `cal_7d`), which is the gate doing exactly its job. **THE BOOK-WIDE SPLIT IS RECORDED UNRULED:** L01, L02, L04, L06 and L11 say Lesson 3; L07, L08 and L13 say Lesson 6; L03 says both. **L06 ITSELF CREDITS LESSON 3, THREE TIMES**, which is the strongest single signal — the encoder lesson points back rather than claiming it. 342 Maker payload comments still say Lesson 6 and were deliberately NOT swept: not every split is drift, and a 342-site sweep on my own judgment is a ruling, not a fix. Applied S144: L11 **v02.29.0** (moderate), Maker **v2.49.1**, `build_family_map` **v1.6.2**. 70/70 gates, census 40,608 → **40,616**. **L11 IS READ AND FIXED, AND UNBANKED — §3.5 MUST BE RULED BEFORE THE BANK, because six of its homes would key answers.**) Prior: **v8.135.1** (v8.135.1, S144, minor — **THE SEVEN-PHASE BOX CONVERGES ON THE LINE AND THEN LEAVES IT, AND EVERY ARTEFACT AGREES WITH EVERY OTHER ONE.** S143 fixed a maneuver whose net rotation was ZERO and which could therefore never reach the line. The rebuilt box reaches it — and reaches it PERPENDICULAR. `+90`, `−90`, `−90` is a net `−90`, so `PHASE_TURN_BACK` leaves the robot facing the line square-on, `RETURNING_TO_LINE` crosses it at a right angle, and the state machine then hands straight to `FOLLOWING_LINE` **with no realigning turn anywhere in the tree.** At that moment the sensor array is PARALLEL to the tape, so all five sensors darken together, `readLine()` returns the centre value, `followLine()`'s error is zero, and the robot drives straight across the line and off the far side. **THIS IS S143's RULE 51 FIRING A SECOND TIME ON THE SAME MANEUVER, AND THE SECOND FIRING IS THE HARDER ONE.** In S143 three artefacts described a four-leg box and the code was a leg short, so the code was the outlier and the fix was a reconciliation. **Here GRAPHIC 10.2 (*LINE FOUND — follow it again*), §3.3 (*ready to cross it*), §7.7 (*resumes line following as if nothing happened*) and Sabotage B5 (which treats square-on as the CORRECT outcome) all agree with the code.** There is no outlier to defer to. **When the artefacts disagree you ask which one is the design; when they all agree you have to ask whether the design is right, and no instrument in this repo can answer that — only the floor can.** RECORDED UNRULED, and the bench item is now a specific falsifiable prediction rather than a general *drive it*: watch the two seconds after `SEEKING` flips to `FOLLOWING`. **AND THE CHALLENGE IS MORE CORRECT THAN THE LESSON** — Challenge 6's wedge meets the line at 30°, which a P-controller can pull out of; the box meets it at 90°, which it cannot. **§8.3's ARITHMETIC WAS WRONG, IN THE ENTRY S143 HAD JUST REWRITTEN.** It read *the box uses it three times, so a 10° error becomes 30°*. The box turns `+90`, `−90`, `−90` — one way and back twice — so a proportional turn error largely CANCELS: net is `90k − 90k − 90k = −90k`, and a single turn off by 10° leaves the finished heading off by **10°, not 30°.** The accumulate-logic was imported from §7D's square, where it is CORRECT because all four turns go the same way (2° × 4 = 8°), and the glossary's *Errors do not cancel* is true of that shape and not of this one. **A RULE THAT IS TRUE OF ONE SHAPE IS NOT TRUE OF EVERY SHAPE THAT LOOKS LIKE IT.** Rewritten to point at the drives first: `DRIVE_OUT` and `DRIVE_PAST` are 35 cm of open-loop travel with nothing watching the heading, and nothing cancels a curve. **§4.3's SODA-CAN COUNT WAS UNVERIFIABLE AND IS NOW DELETED (rule 50).** *Round, shiny, narrow and light — it fails four of the five rows* names four adjectives across three rows: round and shiny both land on Surface, and a can PASSES Height at ~12 cm against the table's 5 cm floor. Replaced by naming the rows it actually fails. **AND THE AMBER LEAK CLOSES ITS SECOND OF THREE.** L10 callout `10.5` carries `data-family="NOTE"` and wore `#856404`, the WARNING family's colour; derived book-wide, the canonical NOTE header is `div-c-3a5a3a` at 16 of 20 and the only two exceptions were this one and L02 §2.7 — exactly the pair S143 deferred to their own reads. L02's is still open. **THE CSS ACCEPTANCE PROOF WAS RE-DERIVED RATHER THAN CARRIED**, which is §24.13 applied to a proof instead of to a count: zero elements in the sixteen lessons carry two classes (the only 7 multi-class attributes in the tree are hand-authored pairs in `index.html` and `going_deeper.html`, outside the generated digest), so the frequency reshuffle cannot decide a cascade. §27.11 digest moved once, rules and declarations UNCHANGED at 574/2,033, the whole textual diff being `.div-c-3a5a3a` 33→34 and `.div-c-856404` 24→23 swapping rank. **The moved baseline was blinding-controlled before it was trusted** (rule 17) — an injected rule still FAILS §27.11 — and the restore was by md5 from a snapshot, not `git checkout`, per S143's refinement to rule 15. **AND TWO GATES CAUGHT MY OWN EDITS BEFORE ANYTHING WAS PRESENTED:** §27.16 on an `&minus;` entity where the literal is visible in source, and §27.13 on the unregenerated stylesheet. Applied S144: L10 **v02.29.1** MINOR — prose and one class attribute, no structure moved, so the visible §5b banner correctly stays at 02.29; `book_gates` **v1.65.9**. 70/70 gates, `callout_id --audit` 1120/0, `keyterm_prefix` 0 to convert, `gate_payload_match` PASS, `quiz_bank --check` 9 banks valid, census unchanged at **40,608**. **BANK L10 v1.0.0 WRITTEN IN THE SAME SESSION AS THE READ AND THE FIX** — 50 before + 25 after, 72/20/8 both sets, banks 9 → 10 of 16, **770 questions**. DJ ruled the WHOLE bank rather than the `before` set alone, so the `after` set is written against §6, §7 and §9 knowing some of it may be re-keyed once the arrival geometry is settled. **Six answers are deliberately not asked, and the header names all six with reasons** — chief among them the finishing pose, the crossing angle and the box-versus-wedge distances, which all sit on the unruled geometry. Three corrected byte figures are load-bearing (A06 +50, A13 the 15→5 line trade, A18's state names); against v02.27.x several would have keyed wrong. **AND THE HEADER-OF-A-THING RULE WAS PAID BEFORE IT COULD FIRE:** every question id the header names was asserted to EXIST in the file, and all five `source:` pins were compared against the live lesson versions rather than typed — the S142 defect class, caught by construction this time.) Prior: **v8.135** (v8.135, S143, moderate — **§16.12 NEW: THE AVOIDANCE BOX IS SEVEN PHASES, ONE JOB EACH — AND THE DEFECT IT FIXES PROVES THAT A GATE CERTIFYING AGREEMENT IS NOT CERTIFYING CORRECTNESS.** L10's maneuver turned `+90`, drove, turned `-90`: **net rotation ZERO**, so the robot finished on its approach heading displaced sideways, and `RETURNING_TO_LINE` ran PARALLEL to the line until the timeout. It could never return. **`gate_payload_match` passed for the maneuver's whole life PRECISELY BECAUSE 202 payload files copied the wrong code faithfully** — faithfulness is not truth, and nothing in the tree pins the book against physics. **EVERYTHING EXCEPT THE CODE WAS RIGHT:** GRAPHIC 10.2's four arrows draw the correct four-leg box, `PHASE_DRIVE_PAST`'s comment says *past the obstacle*, `PHASE_TURN_BACK`'s says *toward the line*, and §3.3 names *out · along · back · hunt* — four movements where the code had three. **The art and the names were the design document and the code was a leg short of it**, which inverts the S136 rule: read the art's own coordinates before assuming the SVG is the thing that is wrong. **AND §8.3 SUPPLIED A PLAUSIBLE WRONG CAUSE FOR ITS OWN BUG** — *the turn-back is almost certainly wrong, go re-prove your 90° turn* — where a perfectly accurate ±90 pair is exactly what GUARANTEES the failure. That is how it survived: the first student to report it would have been sent to tune the one thing that makes it worse. **DJ ruling: seven phases, one job each**, verified by simulating the pose after EVERY phase — `TURN_AWAY` · `DRIVE_OUT` (new, `AVOID_OUT_CM = 15.0`) · `TURN_ALONG` (new) · `DRIVE_PAST` · `TURN_BACK` · `SEEK_LINE` · `COMPLETE`, ending 20 cm along and 15 cm out, FACING the line, crossing perpendicular after 15 cm against a 5 s timeout; the old shape run as the control prints PARALLEL. **DJ also ruled BOTH SOLUTIONS SHIP**: the box is the base build and the **wedge** — drop the parallel leg, turn back PAST the original heading, close on a slant — is L10 Challenge 6, so the trade is the assignment rather than the answer. 112 `RobotConfig.h` and 90 `main.cpp` payloads re-derived from the parsed object; all three sabotage plants preserved. **THE AVR TOOLCHAIN IS NOW A ONE-COMMAND INSTALL AND BYTE CLAIMS ARE CHECKABLE:** `apt-get install gcc-avr avr-libc`, the eight Pololu/Arduino repos cloned beside `pio_harness.sh`, and `pololu/Zumo32U4@2.0.1` is the SAME code as the harness's `zumo-32u4-arduino-library` with the other seven LIBDIRS its own declared `depends=`. **THE CONTROL WAS EXACT: the PRE-change finished build compiles to 20,364 bytes, byte-for-byte the figure the lesson printed**, which is what makes the rest trustworthy — and **four byte claims were wrong, three of them before this session**: Step 6 *+20* → **+2**, Step 7 *+192* → **+194**, Step 8 *+84* → **+660**, Step 9 *20,364* → **20,516**. `after_step_8` is new, built to §15.6. **L09 READ AND FIXED (eight findings) AND BANKED (75 questions, banks 8 → 9 of 16).** §3.4's state table and BC01 Q3 both said `AT_INTERSECTION` *stops and identifies* where it does NEITHER — four sources agreed against them. Step 6 claimed L08's main mentions `isRunning` *five* times; it is **ten, on nine lines**. §7.2's Test 6 asked for a 20 cm gap and expected the robot to cross it against a 500 ms budget. Step 7's checkpoint opened *B still rescues you* and then explained why it cannot. **AND §3.6's timed-turn sentence WAS WRONG TWICE IN THE FIXING** — *once*, then *twice*, where the true answer spans L03, L05 and L10; it shipped with **no count at all**. **A COUNT INSIDE A SENTENCE IS A CLAIM: DERIVE IT OR DELETE IT.** **THE CATCH-UP CONVENTION IS SPLIT AND IS RECORDED UNRULED:** L07–L10 use OFFSET (`step_N → after_step_(N-1)`), L11–L16 use IDENTITY, cleanly across all 64 rows — misread as a defect mid-session, repointed and **reverted**, and a gate written on that misreading would have certified 64 rows and failed 32. Applied S143: L09 **v05.27.0**, L10 **v02.29.0**, Maker **v2.49.0**, `book_gates` **v1.65.8**. 70/70 gates; the §27.11 digest moved FOUR times, rules and declarations UNCHANGED at 574/2,033 every time, and the acceptance test has a proof rather than an inspection — **no element in the sixteen lessons carries two classes, so a frequency reshuffle cannot decide a cascade.**) Prior: **v8.134** (v8.134, S142, moderate — **§16.10 AND §16.11 NEW: THE WHEEL GEOMETRY AND THE Kp RANGE ARE NOW CANON, AND BOTH WERE ALREADY DERIVABLE FROM THE BOOK.** §16.10 records that **39 mm is the diameter OVER THE TRACK** (sprocket alone is 35 mm), with sprocket spacing 85 mm, 12 CPR on the motor shaft and 909.7 counts per sprocket revolution at the fleet's 75.81:1 — plus the open question of whether travel follows the outer surface or the track's pitch line, which lands **inside §7's own ±2 cm tolerance** either way and is a bench item, not a correction. §16.11 rules **Kp 0.1–0.3, starting at 0.10, on the 75:1 fleet** (DJ ruling). **THE Kp NUMBER WAS DERIVED, NOT CHOSEN, AND L08 PROVED IT AGAINST ITSELF**: §3.1 states the correction range as *roughly −600 to +600*, and **600 = 0.3 × 2000**, the maximum error on the 0–4000 scale — so that sentence can only have been written against a 0.3 ceiling, and §7.3 and the Quick Reference at 0.08–0.25 were the drift. **A number that a sentence DERIVES FROM cannot be swapped without rewriting the sentence** (the S137 F=ma shape): Challenge 5's three adaptive gains had been sitting on the OLD range's endpoints and moved with it, 0.08/0.15/0.25 → 0.10/0.20/0.30. A 100:1 Kp suggestion is explicitly **NOT book content** — it lives in the teacher-facing resource doc flagged unverified, because no 100:1 robot has ever been tested here and the two physical arguments point opposite ways; §16.1's rule applies unchanged, the book speaks to the robot in front of the student. **ALSO S142, AND IT IS THE ENTRY'S REUSABLE HALF: `gate_payload_match`'s LINE TEST WAS A SUBSTRING TEST AND WAS BLIND ON ONE SIDE.** The line-wise fallback asked `l in corpus` — containment in the whole corpus TEXT — so a payload line that had **LOST a leading qualifier the lesson carries** matched trivially inside the longer lesson line. Measured, not theorised: during the S142 `static` pass, reverting exactly one of 136 payloads left the gate printing **PASS**. Additions it always saw; a dropped `static`, `const` or `unsigned` it could not see **at all** — the §15.6 class it exists to hold. Now line EQUALITY against a stripped corpus line set; control-run **five shapes × two gate versions** from a snapshot with a byte-exact restore, and the two already-caught shapes prove nothing was lost. `gate_payload_match` **v1.7.1 → v1.8.0**. **THE WHOLE `static` PASS RESTED ON THAT GATE AND THE CLAIM WAS WRONG WHEN IT WAS MADE** — the safety argument was *the gate will catch a partial application*, and it would not have; the 136 payloads were verified instead by **re-deriving them from the parsed PAYLOADS object**, and a two-translation-unit compile proved internal linkage safe. **L08 READ END TO END, SIX FINDINGS.** Step 2's *complete file* reveal had **deleted the TRIM block** while Step 8's `handleGap()` uses it — compiled against the book's own reveal, `'TRIM' was not declared in this scope`; the Maker was correct in all seven payloads and only the book was wrong. §8A.2 used raw `read()` against a **calibrated** threshold, the exact thing Step 5 teaches against, in the same lesson. §8A.4 claimed to be *the complete pattern you've implemented* while naming **three identifiers that exist nowhere** — `SETPOINT`, `MAX_CORRECTION` and a bare `Kp` — and clamping the correction where the lesson clamps the speeds; it is now Step 7's own function, asserted line-for-line identical. Step 8's hint cited *Lesson 6's timers* and **L06 contains zero `millis()`**; repointed to L05's beep-interval trick, the same non-blocking stamp-and-compare shape. And two variables called *module-private* had external linkage — both now `static`, the file-scope sense L07 gave `averageCounts()`, across **136 Maker payloads (L08–L16) and 3 lesson reveals**, with L05's single-file `jumper_check` global deliberately untouched. **BANK L08 v1.0.0 (75 questions), written in the session that read the lesson** per QUIZ_SPEC §0; five of the six findings are load-bearing in it. **THE HEADER OF A THING IS NOT THE THING, TWICE MORE — AND BOTH WERE CREATED BY THE REBALANCE PASS THAT FOLLOWED THE DRAFT**, which is the mechanism exactly: ids shift and the header keeps naming the old ones. Worse, the first verifier written to catch them checked ids against a mapping typed into the script rather than parsed from the file, so it **validated intent and printed ALL HOLD against a file that had not been written**. Rewritten to parse the header's own claims — at which point it failed, which is how it was known to work. Applied S142: L07 **v04.31.3**, L08 **v04.31.0** (moderate), L10 **v02.27.2**, Maker **v2.46.0**, `book_gates` **v1.65.4**. 70/70 gates; CSS regenerated three times, rules and declarations UNCHANGED at 574/2,033 with all 574 declaration blocks byte-identical every time.) Prior: **v8.133** (v8.133, S141, moderate — **§15.6 NEW: A STEP PAYLOAD IS THE FILE AS IT STANDS AT THAT STEP, NOT AS IT ENDS UP.** L06's `after_step_7` through `after_step_11` shipped **Step 13's FIXED** `driveDistance()` and `turnDegrees()` — TRIM on the left motor and an `averageCounts()` gate — so the five DISCOVERIES entries that hand a catching-up student the file for **Step 8, Discovery 6.7, Step 10, Discovery 6.8 and Step 12** arrived with **both of the bugs Step 13 exists to find already repaired.** Steps 7 and 10 build those functions BROKEN **by design** (S24 ruling), and Step 13 — *The Square That Won't Close* — quotes the student's own file back at them: *"Look at what `driveDistance()` actually commands: `motors.setSpeeds(speed, speed);`"*. **Neither quoted line was in their file, and the square they were told would fail would have closed.** Cause named by the Maker's own changelog: **v2.14's TRIM pass regenerated `PAYLOADS[6..9]` wholesale** from the corrected states instead of only the states after the repair. **NOTHING COULD SEE IT FOR 117 SESSIONS**, because `gate_payload_match` asserts a payload byte-derives from **SOME** lesson `<pre>` and never from the `<pre>` belonging to the step it is NAMED for (§24.8) — a payload carrying a LATER step's code satisfies it perfectly. **GATE 70 NEW**, written in the same pass as the ruling (§24.2), predicate **DERIVED rather than pinned** (S126 rule 19): the expected bodies are EXTRACTED from the lesson's Step 7 and Step 10 reveals, and `finished`'s expectation is those bodies with Step 13's two substitutions applied, both of which Step 13 publishes as standalone single-line `<pre>` blocks — so rewording a comment moves both sides together and only a real drift fires. **ITS FAR-SIDE ARM WAS BLIND AND A BLINDING CONTROL CAUGHT IT** (S139 rule 17): the first draft asserted only that `finished` did not hold the BROKEN body and that the words `TRIM` and `averageCounts` appeared somewhere, and deleting `+ TRIM` from `finished` left it **SILENT** — the block matched neither shape and both words survived elsewhere in the file. **A gate that checks for a word is checking for a word.** Rewritten to compare CODE LINES against the derived expectation; it now fires. Control-run three ways, each firing gate 70 **ALONE** at 69 of 69 others green: the defective payloads restored FAILS naming all seven; `finished` de-TRIMmed FAILS; and **one comment reworded in the lesson's Step 7 reveal FAILS**, which is the derivation proving itself. **AND THE L06 CONTENT READ FOUND THREE MORE THINGS NO GATE CAN SEE.** §5.4's without-`abs()` demonstration commented the loop `// already true — loop never runs`, where the paragraph directly beneath it and BRAIN CHECK 01 Q5 both say **false** — and the comment contradicts itself, since a true condition runs. Challenge 6's *Where to look* promised **two while-loops** where its own pseudocode and template both use one `while` with an `if` inside. **AND OBSERVATION EXPERIMENT 3 PREDICTED AN OVERSHOOT THAT CANNOT HAPPEN** — *"drag the right and it sails straight past the mark"* — where **Step 13's own Bug 2 attributes BOTH error directions to the LEFT track**, and the two cannot both be true. The gate reads `abs(encoders.getCountsLeft())`, so retarding the right side turns the robot right, putting the left wheel on the **outside** of the arc: 30 cm of left-wheel rotation buys LESS than 30 cm of travel, and overshoot needs the left encoder to *under*-count, which nothing done to the right track can cause. **The hook survives and improves**: the two drags now produce **two different lies** rather than two directions of one — the left moves the number and `averageCounts()` halves it, the right bends the HEADING and no encoder on this robot can see a curve, which is Step 13's own WARNING and plants Lesson 12 four lessons early. BRAIN CHECK 03 Q5 carried the same wrong clause and moves with it. **THE OLD EXPERIMENT 3 IS NOT SEPARATELY ARCHIVED (DJ ruling: *"Don't worry about it"*)** — it is recoverable from the pushed tree at `8b71826`, and neither `ZUMO_SHELVED_CARDS.md` (never live) nor `ZUMO_PARKED_EXIT_ITEMS.md` (§10 exit-region, §25 conversions) will take live prose that was rewritten; that class still has no home. Applied S141: Maker **v2.45.6** (five payloads reverted, `finished` untouched, five changed lines), L06 **v04.32.1** MINOR — prose and one code comment, no structure moved, so the visible §5b banner correctly stays put. `book_gates` **v1.65.0**, 70 gates.) Prior: **v8.132** (v8.132, S140, moderate — **WHEN TWO SECTIONS DISAGREE, READ THE CITATIONS: THE OLDER ONE USUALLY PREDATES THE NEWER, AND NO INSTRUMENT CAN SEE IT.** L05's Observation Experiment 3 placed a box at 45° off the flank and its hint predicted RIGHT would answer — while §3.4a, citing Pololu's own measurement, states that roughly 19°–72° off each side is a blind wedge with nothing aimed into it. 45° is 26° past FRONT's edge and 27° short of RIGHT's: almost exactly the middle. **THE TELL WAS THE CITATION** — the hint cites §3.4, not §3.4a, so the experiment predates the section and nobody revisited it. A student running it would have read 0 and concluded the hardware had failed, which is the precise inference §3.4a exists to prevent four sections earlier. DJ ruled option A: rewritten to DEMONSTRATE the dead spot, with a second move out toward 90° so the card still proves the side detector works — and this is the last lesson in which the side receivers exist at all. **AND A KEYWORD CAN BE USED, GLOSSED AND NEVER TAUGHT.** `static` appears in L05 §5.11 as a retained local with no prose, again in L06's `averageCounts()` at file scope with no prose, and is explained only in L07 — two unrelated meanings of one word, met twice before either is defined. Both now teach their own sense and name the other, following the book's existing *two different jobs* pattern (L03's const/constrain, L10's two enums); the glossary entry is now **Static Variable (retained local)**. **L03's LAST TWO [CONTENT PLACEHOLDER] BLOCKS ARE RESOLVED**: brushed-vs-brushless written into §4.2 as prose, seated after the motor photo so GRAPHIC 3.18 stays with the paragraph it illustrates; the Roomba story SKIPPED by DJ ruling, because §1 *The Crooked Robot Problem* already carries that thesis 1,150 lines above it. **ONE FINDING WAS REVERSED BY MEASUREMENT** (§24.8, the S139 shape again): L05's BC02 shares zero character-exact lines with its §2 objectives, which reads as a §25.5 violation until all sixteen lessons are measured — **only L06 satisfies §25.5 at 6/6**, every other lesson is 0–4, so it is a book-wide condition and not an L05 defect. **BANKS L03 v1.0.0 (83 questions) and L05 v1.0.0 (87), both authored after the read and the fixes**; five L05 questions rest on the corrected Experiment 3 geometry and would have been keyed backwards against v04.28.1. **AND THE HEADER OF A THING IS NOT THE THING**: both new banks named question ids that had shifted during drafting, caught only by re-deriving them from the file. L03 v03.41.0 · L05 v04.29.0 · L06 v04.32.0 · `book_gates` v1.64.5; the §27.11 digest moved four times, all usage rank, rules and declarations unchanged at 574/2,033 throughout.) Prior: **v8.131** (v8.131, S139, moderate — **A VERSION NAMED IN PROSE IS NOT A VERSION CLAIM, AND AN ENTRY FILED IN ONE HOME IS NOT FILED.** S138 bumped the Bible version home to v8.130.2 and wrote its entry into line 17's `Current:` chain, but never inserted the line-start changelog entry. Every prior session edited BOTH homes; the diff proves it — S136 and S137 each show `@@ -17 +17 @@` *and* `@@ -97,0 +98,2 @@`, S138 shows only the first. `session_versions.bible_consistency()` reads the line-start list, so it believed S137 had just run and reported LIVE.md and the S139 handoff as wrong when both were right. **THE READER WAS CORRECT AND THE BOOK WAS WRONG, WHICH IS THE REVERSE OF THE USUAL CASE.** Filed at S139 using S138's own chain-copy text verbatim: 2 added, 0 deleted, the same hunk shape as S136 and S137. Note the two homes have NEVER been byte-identical — S137's filed entry is 2,607 chars against the chain's 2,816, differently worded and differently tailed — so filing is authoring, not copying. **`session_versions` v1.24.0 -> v1.24.1.** `check()` read the WHOLE handoff into `_versions_in`, which builds its dict with `re.findall`, so the LAST occurrence of a key won: the S139 handoff states `Bible **v8.130.2**` in its STATE block and eighty lines later refers in prose to `(Bible v8.130.1)`, and the prose won. LIVE.md never had this defect because its read is already scoped to line 6, asserted to start with `**Versions:**`; the handoff was the outlier and now takes the same discipline — a WINDOW anchored on the VERSION BLOCK marker, one line per line of the emitted block. `_versions_in` is untouched, because it runs on generated text too and a rule about prose is meaningless there. **THE MARKER IS NOW EMITTED**: its own sentence has read *emitted by session_versions.py --handoff, never hand-typed* since S138 while nothing emitted it, so `_fixture_clean` — which rebuilds the handoff from `--handoff` — produced a file that claim was false about. **No whole-file fallback**, because a fallback reinstates the defect on exactly the malformed file that most needs checking; a missing marker is LOUD. And a generated key ABSENT from the window is now reported rather than skipped — the old `if k in hw` passed silently over anything the file failed to mention, which under a windowed read would have turned a reflowed block into silence. **CONTROL H, three directions** (prose does not shadow · a wrong value inside the block is still loud · a missing marker fails), plus a separate blinding run seeding the real failure mode — a stale STATE block with correct prose — which still reports. ALL EIGHT CONTROLS PASS. **L04 READ FINDINGS: 11 OF 13 APPLIED** (the twelfth was recovered from the S138 chat, the thirteenth found by census here). Challenge 4's TEMPLATE did not compile — `LINE_SEEN` used and never declared, card naming only two blanks; now three blanks, and the blank is right because §8A.2 states that choosing thresholds from real data IS the skill, and Challenge 1 already blanks one. `LEARN 4.38` cited *Lesson 2 §3.2b*, which L02 does not have (its §3.2 is *Data Types*, with `bool` an h4 inside it). §8A.8's *counts hundreds per crossing* in three places contradicted its own arithmetic — 10 Hz for half a second derives FIVE — now *counts every loop, not every crossing*, which stays true at any tape speed where *five* assumes the example dwell. §5.5 presented arrays as new when L03 §8A.5 already taught them; it now credits Lesson 3, the S137 *nobody explained it* shape. Step 2's reveal put GLOBAL VARIABLES directly under CONSTANTS against its own pseudo-code (*right after the prototypes block*) and against the shipped file, whose Maker payload order is HARDWARE OBJECTS -> CONSTANTS -> FUNCTION PROTOTYPES -> GLOBAL VARIABLES; the banner is back. C4's function list named `[0]`/`[2]` where goal, pseudo-code and reveal all loop all three. **THRESHOLD CANON: 600 BOOK-WIDE IN L04, WITH THE MARGIN STATED (DJ ruling).** The census split cleanly — all six 500s were TEACHING (§8A.1, §8A.2, the KEY TERM, §8A.3's worked example, the inline exercise, the Glossary) and all six 600s were PRACTICE (§8A.8, Challenges 1, 2 and 4) — so a student applying the rule just taught wrote 500 and found every reveal saying 600, with no sentence explaining the change. **EIGHT SITES SWAPPED; TWO WERE REWRITTEN, NOT SWAPPED** — the KEY TERM and the Glossary carried the *halfway* reasoning, and substituting 600 into *500 is the natural darker-than-halfway line* manufactures a false sentence, the S137 F=ma failure exactly. Both now keep 500 as the midpoint and state 600 as a deliberate margin above it. **§7's *seven-section layout* finding was REVERSED BY MEASUREMENT** — L02 §3.1's Color Key lists eight rows with Prototypes marked unnumbered, i.e. seven numbered, so the sentence is true; the banner census counted `// ===== =====` banners, which Header and Include never carry, giving a ceiling of 6 that no lesson could exceed. **The instrument could not return the answer the claim asserted** (S137 rule 38's shape). This retires the L03 §5.1 queue item; the CONSTANTS vs CONFIGURATION vocabulary drift underneath it stays open. `waitForStart()` and `checkBattery()` moved out of the Quick Reference's *Line sensor functions* table into a new **Startup & safety** group (`#qr-startup`) — none of the ten existing groups fitted, and two homes for one fact is how banner/fence/pill drift began. **L04 v04.28.3 -> v04.29.0, MODERATE** — a new `<h4>` and `<table>` means structure moved, so both §5b homes move and the visible banner was DERIVED from the hidden comment, never hand-typed. **CSS regenerated three times in one session; digest `ce43da62` -> `d7702428` -> `3ce57062` -> `2cd8cd62`, rules and declarations UNCHANGED at 574/2,033 every time, class set byte-identical, and the acceptance test was the RESOLVED STYLING (rule 24): all 574 declaration blocks compare byte-identical, so nothing renders differently.** `book_gates` v1.64.2 -> v1.64.3. **QUIZ_SPEC v1.0.0 -> v1.1.0: THE FORMAT MIX WAS ARITHMETICALLY IMPOSSIBLE.** MC ~70%, TF cap ~20%, matching *one or two, no more* — at both ceilings a 50-question set has an MC FLOOR of 76%, so ~70% was unreachable by construction, and L02 met it only by running TF at 22%, over its own cap, unnoticed because nothing validates a mix. **The matching allowance moved to ~10%, NOT the TF cap, and the reason is signal**: true/false is 50% guessable so buying the ratio with it lowers what the gate measures, while matching's mandatory `extra_answers` absorbs the guess. The superseded cap was ALSO restated in the per-format rules and was fixed there in the same pass — a rule restated in two sections is two rules. **L02 IS NOT REBALANCED** and the amendment says so, so it is not mistaken for conformant when used as a template. **L04 QUIZ BANK WRITTEN, v1.1.0, 73 questions** (before 51: MC 69% / TF 20% / matching 12%; after 22: MC 68% / TF 23% / matching 9%), every question cited, authored AFTER the read and the fixes — READ -> FIX -> QUIZ — and pinning `lesson_04: v04.29.0`, because five of its questions would have been keyed wrong against v04.28.3. **The wave-test direction and Act Two's row-1 overflow are asked in NEITHER set**: both are open bench findings, and a `before` set must be answerable from the text alone. Banks 1 -> 2 of 16. 69/69 gates, family map unmoved at 1119, `callout_id --audit` 1119/0, `strip_inline --verify` 0 dead, `gate_payload_match` PASS, census 40,468 [lesson_inventory.py].) Prior: **v8.130.2** (v8.130.2, S138, minor — **AN ASSET IS NOT A DELIVERY UNTIL SOMETHING REFERENCES IT.** `L04_GRAPHIC_4-07_five_sensor_windows.svg` was pushed on its own and took the suite from 69/69 to **two failures** — §17.3c's raster census (31 walked, 32 present) and §10's worklist, because an unreferenced file lands in `image_audit`'s unreferenced bucket, 30 → 31. **Three counters moved for a file no page pointed at**, which is the shape worth recording: the tree's instruments watch the DIRECTORY as well as the prose, so a half push is louder than no push. Wired into L04 twice — §1 as the reveal, retiring the `[IMAGE 4.5]` placeholder, and §4.1 as the numbered reference, retiring a **borrowed Lesson 11 diagram** that had stood in with a *temporary* caption and array-slot labels. **`IMAGE 4.1` RETIRES WITHOUT BEING SHOT** — the shot list, the Figures table and the reserved filename all described the same photograph, so the fall figure debt falls **11 → 9** and `IMAGE_SHOT_LIST.md` goes 22 photos → 21. **THE ORIENTATION WAS SETTLED BY MEASUREMENT AND CONFIRMED ON A BOARD.** Pololu 0J63 §3.5 gives the convention — sensor 1 is the robot's left-most — but the photograph is an underside view with the blade pointing *away*, which mirrors it; three independent signals agreed (the `FRONT` arrow, the silkscreen with `RGT`/`DN4` on the viewer's left and `DN2`/`LFT` on the right matching §3.2's own pin table, and the moulded chassis logo reading reversed), and DJ confirmed. Chips run **5 4 3 2 1** left to right and the margin labels read **ROBOT'S RIGHT** / **ROBOT'S LEFT**, so the figure now DEMONSTRATES the mirror §4.1 warns about instead of falling into it. **AN ILLUSTRATOR ROUND-TRIP IS NOT AN EDIT.** Four passes resampled the embedded photograph every time — JPEG 1200×1148 209 KB became PNG RGBA 1208×1152 at 1.36 MB, three pixels wider per save — dropped `role`/`<title>`/`<desc>`, flattened hand-written attributes into `.st0` classes, baked text centring into `translate()` so nothing re-centres, and on one pass **injected a `Lorem ipsum` textPath** where the footer rule had been, deleting the rule in the process. Every label edit was ultimately made in TEXT against the original file; the shipped photo is byte-identical to the first upload and the file is 287 KB rather than 1.82 MB. **AND A GENERATED BOARD IS NOT A PHOTOGRAPH** — a blueprint-styled redraw was declined because its silkscreen reads `OJ8696` where the board reads `0J8696`; **one provable wrong glyph means all fifteen labels are model output rather than evidence**, which is §14 exactly, and it was also a 2.1 MB dark-field raster with no live text. **`gate_payload_match` WENT PASS → FAIL INSIDE THE S137 CLOSE PUSH AND THE HANDOFF STILL CLAIMED PASS** — it is not one of the 69, so nothing runs it unless a session opener does. Its `EXEMPT` keys pin literal starter LINES, and S137 rewriting those lines orphaned three of them: **S126 Rule 19 arriving as a live defect one layer down** — a key that pins a spelling certifies a spelling, not a property. Repointed and one added, `gate_payload_match` **v1.7.1**, blinding control loud, orphan census across all 21 keys clean. Baselines moved with their reasons in place: §17.3c 31 → **32**, §21 image references 1,201 → **1,202** (two placements in, one borrow out), §27.11 digest — rules and declarations UNCHANGED at 574/2,033, zero born, zero died, **class SET byte-identical in both directions**, only usage RANK reshuffling the file, `strip_inline --verify` 0 dead names. `book_gates` **v1.64.2**. Applied S138: L04 **v04.28.3**, MINOR (a figure lands and a borrow leaves; the §5b visible banner correctly stays put). 69/69 gates, family map unmoved at 1119, `build_css --check` current at 574 rules, census **40,468** [lesson_inventory.py].) Prior: **v8.130.1** (v8.130.1, S137, minor — **§24.10 AMENDED: THE RULE NAMED A TOOL, SO CHANGING TOOLS ROUTED AROUND IT.** DJ ruling: *"Grep just seems to be a waste and you miss so much when you grep. When possible use other methods."* **§24.10 has said this since S91 and it was violated TWICE in the session that amended it, both times by an operator who believed he was complying because he had not typed the word `grep`.** A Python `if 'for loop' in line` walk over L03 returned two hits, and those two were carried to DJ as a finding with two rulings attached — **a population of unknown size reported as a measurement.** The scan cannot see a challenge that needs a loop and never says the word, which is the majority case; and it could not see that **the underlying question had ALREADY BEEN RULED in S57 and shipped** (L04 §8A.6 owns the `for` tutorial, L05 §5.15 is the spiral rung, Bible §11 v8.36.1 came out of it), because **no instrument that reads the tree can see a ruling.** The amendment adds five clauses and pins a PROPERTY rather than a spelling (S126 Rule 19, applied to canon instead of to a gate): a **text match** is `grep`, `rg`, editor find, `str.find`, `in`, `re.search` over lines, or an `--audit` flag doing any of those internally — **one instrument in different clothes.** **A TEXT MATCH LOCATES; IT NEVER ANSWERS**, because the one thing it cannot return is the case that lacks the string, so a hit is READ IN FULL before it is reported and an empty result is never reported as absence — which is the positive form of S136’s *grep the figure book-wide and read every sentence it appears in*: **the scan is step one of two, and stopping after step one is the defect.** Clause 3 extends from COUNTS to CLAIMS: a sentence with no named instrument is a lead, and **a lead never carries a ruling request**, since attaching a decision to an unverified premise asks DJ to rule on something that may not be true. New clause 5: **ask whether the session record already answers it before measuring the book.** **AND THE AMENDMENT OWES TOOLING, NOT WORDS** — this entry’s own thesis is that a rule recalled at the moment of temptation does not work, so every reach for a text match where the parser cannot answer is now a recorded FEATURE REQUEST against `lesson_inventory.py`; S137 opens one: *enumerate, per challenge card, the constructs the student is asked to WRITE*, which is what §11 requires before any §9 can be certified and which nothing in the tree can answer. **Ungated by design** — §24.2 owes no gate because there is nothing in the book to assert against, the §24.6b/c/§24.7/§24.8 shape. Doc only: no lesson file changed, no instrument changed, no gate changed. 69/69 gates, census unchanged at **40,458** [lesson_inventory.py], family map unmoved at 1119, `build_css --check` current at 574 rules.) Prior: **v8.130** (v8.130, S136, moderate — **NO INSTRUMENT READS PROSE, AND THE BOOK HAS BEEN GRADED BY INSTRUMENTS FOR 135 SESSIONS.** A content read of L01 and L02 end to end found **seven defects across the two lessons, and not one of them was visible to any of the 69 gates** — because a gate measures structure and a FALSE CLAIM IN PROSE IS PERFECTLY WELL-FORMED STRUCTURE. **THREE WERE CONTRADICTIONS BETWEEN LESSONS, INVISIBLE INSIDE EITHER ONE.** (1) L01 §3.0 promised a **sumo match** the course never runs, contradicted in place by L03's *“our non-sumo RoboCup bots”* and L14 listing sumo under *beyond these pages*; a scan of every course-scope promise in all sixteen lessons returned **exactly one hit**. (2) **L01 and L02 gave OPPOSITE diagnoses of the same symptom**: L01's troubleshooting table blamed garbled serial on a **wrong baud rate** and told the student to match 115200, where L02 §8, L02 §6 Step 2 and L02's own Glossary all state that the Zumo talks over **native USB**, which negotiates its own speed, so the number in `Serial.begin()` is ignored on this hardware. **L01 is the lesson they are in when it happens.** (3) L02 §3.2 pointed at **Lesson 7** for `float`, which teaches it nowhere; `float` has **no KEY TERM in any of the sixteen lessons**, and the first time a student must TYPE one is **L06**'s pentagon challenge, one lesson BEFORE the promise came due. **AND A NUMBER THAT CONTRADICTED THE FLEET'S OWN CHEMISTRY:** L02's Quick Reference said `readBatteryMillivolts()` reads **about 6000 with fresh batteries** — that is **1.5 V per cell, alkaline**. The fleet runs **4-cell NiMH**, and L01 (~5,400 fresh), L03 (5400 / 4800 / 4200, and naming alkaline separately at ~6,300) and the `BATTERY_GOOD = 4800` / `BATTERY_LOW = 4200` constants in L07 and L08 all agree. **L01's OTHER `6000` WAS CHECKED AND DELIBERATELY LEFT** — it is a Challenge 11 prompt, *“try a threshold of 6000 — what happens, and why?”*, whose whole point is that the threshold is unreachable. **NOT EVERY HIT IS A DEFECT; READ BEFORE YOU FIX.** **THE COMPILER SETTLED WHAT READING COULD NOT.** L02's *The Broken Code* challenge named lines **3 / 7 / 11**; built against a stub header with `g++ -fsyntax-only`, the mistakes are on **2 / 5 / 8**. **And the build revealed something no reading would have:** on the FIRST build the missing parenthesis **does not appear at all** — the line-2 semicolon merges the declaration into `setup()`, so the parser never reaches it, and a challenge that promises three errors shows two. A student counts, finds two, and concludes they have missed one that is not findable. The reveal now names 2 / 5 / 8, explains the masked error, and closes *“a build that reports two errors does not mean there are only two.”* **THE BOOK'S OWN CODE CAN BE A WITNESS.** The buzzer's non-blocking behaviour could not be confirmed from the library (unreachable), but **L01's chime proves it**: `playFrequency(440, 800, 15); delay(900);` and a jingle whose every delay runs LONGER than the note it follows. Those delays are meaningless if the call blocks. **The behaviour was already taught — inside GRAPHIC 1.19, and ONLY there**: invisible to a skimmer, unreachable by a screen reader, absent from both Quick References, and silently required by Bonus 5, whose hint asked how long a trip through `loop()` takes. **A FACT THAT LIVES ONLY IN AN SVG IS NOT IN THE BOOK.** Now stated in L01 prose, in L02's Quick Reference sound row, and in the Bonus 5 hint. **ALSO FOUND: L02's Quick Reference anatomy list omitted the Function Prototypes row** that §3.1 insists on, Brain Check Q1 requires by name, Step 7 breaks the build to teach — and that its own code template three inches above it contains. **THE QUIZ WORK, AND WHY ITS ORDER IS CANON.** `quizzes/` now holds the L02 bank (**58 before + 18 after**), `QUIZ_SPEC.md`, and `quiz_bank.py` (nine selftest controls, all loud). **FOUR OF THE SEVEN DEFECTS SAT DIRECTLY UNDER QUESTIONS THE BANK ASKS.** A bank written against the unread L02 would have keyed **6000** correct and marked the students who read carefully WRONG. **AN AUTO-GRADED GATE THAT PUNISHES THE ATTENTIVE IS WORSE THAN NO GATE**, so the order is fixed per lesson: **READ → FIX → QUIZ**, never quiz first. **STATUS IS DERIVED, NOT WRITTEN** — `quiz_bank.py --status` reads the tree, because a hand-kept progress list is wrong the first time somebody forgets, and wrong SILENTLY (§24.13). **THE SOURCE PIN EARNED ITSELF INSIDE ONE SESSION:** bumping L01/L02 at close made the bank's `source:` block stale, and the pin is what caught it. **Fill-in-the-blank is BANNED** — Canvas string-matches it, so `getSingleDebouncedPress` fails on a capital G and a student loses entry to build time over spelling; the validator rejects the type. **Matching requires `extra_answers`** — five prompts against five answers hands the last pair to anyone who knows the rest. **CANON DEBT, RECORDED NOT FIXED: no gate holds a quiz bank.** `quiz_bank.py --check` is loud but `book_gates.py` never calls it, so a broken bank can be pushed — the same debt shape as the nav pills in v8.129, logged the same day it was created. Applied S136: L01 **v03.28.2** · L02 **v03.21.2**, both MINOR (prose and table cells only), so the visible §5b banners correctly stay put. `quizzes/` is a NEW top-level directory. 69/69 gates, family map unmoved at 1119, `build_css --check` current at 574 rules, `image_audit --check` current at 16 of 146.) Prior: **v8.129** (v8.129, S136, moderate — **A LABEL IS NOT THE THING IT NAMES, AND THE TWO DRIFT SEPARATELY.** A section has THREE independent spellings — the banner cap, the HTML section fence, and the nav pill — and this session found live disagreements in all three, none of which any of the 69 gates could see, because **no gate ties a pill to its banner.** **THE BANNER: §3 read `Theory` in fifteen lessons and `Code Anatomy` in L02**, and §8 was thirteen bare caps titled *Troubleshooting* against three carrying the label plus a real lesson-specific title. DJ ruled toward the three; **thirteen new titles were written from each section's own content**, never invented — L04's *When the Sensors Lie* is that section's own opening sentence, *“the program compiles, uploads, runs — and lies.”* **§4's SEVEN labels were measured and LEFT ALONE** (Hardware ×8, The Course ×3, Setup, Planning, Prep, Timing, Budget): each is accurate to its own lesson, so that split is a descriptor working as designed rather than drift, and normalising it would have destroyed information. **GATE §6.8a CAUGHT THE HALF-RENAME AND IS WHY THE §3 FIX DID NOT SHIP BROKEN:** the fence is DERIVED from the eyebrow's post-middot name, so changing L02's banner alone failed the suite immediately at *expected SECTION 3: THEORY*. One edit became two because the gate said so. **THE NAV PILL IS A SECOND LABELLING LAYER AND NOTHING WAS WATCHING IT.** Two of its entries were defects rather than preferences: **L14's `8. Tune & Test` pointed at a Troubleshooting section** (tuning is L15), and **L16's `9. Tiers` sat over a banner reading Challenges** — in both cases the pill had drifted away from its own section, and a student clicking either landed somewhere other than the label promised. **THE RULED VOCABULARY, pills 1–9, now byte-identical across all sixteen files:** `1. Intro · 2. Objectives · 3. Theory · 4. Hardware · 5. Code · 6. Build · 7. Verify · 8. Diagnose · 9. Challenges`. **THE PATTERN DJ RULED IS THAT PILLS ARE SHORT HANDLES AND BANNERS ARE SECTION NAMES**, which is what makes `Diagnose` over a Troubleshooting banner and `Wrap-Up` over an Exit Ticket banner coherent rather than inconsistent — they are not supposed to match. **PILL 7 WAS RULED ON THE WORD, NOT THE SHAPE.** `Commission It` is the precise engineering term and was DECLINED on measurement: the word appears **nowhere else in the book**, so it would reach a student as a nav pill with no definition behind it. `Diagnose` was rejected for §7 on the evidence that **§7 and §8 are opposites** — entered with a working robot and a broken one respectively — and naming both the same tells a student they are one errand. `Verify` was taken because it is equally true of L01's *press A and watch* and L16's three-run A/B protocol, which is the whole span the word must cover. **PILL 10 HAS ONE EXCEPTION AND IT WAS MEASURED BEFORE IT WAS GRANTED:** a read of all sixteen §10 sections found exactly TWO that are not a Brain Check exit ticket, L14 and L16, so fifteen take `Wrap-Up` and **L14 keeps `Match Day`** because its §10 is `10.1 Morning Routine` / `10.2 Pre-Match Routine`. **AND THE SESSION'S OTHER HALF IS THE ONE NO INSTRUMENT COULD HAVE DONE: A CONTENT READ OF L01 FOUND A PROMISE THE COURSE NEVER KEEPS.** §3.0 explained closed-loop control with *“when your Zumo fights a sumo match later in this course”* — and the course is Rescue Line. **The book already contradicted itself in two places**, L03 saying *“our non-sumo RoboCup bots”* and L14 listing sumo under *“beyond these pages”*; a scan of every course-scope promise in all sixteen lessons returned **exactly one hit**, and it was this one. Rewritten to line following, which is what L08 teaches. **THE 69 GATES MEASURE STRUCTURE, AND A FALSE CLAIM IN PROSE IS WELL-FORMED** — §24.13's re-derive does not reach a sentence, so the error survived 135 sessions. **ALSO FOUND AND DELIBERATELY NOT RULED:** the `for` loop has THREE homes and no owner — introduced in L01's INSIGHT, dissected in L04 §8A.6, and the objective *write a for loop and trace it* owned by L05 — while L04 §8A.6 opens *“nobody explained it,”* which is true only if L01's INSIGHT is skipped. **CANON DEBT, RECORDED NOT FIXED:** the pill vocabulary and the pills-are-handles pattern live ONLY in this entry, the same shape as §3.1a, §24.14 and §21.1 before their sections were written — **a rule whose home is a changelog entry holds only where someone happens to look.** No gate holds a pill. Applied S136, all sixteen MINOR — labels and one sentence only, no structure moved, so the visible §5b banners correctly stay put: L01 **v03.28.1** · L02 **v03.21.1** · L03 **v03.40.1** · L04 **v04.28.1** · L05 **v04.28.1** · L06 **v04.31.1** · L07 **v04.31.1** · L08 **v04.30.1** · L09 **v05.26.1** · L10 **v02.27.1** · L11 **v02.28.2** · L12 **v01.31.2** · L13 **v02.28.2** · L14 **v02.33.1** · L15 **v02.30.1** · L16 **v02.22.1**. No stylesheet movement: `build_css --check` current at 574 rules, family map unmoved at 1119, `image_audit --check` current at 16 of 146, census unchanged at **40,455** [lesson_inventory.py].) Prior: **v8.128** (v8.128, S135, moderate — §10 gate 69: a figure is planned by its TAG, which lives in the figures index, not where the figure does; and §18.1's *1-2 prior concepts* is a FLOOR, not a ceiling.) Prior: **v8.127.1** (v8.127.1, S134, minor — **A COUNT IN THE v8.127 ENTRY WAS WRONG AND THE TRIPLE-CHECK FOUND IT.** It stated that 19 body KEY TERM blocks carry `id="term-*"`. **18 do.** The nineteenth, L03 `3.44`, carries `id="glossary-trim"` — a GLOSSARY-prefixed id on a BODY block, which is its own unruled lead. The first measurement counted heads carrying ANY id and the sentence reported them as one spelling: §24.13, a count re-read rather than re-derived, and §24.6c on a number that had already reached canon. Corrected in §24.14d, this changelog, the handoff and LIVE.md. `keyterm_prefix` **v1.0.1**, doc only — no behaviour, no gate and no lesson file changed, and the anchors themselves were verified intact at 169 before and 169 after with zero lost and zero gained.) Prior: **v8.127** (v8.127, S134, moderate — **§24.14d NEW: A CALLOUT FAMILY NAMES ITSELF IN THE BODY, AND THE REGION DECIDES.** KEY TERM was the only large family in the book that did not name itself - 13 of 238, against NOTE 113/133, CHECKPOINT 102/112, TIP 79/85 and four families at 100% - and those 13 were exactly the blocks a normalisation pass was about to strip. DJ ruled the prefix IN for the body and OUT for the glossary, where the banner already says it. 83 blocks converted; the pin falls 125 to 55 because 70 holds expired by themselves. Gate 68 new, and its first draft failed its own control: a coverage count measures blocks SCANNED, not blocks ASSERTED, so a gate carrying exceptions owes a HOLD ARM. Full entry in the CHANGELOG below.) Prior: **v8.126** (v8.126, S133, moderate — **§2.1 AND §2.2 NEW, AND §27.15b GAINS THE FINDING THAT COST TWO REVERTS: A GENERATED CLASS NAME IS NOT A HANDLE.** A rank-assigned `-N` suffix means adding uses to one of a pair SWAPS them, silently re-resolving twelve untouched lessons - and it fires through inline authoring too. Carried by `ul[data-objectives]` in `css/semantic.css`; the selector is an ATTRIBUTE because a semantic class in markup runs the generator backwards. Also: 24 body KEY TERMs lose the em dash, all 237 back-to-top links become one spelling, §14.1's template filename is corrected, and the CANON tier now matches on a WORD BOUNDARY - measured inert across all 1,119 labels. Gates 66 and 67 new. Full entry in the CHANGELOG below.) Prior: **v8.125** (v8.125, S132, moderate — **§27.15f NEW: THE REVEAL BOX GRADUATES.** 453 reveals in thirteen spellings plus 55 with no class at all — and those 55 had no `cursor: pointer`, the one part of the drift a reader would have felt. Border ruled `#909090` by §27.15e’s existing 3:1 test, not by a new decision; the summary line normalised from twelve spellings. L11’s four ANSWER callouts held by DJ ruling. Gate 65 new. Full entry in the CHANGELOG below.) Prior: **v8.124** (v8.124, S132, moderate — **THE GLOSSARY IS ONE COMPONENT.** 151 entries in five schemas become the canon term card; 54 of them had no `data-family` and no `data-callout` and could not fail a gate. §24.14b NEW: a callout in the glossary region is a KEY TERM, 97 of 97 — a section id is STRUCTURE, not decoration. The pin falls 212 to 125 because gate 62's coverage arm said the holds had expired. §6.9's entry format and §8's 11-type table both become pointers to `BookComponentStandard`. Gate 64 new. Full entry in the CHANGELOG below.) Prior: **v8.123** (v8.123, S131, moderate — **GATE 63 NEW: A FIGURE IS LANDED BY AN ASSET, NEVER BY A DECORATION.** S130's 884 marks landed in the prose `image_audit`'s neighbour arm reads, and six real shots reported LANDED with nothing taken; no gate failed, and the only signal read as *re-run me*. The predicate is the AUTHORED `data-mark`, not the directory. `image_audit` v1.2. Full entry in the CHANGELOG below.) Prior: **v8.122.1** (v8.122.1, S130, minor — **§12.6 records the census definition change.** Every census figure before S130 runs high by exactly the lesson count; the offset is derived, not magic. Recorded rather than retro-edited, because a reader comparing 40,698 with 40,683 computes a phantom −15 and hunts fifteen lines that never existed.) Prior: **v8.122** (v8.122, S130, moderate — **THE MARKS ARC LANDS; §20.5 AND §24.14c NEW.** 884 emoji become marks, all 1,069 callouts gain an authored identity, the GLYPH tier is DELETED as the second decoration-keyed tier to die, and 212 blocks move to a preserved pin. Gate 60 new. Full entry in the CHANGELOG below.) Prior: **v8.121** (v8.121, S129, moderate — **§5.1 stops reading the glyph; `glyph_scan.py` v1.0 new; the detector learns `</>`; §21 derived at 1,134. Full entry in the CHANGELOG below.) Prior: **v8.120** (v8.120, S128, moderate — **§24.14a NEW: THE FAMILY LIVES IN THE MARKUP.** The GLYPH tier resolved 209 blocks by the decorative emoji alone, so the marks arc could not start until the family moved into the markup. data-family on all 1,069; the generator reading it reproduces its table byte-identically and the blinded glyph tier still returns 1069/1069. Gate 59 new. Full entry in the CHANGELOG below.) Prior: **v8.119** (v8.119, S127, moderate — **§27.16 NEW: ONE SPELLING PER CHARACTER, AND THE RULE IS A PROPERTY RATHER THAN A LIST.** DJ ruling: *"Ok, Yes change them to the symbols."* The book spelled the same character two ways — **5,935 non-ASCII characters were already literal while 4,827 were entities** — so every byte-reading instrument counted a fraction of each population. **The discrepancy the handoff carried is now settled and its cause is bigger than the two numbers.** S126's glyph census of *95 distinct / 3,258 occurrences* is reproducible at exactly one scope out of an eight-way grid — literal bytes of the sixteen lesson files, Unicode symbol categories — which yields **95 / 3,273**, and the 15-occurrence gap **IS `≈` U+2248**, the only character in the census with a count of 15. **But that scope is itself the defect:** decoding entities, the true figure is **110 distinct / 3,984**. **34 characters were written BOTH ways and 15 existed ONLY as entities** — 🏠×16, ©×16, 🕵×8, ⭐×3, plus 🎓 and the three medals — and those fifteen appear in **no census ever taken of this book**. Every glyph count in the handoff was therefore low: 💡 114 is **118**, 🔑 158 is **174**, 🎯 113 is **125**. §24.8 again, on a denominator: the population you can enumerate by spelling is not the population that renders. **THE OPPOSITE DIRECTION WAS CONSIDERED AND FAILS ON ITS OWN TERMS.** DJ asked whether writing everything as an entity would be safer. It would not: entity form is not one spelling but three, and **the book already carries 14 characters spelled in more than one entity form** — em dash `&mdash;` 2,129 against `&#8212;` 117, and the apostrophe at `&#39;` 27 against `&#x27;` 44 with no literal at all. An all-entity rule needs a SECOND rule for named vs decimal vs hex, and that rule has already drifted three ways unruled. **Literal is the only direction that collapses to exactly one spelling**, and the only legible one: **103 of the book's 143 non-ASCII characters have no named entity**, so an all-entity book prints `&#128161;` where an editor needs to see a lightbulb. **THE RULE: write the character however it can be SEEN in the source.** Literal when the literal form is distinguishable; an entity when it is not. **The second clause is a PROPERTY, and it reaches exactly three characters** — no-break space, narrow no-break space, and **the non-breaking hyphen U+2011, which was missed on the first pass and is the sharper case of the three**: literal U+2011 renders and reads identically to `-`, so normalising it would silently convert 60 deliberate no-break decisions into characters nobody can see or verify. Named where a name exists, decimal where none does — `&nbsp;`, `&#8209;`, `&#8239;` — and **the gate DERIVES the no-name claim from `html.entities` rather than asserting it**. **SCOPE IS TEXT NODES, AND THE TWO EXCLUSIONS ARE PROPERTIES NOT TASTES:** `<script>` and `<style>` are RAW TEXT the parser never decodes, so `&#39;` there is six literal characters of JavaScript and rewriting it would change what the program says — **the Maker's payloads live exactly there**; and attribute interiors stay untouched, where `&quot;` inside a double-quoted value is mandatory. **4,789 conversions across 20 files, 239 held, 357 protected occurrences left alone.** **THE PROOF IS NOT THE DIFF.** The decoded text of all 21 files is **byte-identical before and after**, asserted inside the tool and again independently against a full-tree snapshot: the page renders exactly as it did and only the bytes on disk moved. **THE COUPLING PREDICTED AS DANGEROUS WAS THE SAFE ONE, WHICH IS WHY IT WAS MEASURED.** 439 entities sit inside `<pre>` and `gate_payload_match` byte-derives Maker payloads from those blocks — but `decode_pres()` calls `H.unescape`, so it compares DECODED text and was always invariant to this. It passed untouched. **WHAT DID BREAK IS S126's RULE 18 ARRIVING AS A LIVE DEFECT: FOUR PREDICATES PINNED A SPELLING.** Gates §25.6, §25.2 and §4.5 searched the source for the literal string `&mdash;`, and `next_pointer` EMITTED it. **`book_gates.py:396` already carried the correct dual form and its broken twin sat eleven lines away at 701** — one assertion, written both ways, in the same file. **AND THE SAME DEFECT HID IN A LOCATOR, WHERE IT IS DANGEROUS RATHER THAN LOUD:** `strip_inline`'s hero-version and footer spans are found by the same pinned strings, and with them blind **L01's held attributes fell 39 → 32** — seven attributes silently unprotected, which is the `--include-held` hazard reached by a different road. Caught by its own Control H, not by a gate. An AST scan of every instrument then enumerated the rest rather than waiting for the next failure, and found **`_LAND`, the landing-zone detector, listing `&larr;` and `&#8592;` but not the literal `←` — already blind to 33 of the book's 36 left-arrows BEFORE this session**. Widened; the book is clean behind it. **TWO DEAD CONSTRUCTS FOUND AND PROVED PRE-EXISTING RATHER THAN BLAMED ON THE SWEEP:** `BONUS_MARK` in `book_gates.py` and `MARK` in `gen_bonus_banner.py` are defined and indexed nowhere, and two stray-checks cannot fire — the bonus banner carries no mark at all, having lost it to S108's no-icons ruling. Verified against the pre-sweep snapshot: **zero marks in any banner before the sweep. That is a live lead for the marks arc, since the banner is where a mark would go.** **GATE 58 NEW**, written in the same pass as the ruling per S126. It asserts the property, not the spelling: it re-runs the sweep over every page and fails on any convertible occurrence, checks that no held character appears literally, and **guards its own definition** — adding a fourth character to `HOLD` fails. Control-run seven ways from a full-tree snapshot with read-back asserts: four defect shapes each fire gate 58 **ALONE** at 57 of 57 others green; a fourth-character `HOLD` drift fires alone; **an entity injected into a `<script>` body leaves gate 58 correctly silent**; and the untouched tree passes at 58 of 58 at BOTH ends with a full-tree diff proving the restore real. `regex_audit` 0 leads. `book_gates` **v1.54**, `strip_inline` **v1.2**, `next_pointer` **v1.2**, `session_versions` **v1.20.0** — whose Control G named `entity_sweep` as registered-but-not-emitted on the first run, the control working as designed. New instrument `entity_sweep.py` **v1.0**, registered. Applied S127, all MINOR — **the rendered output is byte-identical, so the visible §5b banners correctly stay put**: L01 **v03.23.1** · L02 **v03.15.1** · L03 **v03.33.1** · L04 **v04.23.1** · L05 **v04.22.1** · L06 **v04.27.1** · L07 **v04.26.1** · L08 **v04.23.1** · L09 **v05.20.1** · L10 **v02.21.1** · L11 **v02.22.1** · L12 **v01.24.1** · L13 **v02.22.1** · L14 **v02.27.1** · L15 **v02.23.1** · L16 **v02.15.1**; `going_deeper` **v01.6.1**, Maker **v2.45.4**, Timer **v1.3.2**. No stylesheet movement: `build_css --check` current at 604 rules, family map unmoved at 1069, census unchanged at **40,700** [lesson_inventory.py].) Prior: **v8.118** (v8.118, S126, moderate — **§27.15e NEW: THE DARK CODE BLOCK GRADUATES, AND A BORDER 398 BLOCKS CARRIED FOR TWO YEARS TURNED OUT TO BE INVISIBLE.** The third and largest graduate: **802 `<pre>` elements** that **fourteen** generated class names stood for, and the fourteen disagreed. Padding was unanimous at 15px; colour, line-height, font-size and overflow-x were each 98–99% one value, so those are drift and normalise. **TWO declarations were a real split and DJ's eye is what resolved the first one.** Border: 398 blocks carried `1px solid #333`, 289 carried none — and that is **the L09/L10 SEAM, not a preference** (L02–L09 bordered 365 of 372; L10–L16 bordered 29 of 281), the same seam S108 found for the §6.5 Box and S123 for the mono stack. **Shown four rendered specimens, DJ said they all looked the same — and the measurement agrees: `#333` on `#1e1e1e` is 1.32:1 against a 3:1 WCAG minimum for a non-text boundary.** The aggregate 57/42 was a true number telling the wrong story; there was never a majority to defer to, only two conventions. Ruled to a border that earns its place. **THE FIRST VALUE PROPOSED WAS WRONG AND ITS OWN GATE CAUGHT IT: `#686868` was computed by hand as "3:1" and is 2.992:1.** Gate 57 asserts the RATIO rather than the spelling, so it failed before the push; a gate checking for the literal `#686868` would have certified an invisible border as the ruled one. **`#696969`** is the lowest grey that genuinely clears 3:1 and is indistinguishable by eye from the value it corrects. Radius ruled **6px** (389 against 265 at 8px). **NO OPT-OUT AND NO EXCEPTION LIST.** The 30 light `#f8f9fa` blocks need none — they declare background, border, radius and padding themselves, so their class beats the element rule on every property it sets, checked rather than assumed. **The 109 blocks that sat inside a dark WRAPPER `<div>` in L02/L03 were UNWRAPPED rather than excepted**, because an opt-out is an exception someone must remember forever; all 109 held exactly one `<pre>` and nothing else, verified across all **eight** wrapper spellings — a population first sampled at ONE and reported as two spellings, §24.8 again. Six bare `<pre>` in L02 join the rule too: three carry `tok-*` syntax spans and are simply code, and three are expected serial output, where **the dark block was already the book's convention**, measured rather than ruled from the three in hand. **§27.15b FIRED FOR REAL, WHICH IS THE ENTRY'S FIRST PROCESS FINDING.** 176 `<pre>` joined `.p-m-0`'s 120 users, `<pre>` became the dominant tag, the rule renamed to `.pre-m-0`, and **119 `<p>` plus one `<ul>` went dead** — caught by `strip_inline --verify`, invisible to every gate, exactly as §27.15b predicts. **The tree was REVERTED to the pushed clone and re-derived with the survivor rename in its correct slot, not patched forward**, and the rename mapped survivors by **identical declarations**, never by name arithmetic — which is what caught the second one, `.div-bg-1e1e1e-2` taking the name `.div-bg-1e1e1e` after the original holder died in the unwrap. **AND THE CONTROL HARNESS COMMITTED S121'S ERROR A SECOND TIME, IN A NEW PLACE.** It snapshotted the stylesheets but not the lesson file one injection touched, and the cleanup then ran `git checkout --` on `Lesson_05.html` — which reverts to **HEAD**, and none of this work was committed, so it silently destroyed that lesson's share of the build. **The untouched-tree control is what exposed it, by failing — the one control whose only job is to have nothing wrong with it.** §12's rule restated on harnesses: **a harness must snapshot every file it can touch, and version control is not a snapshot for work that has never been committed.** Redone from a full-tree snapshot, six ways, each firing gate 57 **ALONE** once the stylesheet is regenerated — including on `#686868`. **§27.11 MOVES TO 604 / 2,141 AND RECONCILES EXACTLY:** 23 rules die carrying 148 declarations, 3 are born carrying 3, one is altered gaining 4, and −148 + 3 + 4 = −141. **PROOF NOTHING ELSE MOVED:** every lesson expanded against its own stylesheet before and after, and **every changed line is a `<pre>`, a removed wrapper `<div>`, or whitespace — ZERO others across all sixteen.** `book_gates` **v1.53**, 57 gates; `regex_audit` 0 leads. Applied S126, all sixteen MODERATE (every page renders differently, so both §5b homes move): L01 **v03.23.0** · L02 **v03.15.0** · L03 **v03.33.0** · L04 **v04.23.0** · L05 **v04.22.0** · L06 **v04.27.0** · L07 **v04.26.0** · L08 **v04.23.0** · L09 **v05.20.0** · L10 **v02.21.0** · L11 **v02.22.0** · L12 **v01.24.0** · L13 **v02.22.0** · L14 **v02.27.0** · L15 **v02.23.0** · L16 **v02.15.0**. Census 41,128 → **40,700** [lesson_inventory.py] — the 428-line fall IS the unwrap, 109 wrappers at four lines each minus the blanks it closed. Family map unmoved at 1069.) Prior: **v8.117** (v8.117, S126, moderate — **§27.12 EXTENDED AND §27.15d NEW: A PAGE CAN STYLE ITSELF AND STILL BE PART OF THE BOOK, BUT ONLY IF THE RESOLVER CAN SEE IT.** Three jobs, and each one found the next. **FIRST, §27.15a's *no exception list* was not true of the tree.** Five inline `<code>` elements in L03 still carried a class — the last five in the book — invisible to S124's strip because that sweep enumerated the class names it knew and these three are spelled differently (§24.8). Two of them, `.code-inline-bg-f5c6cb` and `-f5c6c0`, are the SAME pink one digit apart on the same construct: a typo, not two colours. DJ: *"Fix the typo, all should be consistent in the entire book."* Fixing the SPELLING would have preserved an exception the ruling forbids, so all five classes were deleted and the elements take the ruled wash. §27.11 moves to **624 / 2,282**, and the −15 declarations IS those three rules at five each — zero born, zero altered, so §27.15b's rename trap does not fire. **SECOND, §27.15d NEW: THE RESOLVER KNEW EXACTLY ONE STYLESHEET.** `expand_classes` read `css/book.css` and nothing else, so a page styling itself from its own `<style>` block was OPAQUE to every gate that reads through it — the class stayed a class and resolved to nothing. **The consequence was concrete and nobody had seen it: `going_deeper.html`'s hero satisfied §25.6 ONLY BECAUSE IT WAS WRITTEN INLINE.** Inline and expanded-from-`book.css` produce the same string; a class the resolver cannot see produces neither. So the seven inline `style` attributes S125 recorded as a scope question were not leftovers at all — six of them were load-bearing for a gate. **DJ ruled A: teach the resolver the page's own `<style>` block.** `lesson_inventory` **v1.3.0** gains `page_css()`, merged with the page winning any collision, which is the seating §27.15c already ruled. **It is INERT for the sixteen lessons, measured rather than assumed: not one carries a `<style>` block at all.** going_deeper's hero and footer convert to classes whose declarations are byte-faithful restatements of the lessons', which is normally the two-spellings drift §27.15c rules against — **it is safe here, and only here, because §25.6 is the watcher: it compares all seventeen heroes by RESOLVED styling, so a drift in either copy fires it.** Proven by injection: one declaration moved from `padding: 30px` to `31px` FAILS §25.6 alone. The page also adopts **Inter**, closing S125's second recorded find — it was loading the webfont and rendering Arial. Four dead `.hero` rules deleted, zero uses, verified before deletion. **THIRD, AND IT IS THE ENTRY'S REAL FINDING: TWO GATES KEYED THEIR SCOPE ON A BARE SUBSTRING, AND THE REPAIR FOR THAT INTRODUCED A WORSE BUG THAT ONLY A COVERAGE ARM COULD SEE.** A COMMENT added to going_deeper's own `<style>` block, reading *…reach through `css/book.css`…*, pulled the page into §27's scope and reported eleven of its own classes as unresolved — because the predicate was `'css/book.css' in src`. **A page that merely MENTIONS a path is not a page that LINKS it.** Replaced with a `<link…href>` match — which demanded the bare path and therefore matched **ZERO of the sixteen lessons**, because they live in `lessons/` and link `../css/book.css` (§27.10, the book does not name its own host). **§27 then PASSED on an empty scope and looked exactly like a gate that was working.** It was caught ONLY because gate 44 had been given a COVERAGE arm in the same pass and §27 had none — **S117/S118 for the third time, and the first time the empty scope was one I had just written.** Both predicates now allow the relative prefix; §27 gains its coverage arm. **§27.12's SCOPE now DERIVES from EITHER stylesheet** (DJ: *"Yes watch pages that link"*), so a page joining the layer by either pipe enters automatically — S125's lesson applied one level up, since §27.15c's new delivery path had left going_deeper PARTLY converted with gate 44 structurally unable to see it. **`index.html` links NEITHER sheet and is held BY NAME** (§25.2a), because DJ ruled its one inline `style` out and **a fix with no gate is an unguarded fix**; the tool pages stay out by §25.6a. **COMPLEMENTARITY MEASURED, NOT ASSERTED (the S106 shape):** an inline style pasted into going_deeper's preamble — a span no other gate covers — FAILS §27.12 **alone** under the new scope and is **completely invisible** under the old one. Control-run from a pristine snapshot with read-back asserts, seven ways, untouched tree passing at both ends. `regex_audit` 0 leads. `book_gates` **v1.52**, `lesson_inventory` **v1.3.0**. Applied S126: L03 reaches **v03.32.0** (moderate — four pills stop being pink, so both §5b homes move); `going_deeper` reaches **v01.6.0** (moderate — body face and hero markup both change). Census unchanged at **41,128** [lesson_inventory.py]; family map unmoved at 1069.) Prior: **v8.116.1** (v8.116.1, S125, minor — **§27.15c IS NOW HELD BY GATE 56, AND IT SHIPPED UNGATED FOR ONE COMMIT.** The ruling was made and applied, and **nothing guarded the path it created.** The sixteen lessons reach the semantic layer through `css/book.css`, which gate 54 holds byte-for-byte; `going_deeper.html` reaches it through a direct `<link>` that no instrument in the tree could see. **MEASURED BY INJECTION BEFORE THE GATE WAS WRITTEN, and each of the four left ALL 55 PRECEDING GATES GREEN AND EXIT 0:** delete the `<link>` and 71 inline `<code>` silently lose the ruled pill; restate `code { background: … }` in the page's own `<style>` and one ruled construct has two spellings again, which is the exact drift §27.15c ruled against; point the `href` at a path that does not exist and the pipe is decorative; seat the `<link>` BELOW the `<style>` block and the layer starts overriding the page instead of the other way round. **THE TWO INSTRUMENTS THAT LOOK LIKE THEY SHOULD HAVE COVERED IT ARE BOTH STRUCTURALLY BLIND, §24.8 TWICE:** `site_parity` compares what the site SERVES against the repo, and a page that stops REFERENCING a file still matches; `font_stack_sweep` rewrites non-compliant stacks, and an ABSENT stack is not non-compliant. **The consumer set is NAMED, not counted** (§25.2a), and the gate carries a COVERAGE arm because a gate that scans zero pages passes (S117/S118) — emptying `_SEM_CONSUMERS` FAILS naming *scanned ZERO consumer pages*. Control-run six ways from a pristine snapshot with read-back asserts (S117/S121), each of the five defect shapes firing gate 56 **ALONE** at 55 of 55 others green, and the untouched tree passing at 56 of 56. `regex_audit` run after the gate was written per S124 and returned 0 leads. **THE GENERAL SHAPE, AND IT IS THE ENTRY'S POINT: A RULING THAT CREATES A NEW DELIVERY PATH CREATES A NEW UNGUARDED PATH, AND THE SESSION THAT RULES IT IS THE SESSION THAT OWES THE GATE.** The coupling cost of §27.15c was written into the handoff as a known risk and no gate was written for it in the same pass — §24.2's shape exactly, one scale down: **write the gate in the same pass as the ruling, not in the handoff that describes it.** `book_gates` reaches **v1.51** (from v1.50), 56 gates. No lesson file changed, `going_deeper.html` unchanged and byte-verified against the pushed clone, `build_css --check` current at 627 rules, census unchanged at **41,128** [lesson_inventory.py], family map unmoved at 1069.) Prior: **v8.116** (v8.116, S125, moderate — **§27.15c NEW: `going_deeper.html` JOINS THE SEMANTIC LAYER, AND ONE BOOK MEANS ONE CONVENTION.** DJ ruling C: *"I want the book to be the same throughout."* S124 recorded the page as carrying a THIRD inline-code treatment — a DARK pill, `#1e1e1e` ground, `#9cdcfe` text, a border, `.88em` — and deliberately left it unruled. Measured this session before proposing anything: **77 `<code>` elements, 71 inline and 6 inside `<pre>`**, and **not one of the 71 carries a class attribute**, the same bare-element population that was invisible to S124's first sweep. **THE S124 TRAP DOES NOT FIRE HERE AND THAT WAS CHECKED RATHER THAN ASSUMED:** the grounds are **62 on `#ffffff` and 9 on `#e3f0e5`**, zero dark, zero `<th>` ancestors, and no container in any chain declares `color: white`. **The content is the lesson construct**, not a different one — 22 identifiers, keywords and types, 19 calls and signatures, 9 expressions, 7 compiler-error strings, 5 literals, 5 preprocessor directives, 4 filenames — and the one shape that could have been a divergence is not: **compiler-error text marked as `<code>` matches lesson practice at 23 bare occurrences across L01–L16**, `'calibrateSensors' was not declared in this scope` appearing verbatim in both places. **THE ARGUMENT AGAINST A DELIBERATELY DIFFERENT CONTEXT IS THAT THE BOOK ALREADY SHIPS THE SPLIT IT WOULD HAVE DEFENDED.** `css/book.css` carries six `.code-block-bg-1e1e1e` variants and nine bare `<code>` live inside L01's `#1e1e1e` blocks under the light pill, so **dark block plus light inline is already canon in sixteen lessons**; going_deeper was the only page where inline matched the block, and it is linked from all sixteen lessons and from `index.html`, so a student crosses that seam constantly. **A CLAIM CARRIED IN FROM THE PRIOR SESSION WAS FALSE AND DJ'S QUESTION IS WHAT TESTED IT.** Asked whether S124 had delivered the pill *by linking*, the answer measured against the tree is **no: nothing in the repo links `css/semantic.css` at all**. The lessons link `css/book.css`, which carries the semantic layer preserved verbatim at byte offset 973 — the pill arrived through a pipe that already existed. §24.6c: **a mechanism inherited from a changelog is a lead, not a finding.** **THE MECHANISM WAS RULED ON WHICH FAILURE IT LEAVES BEHIND, NOT ON EFFORT.** Rewriting the page's own `code` rule in place spells the pill in TWO files and **nothing watches the second** — gate 54 asserts the layer appears verbatim inside `css/book.css` and is structurally blind to a third copy inside a page's `<style>` block, so that option is only safe with a gate written alongside it and is therefore the MORE expensive one. Linking `css/book.css` instead was measured and rejected: **84,541 bytes and 627 lesson-specific rules pulled into a page that uses none of them, to obtain four declarations.** So going_deeper becomes the **FIRST DIRECT CONSUMER of `css/semantic.css`**, and the layer now has two delivery paths and one source. **THE COUPLING COST IS REAL AND IS RECORDED RATHER THAN WAVED OFF:** the page will inherit whatever graduates into the layer next, uninvited, and it defines its own `.callout` while callout families are a queued graduation candidate. The `<link>` is seated ABOVE the page's `<style>` block on purpose, so the page wins any collision on a declaration it sets — but only on the declarations it sets. **FOUR DECLARATIONS LEFT THE PAGE AND NOTHING WAS ADDED:** the local `code` rule, its mono `font-family`, the `pre` rule's mono `font-family`, and the page's own `pre code` reset, which the layer already holds. Six changed lines total, each edit asserted at exactly one occurrence with post-asserts that `#9cdcfe` and every local `ui-monospace` string are gone. **NO STYLESHEET MOVEMENT AND NO CASCADE:** going_deeper is not a `build_css` source, so §27.8b's three-step sequence never ran, `build_css --check` stays current at **627 rules**, §27.11 holds at 627 / 2,297 and the family map is unmoved at 1069. `site_parity` moves 139 → 140 referenced assets and still reports PARITY, because the site already serves the file. **GATE 44 STILL DOES NOT REACH THIS PAGE AND THAT IS NOW A SCOPE QUESTION RATHER THAN AN OVERSIGHT:** §27.12 keys on `css/book.css` appearing in the file, so going_deeper's seven inline `style` attributes stay outside it while the page is now PARTLY converted — recorded, not ruled. **THE VISIBLE §5b BANNER WAS MISSED AND THE GATE CAUGHT IT**, failing with *hidden=01.5 but visible=01.4* before anything was presented; the first sweep for version homes grepped for `version:` and `Version 1` and the banner reads `Version 01.4`. **A SEPARATE FIND, RECORDED NOT RULED:** going_deeper preconnects to Google Fonts and loads **Inter**, and never uses it — its only `font-family` declarations are Arial for `body` and the ruled mono stack; every lesson body is Inter, so the page is off-family in body text and pays for a webfont it discards. Applied S125: `going_deeper` reaches **v01.5.0** (from v01.4.2), MODERATE — 71 elements render differently, so both §5b homes move. Census unchanged at **41,128** [lesson_inventory.py]; `book_gates` unchanged at v1.50, 55 gates, all passing.) Prior: **v8.115** (v8.115, S124, moderate — **§27.15a NEW: THE PILL IS RULED IN, AND AN ELEMENT RULE REACHES ELEMENTS NO AUTHOR EVER LISTED.** DJ ruling: *"Pill everywhere."* The second graduate into `css/semantic.css`, and the first to leave by DELETION — **2,132 class attributes stripped from all sixteen lessons**, ten rules dead, one born, net −9. **The ruling was taken TWICE on purpose**: once for inline code generally, and again explicitly for the 28 `.code-ff-uimonosp` uses that had drifted to mono-without-pill, on the evidence that **36 operator `<code>` elements elsewhere in the book were ALREADY pilled against 12 that were not** — the same operator appearing both ways, the 12 confined to L02 and L03. There was no operator convention to preserve, only local drift, so the rule ships with no exception list and no exception a gate must encode. **THE L09/L10 SEAM THE HANDOFF DESCRIBED IS NOT CLEAN**: re-measured without inheriting S123's figures, every back-half lesson still carried pills (L10 27, L12 27, L13 20, L14 16, L16 9) and every front-half lesson carried bare ones (L02 24, L08 23, L07 18); L01 is 52% bare, not the 49% carried forward. **SIX CLASSES STOOD FOR ONE CONSTRUCT AND DISAGREED ABOUT IT** — `.code-inline-bg-e8e8e8` through `-6` shipped three font sizes (0.9em, 0.85em, and none, which renders 1em), one added `color:#333`, one added `white-space:pre-wrap`. All six dropped; the normalisations are the POINT of one pill, and the pre-wrap was **verified inert before it went** — its string has no repeated spaces and no newline. **THE GROUND IS `rgba(0, 0, 0, 0.08)`, NOT AN OPAQUE GREY, DJ RULING B.** Inline code sits on **43 distinct backgrounds** and only 46% of it is on the cream page; **686 pills already sit on 27 coloured grounds**, so neutral-grey-on-coloured was already the status quo. **Eight `<code>` elements sit on DARK grounds whose containers each declare `color:white`** — seven bare ones in dark table headers (L04 `#a04a5f`, L05 `#352405` ×2, L10 `#00393d` ×2, L12 `#433014` ×2) and L14's score formula in a `#1f2a3d` banner — and an opaque pill renders white text on light grey in all eight, silently. **THE SEVEN WERE INVISIBLE TO THE FIRST SWEEP BECAUSE THEY CARRY NO CLASS**: the audit looked at the elements that still had one, found the two that did, and reported two. **§24.8 again — the population you can enumerate is not the population an element rule reaches** — and DJ found the hole by asking *"why would I want to change from code to span?"* about a proposal that was reaching for the MARKUP on a sample of one. With eight cases the markup was obviously innocent and the rule was obviously unscoped. **THE OPTIONS WERE ALSO MISPRICED TO DJ AND HE CAUGHT THAT TOO.** A translucent wash was first recommended as *the conservative choice* and it is the opposite — it repaints all 3,336 elements including 2,097 already ruled, where an opaque pill plus a scoped opt-out changes 1,233. Then, told the wash was *more work*, the correct answer was that it is a ONE-LINE value change, identical labour: **3,336 is a count of what CHANGES, not of what it COSTS**, and conflating the two nearly lost DJ the option he preferred. **THE `pre code` RESET IS PART OF THE RULE.** 15 bare `<code>` live inside `<pre>`, nine in L01's `#1e1e1e` blocks; an unscoped background paints a pill inside every dark code block in the book. **Gate 53 checks declarations that EXIST, §27.13 checks the GENERATED block, and gate 54 checks the layer is preserved VERBATIM — a verbatim copy of a wrong rule passes all three.** **GATE 55 NEW**, holding the reset and asserting the ground is still a wash; control-run three ways, each firing gate 55 ALONE: reset deleted FAILS, ground made opaque FAILS, reset that does not clear the background FAILS. **`going_deeper.html` REACHED THE SAME RESET INDEPENDENTLY** and is recorded as carrying a THIRD inline-code treatment nobody had counted — a DARK pill, `#1e1e1e` ground, `#9cdcfe` text, a border, `.88em`, across 71 inline uses — **not ruled**, because that page does not link this stylesheet. **THE GENERATOR PROPERTY THAT COST A FULL REDO, AND IT IS NEW CANON (§27.15b): STRIPPING A CLASS FROM SOME OF ITS USERS RENAMES IT FOR THE SURVIVORS.** `build_css` names a rule after its DOMINANT TAG, so when four `<code>` elements left `.code-ff-uimonosp-2`, the three compiler-error `<span>`/`<strong>` elements sharing it were renamed to `.span-ff-uimonosp` and went dead. **No gate saw it; `strip_inline --verify` did.** The recovery order is load-bearing: **strip → regenerate → move the survivors onto the name pass 1 emitted → regenerate again.** Renaming after a SECOND pass is unrecoverable, because the generator can only preserve a class it can still resolve — that mistake was made, and the fix was to revert and re-derive from a clean clone rather than to patch forward. **§27.11 MOVED TO 627 / 2,297**, and the −9 is fully accounted: ten rules die (six pill variants, three mono, `.code-c-white`), one is born (`.span-ff-uimonosp`). Declarations fall further than rules because the six disagreed. **THE WHOLE BUILD WAS RE-DERIVED FROM A CLEAN CLONE AND DIFFED AGAINST THE FIRST ATTEMPT**: all sixteen lesson files and the entire generated block came back **byte-identical**, which is the strongest available evidence that the strip, the survivor rename and the thirty-two version homes are right — two independent runs agreeing beats either run verifying itself. `book_gates` **v1.50** (55 gates); `regex_audit` caught a match-and-discard alternation in gate 55 on its first run and it was made non-capturing, restoring 0 leads. Applied S124, all sixteen MODERATE (every page renders differently, so both §5b homes move): L01 **v03.22.0** · L02 **v03.14.0** · L03 **v03.31.0** · L04 **v04.22.0** · L05 **v04.21.0** · L06 **v04.26.0** · L07 **v04.25.0** · L08 **v04.22.0** · L09 **v05.19.0** · L10 **v02.20.0** · L11 **v02.21.0** · L12 **v01.23.0** · L13 **v02.21.0** · L14 **v02.26.0** · L15 **v02.22.0** · L16 **v02.14.0**. Census unchanged at **41,128** [lesson_inventory.py]; family map unmoved at 1069.) Prior: **v8.114** (v8.114, S123, moderate — **§27.15 NEW: THE SEMANTIC LAYER. THE STYLESHEET COULD NOT SAY WHAT ANYTHING IS.** DJ, cutting through a ruling I had framed as a paint choice: *"I thought since we weren't using canvas we didn't need the inline code."* **Correct, and measured — the lessons carry ZERO inline `style` attributes and 25,752 `class` attributes.** The inline representation `build_css` round-trips through exists only inside one run of the generator; §27 retired the constraint that produced the model and the model outlived it. **THE CONSEQUENCE IS ARCHITECTURAL, NOT COSMETIC:** because names are re-derived from VALUES every run, the generated block can only reproduce the status quo — `.code-inline-bg-e8e8e8` can never become `.code`, and no element selector, semantic class or custom property can survive in it. **It is a fixed point of the migration, not a design system**, which is why there was nothing a second book could inherit and why `ZUMO_COLOR_LEDGER.md`'s first item is not a colour but whether custom properties can be emitted at all. One disease, two symptoms. Measured: **540 of 636 class names encode a value**, covering 22,816 of 25,752 uses. **`css/semantic.css` IS THE ONE STYLESHEET FILE HAND-EDITED ON PURPOSE**, preserved verbatim at the top of `css/book.css` and never re-derived; `book.css` stays DO-NOT-HAND-EDIT (§24.12). **THE CHANGE IS ADDITIVE BY CONSTRUCTION: zero lesson edits, all 540 value-named classes keep working, and a tree without `semantic.css` still builds.** Retiring the round-trip outright is explicitly NOT ruled — it would touch 540 class names, seven instruments reading `expand_classes` and 25 gate references — so the door is open and walking through it is optional, incremental and stoppable. **FIRST GRADUATE: `code, pre`.** 1,199 bare `<code>` elements across L10–L16 were reached by NO author rule, so their typeface was the browser's rather than the book's, while 2,136 styled ones in L01–L09 carried the ruled stack via a class — one construct, two conventions, **split at the L09/L10 seam, the same seam S108 found for the §6.5 Box**. The rule now reaches **3,336 inline `<code>` and 832 `<pre>`**. It declares `font-family` and nothing else: **the grey pill is unruled and deliberately deferred**, and when ruled it is ruled in ONE line for every book. **GATE 53 COULD NOT HAVE FOUND THIS AND SAID SO BY PASSING** — it checks declarations that EXIST, and an unstyled element has none (§24.8, on a gate shipped four hours earlier in the same session). **GATE 54 EXISTS BECAUSE THE FAILURE MODE IS SILENT:** measured by injection before the layer was built, an element rule pasted into `css/book.css` did not error and did not warn — the next regeneration DELETED it and every gate stayed green. It asserts presence, byte-equality with the source, both markers, at least one element selector, and no value-named class. **Its independence is MEASURED:** adding a value-named class to `semantic.css` and regenerating leaves `build_css --check` reporting *current* and §27.13 green, and gate 54 alone names it — the S106 complementarity shape. **§27.11 AND CONTROL D ARE NOW SCOPED TO THE GENERATED BLOCK, SCOPED RATHER THAN WEAKENED.** Scoping closed a mixed population this session created: the baseline counted class rules but every declaration, so one added element rule read as *"a rule gained or lost a property"* — the precisely wrong diagnosis in the gate's own words. **Rules and declarations hold UNCHANGED at 636 / 2,332**, which is the proof that adding the layer moved nothing generated; only the digest moves, because its span now begins at the GENERATED BLOCK marker. `build_css` **v1.3.0**, `book_gates` **v1.49** (54 gates). No lesson file changed; census unchanged at **41,128** [lesson_inventory.py]; family map unmoved at 1069. **OPEN, NOT RULED:** whether `css/semantic.css` should carry a version home of its own, as `BookComponentStandard.md` does — it is canon and nothing versions it.) Prior: **v8.113** (v8.113, S123, moderate — **§6.5a-T EXTENDED: THE BOOK HAS ONE MONO STACK, AND THE LEAD THAT OPENED THE ARC NAMED THE MINORITY.** The queue carried a *Consolas* lead: 15 declarations in `css/book.css`. Measured, it was **422 declarations across 12 lessons** consolidating into those 15 rules, in **five spellings resolving to THREE different fallbacks off Windows and ONE face on Windows** — which is why it survived, because the only readers who could see it were the ones who could not report it. On a Mac the dark code block rendered Monaco while inline code rendered the browser default: two code faces on one page. **THEN GATE 53 RESIZED THE PROBLEM ON ITS FIRST RUN.** The book's DOMINANT code type was never Consolas at all — `'Courier New', monospace` held **21 rules and 2,825 uses** against Consolas's 14 and 459, and `font_stack_sweep` is structurally blind to it because its first face is not a named substitution risk. **§24.6c one level up: a lead names a SYMPTOM, and scoping the instrument to the lead's own vocabulary guarantees you measure only what you were already told.** The right question was never *where is Consolas* but *how many mono stacks does this book have*. That error reached DJ as a priced ruling — the A/B/C specimen was costed on 459 uses when the population was 3,294, and A was described as a downgrade when for 86% of the book it was already the status quo. Re-priced and re-ruled **B-FULL**. **THE RULED STACK:** `ui-monospace, SFMono-Regular, Menlo, Consolas, 'Liberation Mono', 'Courier New', monospace`, now on all 38 declarations. **GATE 53 NEW, AND ITS INDEPENDENCE IS MEASURED RATHER THAN ASSERTED:** injecting `ui-monospace, monospace` — safe first face, wrong tail — leaves the sweep reporting **0 rewrites** while gate 53 names the file; a third control on `timer.html` fires gate 53 and NOT §27.11, because the stylesheet baseline cannot see a tool page. **THE TOOL PAGES JOIN THE SWEEP'S SCAN**, for the reason S108 put `css/book.css` in it: four declarations in `going_deeper.html`, `newproject.html` and `timer.html` that NO instrument in the tree could see, the sweep never opening them and `build_css` not modelling them (§25.6a). The web-served exemption is now DERIVED per file — going_deeper serves Inter, the other two do not, so a blanket `.html` rule would have been wrong in one direction — and the mono target is CONTEXT-dependent, since an SVG cannot resolve the `ui-monospace` keyword. **§27.11 MOVED TWICE AND THE SECOND MOVE IS THE ONE THAT COUNTS:** 641/2,350 → 636/2,332, diffed by SELECTOR — **39 mono rules before, 10 DIED and 29 ALTERED, 39 = 10 + 29, and nothing non-mono moved.** Five of the ten are font-named classes replaced by `.code-ff-uimonosp*`; the other two, `.code-block-333-6` and `.code-inline-bg-e8e8e8-7`, **COLLAPSED into surviving twins** — proved by comparing their declarations with `font-family` removed, not inferred from the rule disappearing. `strip_inline --verify` 0 dead class names. Re-controlled after the move against a dropped `color: white;`, which still FAILS. **A REGISTRATION GAP THE SESSION EXPOSED BY WALKING INTO IT:** `timer.html` carries a version home and was registered in NOTHING — `session_versions` had never tracked it, so its version could only ever have been hand-typed, the exact §12.6 shape. Registered, and its Control G confirmed it reaches both emitted blocks. `book_gates` **v1.48** (53 gates), `font_stack_sweep` **v1.3.0** with new controls I/J/K, because every pre-existing control called `rewrite()` without a path and could therefore see none of this behaviour — an assert that cannot fail is not evidence. Applied S123, all MINOR (style only; no content or structure moved, so the visible §5b banners correctly stay put): L02 **v03.13.1** · L03 **v03.30.1** · L04 **v04.21.1** · L05 **v04.20.2** · L06 **v04.25.1** · L10 **v02.19.2** · L14 **v02.25.1** · L16 **v02.13.1**; `going_deeper` **v01.4.2**, Maker **v2.45.3**, Timer **v1.3.1**. Census unchanged at **41,128** [lesson_inventory.py]; family map unmoved at 1069.) Prior: **v8.112** (v8.112, S123, moderate — **§3.1b EXTENDED: THE TITLE IS FED, THE OPENER IS UNIQUE, AND THE APOSTROPHE IS RULED.** DJ asked for the title to be fed rather than typed, and the reason is that gate 51 already caught the drift while nothing closed it — the gate named a mismatch and a human retyped the title, which is the hand-typing the gate exists to catch, one step later. **New tool `title_feed.py` v1.0** rewrites the `<p>In <strong>Lesson N: Title</strong>,` span from the §6.5a strip, section-scoped, at an absolute offset with a byte-delta assert. **Its correctness proof is that applying it to a conforming book rewrites NOTHING**, which doubles as the cross-check against gate 51: the two derive the title separately, so a divergence turns that control red. **THE HANDOFF'S UNIQUENESS CLAIM WAS TRUE ONLY UNDER TWO CONDITIONS, AND IT NAMED ONE.** Page-wide, `In <strong>Lesson` occurs **3 times in L03, 2 in L09 and 3 in L12** — backward references. Under the colon form AND the section scope it occurs **exactly once in every lesson 01–15 and zero times in L16**. A page-wide rewriter would have overwritten L12's *"In Lesson 4 you calibrated…"* with the wrong lesson's title, which is S122's scope lesson arriving as a live hazard rather than as a maxim. **GATE 52 NEW, AND IT HOLDS A HOLE GATE 51 CANNOT SEE.** Gate 51 counts occurrences of the CORRECT opener and requires one; a section holding the correct opener PLUS a stale duplicate satisfies that count. Measured by injection — a duplicate opener spelling *Line Following* pushed into L07 left **all 51 preceding gates green, exit 0**. **Not a synthetic shape: S122 committed exactly this defect to L05**, by reading an audit result instead of the file. The gate IMPORTS the target regex from `title_feed` rather than re-implementing it (S83), so its spelling cannot drift away from the generator's. Control-run four ways from a pristine snapshot with read-back asserts (S121): clean PASSES, a duplicate FAILS naming **07: 2 §3.1b opener(s) inside one section**, a wrong target FAILS naming *points at Lesson 9, expected 8*, and a removed section FAILS. **THE APOSTROPHE IS RULED B AND THE DERIVATION IS NOW BYTE-FAITHFUL.** Three owners — gate 51's matching expression, `next_pointer.esc()` and `title_feed.to_prose()` — each rewrote an ASCII apostrophe to `&rsquo;`, so a title called *derived* was transformed in one character; the lossy step had been recorded and deliberately parked since S121. Exactly one of sixteen titles carries an apostrophe. **Measured before ruling: L11's *Time Lies, Distance Doesn't* was spelled STRAIGHT in 21 places** — the strip attribute in all sixteen lessons, plus L11's own `<title>`, `<h1>` and §5b footer, L16's table cell, and a JS string in `newproject.html` — **and CURLY in exactly 3, all inside `Lesson_10.html`**. The transform was making the book's dominant spelling the minority one on the single page that generated it. Priced against the alternative: curly everywhere touches 17 files including a `<title>` tag and a JS string literal where the entity renders as its own source text. DJ ruled the straight form; **all three owners moved in the same commit**, because one left behind re-creates the drift on the next apply. One spelling now stands at **24 of 24**, verified by re-measuring the whole book rather than by inspecting the three sites edited. The ampersand branch is untouched and still required, since the strip attribute already carries `&amp;`. **THE THREE SITES HAD THREE DIFFERENT OWNERS**, which is the practical finding: the opener belongs to `title_feed`, the link block to `next_pointer`, and the NEXT LESSON callout to nobody — authored prose, hand-edited. A generator arc that assumes every occurrence of a derived string is generated will leave the authored one behind. **NO STYLESHEET MOVEMENT:** the edit is prose-only, no class changed, so §27.8b's three-step sequence never ran and `build_css --check` stayed current at 641 rules; family map unmoved at 1069. `book_gates` **v1.47** (52 gates), `next_pointer` **v1.1**, `session_versions` **v1.19.0** — its Control G named `title_feed` as registered-but-not-emitted on the first run, which is the control working as designed. Applied S123: L10 **v02.19.1**, MINOR — three characters change and the page structure does not, so the visible §5b banner correctly stays at 02.19. Census unchanged at **41,128** [lesson_inventory.py].) Prior: **v8.111** (v8.111, S122, moderate — **§3.1a AND §3.1b NOW HAVE NUMBERED SECTION BODIES, AND §3.1b IS NEW.** v8.110 recorded as canon debt that *§3.1 itself has no numbered section body*; **measured against the file, that was false** — §3.1's body has sat under §3 since v8.109. Only §3.1a was bodiless. §24.6c: a claim inherited from a changelog is a lead, not a finding. **§3.1b NEW: every lesson 01–15 carries the canonical *What's Next?* section**, DJ ruling S122 after a census found the book ending **four different ways** — seven lessons with a heading in THREE spellings, five with a bare `Next:` paragraph, one stacking both, four just stopping. **The shape is a FLOOR** (§25.8): heading, a bold opener naming the next lesson, at least one list, seated above Engineer's Log; a lesson carrying more keeps it, and **nothing was deleted to reach conformance** — the five `Next:` paragraphs were MOVED inside and L01's block relocated. **The title is DERIVED from the §6.5a strip**, which caught L07's prose saying *Line Following* and L08's saying *Intersections and Dead Ends* while the generated link block ninety lines below the same page said *Line Following with P-Control* and *Intersections & Dead Ends*. **THE SESSION'S REUSABLE FINDING IS SCOPE:** three separate false results came from asking a SECTION question with a PAGE-wide instrument — a duplicate opener added to L05 where one already existed, a stray-`Next:` flag that could not tell *moved inside* from *left outside*, and an assert that counted L12's BACKWARD references. Gate 51 is section-scoped throughout and caught L09's opener buried mid-paragraph after a hand-rolled audit passed it. **Tier-1 normalizations from the same census:** 17 blocks to one spelling of `🧠 THE LOGIC (Pseudocode)`, **89 back-to-top links to one markup** of 237, and **98 `&#9744;` entities to the literal ☐** per v8.104. **§27.11 moved twice and the second move DIED RULES:** 643/2,357 → 641/2,350, `.link-c-2e86ab-3` and `.link-c-2e86ab-4` gone and the −7 declarations ARE those two rules — both existed only to shrink the back-to-top link, and zero uses remained, **verified by grep across every page rather than inferred from the rule disappearing.** **§26.10 NEW: the colour ledger is `ZUMO_COLOR_LEDGER.md`**, opened with 16 items and the blocking measurement that **155 hexes appear inside 562 class names**, so a repaint is a RENAME. The Engineer's Log stripe split (`#0e1a2c` L01–L11, `#6f7582` L12–L16) was **PARKED, not ruled** — DJ: *"We haven't gone through and done all the colors have you?"* `book_gates` **v1.46.2**, 51 gates. Applied S122, all MODERATE except L05 (id-only, invisible, MINOR): L01 **v03.21.0** · L02 **v03.13.0** · L03 **v03.30.0** · L04 **v04.21.0** · L05 **v04.20.1** · L06 **v04.25.0** · L07 **v04.24.0** · L08 **v04.21.0** · L09 **v05.18.0** · L10 **v02.19.0** · L11 **v02.20.0** · L12 **v01.22.0** · L13 **v02.20.0** · L14 **v02.25.0** · L15 **v02.21.0** · L16 **v02.13.0**. Census 41,028 → **41,128** [lesson_inventory.py].) Prior: **v8.110** (v8.110, S121, moderate — **§3.1a NEW: EVERY LESSON 01-15 ENDS WITH A WORKING LINK TO THE NEXT LESSON (DJ ruling: *"Yes, fix it completely"*).** S120 left the forward pointer as an open ruling and the measurement that reopened it found something the construct census could not see. **Counting CONSTRUCTS, seven lessons carried neither a *What's Next* heading nor a `Next:` paragraph. Counting CONTENT, thirteen of fifteen already named the next lesson in their final quarter** — the pointer was nearly universal and nearly invisible, living in the last sentence of whatever block happened to end the lesson. Only **L11 and L12** had no forward reference of any kind, L11's sole tail mention pointing *backward* to L10. So the ruling is not *add a pointer to seven lessons*; it is *the practice is already the norm, and two lessons are missing it.* DJ: *"keep the 2 new ones like the other 13."* **THE REAL DEFECT WAS NOT THE POINTER BUT THE DEAD AFFORDANCE.** Measured by parse: **zero of sixteen lessons contained a single `<a href>` to any lesson file anywhere in their tail.** A student reading *"In **Lesson 7: Code Organization**, you'll learn…"* sees bolded text shaped exactly like a link, clicks it, and nothing happens. **THE SEAT IS THE RULING'S OTHER HALF, AND IT IS A PEDAGOGICAL CHOICE RATHER THAN A TYPOGRAPHICAL ONE.** The forward-pointer PROSE sits at **84% of the page**, above Engineer's Log — the entry that feeds the TDP, which `ZUMO_Syllabus_WORKING.md` weights at 25% of the grade — and above the bonus block, glossary and quick reference. A link there routes a student out of the lesson *before the graded work*, which is §3.1's failure one scale down: a lesson ending itself early instead of the book ending early. **The block therefore sits immediately above the §5b footer**, the last element on the page and the only seat that exists, exactly once, in all sixteen lessons (the `end Image Index wrapper` comment was the obvious alternative and is present in only nine). **GENERATED, NOT REPAIRED**, on the §6.8a precedent: new tool **`next_pointer.py` v1.0**, whose titles are DERIVED from the §6.5a lesson strip after asserting the strip is byte-identical across all sixteen — no lesson title is typed anywhere in this session's work. **L16 generates no block and the gate fails if it acquires one** (§3.1: it ends the book). **THE ANCHOR IS A TEACHER AFFORDANCE AND IS RECORDED AS ONE.** DJ ruled *"we need an anchor link in all books"*; the block carries `id="next-lesson"` in all fifteen, which gives a stable Canvas address. It is honestly labelled: the §6.5a strip is inside `<nav class="nav">` at `position: sticky; top: 0`, so all sixteen lesson links are on screen at the bottom of every page and the anchor buys navigation nothing a student lacked. **L03's orphan `id="whats-next"` is the counter-example that made the point**: the string `whats-next` occurs exactly once in the entire repo, in L03's own heading, targeted by no link on any page — and §27.14 cannot see it, because that gate asserts every LINK resolves to an id and never that every ID is reached. **§27.11's DIGEST MOVED AND THE RULES DID NOT**: 643/2,357 unchanged, zero born, zero died, zero altered, S113's shape for the fourth time. Cause is benign and worth naming because it will recur — `build_css` orders rules by usage RANK, so fifteen new uses of `.link-c-2e86ab` (398→413) and `.p-mt-22px` (29→44) relocated one rule and changed two count comments, 20 changed lines, all of them a comment or that relocation. **Every class in the new block already existed**, which is why no lesson was re-classed and §27.8b's three-step sequence never ran. `book_gates` **v1.46**, 50 gates, gate 50 control-run four ways — a deleted block FAILS naming **07**, a wrong target FAILS naming *points at Lesson 9, expected 6*, a block on L16 FAILS, and a block moved below the footer FAILS — plus the moved digest re-controlled against a dropped `color: white;`. **THE PROCESS FAILURE IS THE ENTRY'S MOST REUSABLE PART, AND IT IS §24.6b IN A PLACE THE RULE DID NOT REACH.** The first control harness restored each injection with `git checkout --`, which reverts to the **COMMITTED** tree — and none of this session's work was committed. So every *restore* silently DELETED the block instead of replacing it, and three of the four controls ran against a tree already failing for an unrelated reason. All four printed FAIL and all four looked like evidence. **What exposed it was the untouched-tree control, which also failed** — the one control whose only job is to have nothing wrong with it. Redone from a pristine snapshot of the working tree, each control isolating cleanly. **Version control is not a control harness when the thing under test has never been committed**, and a harness needs its own snapshot. **CANON DEBT, RECORDED NOT FIXED:** §3.1 itself has no numbered section body — it exists only in this version line and in the CHANGELOG, so §3.1a is seated the same way. That is precisely the shape v8.103 closed for §24.14 and v8.101 logged for §21.1: **a rule whose home is a changelog entry holds only where someone happens to look.** Applied S121, all fifteen MODERATE (the pages render differently, so both §5b homes move): L01 **v03.20.0** · L02 **v03.12.0** · L03 **v03.29.0** · L04 **v04.20.0** · L05 **v04.20.0** · L06 **v04.24.0** · L07 **v04.23.0** · L08 **v04.20.0** · L09 **v05.17.0** · L10 **v02.18.0** · L11 **v02.19.0** · L12 **v01.21.0** · L13 **v02.19.0** · L14 **v02.24.0** · L15 **v02.20.0**; L16 unchanged. Census 40,979 → **41,028** [lesson_inventory.py] — 45 lines for fifteen blocks plus 4 for the two authored pointers, which is the arithmetic check that nothing else moved. Family map unmoved at 1069.) Prior: **v8.109** (v8.109, S120, moderate — **§3.1 NEW: THE BOOK ENDS AT LESSON 16, FOR EVERY READER (DJ ruling).** The L14 flow review DJ queued at S115 found **L14 closing the course two lessons early.** Its *What's Next?* offered L15 and L16 as post-competition ***options*** in a three-bullet menu whose third bullet was *"Different competitions: sumo wrestling, maze solving, soccer"* — an after-the-book list in peer position with the capstone. **The byte chain said the opposite:** L16 §4.2 audits *"where YOUR 28,034 bytes are"*, which IS L15's finished build, so L16 hard-requires the lesson L14 called optional. Prose and code disagreed and the prose was wrong. **A SECOND ENDING WAS OFFERED AND DJ RULED IT OUT** — the calendar stops most students at L13 Step 3, and the obvious move was to author a Fall-terminal close there. DJ: *"I don't think we need to make the book tell them they are done at 13… if this book is used by others, then 13 won't be the end. It would be 16."* **The corollary is that this Bible carries NO calendar canon and must not acquire any** — measured **against the file as it stood before this entry**: `trimester`, `Fall`, `Winter`, `calendar` and `class period` each occurred **zero** times. **They occur now, in this entry and in §3.1, and that is the trap worth recording** — the first draft of this sentence asserted the zero as a present-tense fact about a file the sentence itself had just falsified, and it survived a full gate pass because no gate counts words. A word count was the wrong instrument in any case: §3.1 forbids a scheduling FACT that a lesson depends on, not the vocabulary for discussing the boundary. The term split lives in `ZUMO_Teacher_Daily_Grid_WORKING.md`, which rules its own mid-L13 break *"a calendar seam, not a book seam."* That separation is exactly what lets an adopter run a different length. **THE GATE CAUGHT THE FIX BEFORE IT SHIPPED, AND THE OLD TEXT HAD BEEN PASSING ON LUCK.** The rewritten bullets carried no terminal punctuation, so *full PID* ran on into the Lesson 16 sentence and §24's promise gate FAILED naming `L14 -> L16: promises ['PID']`. **The ORIGINAL bullets had the same run-on** and passed only because the distance to the next period exceeded the gate's 110-character window — a gate satisfied by the geometry of the defect rather than by its absence. Closed by punctuating each bullet as its own sentence, not by touching the gate. **Ungated by design:** *no lesson closes the course early* has no machine predicate, the §24.6b/§24.6c shape. **Recorded as an open ruling, measured not guessed:** the `<strong>Next:</strong>` construct exists in exactly **three** lessons (13, 14, 15) and a *What's Next?* heading in **seven** (01, 03, 05, 06, 07, 08, 14), overlapping only in L14 — whether every lesson 1–15 must carry a forward pointer, and which construct, is unruled. Applied S120, both MODERATE (the pages render differently, so both §5b homes move): L14 **v02.23.0**, L15 **v02.19.0** — L15 gaining the `Next:` pointer it had never carried, seated on L13's live placement. Census 40,978 → **40,979** [lesson_inventory.py]. No stylesheet change: `build_css --check` current at 643 rules, family map unmoved at 1069.) Prior: **v8.108.1** (v8.108.1, S119, minor — **§25.10l IS NOW HELD BY GATE 49, and it is a CONSTANT rather than a baseline.** The rule was ruled in v8.108 and nothing enforced it. BC01 measured at five items in 13 of 13 converted lessons before L15 and 14 of 14 after, and no gate held the number — so S119 came within one ruling of breaking a 14/14 norm silently, on a question asked from a false premise. **What makes this different from §21's coverage or the family map's total is that it can never legitimately move again:** the conversion arc closed at S119 and `BC_PENDING` is empty, so there is no future edit entitled to change it. If it moves, something was edited that nobody ruled on. The gate carries a COVERAGE arm as well as a count arm, because a gate that scans zero lessons passes (S117, S118). Control-run four ways: an item deleted FAILS naming **4**, an item added FAILS naming **6**, the anchor removed FAILS on COVERAGE naming 13 of 14, and the unperturbed book passes. `book_gates` **v1.45**, 49 gates. **Canon debt, recorded not fixed:** §25.10l's constant now lives ONLY in `book_gates.py`, the same shape as §21.1's thresholds.) Prior: **v8.108** (v8.108, S119, moderate — **§25.10l NEW: WHICH EXIT BLOCK EXERCISES AN OBJECTIVE IS DECIDED BY THE OBJECTIVE'S VERB, NOT BY A COUNT.** L15 is the FOURTEENTH AND LAST conversion; `BC_PENDING` is now empty. The ruling arose because a question was asked with a wrong premise and DJ ruled past it: on being told *"objective 7 has no BC01 item"* he ruled that one be added — and the premise was false, because **four of L15's seven objectives had no BC01 item**, BC01 having never been an objective map at all (BC02 is). Measured before acting: **BC01 = 5 items in 13 of 13 converted lessons**, a norm no gate holds. Three options were priced — swap at five, add to six, or one item per objective at seven — and **DJ ruled a fourth, D, on the teaching rather than the structure**: objective 7's verb is *Diagnose*, BC01 sits before §6 and asks what the BUILD depends on, and diagnostic vocabulary is what a student reaches for at the BENCH in §7 when something is already twitching. So objective 7 is exercised in **BC03**, BC01 stays at five and 14/14, and **BC03 goes to six** — legal without a ruling because §25.8 made Knowledge Check a FLOOR and L02 already ships seven. The general rule extends §25.10j from checklist rungs to OBJECTIVES: a capability claim is BC02's, an *understand why* or *diagnose* claim is BC03's, and displacing a BC01 item to make room for one is paying the wrong price. **§25.5 ON A GLYPH-LESS §2, AND L01 RULED LEGACY.** L15 and L01 are the only two lessons whose §2 carries no `☐`, and the S118 survey read that as *zero objectives* — **it was counting the GLYPH, not the objectives**. L15 has seven, verb-first, under the standard lead-in, so §25.10k's reword was unnecessary and §25.5 applied unchanged; BC02 supplies the literal ☐ and the equality assert normalises it away, which is L01's shape and not L13's. **L01's own BC02 does NOT carry L01's objectives** — six against ten, sourced from a technical-skills list §2 never states — and **DJ ruled L01 LEGACY**, pre-§25.5 rather than a competing precedent. **BC03's ANCESTOR WAS INVISIBLE TO THE NAME SWEEP, WHICH IS §25.10e REPEATING.** S117 predicted L15 as all-authoring; it is not. §10's *What you should be able to say out loud, without notes* is a five-item block doing BC03's job under a name no retired list carries, found by READING §10. Five migrated, one authored, answers and §-citations authored throughout — §7B verified to define both K_u and T_u, including that the robot supplies T_u = 2 ÷ WEAVE. **§25.10i RAN AND RETURNED NOTHING, WHICH IS ALSO A RESULT.** All 5 × 7 pairs scored: matrix max **0.182**, against L11's duplicates at 0.53–0.86 and its KEEPERS at 0.36 on the same scorer. No item folds. The scorer was control-run first against L11's four independently recorded pairings and reproduced all four RANKINGS while its magnitudes differ from S116's, **so the ranking was trusted and the S117 absolute threshold was not** — a calibration limit stated rather than assumed. Recorded and deliberately not tuned away: reading says say-2 restates Objective 3 and say-3 restates Objective 6, and the scorer sees 0.154 and 0.111 because the sentences share one content token. **BC02 CLAIM PLUS BC03 QUESTION ON ONE FACT IS THE DESIGN, NOT A DEFECT** — §25.10j's B ruling exactly: the checkbox collects the claim, the question tests it. **THE PROCESS FAILURE IS THE ENTRY'S REAL FINDING, AND IT IS AN INSTRUMENT THAT RESOLVES THE DIFFERENCE IT IS ASKED TO DETECT.** §27.8b's third step was run as `strip_inline --apply` without `--include-held` — the flag is written in the handoff in those words — which reverted the four byte-exact-across-lesson constructs from `class=` to inline `style=` in **all fifteen** other lessons. **The expansion proof PASSED on that broken tree.** Expansion resolves a class back to its declarations, so a lesson left inline expands byte-identically to one correctly classed: the test proves RENDERING equivalence and cannot see FILE-STATE divergence, and it is the very test S118 canonized for proving a rename innocent. Only the line diff saw it, at 570 changed lines. **§24.8 on a proof rather than on a gate: an instrument whose transform erases the distinction under test reports agreement no matter which way the answer falls.** Redone correctly, only **ONE** file differs from the pushed clone plus the stylesheet — S117's shape, not S118's — proved by both instruments agreeing at zero. **FOUR BASELINES MOVED, EACH CONTROLLED IN THE FAILING DIRECTION:** §21 coverage 245 → **250**, sole delta the four block icons plus the column emblem; family map 1065 → **1069**, controlled at an identical generator version with exactly ONE family moving, BRAIN CHECK 52 → 56 and the other 29 byte-identical; §27.11 digest to `f852cf656a9bda51` with rules and declarations **UNCHANGED** at 643/2,357, zero born, zero died, zero altered — S113's shape; and `'15'` leaving `BC_PENDING`, which is what §25.2a's NAMED sets buy, since it fails naming **L15**. After the moves a deleted callout FAILS naming **1068/1069**, `'15'` restored FAILS naming L15, a deleted `color: white;` FAILS, and a broken image ref FAILS. `book_gates` **v1.44.5**, `build_family_map` **v1.3.7**. Applied S119: L15 **v02.18.0**, moderate — the page renders differently, so both §5b homes move. Census 40,780 → **40,978** [lesson_inventory.py].) Prior: **v8.107** (v8.107, S118, moderate — §25.10k NEW: objectives that are noun phrases are reworded in Section 2, not in BC02; L13 is the THIRTEENTH conversion, all three non-BC02 blocks authored from nothing, and the build is a CLASS RENAME that carried nine other lessons. Full entry in the CHANGELOG below.) Prior: **v8.106** (v8.106, S117, moderate — **§25.10j NEW: A CHECKLIST RUNG THAT ASKS A *WHY* IS A KNOWLEDGE-CHECK ITEM WEARING A CHECKBOX.** L12 is the TWELFTH conversion. Its ten-rung *Exit Ticket* held four rungs no §2 objective covers and that are not bench observations either — each a knowledge claim carrying its own §-citation, and each with nowhere legal to go, since §25.5 makes BC02's **I can…** group §2's objectives character-exact and a third labelled group would be a construct invented to hold four items. **DJ ruling S117, presented as two options and ruled B:** *A, the student ticks a box claiming they know; B, the student is asked, and finds out whether they actually do.* The rung supplies the stem and the citation; the session authors the interrogative form and the answer. **The test is the VERB, not the tense** — a capability claim is BC02's, an observation claim is BC02's **I have…** group, and an *understand why* claim is BC03's, because a checkbox cannot tell a student who knows from a student who is willing to tick. This runs OPPOSITE to L07's and L09's precedent of moving an item *out of* BC03, and that is its value: L12's BC03 had **no ancestor at all** and would have been four items authored from nothing; it is now four authored from four stems the lesson wrote itself. **§25.10i APPLIED, AND IT RULED ONLY ONE PAIR.** A scored pairwise diff of all **10 × 6** combinations separates cleanly on exactly one: rung 1 scores **0.78** against Objective 1, and the next-best pair in the entire matrix scores **0.42**, so `min(dup) > max(keep)` holds on a single NAMED pair. **Unlike L11, none of L12's past-tense rungs is a duplicate** — rungs 2, 3, 4, 6 and 10 are bench observations and outcome claims, folded into **I have…** per §25.10a/b with no ruling needed, so S116's open question about *did you do it* versus *can you do it* **stays open** and recurs on L13/L15 only if either carries a past-tense checklist. **AN INSTRUMENT-VS-READING DISAGREEMENT IS RECORDED AND DELIBERATELY NOT RESOLVED:** the scorer ranks rung 2 against Objective 6 highest of the keepers at 0.42 on the shared words *robot / hands / turn / gyro*, while reading says rung 2 is a §7A bench observation and Objective 6's content lands on rungs 4 and 5 at 0.27 and 0.21. **The normaliser was not tuned to produce that pairing**; only the pair above the separation was acted on. **BC01 had NO ancestor** — zero pre-§6 `check` reveals, zero `quiz`, zero TRY IT, all thirteen pre-§9 reveals being `catchup` — so five items were authored and §-ordered §3.1→§3.2→§3.3→§3.4→§3.5, each citation read and verified to CONTAIN its answer (v8.58.1), and each deliberately non-overlapping with BC03: BC01 asks why NO encoder can ever detect slip, BC03 what the two disagreeing numbers on the OLED tell you that neither instrument could alone. **BC02 is L12's own six §2 objectives migrated character-exact** (§25.5), asserted equal in the builder, with the literal ☐ and not `&#9744;`. **BC04 has no ancestor either**; three prompts authored, no reveals, the Engineer's Log surviving as its own block after it (S80), and the 🏆 *one sentence to walk out with* plus the *Next lesson* prose kept OUTSIDE the family and reseated after BC04 on L10's arrangement. **§25.10c ACHIEVABILITY, MEASURED AGAINST §7's OWN PROSE:** §7's opening TIP supplies a no-materials fallback — *"No delrin? Hold the robot… guaranteed slip, zero materials"* — which covers 7A, 7B and 7C, all motors-off or turn-in-place. **It cannot cover 7E**, which drives four 30 cm sides and four corners. That rung is now behind the Mark-done lock; its wording was generalised from *on the delrin* to *on a slick floor*, which does not remove the dependency. **OPEN FOR DJ, and it is a fact about the room rather than the book:** if every student gets floor time the lock is earnable, and if access is rationed the rung comes out of the locked list. **THREE BASELINES MOVED, EACH CONTROLLED IN BOTH DIRECTIONS.** §27.11 to **643 rules / 2,357 declarations**, diffed by SELECTOR: exactly one gone (`.div-ddd-3`, the consumed Exit-Ticket checklist box, 5 declarations — the −5 IS that rule), **zero born, zero altered, so no class RENAME**, S116's shape rather than S115's. §21 coverage 235 → **240**, sole delta `BrainGear_Incomplete.png` 55 → 60, which is four block icons plus the column emblem. Family map 1057 → **1061**, controlled at an identical generator version: exactly ONE family moves, BRAIN CHECK 44 → 48, the other 29 byte-identical. After the moves a deleted `color: white;` still FAILS, a deleted callout FAILS naming **1060/1061**, a broken image ref FAILS, and leaving `'12'` in `BC_PENDING` FAILS **naming L12**. **ONLY TWO FILES DIFFER FROM THE PUSHED CLONE** — `Lesson_12.html` and `css/book.css`; the other fifteen lessons returned byte-identical through restore → regenerate → apply, which is measured and not assumed (S115). **THE PROCESS FAILURE, AND IT IS S116's REPEATING:** the §24.14 deletion control **misfired three times** — a regex that matched nothing, then a class absent from L04, then a class the family map does not count as a callout — and on two of those the script died before writing while the gate then printed `exit=0` against an untouched tree. Caught only by reading what the control DID rather than what the gate said afterwards. The fourth attempt added a **disk read-back assert before the gate runs**, and it fired correctly at 1060/1061. **Knowing §24.6b confers no immunity; the read-back does.** **AND A SECOND, WORSE ONE, IN THIS VERY FILE:** the first attempt at this entry used `open(p,'w')` and hit a `UnicodeEncodeError` mid-write, **truncating the Bible to 0 bytes**. Recovered from the clone in one command, and the rule §12 already carried is now paid for twice: **encode the whole blob FIRST, then temp-file + `os.replace`** — a truncating `open()` must never be reachable from a call that can raise. `book_gates` **v1.44.3** (from v1.44.2), `build_family_map` **v1.3.5** (from v1.3.4). Applied S117: L12 **v01.20.0**, moderate — the page renders differently, so both §5b homes move. Census 40,391 → **40,579** [lesson_inventory.py].) Prior: **v8.105** (v8.105, S116, moderate — **§25.10i NEW: A PAST-TENSE CHECKLIST RUNG IS NOT AUTOMATICALLY A DUPLICATE OF THE OBJECTIVE IT ECHOES.** L11 is the ELEVENTH conversion. Its *Skills Checklist* held seven rungs against six §2 objectives, and four of the seven restate an objective in the past tense — *"I converted encoder counts…"* against *"Convert encoder counts…"*. Ruled by a SCORED PAIRWISE DIFF of all 7 × 6 combinations rather than by eye: the four score **0.55–0.73** against their partner objective and the three survivors score **0.31–0.38** against their nearest, and the builder asserts `min(dup) > max(keep)` so a wrong pairing cannot pass. The four are PARKED with their pairing table, not deleted (S77). Rungs 4/5/6 are bench observations no objective covers and become BC02's **I have…** group per §25.10a/b. **The open question is recorded and NOT ruled:** the past tense asks *did you do it* where the objective asks *can you do it*, and if that distinction is worth keeping, all seven fold and the student ticks the same claim twice. **Also recorded: the achievability edge.** Folded rung *"Run 7E and watched the stopwatch version fail on a tired battery"* now sits behind the Mark-done lock and needs a TIRED BATTERY to earn — §7E mandates the run, so the lesson plants the rep, but this is the §25.10c shape where an unearnable item makes the lock unreachable. BC01 had NO ancestor (zero pre-§6 reveals, zero `check`, zero TRY IT): five items authored and §-ordered §3.1→§3.2→§3.3→§3.4→§3.5, each citation READ and verified to contain its answer. BC03 migrates all five *Reflection Questions* verbatim — asserted, not eyeballed — and authors the five answers and the five §-citations the ancestor never had. BC04 has no ancestor either; three prompts authored, no reveals, and the Engineer's Log survives as its own block after it (S80 precedent). **THREE BASELINES MOVED, EACH CONTROLLED BOTH WAYS:** §27.11 645/2,365 → **644/2,362**, diffed by SELECTOR — exactly one gone (`.div-bg-eafaf1`, the checklist's green box, 3 declarations, and the −3 IS that rule), **zero born, zero altered, so NO class RENAME this time unlike S115**; §21 coverage 230 → **235**, sole delta `BrainGear_Incomplete.png` 45 → 50; family map 1053 → **1057**, controlled at an identical generator version with exactly ONE family moving, BRAIN CHECK 40 → 44, the other 29 byte-identical. After the moves a deleted `color: white;` still FAILS, a deleted callout still FAILS at 1056/1057, a broken image ref still FAILS. **TWO PROCESS FAILURES, BOTH MINE.** (1) The 4/3 checklist split was HAND-PREDICTED and asserted — and **the assert failed**, because the normaliser stripped only *"I can/I have"* and read two real duplicates as unique; re-derived rather than tuning the normaliser to match the guess, which would have been writing the gate to the sweep. (2) A control run misfired **three times** — the injection never landed and the gate printed PASS against an unmodified tree, and that PASS was nearly recorded as evidence. **§24.6b is not "assert something changed" but assert the injection landed in the shape intended, and a control that dies before it writes is a control that never ran.** `book_gates` **v1.44.2**, `build_family_map` **v1.3.4**. Applied S116: L11 **v02.18.0** (moderate — the page renders differently, so both §5b homes move). Census 40,206 → 40,391 [lesson_inventory.py].) Prior: **v8.104.1** (v8.104.1, S115, minor — an INCOMPLETE PUSH: nine lessons rewritten by the CSS class rename were omitted, caught only by running the gates in the pushed clone. Nine MINOR bumps.) Prior: **v8.104** (v8.104, S115, moderate — **§25.2a NEW: one predicate for "converted", and the converted SET is NAMED not counted; DJ ruling: L14/L16 exempt, L10/L11/L12/L13/L15 convert; L10 is the tenth conversion. `book_gates` v1.44.1, 48 gates. Census 40,206.) Prior: **v8.103** (v8.103, S114, moderate — **§24.14 NEW: EVERY CALLOUT BLOCK RESOLVES TO A FAMILY.** The rule gate 47 has enforced since S112 finally has a section. **The rule was never missing — only its home was.** Gate 47 enforced it, `book_gates.py` carried a full comment block on why, and `ZUMO_FAMILY_MAP.md` recorded the whole S112 derivation. What did not exist was the numbered section here: §24 ran 24.13 straight to 24.15. v8.101 logged that as a debt rather than closing it — the same shape as §21.1's thresholds living only in `book_gates.py`, and the same §24.2 failure: **a rule canonized without its home holds only where someone happens to look.** The section states the ORDER — **family from CONTENT; the mark and the colour are OUTPUTS** — and records why: 252 of 1,048 blocks were resolved by HEX until S112, so a repaint moved the ground under a quarter of the map, and thirteen L12 blocks wearing one glyph and one green were three families with a correct STILL GREEN ruling **overridden by paint**. It also states plainly that the printed denominator is a **frozen baseline, not a count**, that `assigned 1049 / 1048` is therefore a line that reads like a count and is not one, and that moving the literal is legal only after a control run proves the per-family delta — because the total alone is not evidence. Transcription of an enforced rule, not new canon: no ruling changed, no lesson file changed, census unchanged at **40,015**.) Prior: **v8.102** (v8.102, S114, moderate — **§12.2 THE HANDOFF'S NUMBER IS GATED, AND THE RULE IT ENFORCES WAS ALREADY CANON.** §12.3 step 4 has said *write `ZUMO_S<N+1>_HANDOFF.md`* since v8.24, and §12.2's own `git rm ZUMO_S<N-1>_HANDOFF.md` line agrees with it. **Both were violated in consecutive sessions anyway.** the prior session wrote its OUTGOING handoff into the INCOMING handoff's filename, editing that file in place across three commits with its STATE block updated each time (`893b8b6` → `4558257` → `8ae3857`) while its title kept naming a session that had already run — so the incoming handoff under its correct name **never existed**, and the text actually pasted at session open matches no committed version of any file. The convention was then inferred from that one defective example rather than from this Bible, and the outgoing handoff went up under the wrong number. **The session number then drifted twice more the same day**, disagreeing across four hand-typed homes. **DJ caught both by reading the numbers.** `book_gates` **v1.42.2 → v1.43.0**: gate 28 now parses the number out of the FILENAME, out of the `# ZUMO — SNN HANDOFF` title and out of the *"paste at top of Session N"* clause, fails when any two disagree, and fails **loudly rather than skipping** when the title shape is missing. Control-run four ways, and two of the four are the real historical defects, not synthetic ones. **What it replaced could not see any of it:** the gate asserted only that exactly ONE handoff exists, which is satisfied by a file renamed `ZUMO_S999_HANDOFF.md` — measured, and it PASSED. **§24.2 arriving late — a rule canonized without its gate holds only where someone happens to look — and §24.8: if the answer were the opposite, the old gate looked identical.** No lesson file changed; census unchanged at **40,019**.) Prior: **v8.101** (v8.101, S114, moderate — **§24.15 NEW: A DRAFTING MODEL CANNOT COUNT THE BOOK.** `GPT_BRIEF_FORMAT.md` was written at S112 and committed to the repo root, where nothing gated it, nothing linked it and no roster named it — the §12.2 shape exactly, a standing rule stored somewhere it will drift silently. Moved here verbatim in substance and **the file is deleted**. The rule: every brief to a drafting model opens with a MEASURED FACTS block, each number naming the instrument that produced it (§24.10), and a task whose answer depends on a number not in that block is either given the number or not sent. **Also recorded, not fixed: §24.14 does not exist.** Gate 47 ships printing `§24.14 every callout block resolves to a family` against a section number this Bible has never carried — the same debt shape as §21.1's thresholds living only in `book_gates.py`. **Also S114, and it is the entry's real finding: THE FAMILY MAP'S DENOMINATOR IS A FROZEN BASELINE, NOT A COUNT.** L03's `[IMAGE 3.4]` placeholder became a real WHAT YOU SHOULD SEE callout carrying a §22 terminal block, and the generator printed **`assigned 1049 / 1048`** — a line that reads like a count and is not one, because 1048 was a literal. Controlled at the identical generator version against the pre-edit tree: the only two lines that move are WHAT YOU SHOULD SEE 27 → 28 and the total, with the other **29 family counts byte-identical**, so the delta is the one added block and nothing else. `build_family_map` **v1.3.1 → v1.3.2** moves the literal to 1049. **Parsing the true total and asserting the baseline separately is deliberately NOT done — it changes what gate 47 means, and that is DJ's ruling.** Until it is ruled, every callout added anywhere in the book fails gate 47 until that literal is edited by hand. Applied S114: L03 **v03.24.1 → v03.25.0** (moderate — a planned figure became live content, so the page renders differently and both §5b homes move). Census 40,013 → 40,019 [lesson_inventory.py].) Prior: **v8.100** (v8.100, S112, moderate — **§27.14 NEW: every link and every id resolves.** 1,237 links and 705 ids across twenty pages had no gate at all; gate 46 is parser-based and control-run on four defect shapes. Its own lesson: **where a gate sits in the file is part of the gate** — appended below the summary it printed PASS after `ALL GATES PASS` and never ran on a failing tree.) Prior: **v8.99** (v8.99, S111, moderate — the challenge-card header takes the §9 band `#7A5905`, superseding v8.87's Antique Bronze; the band palette is re-ruled at EIGHT bands with `build_palette.py` v1.1; `[IMAGE 2.5]` retires into a live code block; and the S111 handoff's *"gradients unruled"* line is corrected — v8.87 banned them and the ban stands.) Prior: **v8.98** (v8.98, S109, moderate — **§24.13 NEW: RE-DERIVE, DO NOT RE-READ — AND A LIST IN PROSE IS NOT THE LIST.** DJ ruling, on being told a taxonomy had been checked three times and was still wrong: *"So when I say double check you don't double check?"* **Re-reading is not checking** — a second look at the same artefact is the same instrument run twice, and a list with a member missing looks complete from the inside (§24.8, failed). A check is a DIFFERENT METHOD or an ASSERT against a number the artefact cannot supply. **The case:** a 17-family consolidation was computed in code, printed `CORE CONCEPT 60` with `unaccounted: 0`, then HAND-TYPED into a chat message as an 11-row table — INSIGHT's 60 blocks dropped in the retyping — and the next build read the chat message rather than the verified structure. Three passes found nothing because all three were re-readings; one line caught it, `assert tot + rem == 1048`. **This is the v3.0 ghost wearing a different noun:** §12.6 forbids hand-typing a VERSION and §24.10 requires a count to name its instrument, but neither covered a TAXONOMY, and the gap was enough — so the rule is stated on SETS. Also S109, and separate from the ruling: six INSIGHT blocks carrying the canonical magnifier wore five non-canon paints across L02/L03/L07, three of them borrowing OTHER families' canon (TIP's `#f0f7f0`, What-You-Should-See's `#d1ecf1`, Checkpoint's `#4caf50`); repainted to `#e9f7f5`/`#2da99d`, canon 31 → 37, three stylesheet rules died and all three had ZERO Bible mentions. `book_gates` **v1.39.2** — gate 27.11's label DERIVED from its own constants after shipping stale at 664/2,434 against a live 660/2,418, and **`BAND_END`** names the §10+ section band, previously typed literally in ELEVEN places; control-run by flipping it to DJ's Steel `#708BAF`, which fires **FOUR** gates — §25.10h, §4.5, §4.5a and §5.1 — where the S108 handoff recorded three and missed §5.1's `GEOM_BASELINE` keys. Applied S109, all MINOR (callout paint only, visible banners unchanged per §5b): L02 **v03.10.2** · L03 **v03.23.2** · L07 **v04.20.2**. Census unchanged at **40,025**.) Prior: **v8.97** (v8.97, S108, moderate — **THE BANNER ARC IS APPLIED.** §6.5's LOCKED *"Cap KEEPS the leading icon"* is **SUPERSEDED: no icons on any of the 237 caps**, bonus block included; all 237 carried a leading emoji and now none does, verified by DOM lookup rather than regex. **NEW §6.5b** — every cap is an EYEBROW above a HEADLINE, *headline = the most descriptive string available, eyebrow = everything before it*; 189 of 237 carry an eyebrow, the other 48 are Glossary / Quick Reference / Figures × 16. The fence rule extends §6.8a: fence = the eyebrow's name after the middot, else the headline, so the fence stays DERIVED — `_fence_title()` knows both shapes and was control-run against a converted lesson and a legacy one. **NEW §6.5a-T** — type treatment E: Inter served the way the front door already serves it, `.page` line-height 1.7→1.65 and `#333`→`#1d1d1f`, and the Windows-only Segoe UI stack RETIRED after eleven sessions in which `font_stack_sweep` reported 0 rewrites because it never opened `css/book.css`. **§4.5's derived family mark is superseded**, word retained, `gen_bonus_banner` v1.3.0, its mark assert INVERTED rather than deleted. **§6.9: `image-index` → `figures`** in three passages. **§6.5 Box CONFIRMED and unified** — the panel had shipped in two forms split at the L09/L10 seam in all five colour groups, invisible to every gate; 104 panels moved and five duplicate rules collapsed. `book_gates` **v1.39.0**, `font_stack_sweep` **v1.1.0**, `gen_bonus_banner` **v1.3.0**, `going_deeper` **v01.2.0** with six anchor ids at last. **THE RAMP WAS PILOTED AND REVERTED:** §5.0.1's Heritage Blue band ramp cannot be applied to one lesson, because five constructs are byte-compared across all sixteen — lesson strip, hero, PART dividers, bonus cap, FINISHED EARLY box — and three gates hard-code `#6c757d`. It is a book-wide change with instrument work FIRST.) Prior: **v8.96** (v8.96, S106, moderate — **§27.12 + §27.13 NEW: THE MIGRATION'S TWO UNGUARDED INVARIANTS.** **§27.12** — a page that links `css/book.css` carries NO inline `style=""`. Measured: pasting one `<p style="color: #ff00aa; font-size: 13px;">` into L05 left **all 43 preceding gates green**, and the element renders correctly while doing it, so nobody looks. Every hand-edit, pasted block and AI-suggested snippet re-opens the hole the migration closed. Scope keyed on the `<link>`, the §25.6a rule, so the four tool pages' own inline styles stay out. **§27.13** — `css/book.css` must regenerate byte-identically from the lessons. This is the guard on §27.8a/b: stop after *regenerate* and skip *apply*, and 46 class names keep their spelling while changing meaning, invisible to gate 41. **Gate 43 cannot cover it, because §26's repaint MOVES gate 43's baseline by design and a moved baseline is a spent gate**; §27.13 re-derives instead of remembering, so a repaint does not spend it. **Complementarity measured in both directions:** a hand-deleted `color: white;` fires 43 and is INVISIBLE to 45 (build_css reads the stylesheet through `expand_classes` — §24.8); one element retyped to a *different resolvable class* leaves all 43 green and fires 45. Neither subsumes the other. **`strip_inline --verify` was offered and NOT added**: it computes gate 41's assertion a second way and never fired independently of it across four controls — an assert that cannot fail is not evidence. `book_gates` **v1.38, 45 gates**.) Prior: **v8.95** (v8.95, S105, moderate — **§27.11 NEW: THE STYLESHEET IS BASELINED, BECAUSE THE MIGRATION MADE IT A SINGLE POINT OF FAILURE.** A declaration used to live in 25,036 places; it now lives once, and **nothing validated `css/book.css` at all**. `build_css --check` cannot: it rebuilds from lessons read through `expand_classes`, which reads the stylesheet — damage it and the expansion is damaged identically, so `--check` says *current*, exit 0. **Measured: deleting one `color: white;` left all 42 gates green and `--check` clean** while the lesson strip's links went dark-on-dark in all sixteen lessons. **Gate 43** baselines 664 rules / 2,434 declarations / a body digest; control-run against a dropped declaration, a changed hex and an injected rule, loud on all three, silent on a generator version bump. **The baseline is meant to move** — §26's repaint moves it, as §21's moved 218→223. General lesson: **consolidation creates a single point of failure, and the instrument that built the artefact cannot be the instrument that guards it.** `book_gates` **v1.37, 43 gates**.) Prior: **v8.94** (v8.94, S105, moderate — **§27.10 NEW: THE BOOK IS DOMAIN-AGNOSTIC.** 478 absolute `href`/`src` attributes plus **18 JavaScript `img.src` string assignments** — 496 total, all in the sixteen lessons — made relative-to-the-page. **The 18 JS refs were invisible to every attribute-shaped search** and surfaced only because the sweep's REMAINDER was audited rather than assumed. Seven off-site references remain and must (Google Fonts 4, template zip 2, jszip CDN 1): domain-agnostic means the book does not name its own host. The going_deeper gate no longer allow-lists the absolute form — it derives expected depth from the page and was control-run against both regression shapes. **NEW gate 42** fails on the domain appearing anywhere in any page in any syntax — it exists because reverting one image `src` to absolute passed all 41 preceding gates, measured by seeding it. `book_gates` **v1.36, 42 gates**.
...
v8.137, S147, moderate — **§16.17 NEW: A CITATION IS A CLAIM ABOUT AN EDITION, NOT ABOUT A NUMBER — AND THE SESSION THAT FOUND IT SHIPPED NOTHING TO THE TREE, WHICH IS THE HONEST HEADLINE.** **DJ RULED AT OPEN that the L14–L16 arc proceeds despite sitting outside the September scope** — *“so we keep the pattern the same until the end. No risk of drift.”* L14 was then read end to end and measured with **twelve compiles**, control first (L11 `after_step_1` reproduced **20,516** from a fourth independent clone). **NOT ONE CHARACTER OF THE FIX WAS APPLIED**; the whole of it is specified in `ZUMO_S148_HANDOFF.md` and the tree stands unchanged at `0e62713`. **THE BYTE CHAIN IS L13's TRAP AGAIN, NOT L12's:** L14 moves **+170 at the top and +176 at the bottom**, so two of its three deltas change — *up 738* is **744**, *734 smaller* is **740**, and only *36 fewer* at 7C survives; B2/B3/B4 remain byte-identical at **25,816** and B4's *8 bytes smaller* in match mode holds exactly (25,780 → 25,772). **Rule 49's revocation is confirmed for the third consecutive session: recompute the deltas from the compiles every time.** **SEVEN DEFECTS INDEPENDENT OF THE BASELINE**, of which two are the same shape and are the reason rule 50 gains a third instance: **two paragraphs each price the same quantity twice, with different numbers** — Step 3's *up 738 … the report card costs 830 bytes*, and B1's *734 bytes SMALLER* against a reveal reading *the 820 missing bytes*. **`830` appears in exactly one sentence in the whole tree and nothing measures it**: `selfTest()` has **no standalone symbol** in the Step 3 binary, LTO having inlined it into `setup()`. Unlike L13's 120/26 an honest number exists in both cases, so the ruling is *replace*, not *delete*. Also: Step 2's *260 lines of code* against a derived **108**; a dangling cross-reference to a **“Section 6.3” that does not exist** (Section 6 is Steps 1–4, and every other cross-ref in the lesson resolves); §4.3's *“add this to your code:”* introducing **no code**, above a callout that says *zero new code*; a **draft self-correction left in shipped prose** in §7E (*“secretly flatten a tire… no — have them cover one line sensor”*); and the Figures table's *twelve-item Competition Day Kit* against a derived **11**. **AND THEN THE SESSION BECAME A RULEBOOK INVESTIGATION, WHICH IS §16.17.** L14 cites §4.2.8 for the LoP procedure in §4.1 and §4.3.7 for the same rule in its Engineer's Log; **neither is a typo, because RCJ 2025 added §4.1 “Terms and Definitions” and pushed all of Section 4 down by one.** §4.3.7 is correct, the Engineer's Log was right, §4.1 was wrong, and **the answer was in the rulebook's changes list rather than in either lesson section** — confirmed independently by RCAP U19 v2026.0, which numbers both rules identically. **THE SAME PASS KILLED THREE INSPECTION SPECS THAT EXIST IN NO EDITION** — the 25×25 cm footprint, the height bullet and the weight bullet — and established that *must fit under obstacles* is wrong twice over, since obstacles are **≥ 15 cm** and the robot goes **around** them (§3.5.4, §3.5.6) while the 25 cm is **bridge** geometry (§3.2.3). **`ROBOCUP_RESCUE_LINE_2026.md` CARRIES THE STALE §4.2.7/§4.2.8, IS THE SOURCE OF L14's ERROR, AND IS ITSELF MISNAMED** — its content is the 2025 edition throughout. **ONE ITEM IS BLOCKED ON DJ AND WAS DELIBERATELY NOT GUESSED:** RCAP U19 is a RoboCup **Asia-Pacific** ruleset that **deletes the black victim, the red evacuation point and ramps entirely**; **the black victim is load-bearing in L13 — in scope, read, fixed and banked** — appearing as a learning objective twice, as Reflection Question 5, in `IMAGE 13.1`'s caption and as a banked question. Mercersburg is in Pennsylvania so RCAP almost certainly does not govern, but the question went unanswered at close, and **only L14 §3.2's scoring table is blocked behind it.** A third upload, *RoboCup Rescue Robot League 2026D*, is a different competition entirely and is recorded so nobody mines it again. **NO GATE HOLDS A RULE CITATION** — a fourth invariant with no gate, joining quiz banks, nav pills and byte figures. Doc only: no lesson file changed, no instrument changed, no gate changed, census unchanged at **40,605**. 70/70 gates, all instruments clean.

v8.136.2, S146, minor — **A TRIPLE CHECK BLINDING-CONTROLLED THIS SESSION'S OWN CONTROL, AND THE ENDPOINT FORM WAS BLIND (rule 59, third firing).** S146 recorded that no blinding control was run on a byte figure because nothing in the tree can see one, and that the substitute control *re-derives the chain out of the edited HTML and asserts it closes on its own stated total*. **Seeded, that assertion does not fire.** Reverting Step 5's figure alone — 25,114 back to the stale 24,946 — leaves 25,072 − 24,694 = 378 exactly true, **all 70 gates green and `gate_payload_match` PASS**, because the endpoints and the total are untouched by any change in the MIDDLE of the chain. The form that DOES fire compares every parsed figure against its COMPILE and every stated delta against its neighbours; run on the same seeded tree it names *chain != compiles*. **Both descriptions were corrected in place rather than left to be re-implemented from a changelog** — a reader of S147 building the endpoint form would have believed the chain was held. **Rule 59 asks what your control would report if the property did not exist; the endpoint form reports PASS either way.** **THE BANK SWEEP WAS CONTROLLED THE SAME WAY AND DOES FIRE:** reintroducing one superseded distractor is named by the forbidden-string sweep and is invisible to both `quiz_bank --check` and all 70 gates — which is *no gate holds a quiz bank*, measured rather than restated. **AND THE TRIPLE CHECK CAUGHT SOMETHING BIGGER THAN A CONTROL:** three files appeared in the delivery directory that this session did not author — a modified `Lesson_14.html` carrying an L14 byte fix and a version bump, a `quizzes/ZUMO_QUIZ_L14.yaml` that exists in no commit, and an edited `ZUMO_TDP_Template_v3.md` — and prose describing that work had reached the Bible, LIVE.md and the outgoing handoff. **`session_versions --check` is what named it**, reporting *LIVE.md L14 written=v02.34.0 files=v02.33.1* on a clean clone: rule 60's shape, a document naming a version the tree does not hold. **All three were quarantined out of the delivery and every deliverable was rebuilt from the one tree measured clean.** Unauthored work is not shippable work however plausible it reads — the L14 byte fix in particular is the correction S146 DECLINED on the reasoning that L14 must be fixed whole or not at all. Doc only: no lesson file changed, no instrument changed, no gate changed, census unchanged at **40,605**.
v8.136.1, S146, minor — **A DOUBLE-CHECK FOUND THREE DISTRACTORS THAT EXPLAINED THEMSELVES BY THE BOOK'S REVISION HISTORY, AND THE ONLY THING THAT COULD SEE THEM WAS AN ASSERTION WRITTEN AGAINST THE ARTEFACT.** DJ asked for a double check. Per §24.13 that is a DIFFERENT METHOD, not a second read, so the bank was re-verified by a script that parses its own header's claims and asserts a forbidden-string set against the questions. **It fired.** L13 bank A13, A15 and A16 each offered the PRE-CORRECTION figure as a distractor — *24,946 — up 232*, *24,902 — down 44*, *368 bytes* — with feedback reading *“those were the figures before the byte re-baselining.”* **A student has never seen those numbers and has no access to the book's edit history**, so the feedback explains a wrong answer by an event outside the book — against QUIZ_SPEC §4, which makes the whole contract of `cite:` *tell the student where to re-read*. **AND ONE OF THEM WAS ACTIVELY DANGEROUS: L14 still carries three `24,902` sites**, so a student reading ahead would have found the distractor corroborated. All three replaced with lesson-internal wrong answers — a flat-byte trap keyed to Step 4 being *the last free step*, a zero-cost trap keyed to the 378 total, and a SIGN error (462 = 180 + 240 + 42) that teaches the thing Step 6 exists to teach. A24's *“measured on the toolchain”* went the same way. **THE FIRST SWEEP MISSED A THIRD OF THEM, AND THAT IS THE ENTRY'S POINT.** The forbidden list held comma-formatted figures, so `368 bytes` slipped it; only adding the PHRASE *re-baselining* caught A16. **A defect class is not a list of its instances — scan for the VOCABULARY that produces it, not the values it happened to produce.** **AND THE SWEEP THAT FOUND THEM ALSO PRODUCED A FALSE POSITIVE THAT WAS READ RATHER THAN ACTED ON** (rule 38): `compiled in` matched *“compiled into both builds”*, which is correct lesson vocabulary. `avr-nm` and *disassembl* also flagged and both STAY — they are the lesson's own words in Step 2 and Mystery B2. **A text match locates; it never answers.** **THE FIX ALSO BROKE ITS OWN FILE AND THE VALIDATOR CAUGHT IT**: the replacement `why` carried an unquoted colon and `quiz_bank.py --check` failed to parse the YAML — a repair is an edit and owes the same gate as the thing it repairs. Applied S146: `quizzes/ZUMO_QUIZ_L13.yaml` **v1.0.0 → v1.0.1**; no lesson file changed, no instrument changed, census unchanged at **40,605**. 70/70 gates, `quiz_bank --check` 13 banks valid, `quiz_bank --selftest` all controls pass.
v8.136, S146, moderate — **§16.16 NEW: A SUPERLATIVE IS A CROSS-LESSON CLAIM, AND AN EXPLANATION CAN BE WRONG WITHOUT ANY NUMBER BEING WRONG — BOTH FOUND IN ONE LESSON, NEITHER VISIBLE TO SEVENTY GATES.** **DJ RULED THE FALL SCOPE AT SESSION OPEN: L01–L13, NOT L01–L12** (`QUIZ_SPEC` §9 had carried that as open since S136; spec **v1.1.0 → v1.2.0**). The ruling moved L13 out of a parked arc and onto the September critical path, and L13 was then read end to end, byte-fixed and banked in the same session. **AND THE BYTE FIX BROKE S145's COROLLARY.** S145 closed with *when the deltas are already right, the swap is safe* — true of L12, whose eighteen figures all moved by exactly 160 so not one sentence needed rewriting. **L13's chain moves +160 at Step 1 and +170 at Step 6**, measured on the toolchain against the standing control (L11 `after_step_1` reproduces 20,516 byte-for-byte): 24,694 · 24,694 · 24,874 · 24,874 · 25,114 · 25,072 against a book reading 24,534 · 24,534 · 24,714 · 24,714 · 24,946 · 24,902. **Step 3's +180 survived; Step 5's +232 is +240, Step 6's −44 is −42, and the total cost is 378 rather than 368.** Carrying the corollary forward as a rule would have shipped *up 232* over a measured 240, **and it would have gated cleanly, because nothing in this tree can see a byte figure** (S145). The corollary is revoked as a general rule: **compile every step and derive every delta from the compiles.** **§7E's 64-BYTE CLAIM WAS FALSE AND ITS EXPLANATION NAMED NO DIFFERENCE AT ALL.** The lesson said the gyro-corner and encoder-corner builds *“differ by just 64 bytes — 24,902 against 24,966, the price of keeping both turn functions alive in the binary.”* **Both turn functions are alive in BOTH builds** — the two projects differ by one call site in `main.cpp`, `turnDegreesGyro` → `turnDegrees`, twice — so the stated reason describes something the two builds share, and measured, **they are byte-identical at 25,072.** Had the 64 been right the sentence would still have been false. Rewritten to say the builds weigh the same, which makes the lesson's own point — *the size of the file will never tell you which line* — **stronger**, and puts §7E in agreement with Mystery B2 four sections later, which teaches it with a sabotage. **STEP 6's DECOMPOSITION WAS ARITHMETICALLY BROKEN INDEPENDENT OF BASELINE, AND WAS DELETED RATHER THAN GUESSED AT.** *“The rewritten hand-off and the recompiled state switch came out 120 bytes tighter … while the victim bookkeeping cost only 26”* nets **−94**, against a stated −44 and a measured −42 — wrong at the old baseline too. Symbol diff on both ELFs (`avr-nm --size-sort -S`): `main` −68, `showStatus` −50, against growth of +81 (`rescueCount` 2, `lastRowCm` 4, four strings 33, one OLED layout helper 42). **118 − 81 = 37, and the remaining 5 is section padding** — so no honest pair of numbers closes on 42 without teaching linker padding inside a lesson about rescue zones. **Rule 50 has three options and only two are legal: derive it or delete it. The third — replace it with a number that nearly works — is what put the sentence there.** **§16.16's RULE, AND IT IS THE REUSABLE HALF: A SUPERLATIVE IS A CLAIM ABOUT THE OTHER FIFTEEN LESSONS.** §4.2 and BRAIN CHECK 01's answer 3 both stated **Lesson 13 is the first lesson in this book that adds zero hardware.** A jumper/rewire sweep across all sixteen says otherwise: **L09 §4 already reads “no new hardware required”** and **L12 §4.1 reads “You do not have to plug anything in. You do not have to move a jumper.”** Both L13 sentences were narrowed to the true claim, which costs nothing — *not one wire moves, not one jumper changes* was always the interesting half. **Rule 33 again: seventy gates certify that a false sentence is well-formed.** **AND THE FIGURES TABLE UNDER-DESCRIBED GRAPHIC 13.2** (silver/white/black, where the caption and the SVG both carry green, `GREEN marker – 1200`). Corrected, and deliberately NOT asked in the bank — the plotted values live only in the SVG, and QUIZ_SPEC §5 forbids figure-only facts; B36 and B37 ask the same numbers only because §8A.1 restates the worked example in prose. **BANK L13 v1.0.1 — 96 QUESTIONS** (56 before + 40 after, 71/18/11 and 72/20/8), authored after the read and the fixes. **Seven items would have keyed wrong against v02.28.3 and one, A24, would have keyed a number that never existed.** Eight answer classes are named in the bank header as deliberately not asked, with reasons — including every 7A measurement, since no raw reading or prox count has ever been taken on this fleet and the 7A table ships blank on purpose. **WHAT WAS LEFT ALONE, WITH THE REASON: L14 CARRIES THREE ORPHANED `24,902` SITES** keyed to L13's old finished figure (lines 699, 732, 861). L14 holds eight byte figures in total, and **correcting three would leave a lesson where some figures are compiled and some are stale with nothing marking which** — worse than a lesson that is uniformly out of date and known to be. **Fix L14 whole or not at all.** **NO BLINDING CONTROL WAS RUN ON A BYTE FIGURE, ON PURPOSE (rule 59).** S145 established that nothing in the tree can see one, so a control there would fire for the wrong reason or not at all. The control that WAS run re-derives the corrected chain OUT of the edited HTML and asserts each STEP DELTA against the compiles — not merely that the endpoints close on the stated total, **which a blinding control proved blind: reverting Step 5's figure alone leaves 25,072 − 24,694 = 378 intact and all 70 gates green** — which tests the artefact rather than the intent. **THE HARNESS SETUP INSTRUCTIONS WERE ALSO WRONG AND ARE CORRECTED IN THE HANDOFF:** `LIBDIRS` names `pololu-buzzer-arduino` and `pololu-oled-arduino`, which S146 did not clone, and does NOT name `l3g-arduino` or `lsm303-arduino`, which it did; the libraries live FLAT under `/home/claude/harness`, not under a `pololu/` subdirectory, and `sudo` does not exist on this box. **Read `LIBDIRS` out of the script; do not carry a repo list forward in a handoff.** Applied S146: L13 **v02.29.0** (moderate — twelve sites including two prose rewrites, so both §5b homes move), `quizzes/QUIZ_SPEC.md` **v1.2.0**, `quizzes/ZUMO_QUIZ_L13.yaml` NEW. 70/70 gates, `gate_payload_match` PASS, `callout_id --audit` 1120/0, `keyterm_prefix` 0 to convert, `quiz_bank --check` 13 banks valid, `site_parity` PARITY, census unchanged at **40,605**.
v8.135.4, S145, moderate — **§16.15 NEW: TRIM'S HOME IS LESSON 3, BOOK-WIDE, AND THE SPLIT S144 RECORDED AS UNRULED WAS TWO SHAPES REPORTED AS ONE NUMBER (DJ ruling: *“Go for 2”* — strip the citation from the relocation comment, flip the rest).** **THE SPLIT WAS NEVER LESSON AGAINST LESSON. It was PROSE against CODE COMMENTS**: derived from the tree, every *Lesson 3* claim sat in lesson prose and every *Lesson 6* claim sat inside a syntax-highlighted comment span, with exactly ONE prose leak, L13's *Straight rows (TRIM, Lesson 6)* in a list whose other three entries each name a single teaching home. **TWO CLAIMS CARRIED FORWARD FROM S144 WERE FALSE, AND BOTH WERE TEXT MATCHES READ AS ANSWERS** (rule 38, §24.10). *L03 says both* is wrong — L03's two Lesson-6 hits are FORWARD pointers to closed-loop control (*“TRIM is the open-loop stepping stone — Lesson 6's encoders are your first taste of the real thing”*, plus an `AUTO-TRIM ALGORITHM (Preview of Lesson 6)` block that is an encoder technique). *L09 says Lesson 6* is a false positive — *“the TRIM-independent, battery-independent turn you built in Lesson 6”* describes `turnDegrees()`. **Neither lesson ever claimed a home, so the prose side was unanimous the whole time.** **THE 342 PAYLOAD COMMENTS ARE TWO SHAPES AND ONLY 194 WERE WRONG.** 148 carry a FOUR-LINE block whose fourth line already reads *“(It was born in Lesson 3, spent in Lesson 6, and filed here.)”* — a RELOCATION narrative in L07's reorg lesson, naming the file the student is actually holding when L07 opens, which per S51 is the Lesson 6 finished build byte-for-byte, and verified true here: L06 declares and uses TRIM in 34 places and links back to Lesson 03. The other 194 hang a bare `(Lesson 6)` on TRIM itself, and **a Lesson 3 credit was searched for within 400 characters of all 194 and found ZERO times.** **THE RULED FIX REMOVES A CITATION RATHER THAN FLIPPING ONE**: Group A's line 1 becomes *“// Remember TRIM? It lived at the top of main.cpp,”* and line 4 carries the history; Group B's 194 become `(Lesson 3)`. **FLIPPING GROUP A TO LESSON 3 WAS MEASURED AND REJECTED, AND THE FIRST ARGUMENT AGAINST IT DID NOT SURVIVE MEASUREMENT** — I was about to claim the flip would make the sentence FALSE, and L03's finished `main.cpp` is **227 lines** against L06's **215**, so *buried in a 200-line file* holds either way. The real cost is that flipping line 1 makes line 4 redundant, and deleting line 4 deletes the only sentence in the book that says TRIM MOVED. **Uniformity bought by deleting a true fact is the wrong trade.** **NOT EVERY SPLIT IS DRIFT, AND THIS ONE WAS BOTH — WHICH IS WHY RULE 32 DID NOT SETTLE IT.** S144 stopped at 342 on the correct instinct that a book-wide sweep on one operator's judgment is a ruling; the instinct was right and the population was MIXED, 194 sites of drift beside 148 carrying information, indistinguishable until the comments were READ IN FULL rather than counted. **A census that reports one number for two shapes has already lost the ruling.** **NO NEW GATE IS OWED AND THE EXISTING HOLD WAS BLINDING-CONTROLLED** (rule 17): reverting exactly ONE of the 342 payload sites fires `gate_payload_match` naming 1, so lesson `<pre>` and payload cannot separate and a half-swept fix cannot ship. **THE RESIDUE IS FIVE SITES AND ALL FIVE READ CORRECTLY** — two in L13 and two in L03 attach *(Lesson 6)* to ENCODERS, one in L09 to `turnDegrees()`; **a co-occurrence is not a claim.** **CANON DEBT FOUND WHILE SEATING THIS SECTION, RECORDED NOT FIXED: §16.14 HAS NO NUMBERED SECTION BODY.** v8.135.3 announces *§16.14 NEW* and the file carries no `### 16.14` line at all, so the cliff ruling lives only in a changelog entry — the exact shape v8.103 closed for §24.14 and v8.110 logged for §3.1: **a rule whose home is a changelog entry holds only where someone happens to look.** §16.12 and §16.13 are also seated BELOW §17's heading, and §16.15 is seated beside them rather than moving three sections in a pass nobody ruled. Applied S145: L07 **v04.31.4**, L08 **v04.31.1**, L13 **v02.28.3**, Maker **v2.49.2**, all MINOR — comment text and one prose parenthetical, no structure moved, so the visible §5b banners correctly stay put. 70/70 gates, `gate_payload_match` PASS, `callout_id --audit` 1120/0, `keyterm_prefix` 0 to convert, `quiz_bank --check` 10 banks valid.

v8.135.3, S144, moderate — **§16.14 NEW: A CLIFF READS BLACK, AND EVERY THRESHOLD ARGUMENT BUILT ON *IT READS WHITE* IS NOW RETIRED (DJ ruling: *“We need to correct the cliff arguments so there is no more confusion moving on.”*).** L11 §3.5 rested on *“No reflection at all — which reads exactly the same as the brightest possible white.”* **Pololu's QTRSensors header states the opposite twice and names the exact case: larger values are lower reflectance, “*a black surface OR A VOID*”**, and `readCalibrated()` clamps at 1000, so a missing floor pins every sensor at full black. **L04's TROUBLESHOOTING TABLE ALREADY SAID SO** — *“robot held in the air reads ‘dark’ because nothing bounces back… air counts as black”* — which makes this the S136 shape with the polarity reversed: **two lessons, opposite answers, one physics question, and THE OLDER LESSON IS RIGHT.** §43's *the older text is usually wrong* is a heuristic, not a rule; the rule is *read the citations*. **THE CONSEQUENCE IS WORSE THAN THE OLD ARGUMENT DESCRIBED, AND IT IS DERIVED FROM THE BOOK'S OWN CODE:** over a cliff all five sensors read ~1000, `isLineVisible()` sums them and returns TRUE, `readLine()` averages five equal weights to dead centre (`LINE_CENTER`), `followLine()` computes an error of ZERO, and **the robot drives off the edge at full speed believing it has found the widest, most perfectly centred line it has ever seen.** `handleGap()` is never called; `GAP_MAX_CM` never gets a vote. Every safeguard in the lesson watches for a line that VANISHES, and at a cliff the line does not vanish — **it arrives.** **THE OLD §8A.4 ARITHMETIC PRICED AN ALARM THAT CANNOT FIRE**, and its payoff — *the sensor is in the wrong PLACE, put it on a boom* — **dies with the premise: a reflectance sensor on a boom reads the void as black just the same.** **THE RULED FIX IS SENSOR TYPE, NOT SENSOR POSITION:** reflectance answers *how much light came back?* and both surfaces answer identically; you need something answering *how far away is the floor?* — ~3 mm over tape, the height of a table over nothing. **AND §8A.2 HAD ALREADY WRITTEN THE RULE FOUR SECTIONS EARLY AND ONE WORD SHORT** — *“is there a sensor that measures the thing I actually want? On this robot, for distance, there always is”* — true for distance TRAVELLED (the encoders), false for distance TO THE FLOOR, and that gap is the whole difference between a lesson you can finish and a rail you have to buy. **SIX HOMES CORRECTED, AND THE SWEEP WAS RE-DERIVED RATHER THAN EYEBALLED** — zero book-wide hits remain for *sees white*, *brightest possible white*, *white that never ends*, *400 ms*, *180 ms*, *25 cm/s*, *cliff arithmetic*, *wrong place* or *4.5 cm*: §3.5 (prose + subheading), GRAPHIC 11.2, §8A.4 (arithmetic replaced by a ONE-MINUTE EXPERIMENT — read white mat, black tape and open air, and compare rows two and three), BRAIN CHECK 01 Q5, **BRAIN CHECK 03 × 2**, Challenge 5 (retitled *The Cliff — Read It Yourself*), and §2 objective 6 **moved in BOTH homes with BC02 per §25.5.** **THE CONCLUSION IS UNCHANGED — *use a barrier, not software* — so L13 §4.1 and L14 both survive untouched and the cascade was contained to L11**, verified by a book-wide sweep before a word was written. **GRAPHIC 11.2 WAS CORRECTED, NOT REDRAWN:** it is 254,800 B of embedded JPEG plus ~8 KB of vector labels, and **the photographs do not lie — only the labels did**, so 24 label edits reused the two-panel layout as *WHAT BLACK TAPE READS* / *WHAT NO FLOOR READS*, verdict `1000 = 1000`. **Its closing line was KEPT VERBATIM because it is still exactly true:** *“There is no threshold that works. Not a badly chosen one — there is NO SUCH NUMBER.”* **AND THE LAYOUT AUDIT EARNED ITS KEEP: three of my new labels overflowed their panels, and a CONTROL RUN ON THE PRE-EDIT FILE returned CLEAN**, which is what proved the overflow was mine rather than one of the instrument's four known defects. **FOUR PROCESS FAILURES, ALL MINE, ALL CAUGHT BY INSTRUMENTS.** (1) **§12 ON MYSELF:** I computed slice offsets, THEN ran length-changing replacements in the same script, THEN sliced — eating `-b-white">` and duplicating a `</details>`. Tag balance and the structure gate both fired. **An edit that changes length invalidates every offset computed before it, including offsets in the same function.** (2) **RULE 46 AGAIN, AND THEN A LOSS IN THE OTHER DIRECTION:** two new callouts failed SIX gates, so they became prose — at which point `callout_id` read **1119/1120**, because the §8A.4 rewrite had also silently dropped `11.22`. **The lost id was DERIVED from the diff, not guessed**, and both original callouts were restored with their original families. (3) **SCOPE:** a blanket entity conversion hit the byte-compared lesson strip's `title="…"` — an ATTRIBUTE INTERIOR, which §27.16 explicitly excludes. §6.5a and §3.1b both fired. (4) **RULE 56 FOR THE SECOND TIME IN ONE SESSION:** `build_family_map`'s content tier pinned the literal *“This is what arithmetic is FOR”* — the exact sentence the correction removed — so fixing the prose made the block unnameable. Repointed, no tier logic changed. **A LITERAL CONTENT TIER OWES AN EDIT EVERY TIME THE SPELLING IT PINS IS THE THING BEING CORRECTED, AND THAT COST IS PREDICTABLE ENOUGH TO BUDGET FOR.** §27.11 moved a second time, **digest ONLY, 574/2,033 both ends**, class set byte-identical, usage RANK only (`.h4-c-555` 9 → 8, one fewer `<h4>` in the new §3.5), blinding-controlled and loud, and the cascade proof RE-DERIVED again. Applied S144: L11 **v02.30.0** (moderate), `book_gates` **v1.65.10**, `build_family_map` **v1.6.3**, GRAPHIC 11.2 corrected in place. 70/70 gates. **L11 IS NOW READ, FIXED AND UNBLOCKED — THE BANK IS THE NEXT THING IT NEEDS.**

v8.135.2, S144, minor — **L11 READ END TO END: A LESSON SAYS A VOID READS WHITE, AND L04 ALREADY SAID IT READS BLACK.** §3.5 — *Why We Cannot Code Our Way Off the Table* — rests on *“No reflection at all — which reads exactly the same as the brightest possible white.”* **Pololu's own source says the opposite and names this exact case, twice, in the QTRSensors header comments: smaller values are higher reflectance (white), larger values are lower reflectance, “*a black surface OR A VOID*”.** `readCalibrated()` clamps to 1000, so a missing floor pins every sensor at full black. **AND L04's OWN TROUBLESHOOTING TABLE ALREADY STATES IT CORRECTLY** — *“robot held in the air reads ‘dark’ because nothing bounces back… air counts as black”* — so this is the S136 shape exactly: **two lessons giving opposite answers to one physics question, invisible inside either one, and the OLDER one is right.** Traced through the book's own code, the consequence is worse than §3.5 describes: over a cliff all five sensors read black, `isLineVisible()` returns TRUE, `readLine()` returns dead centre (all-equal weights average to 2000 = `LINE_CENTER`), the error is zero, and **the robot drives off the edge at full speed believing it is perfectly centred on a wide line.** The gap machinery never engages at all; `GAP_MAX_CM` never gets a vote. **§8.4A's ARITHMETIC THEREFORE PRICES AN ALARM THAT CANNOT FIRE**, and its payoff — *the sensor is in the wrong PLACE, put it on a boom* — does not survive either, because a reflectance sensor on a boom reads the void as black too. **The fix is sensor TYPE, not sensor POSITION: you need something that measures DISTANCE, where a missing floor is unambiguously far.** RECORDED, **NOT YET REWRITTEN** — the conclusion (*use a barrier, not software*) is unchanged and L13 §4.1 and L14 both cite only that conclusion, so **the cascade is contained to L11**; but the argument under it is a different argument and that is a DJ ruling. **THREE DEFECTS FIXED.** (1) **STEP 6 BROKE THE BUILD AND §8 BLAMED THE STUDENT.** It said delete `unsigned long gapStartTime` and asserted *“Nothing reads gapStartTime any more.”* Derived from the `after_step_5` payload: nothing reads it, and **two lines still WRITE it**, in `FOLLOWING_LINE` and `RETURNING_TO_LINE`. C++ needs the declaration for a write, so the step yields `'gapStartTime' was not declared` twice — against §6's own promise that *“Every one of them compiles… Nothing is broken in the middle.”* §8 already carried that error and called it the student's fault, **repeating the same wrong word** (*every reader must be gone*). **“Nothing reads it” is not “nothing mentions it.”** The Maker's `finished` payload had all four sites removed and was correct throughout — the S142 L08 Step 2 shape, **Maker right, book wrong.** (2) **7E's `handleGap()` WAS TRUNCATED MID-FUNCTION**, missing the drive line and the closing brace — an unbalanced brace in *“the experiment the whole lesson was built for”*. The Maker's `cal_7e` is complete. (3) **STEP 6's BYTE CLAIM WAS WRONG IN THE OPPOSITE DIRECTION.** It read *“the finished program is smaller than the one you started with… the better answer was also the cheaper one.”* **Measured on the S143 toolchain against an exact control — L11 `after_step_1` reproduces 20,516 bytes, byte-for-byte S143's verified L10 figure — the finished build is 20,702. It is 186 bytes BIGGER.** Rewritten to say so, and the honest version is the better lesson: correctness is something you spend flash on deliberately, with 7,970 bytes still spare. **This is the first NEW claim the toolchain has caught, which is what it was built for.** **A CALLOUT IS NEVER A FREE EDIT, MEASURED AGAIN:** a seventh INSIGHT block in L11 failed **six** gates at once — §5.1 geometry, §21 image-reference coverage, §27.13, §24.14, §24.14a and §24.14c — every one a baseline pinned at six. The byte fact became prose instead and all six cleared; **the callout was not worth moving six baselines.** `callout_id` also caught the id collision before the gates did (11.15 was taken; reassigned to a DERIVED 11.41, not a typed one). **AND A CONTENT TIER THAT PINS A SPELLING OWES AN EDIT WHEN THE SPELLING IS THE DEFECT:** `build_family_map`'s INSIGHT tier matched the literal headline *‘The stopwatch has no readers left’* — the very sentence that was false — so correcting the prose made the block unnameable. Repointed, no tier logic changed, **blinding control run and loud**, `build_family_map` **v1.6.1 → v1.6.2**. (4) **TRIM WAS CREDITED TO LESSON 6 FIVE TIMES IN L11**, where Lesson 3 is *titled* “Motor TRIM — No Two Motors Are the Same” (250 mentions against L06's 39). Challenge 4 contradicted itself inside one card: **Builds on** said Lesson 3, its own Run 1 comment said Lesson 6. Fixed to 7-of-7 Lesson 3 — and **`gate_payload_match` immediately caught two Maker payloads left behind** (`cal_7b`, `cal_7d`), which is the gate doing exactly its job. **THE BOOK-WIDE SPLIT IS RECORDED UNRULED:** L01, L02, L04, L06 and L11 say Lesson 3; L07, L08 and L13 say Lesson 6; L03 says both. **L06 ITSELF CREDITS LESSON 3, THREE TIMES**, which is the strongest single signal — the encoder lesson points back rather than claiming it. 342 Maker payload comments still say Lesson 6 and were deliberately NOT swept: not every split is drift, and a 342-site sweep on my own judgment is a ruling, not a fix. Applied S144: L11 **v02.29.0** (moderate), Maker **v2.49.1**, `build_family_map` **v1.6.2**. 70/70 gates, census 40,608 → **40,616**. **L11 IS READ AND FIXED, AND UNBANKED — §3.5 MUST BE RULED BEFORE THE BANK, because six of its homes would key answers.**

v8.135.1, S144, minor — **THE SEVEN-PHASE BOX CONVERGES ON THE LINE AND THEN LEAVES IT, AND EVERY ARTEFACT AGREES WITH EVERY OTHER ONE.** S143 fixed a maneuver whose net rotation was ZERO and which could therefore never reach the line. The rebuilt box reaches it — and reaches it PERPENDICULAR. `+90`, `−90`, `−90` is a net `−90`, so `PHASE_TURN_BACK` leaves the robot facing the line square-on, `RETURNING_TO_LINE` crosses it at a right angle, and the state machine then hands straight to `FOLLOWING_LINE` **with no realigning turn anywhere in the tree.** At that moment the sensor array is PARALLEL to the tape, so all five sensors darken together, `readLine()` returns the centre value, `followLine()`'s error is zero, and the robot drives straight across the line and off the far side. **THIS IS S143's RULE 51 FIRING A SECOND TIME ON THE SAME MANEUVER, AND THE SECOND FIRING IS THE HARDER ONE.** In S143 three artefacts described a four-leg box and the code was a leg short, so the code was the outlier and the fix was a reconciliation. **Here GRAPHIC 10.2 (*LINE FOUND — follow it again*), §3.3 (*ready to cross it*), §7.7 (*resumes line following as if nothing happened*) and Sabotage B5 (which treats square-on as the CORRECT outcome) all agree with the code.** There is no outlier to defer to. **When the artefacts disagree you ask which one is the design; when they all agree you have to ask whether the design is right, and no instrument in this repo can answer that — only the floor can.** RECORDED UNRULED, and the bench item is now a specific falsifiable prediction rather than a general *drive it*: watch the two seconds after `SEEKING` flips to `FOLLOWING`. **AND THE CHALLENGE IS MORE CORRECT THAN THE LESSON** — Challenge 6's wedge meets the line at 30°, which a P-controller can pull out of; the box meets it at 90°, which it cannot. **§8.3's ARITHMETIC WAS WRONG, IN THE ENTRY S143 HAD JUST REWRITTEN.** It read *the box uses it three times, so a 10° error becomes 30°*. The box turns `+90`, `−90`, `−90` — one way and back twice — so a proportional turn error largely CANCELS: net is `90k − 90k − 90k = −90k`, and a single turn off by 10° leaves the finished heading off by **10°, not 30°.** The accumulate-logic was imported from §7D's square, where it is CORRECT because all four turns go the same way (2° × 4 = 8°), and the glossary's *Errors do not cancel* is true of that shape and not of this one. **A RULE THAT IS TRUE OF ONE SHAPE IS NOT TRUE OF EVERY SHAPE THAT LOOKS LIKE IT.** Rewritten to point at the drives first: `DRIVE_OUT` and `DRIVE_PAST` are 35 cm of open-loop travel with nothing watching the heading, and nothing cancels a curve. **§4.3's SODA-CAN COUNT WAS UNVERIFIABLE AND IS NOW DELETED (rule 50).** *Round, shiny, narrow and light — it fails four of the five rows* names four adjectives across three rows: round and shiny both land on Surface, and a can PASSES Height at ~12 cm against the table's 5 cm floor. Replaced by naming the rows it actually fails. **AND THE AMBER LEAK CLOSES ITS SECOND OF THREE.** L10 callout `10.5` carries `data-family="NOTE"` and wore `#856404`, the WARNING family's colour; derived book-wide, the canonical NOTE header is `div-c-3a5a3a` at 16 of 20 and the only two exceptions were this one and L02 §2.7 — exactly the pair S143 deferred to their own reads. L02's is still open. **THE CSS ACCEPTANCE PROOF WAS RE-DERIVED RATHER THAN CARRIED**, which is §24.13 applied to a proof instead of to a count: zero elements in the sixteen lessons carry two classes (the only 7 multi-class attributes in the tree are hand-authored pairs in `index.html` and `going_deeper.html`, outside the generated digest), so the frequency reshuffle cannot decide a cascade. §27.11 digest moved once, rules and declarations UNCHANGED at 574/2,033, the whole textual diff being `.div-c-3a5a3a` 33→34 and `.div-c-856404` 24→23 swapping rank. **The moved baseline was blinding-controlled before it was trusted** (rule 17) — an injected rule still FAILS §27.11 — and the restore was by md5 from a snapshot, not `git checkout`, per S143's refinement to rule 15. **AND TWO GATES CAUGHT MY OWN EDITS BEFORE ANYTHING WAS PRESENTED:** §27.16 on an `&minus;` entity where the literal is visible in source, and §27.13 on the unregenerated stylesheet. Applied S144: L10 **v02.29.1** MINOR — prose and one class attribute, no structure moved, so the visible §5b banner correctly stays at 02.29; `book_gates` **v1.65.9**. 70/70 gates, `callout_id --audit` 1120/0, `keyterm_prefix` 0 to convert, `gate_payload_match` PASS, `quiz_bank --check` 9 banks valid, census unchanged at **40,608**. **BANK L10 v1.0.0 WRITTEN IN THE SAME SESSION AS THE READ AND THE FIX** — 50 before + 25 after, 72/20/8 both sets, banks 9 → 10 of 16, **770 questions**. DJ ruled the WHOLE bank rather than the `before` set alone, so the `after` set is written against §6, §7 and §9 knowing some of it may be re-keyed once the arrival geometry is settled. **Six answers are deliberately not asked, and the header names all six with reasons** — chief among them the finishing pose, the crossing angle and the box-versus-wedge distances, which all sit on the unruled geometry. Three corrected byte figures are load-bearing (A06 +50, A13 the 15→5 line trade, A18's state names); against v02.27.x several would have keyed wrong. **AND THE HEADER-OF-A-THING RULE WAS PAID BEFORE IT COULD FIRE:** every question id the header names was asserted to EXIST in the file, and all five `source:` pins were compared against the live lesson versions rather than typed — the S142 defect class, caught by construction this time.

v8.135, S143, moderate — **§16.12 NEW: THE AVOIDANCE BOX IS SEVEN PHASES, ONE JOB EACH — AND THE DEFECT IT FIXES PROVES THAT A GATE CERTIFYING AGREEMENT IS NOT CERTIFYING CORRECTNESS.** L10's maneuver turned `+90`, drove `AVOID_PAST_CM`, turned `-90`. **Net rotation ZERO**, so the robot finished on the heading it arrived with — along the line — displaced perpendicular by the one drive, and `RETURNING_TO_LINE` then ran PARALLEL to the line and could only ever run `RETURN_TIMEOUT` out. **Convention-free proof: two turns that cancel cannot converge on a line they are already parallel to.** Every student who built it would have watched it fail. **`gate_payload_match` PASSED for the maneuver's entire life, precisely BECAUSE 202 payload files copied the wrong code faithfully** — which is what a PASS looks like when the source is wrong. The gate pins agreement between the book and the Maker; **nothing in the tree pins the book against physics, and something outside it has to.** **EVERYTHING EXCEPT THE CODE WAS RIGHT, AND THAT INVERTS S136's RULE.** GRAPHIC 10.2's four arrow polygons draw *out · along · back · hunt* — read from their own coordinates, not from the caption. `PHASE_DRIVE_PAST`'s comment says *Drive forward, past the obstacle*; `PHASE_TURN_BACK`'s says *Turn back toward the line*; §3.3's summary names four movements where the code had three. **The art and the names were the design document and the code was a leg short of it** — so *a fact that lives only in an SVG is not in the book* gains its converse: **when the art and the code disagree, ask which one is the design.** I came within one ruling of rewriting three correct artefacts to match one wrong one. **AND §8.3 ANTICIPATED THE EXACT SYMPTOM AND GAVE A PLAUSIBLE WRONG CAUSE** — *"The turn-back is almost certainly wrong. Go back to §7A and re-prove your 90° turn"* — where a perfectly accurate ±90 pair is what GUARANTEES the failure. **A book that contains a convincing wrong explanation for its own bug will absorb the first report from the floor and send it into the wrong file.** That is the survival mechanism, and it is worth more than the geometry. **THE RULING (DJ, S143): SEVEN PHASES, ONE JOB EACH, VERIFIED BETWEEN EVERY ONE.** `PHASE_TURN_AWAY` · `PHASE_DRIVE_OUT` (new; `AVOID_OUT_CM = 15.0`, new) · `PHASE_TURN_ALONG` (new) · `PHASE_DRIVE_PAST` · `PHASE_TURN_BACK` · `PHASE_SEEK_LINE` · `PHASE_COMPLETE`. Pose asserted after each phase rather than at the end: the box finishes 20 cm along and 15 cm out, **FACING** the line, and the hunt crosses **perpendicular after 15 cm (~1.0 s against a 5 s timeout)**, 20 cm past the block with 15 cm of clearance alongside it. The old three-phase shape was run as the control in the same script and prints PARALLEL — **a check that can only pass one of them.** Seven rather than five because §8A's own argument is one job per step, and the graphic's phase panel was re-flowed to match rather than the argument bent to fit. **DJ ALSO RULED THAT BOTH SOLUTIONS SHIP.** The box is the base build; the **wedge** — drop the parallel leg entirely and turn back PAST the original heading so the robot closes on the line at a slant — is **L10 Challenge 6**, riding the `finished` preload. Measured against each other: box 5 actions, crosses square-on after ~15 cm; wedge 2 actions, ~30 cm at 120°, all of it open-loop. **The trade is the assignment, not the answer.** The card leads with the sign trap because I fell into it in front of DJ: instinct says a SHORTER turn-back angles you toward the line, and simulation shows it DIVERGES. **112 `RobotConfig.h` and 90 `main.cpp` payloads** rewritten and re-derived from the parsed `PAYLOADS` object rather than trusted from replacement counts. **All three sabotage plants preserved**: `bonus_b3` keeps its 5 cm, `bonus_b5`'s wrong-way turn-back was rebuilt on the new shape, and the timed build now has FIVE blind `delay()` windows instead of three, which strengthens Step 8's own argument. **The gate earned its keep twice**, catching a banner fixed in the lesson but not in 90 payloads, and `AVOID_OUT_CM` living in payloads with no reveal to derive from. **§16.13 NEW: THE AVR TOOLCHAIN IS A ONE-COMMAND INSTALL, SO BYTE CLAIMS ARE CHECKABLE.** `apt-get install -y gcc-avr avr-libc` yields `avr-gcc 7.3.0+Atmel3.7.0`; the eight repos clone from GitHub and symlink beside `pio_harness.sh`. **`pololu/Zumo32U4@2.0.1` and the harness's `zumo-32u4-arduino-library` are the SAME code under two naming systems** — verified from the tag's own `library.properties`, `name=Zumo32U4` — and the other seven LIBDIRS are that library's declared `depends=`, which PlatformIO resolves transitively and the harness cannot. `platformio.ini` was never touched. **THE CONTROL WAS EXACT AND IT IS WHY THE NUMBERS STAND: the PRE-change finished build compiles to 20,364 bytes, byte-for-byte the figure the lesson printed.** **Four byte claims corrected, THREE of them wrong before this session:** Step 6 *+20* → **+2**, Step 7 *+192* → **+194**, Step 8 *+84* → **+660**, Step 9 *20,364* → **20,516**. And one claim the compile CONFIRMS: `after_step_7` is **byte-identical** before and after, so a constant and two enum members cost exactly zero — precisely what Step 2's checkpoint promises. **`after_step_8` IS NEW**, built to §15.6 as the file AS IT STANDS AT THAT STEP: `after_step_7` plus ONLY Step 8's two cases, with Step 9's `showStatus` switch and Lesson 10 banners deliberately absent. **L09 READ END TO END, EIGHT FINDINGS, THEN BANKED.** §3.4's state table said `AT_INTERSECTION` *"Stop, identify intersection type"* and it does NEITHER — both happen in `FOLLOWING_LINE`, and the Quick Reference's own rule says stopping is never deferred to a later state; **BRAIN CHECK 01 Q3 repeated the same wrong verbs**, so the lesson gave two answers to a question the bank would key. Step 6 told the student L08's main mentions `isRunning` *five* times; re-derived from the parsed object it is **ten, on nine lines** — the L02 *lines 3/7/11* shape. §7.2's Test 6 asked for a 20 cm gap and expected a crossing, where `MAX_GAP_TIME` is 500 and L08's own table says 5 cm crosses and 15 cm times out. Step 7's checkpoint opened *"B still rescues you"* and then explained why it cannot. Plus a double-escaped entity rendering as literal `do&hellip;while`, a dangling em-dash, and six TIP/NOTE callouts wearing the WARNING family's amber — **the leak also sits on L02 §2.7 and L10 10.5 and those were deferred to their own reads.** **BANK L09 v1.0.0, 50 before + 25 after, 72/20/8 both sets**, five findings load-bearing: against v05.26.1, B12 would have keyed the wrong answer and only one of A14/A15 was answerable. Banks 8 → 9 of 16, **695 questions**. **§3.6's TIMED-TURN SENTENCE WAS WRONG TWICE IN THE COURSE OF FIXING IT** — first *"used exactly once"*, then *"twice"*, where the book writes one in L03's Spin Test and timed square, L05's obstacle challenge and L10's deliberate first attempt. It shipped with **no count at all. A COUNT INSIDE A SENTENCE IS A CLAIM: DERIVE IT OR DELETE IT** — and the same session moved *five* to *seven* in eight other L10 sentences. **THE CATCH-UP CONVENTION IS SPLIT AND IS RECORDED UNRULED:** derived across all 64 CATCH-UP rows, **L07–L10 use OFFSET (`step_N → after_step_(N-1)`, the file you START the step from) and L11–L16 use IDENTITY**, perfectly cleanly. It may be deliberate, since L07–L10 all open with *copy your previous project*. **I misread it as a defect, repointed L10's `step_8`, and reverted**; the Maker changelog was rewritten to remove the false claim. **A gate written on that misreading would have certified 64 rows and failed 32** — rule 19 paid in advance, and the reason no gate is written until DJ rules. Applied S143: L09 **v05.27.0**, L10 **v02.29.0** (both moderate), Maker **v2.49.0**, `book_gates` **v1.65.8**, GRAPHIC 10.2 re-flowed with its four route arrows UNTOUCHED. 70/70 gates; the §27.11 digest moved **four times**, rules and declarations UNCHANGED at 574/2,033 every time, and the acceptance test now has a proof rather than an inspection: **no element in the sixteen lessons carries more than one class, so a frequency reshuffle cannot decide a cascade.**

v8.134, S142, moderate — **§16.10 AND §16.11 NEW: THE WHEEL GEOMETRY AND THE Kp RANGE ARE NOW CANON, AND BOTH WERE ALREADY DERIVABLE FROM THE BOOK.** §16.10 records that **39 mm is the diameter OVER THE TRACK** (sprocket alone is 35 mm), with sprocket spacing 85 mm, 12 CPR on the motor shaft and 909.7 counts per sprocket revolution at the fleet's 75.81:1 — plus the open question of whether travel follows the outer surface or the track's pitch line, which lands **inside §7's own ±2 cm tolerance** either way and is a bench item, not a correction. §16.11 rules **Kp 0.1–0.3, starting at 0.10, on the 75:1 fleet** (DJ ruling). **THE Kp NUMBER WAS DERIVED, NOT CHOSEN, AND L08 PROVED IT AGAINST ITSELF**: §3.1 states the correction range as *roughly −600 to +600*, and **600 = 0.3 × 2000**, the maximum error on the 0–4000 scale — so that sentence can only have been written against a 0.3 ceiling, and §7.3 and the Quick Reference at 0.08–0.25 were the drift. **A number that a sentence DERIVES FROM cannot be swapped without rewriting the sentence** (the S137 F=ma shape): Challenge 5's three adaptive gains had been sitting on the OLD range's endpoints and moved with it, 0.08/0.15/0.25 → 0.10/0.20/0.30. A 100:1 Kp suggestion is explicitly **NOT book content** — it lives in the teacher-facing resource doc flagged unverified, because no 100:1 robot has ever been tested here and the two physical arguments point opposite ways; §16.1's rule applies unchanged, the book speaks to the robot in front of the student. **ALSO S142, AND IT IS THE ENTRY'S REUSABLE HALF: `gate_payload_match`'s LINE TEST WAS A SUBSTRING TEST AND WAS BLIND ON ONE SIDE.** The line-wise fallback asked `l in corpus` — containment in the whole corpus TEXT — so a payload line that had **LOST a leading qualifier the lesson carries** matched trivially inside the longer lesson line. Measured, not theorised: during the S142 `static` pass, reverting exactly one of 136 payloads left the gate printing **PASS**. Additions it always saw; a dropped `static`, `const` or `unsigned` it could not see **at all** — the §15.6 class it exists to hold. Now line EQUALITY against a stripped corpus line set; control-run **five shapes × two gate versions** from a snapshot with a byte-exact restore, and the two already-caught shapes prove nothing was lost. `gate_payload_match` **v1.7.1 → v1.8.0**. **THE WHOLE `static` PASS RESTED ON THAT GATE AND THE CLAIM WAS WRONG WHEN IT WAS MADE** — the safety argument was *the gate will catch a partial application*, and it would not have; the 136 payloads were verified instead by **re-deriving them from the parsed PAYLOADS object**, and a two-translation-unit compile proved internal linkage safe. **L08 READ END TO END, SIX FINDINGS.** Step 2's *complete file* reveal had **deleted the TRIM block** while Step 8's `handleGap()` uses it — compiled against the book's own reveal, `'TRIM' was not declared in this scope`; the Maker was correct in all seven payloads and only the book was wrong. §8A.2 used raw `read()` against a **calibrated** threshold, the exact thing Step 5 teaches against, in the same lesson. §8A.4 claimed to be *the complete pattern you've implemented* while naming **three identifiers that exist nowhere** — `SETPOINT`, `MAX_CORRECTION` and a bare `Kp` — and clamping the correction where the lesson clamps the speeds; it is now Step 7's own function, asserted line-for-line identical. Step 8's hint cited *Lesson 6's timers* and **L06 contains zero `millis()`**; repointed to L05's beep-interval trick, the same non-blocking stamp-and-compare shape. And two variables called *module-private* had external linkage — both now `static`, the file-scope sense L07 gave `averageCounts()`, across **136 Maker payloads (L08–L16) and 3 lesson reveals**, with L05's single-file `jumper_check` global deliberately untouched. **BANK L08 v1.0.0 (75 questions), written in the session that read the lesson** per QUIZ_SPEC §0; five of the six findings are load-bearing in it. **THE HEADER OF A THING IS NOT THE THING, TWICE MORE — AND BOTH WERE CREATED BY THE REBALANCE PASS THAT FOLLOWED THE DRAFT**, which is the mechanism exactly: ids shift and the header keeps naming the old ones. Worse, the first verifier written to catch them checked ids against a mapping typed into the script rather than parsed from the file, so it **validated intent and printed ALL HOLD against a file that had not been written**. Rewritten to parse the header's own claims — at which point it failed, which is how it was known to work. Applied S142: L07 **v04.31.3**, L08 **v04.31.0** (moderate), L10 **v02.27.2**, Maker **v2.46.0**, `book_gates` **v1.65.4**. 70/70 gates; CSS regenerated three times, rules and declarations UNCHANGED at 574/2,033 with all 574 declaration blocks byte-identical every time.

v8.133, S141, moderate — **§15.6 NEW: A STEP PAYLOAD IS THE FILE AS IT STANDS AT THAT STEP, NOT AS IT ENDS UP.** L06's `after_step_7` through `after_step_11` shipped **Step 13's FIXED** `driveDistance()` and `turnDegrees()` — TRIM on the left motor and an `averageCounts()` gate — so the five DISCOVERIES entries that hand a catching-up student the file for **Step 8, Discovery 6.7, Step 10, Discovery 6.8 and Step 12** arrived with **both of the bugs Step 13 exists to find already repaired.** Steps 7 and 10 build those functions BROKEN **by design** (S24 ruling), and Step 13 — *The Square That Won't Close* — quotes the student's own file back at them: *"Look at what `driveDistance()` actually commands: `motors.setSpeeds(speed, speed);`"*. **Neither quoted line was in their file, and the square they were told would fail would have closed.** Cause named by the Maker's own changelog: **v2.14's TRIM pass regenerated `PAYLOADS[6..9]` wholesale** from the corrected states instead of only the states after the repair. **NOTHING COULD SEE IT FOR 117 SESSIONS**, because `gate_payload_match` asserts a payload byte-derives from **SOME** lesson `<pre>` and never from the `<pre>` belonging to the step it is NAMED for (§24.8) — a payload carrying a LATER step's code satisfies it perfectly. **GATE 70 NEW**, written in the same pass as the ruling (§24.2), predicate **DERIVED rather than pinned** (S126 rule 19): the expected bodies are EXTRACTED from the lesson's Step 7 and Step 10 reveals, and `finished`'s expectation is those bodies with Step 13's two substitutions applied, both of which Step 13 publishes as standalone single-line `<pre>` blocks — so rewording a comment moves both sides together and only a real drift fires. **ITS FAR-SIDE ARM WAS BLIND AND A BLINDING CONTROL CAUGHT IT** (S139 rule 17): the first draft asserted only that `finished` did not hold the BROKEN body and that the words `TRIM` and `averageCounts` appeared somewhere, and deleting `+ TRIM` from `finished` left it **SILENT** — the block matched neither shape and both words survived elsewhere in the file. **A gate that checks for a word is checking for a word.** Rewritten to compare CODE LINES against the derived expectation; it now fires. Control-run three ways, each firing gate 70 **ALONE** at 69 of 69 others green: the defective payloads restored FAILS naming all seven; `finished` de-TRIMmed FAILS; and **one comment reworded in the lesson's Step 7 reveal FAILS**, which is the derivation proving itself. **AND THE L06 CONTENT READ FOUND THREE MORE THINGS NO GATE CAN SEE.** §5.4's without-`abs()` demonstration commented the loop `// already true — loop never runs`, where the paragraph directly beneath it and BRAIN CHECK 01 Q5 both say **false** — and the comment contradicts itself, since a true condition runs. Challenge 6's *Where to look* promised **two while-loops** where its own pseudocode and template both use one `while` with an `if` inside. **AND OBSERVATION EXPERIMENT 3 PREDICTED AN OVERSHOOT THAT CANNOT HAPPEN** — *"drag the right and it sails straight past the mark"* — where **Step 13's own Bug 2 attributes BOTH error directions to the LEFT track**, and the two cannot both be true. The gate reads `abs(encoders.getCountsLeft())`, so retarding the right side turns the robot right, putting the left wheel on the **outside** of the arc: 30 cm of left-wheel rotation buys LESS than 30 cm of travel, and overshoot needs the left encoder to *under*-count, which nothing done to the right track can cause. **The hook survives and improves**: the two drags now produce **two different lies** rather than two directions of one — the left moves the number and `averageCounts()` halves it, the right bends the HEADING and no encoder on this robot can see a curve, which is Step 13's own WARNING and plants Lesson 12 four lessons early. BRAIN CHECK 03 Q5 carried the same wrong clause and moves with it. **THE OLD EXPERIMENT 3 IS NOT SEPARATELY ARCHIVED (DJ ruling: *"Don't worry about it"*)** — it is recoverable from the pushed tree at `8b71826`, and neither `ZUMO_SHELVED_CARDS.md` (never live) nor `ZUMO_PARKED_EXIT_ITEMS.md` (§10 exit-region, §25 conversions) will take live prose that was rewritten; that class still has no home. Applied S141: Maker **v2.45.6** (five payloads reverted, `finished` untouched, five changed lines), L06 **v04.32.1** MINOR — prose and one code comment, no structure moved, so the visible §5b banner correctly stays put. `book_gates` **v1.65.0**, 70 gates.

v8.132, S140, moderate — **WHEN TWO SECTIONS DISAGREE, READ THE CITATIONS: THE OLDER ONE USUALLY PREDATES THE NEWER, AND NO INSTRUMENT CAN SEE IT.** L05's Observation Experiment 3 placed a box at 45° off the flank and its hint predicted RIGHT would answer — while §3.4a, citing Pololu's own measurement, states that roughly 19°–72° off each side is a blind wedge with nothing aimed into it. 45° is 26° past FRONT's edge and 27° short of RIGHT's: almost exactly the middle. **THE TELL WAS THE CITATION** — the hint cites §3.4, not §3.4a, so the experiment predates the section and nobody revisited it. A student running it would have read 0 and concluded the hardware had failed, which is the precise inference §3.4a exists to prevent four sections earlier. DJ ruled option A: rewritten to DEMONSTRATE the dead spot, with a second move out toward 90° so the card still proves the side detector works — and this is the last lesson in which the side receivers exist at all. **AND A KEYWORD CAN BE USED, GLOSSED AND NEVER TAUGHT.** `static` appears in L05 §5.11 as a retained local with no prose, again in L06's `averageCounts()` at file scope with no prose, and is explained only in L07 — two unrelated meanings of one word, met twice before either is defined. Both now teach their own sense and name the other, following the book's existing *two different jobs* pattern (L03's const/constrain, L10's two enums); the glossary entry is now **Static Variable (retained local)**. **L03's LAST TWO [CONTENT PLACEHOLDER] BLOCKS ARE RESOLVED**: brushed-vs-brushless written into §4.2 as prose, seated after the motor photo so GRAPHIC 3.18 stays with the paragraph it illustrates; the Roomba story SKIPPED by DJ ruling, because §1 *The Crooked Robot Problem* already carries that thesis 1,150 lines above it. **ONE FINDING WAS REVERSED BY MEASUREMENT** (§24.8, the S139 shape again): L05's BC02 shares zero character-exact lines with its §2 objectives, which reads as a §25.5 violation until all sixteen lessons are measured — **only L06 satisfies §25.5 at 6/6**, every other lesson is 0–4, so it is a book-wide condition and not an L05 defect. **BANKS L03 v1.0.0 (83 questions) and L05 v1.0.0 (87), both authored after the read and the fixes**; five L05 questions rest on the corrected Experiment 3 geometry and would have been keyed backwards against v04.28.1. **AND THE HEADER OF A THING IS NOT THE THING**: both new banks named question ids that had shifted during drafting, caught only by re-deriving them from the file. L03 v03.41.0 · L05 v04.29.0 · L06 v04.32.0 · `book_gates` v1.64.5; the §27.11 digest moved four times, all usage rank, rules and declarations unchanged at 574/2,033 throughout.

v8.131, S139, moderate — **A VERSION NAMED IN PROSE IS NOT A VERSION CLAIM, AND AN ENTRY FILED IN ONE HOME IS NOT FILED.** S138 bumped the Bible version home to v8.130.2 and wrote its entry into line 17's `Current:` chain, but never inserted the line-start changelog entry. Every prior session edited BOTH homes; the diff proves it — S136 and S137 each show `@@ -17 +17 @@` *and* `@@ -97,0 +98,2 @@`, S138 shows only the first. `session_versions.bible_consistency()` reads the line-start list, so it believed S137 had just run and reported LIVE.md and the S139 handoff as wrong when both were right. **THE READER WAS CORRECT AND THE BOOK WAS WRONG, WHICH IS THE REVERSE OF THE USUAL CASE.** Filed at S139 using S138's own chain-copy text verbatim: 2 added, 0 deleted, the same hunk shape as S136 and S137. Note the two homes have NEVER been byte-identical — S137's filed entry is 2,607 chars against the chain's 2,816, differently worded and differently tailed — so filing is authoring, not copying. **`session_versions` v1.24.0 -> v1.24.1.** `check()` read the WHOLE handoff into `_versions_in`, which builds its dict with `re.findall`, so the LAST occurrence of a key won: the S139 handoff states `Bible **v8.130.2**` in its STATE block and eighty lines later refers in prose to `(Bible v8.130.1)`, and the prose won. LIVE.md never had this defect because its read is already scoped to line 6, asserted to start with `**Versions:**`; the handoff was the outlier and now takes the same discipline — a WINDOW anchored on the VERSION BLOCK marker, one line per line of the emitted block. `_versions_in` is untouched, because it runs on generated text too and a rule about prose is meaningless there. **THE MARKER IS NOW EMITTED**: its own sentence has read *emitted by session_versions.py --handoff, never hand-typed* since S138 while nothing emitted it, so `_fixture_clean` — which rebuilds the handoff from `--handoff` — produced a file that claim was false about. **No whole-file fallback**, because a fallback reinstates the defect on exactly the malformed file that most needs checking; a missing marker is LOUD. And a generated key ABSENT from the window is now reported rather than skipped — the old `if k in hw` passed silently over anything the file failed to mention, which under a windowed read would have turned a reflowed block into silence. **CONTROL H, three directions** (prose does not shadow · a wrong value inside the block is still loud · a missing marker fails), plus a separate blinding run seeding the real failure mode — a stale STATE block with correct prose — which still reports. ALL EIGHT CONTROLS PASS. **L04 READ FINDINGS: 11 OF 13 APPLIED** (the twelfth was recovered from the S138 chat, the thirteenth found by census here). Challenge 4's TEMPLATE did not compile — `LINE_SEEN` used and never declared, card naming only two blanks; now three blanks, and the blank is right because §8A.2 states that choosing thresholds from real data IS the skill, and Challenge 1 already blanks one. `LEARN 4.38` cited *Lesson 2 §3.2b*, which L02 does not have (its §3.2 is *Data Types*, with `bool` an h4 inside it). §8A.8's *counts hundreds per crossing* in three places contradicted its own arithmetic — 10 Hz for half a second derives FIVE — now *counts every loop, not every crossing*, which stays true at any tape speed where *five* assumes the example dwell. §5.5 presented arrays as new when L03 §8A.5 already taught them; it now credits Lesson 3, the S137 *nobody explained it* shape. Step 2's reveal put GLOBAL VARIABLES directly under CONSTANTS against its own pseudo-code (*right after the prototypes block*) and against the shipped file, whose Maker payload order is HARDWARE OBJECTS -> CONSTANTS -> FUNCTION PROTOTYPES -> GLOBAL VARIABLES; the banner is back. C4's function list named `[0]`/`[2]` where goal, pseudo-code and reveal all loop all three. **THRESHOLD CANON: 600 BOOK-WIDE IN L04, WITH THE MARGIN STATED (DJ ruling).** The census split cleanly — all six 500s were TEACHING (§8A.1, §8A.2, the KEY TERM, §8A.3's worked example, the inline exercise, the Glossary) and all six 600s were PRACTICE (§8A.8, Challenges 1, 2 and 4) — so a student applying the rule just taught wrote 500 and found every reveal saying 600, with no sentence explaining the change. **EIGHT SITES SWAPPED; TWO WERE REWRITTEN, NOT SWAPPED** — the KEY TERM and the Glossary carried the *halfway* reasoning, and substituting 600 into *500 is the natural darker-than-halfway line* manufactures a false sentence, the S137 F=ma failure exactly. Both now keep 500 as the midpoint and state 600 as a deliberate margin above it. **§7's *seven-section layout* finding was REVERSED BY MEASUREMENT** — L02 §3.1's Color Key lists eight rows with Prototypes marked unnumbered, i.e. seven numbered, so the sentence is true; the banner census counted `// ===== =====` banners, which Header and Include never carry, giving a ceiling of 6 that no lesson could exceed. **The instrument could not return the answer the claim asserted** (S137 rule 38's shape). This retires the L03 §5.1 queue item; the CONSTANTS vs CONFIGURATION vocabulary drift underneath it stays open. `waitForStart()` and `checkBattery()` moved out of the Quick Reference's *Line sensor functions* table into a new **Startup & safety** group (`#qr-startup`) — none of the ten existing groups fitted, and two homes for one fact is how banner/fence/pill drift began. **L04 v04.28.3 -> v04.29.0, MODERATE** — a new `<h4>` and `<table>` means structure moved, so both §5b homes move and the visible banner was DERIVED from the hidden comment, never hand-typed. **CSS regenerated three times in one session; digest `ce43da62` -> `d7702428` -> `3ce57062` -> `2cd8cd62`, rules and declarations UNCHANGED at 574/2,033 every time, class set byte-identical, and the acceptance test was the RESOLVED STYLING (rule 24): all 574 declaration blocks compare byte-identical, so nothing renders differently.** `book_gates` v1.64.2 -> v1.64.3. **QUIZ_SPEC v1.0.0 -> v1.1.0: THE FORMAT MIX WAS ARITHMETICALLY IMPOSSIBLE.** MC ~70%, TF cap ~20%, matching *one or two, no more* — at both ceilings a 50-question set has an MC FLOOR of 76%, so ~70% was unreachable by construction, and L02 met it only by running TF at 22%, over its own cap, unnoticed because nothing validates a mix. **The matching allowance moved to ~10%, NOT the TF cap, and the reason is signal**: true/false is 50% guessable so buying the ratio with it lowers what the gate measures, while matching's mandatory `extra_answers` absorbs the guess. The superseded cap was ALSO restated in the per-format rules and was fixed there in the same pass — a rule restated in two sections is two rules. **L02 IS NOT REBALANCED** and the amendment says so, so it is not mistaken for conformant when used as a template. **L04 QUIZ BANK WRITTEN, v1.1.0, 73 questions** (before 51: MC 69% / TF 20% / matching 12%; after 22: MC 68% / TF 23% / matching 9%), every question cited, authored AFTER the read and the fixes — READ -> FIX -> QUIZ — and pinning `lesson_04: v04.29.0`, because five of its questions would have been keyed wrong against v04.28.3. **The wave-test direction and Act Two's row-1 overflow are asked in NEITHER set**: both are open bench findings, and a `before` set must be answerable from the text alone. Banks 1 -> 2 of 16. 69/69 gates, family map unmoved at 1119, `callout_id --audit` 1119/0, `strip_inline --verify` 0 dead, `gate_payload_match` PASS, census 40,468 [lesson_inventory.py].

v8.130.2, S138, minor — **AN ASSET IS NOT A DELIVERY UNTIL SOMETHING REFERENCES IT.** `L04_GRAPHIC_4-07_five_sensor_windows.svg` was pushed on its own and took the suite from 69/69 to **two failures** — §17.3c's raster census (31 walked, 32 present) and §10's worklist, because an unreferenced file lands in `image_audit`'s unreferenced bucket, 30 → 31. **Three counters moved for a file no page pointed at**, which is the shape worth recording: the tree's instruments watch the DIRECTORY as well as the prose, so a half push is louder than no push. Wired into L04 twice — §1 as the reveal, retiring the `[IMAGE 4.5]` placeholder, and §4.1 as the numbered reference, retiring a **borrowed Lesson 11 diagram** that had stood in with a *temporary* caption and array-slot labels. **`IMAGE 4.1` RETIRES WITHOUT BEING SHOT** — the shot list, the Figures table and the reserved filename all described the same photograph, so the fall figure debt falls **11 → 9** and `IMAGE_SHOT_LIST.md` goes 22 photos → 21. **THE ORIENTATION WAS SETTLED BY MEASUREMENT AND CONFIRMED ON A BOARD.** Pololu 0J63 §3.5 gives the convention — sensor 1 is the robot's left-most — but the photograph is an underside view with the blade pointing *away*, which mirrors it; three independent signals agreed (the `FRONT` arrow, the silkscreen with `RGT`/`DN4` on the viewer's left and `DN2`/`LFT` on the right matching §3.2's own pin table, and the moulded chassis logo reading reversed), and DJ confirmed. Chips run **5 4 3 2 1** left to right and the margin labels read **ROBOT'S RIGHT** / **ROBOT'S LEFT**, so the figure now DEMONSTRATES the mirror §4.1 warns about instead of falling into it. **AN ILLUSTRATOR ROUND-TRIP IS NOT AN EDIT.** Four passes resampled the embedded photograph every time — JPEG 1200×1148 209 KB became PNG RGBA 1208×1152 at 1.36 MB, three pixels wider per save — dropped `role`/`<title>`/`<desc>`, flattened hand-written attributes into `.st0` classes, baked text centring into `translate()` so nothing re-centres, and on one pass **injected a `Lorem ipsum` textPath** where the footer rule had been, deleting the rule in the process. Every label edit was ultimately made in TEXT against the original file; the shipped photo is byte-identical to the first upload and the file is 287 KB rather than 1.82 MB. **AND A GENERATED BOARD IS NOT A PHOTOGRAPH** — a blueprint-styled redraw was declined because its silkscreen reads `OJ8696` where the board reads `0J8696`; **one provable wrong glyph means all fifteen labels are model output rather than evidence**, which is §14 exactly, and it was also a 2.1 MB dark-field raster with no live text. **`gate_payload_match` WENT PASS → FAIL INSIDE THE S137 CLOSE PUSH AND THE HANDOFF STILL CLAIMED PASS** — it is not one of the 69, so nothing runs it unless a session opener does. Its `EXEMPT` keys pin literal starter LINES, and S137 rewriting those lines orphaned three of them: **S126 Rule 19 arriving as a live defect one layer down** — a key that pins a spelling certifies a spelling, not a property. Repointed and one added, `gate_payload_match` **v1.7.1**, blinding control loud, orphan census across all 21 keys clean. Baselines moved with their reasons in place: §17.3c 31 → **32**, §21 image references 1,201 → **1,202** (two placements in, one borrow out), §27.11 digest — rules and declarations UNCHANGED at 574/2,033, zero born, zero died, **class SET byte-identical in both directions**, only usage RANK reshuffling the file, `strip_inline --verify` 0 dead names. `book_gates` **v1.64.2**. Applied S138: L04 **v04.28.3**, MINOR (a figure lands and a borrow leaves; the §5b visible banner correctly stays put). 69/69 gates, family map unmoved at 1119, `build_css --check` current at 574 rules, census **40,468** [lesson_inventory.py].

v8.130.1, S137, minor — **§24.10 AMENDED: THE RULE NAMED A TOOL, SO CHANGING TOOLS ROUTED AROUND IT.** DJ ruling: *"Grep just seems to be a waste and you miss so much when you grep. When possible use other methods."* §24.10 has said this since S91 and it was violated TWICE in the session that amended it, both times by an operator who believed he was complying because he had not typed the word `grep`. A Python `if 'for loop' in line` walk over L03 returned two hits, and those two were carried to DJ as a finding with two rulings attached — **a population of unknown size reported as a measurement.** The scan cannot see a challenge that needs a loop and never says the word; and it could not see that **the question had ALREADY BEEN RULED in S57 and shipped** (L04 §8A.6 owns the `for` tutorial, L05 §5.15 is the spiral rung, §11 v8.36.1 came out of it), because **no instrument that reads the tree can see a ruling.** Five clauses, pinning a PROPERTY rather than a spelling (S126 Rule 19 applied to canon instead of to a gate): a **text match** is `grep`, `rg`, editor find, `str.find`, `in`, `re.search` over lines, or an `--audit` flag doing any of those internally — **one instrument in different clothes.** **A TEXT MATCH LOCATES; IT NEVER ANSWERS**, because the one thing it cannot return is the case that lacks the string, so a hit is READ IN FULL before it is reported and an empty result is never reported as absence — the positive form of S136’s *grep the figure book-wide and read every sentence it appears in*: **the scan is step one of two, and stopping after step one is the defect.** Clause 3 extends from COUNTS to CLAIMS: a sentence with no named instrument is a lead, and **a lead never carries a ruling request**, since attaching a decision to an unverified premise asks DJ to rule on something that may not be true. New clause 5: **ask whether the session record already answers it before measuring the book.** **AND THE AMENDMENT OWES TOOLING, NOT WORDS** — this entry’s own thesis is that a rule recalled at the moment of temptation does not work, so every reach for a text match where the parser cannot answer is now a recorded FEATURE REQUEST against `lesson_inventory.py`; S137 opens one: *enumerate, per challenge card, the constructs the student is asked to WRITE*, which is what §11 requires before any §9 can be certified and which nothing in the tree can answer. **Ungated by design** — §24.2 owes no gate because there is nothing in the book to assert against, the §24.6b/c / §24.7 / §24.8 shape. Doc only: no lesson file, no instrument and no gate changed for this entry.

v8.130, S136, moderate — **NO INSTRUMENT READS PROSE.** A content read of L01 and L02 end to end found seven defects, **none visible to any of the 69 gates**: a gate measures structure, and a false claim in prose is well-formed structure. **Three were contradictions BETWEEN lessons, invisible inside either one** — the sumo match L01 promised and L03/L14 deny; L01 blaming garbled serial on a baud rate that L02 §8, §6 Step 2 and the L02 Glossary all say this hardware ignores; and L02 pointing at Lesson 7 for `float`, which teaches it nowhere, when L06 is the first lesson that makes a student type one. **A fourth was a number that contradicted the fleet's chemistry** — L02's Quick Reference read 6000 mV fresh, which is alkaline; our 4-cell NiMH packs read ~5400, as L01, L03, L07 and L08 already agreed. **L01's other 6000 was checked and LEFT** — a Challenge 11 prompt whose point is an unreachable threshold. **Not every hit is a defect.** **The compiler settled what reading could not:** *The Broken Code* named lines 3/7/11, and `g++ -fsyntax-only` against a stub header puts them at 2/5/8 — and showed that the third error is INVISIBLE on the first build, because the line-2 semicolon swallows `setup()` before the parser reaches line 5. **The book's own code can be a witness:** L01's chime writes `delay(900)` after an 800 ms note, which is only meaningful if `playFrequency()` returns immediately — proving the buzzer non-blocking without the library source. That behaviour was already taught **only inside GRAPHIC 1.19**; **a fact that lives only in an SVG is not in the book**, and it is now in prose. **`quizzes/` created** — L02 bank (58 before + 18 after), `QUIZ_SPEC.md`, `quiz_bank.py` with nine loud selftest controls. **Four of the seven defects sat under questions the bank asks**, so the order is canon: **READ → FIX → QUIZ.** A bank written against the unread L02 would have keyed 6000 correct and marked careful readers wrong; **an auto-graded gate that punishes the attentive is worse than no gate.** Status is DERIVED by `quiz_bank.py --status`, never written down. Fill-in-the-blank BANNED (Canvas string-matches; a capital letter costs a student their entry to build time). Matching requires `extra_answers`. **The source pin proved itself inside one session** — the close-of-session version bump made the bank's pin stale and the pin is what caught it. **DEBT: no gate holds a quiz bank.** L01 v03.28.2 · L02 v03.21.2, both MINOR. 69/69 gates.

v8.129, S136, moderate — **A LABEL IS NOT THE THING IT NAMES, AND THE TWO DRIFT SEPARATELY.** A section carries THREE independent spellings — the banner cap, the HTML section fence, and the nav pill — and live disagreements were found in all three. **None of the 69 gates could see any of them, because no gate ties a pill to its banner.** **THE BANNER, TWO SPLITS.** §3 read `Theory` in fifteen lessons and `Code Anatomy` in L02. §8 was thirteen bare caps titled *Troubleshooting* against three (L14/L15/L16) carrying the label PLUS a lesson-specific title; DJ ruled toward the three, so **thirteen new titles were written from each section's own content** rather than invented — L04's *When the Sensors Lie* is that section's own opening sentence, *“the program compiles, uploads, runs — and lies.”* **§4's SEVEN LABELS WERE MEASURED AND LEFT ALONE** (Hardware ×8, The Course ×3, Setup, Planning, Prep, Timing, Budget). Read paired with their headlines, each is accurate to its own lesson — L07 *Planning / Before You Split the File*, L16 *Budget / The Chip Is Full* — so that split is a descriptor working as designed, and normalising it would have DESTROYED information. **Not every split is drift, and the test is whether the variation carries meaning.** **GATE §6.8a CAUGHT THE HALF-RENAME.** Changing L02's §3 banner alone failed the suite immediately at *expected SECTION 3: THEORY*, because the fence is DERIVED from the eyebrow's post-middot name (S108). One edit became two because the gate said so — the gate doing exactly the job S82 built it for, on a construct the session had not thought about. **THE NAV PILL IS A SECOND LABELLING LAYER AND NOTHING WAS WATCHING IT.** Two entries were DEFECTS rather than preferences: **L14's `8. Tune & Test` pointed at a Troubleshooting section** (tuning is L15), and **L16's `9. Tiers` sat over a banner reading Challenges.** In both the pill had drifted from its OWN section, so a student clicking either landed somewhere the label had not promised. **The ruled vocabulary, pills 1–9, now byte-identical across all sixteen files:** `1. Intro · 2. Objectives · 3. Theory · 4. Hardware · 5. Code · 6. Build · 7. Verify · 8. Diagnose · 9. Challenges`. §8A's pills stay lesson-specific (`If & For`, `Sensor Pairs`, `Calibration`) on the same argument as §4. **THE PATTERN DJ RULED: PILLS ARE SHORT HANDLES, BANNERS ARE SECTION NAMES.** That is what makes `Diagnose` over a *Troubleshooting* banner and `Wrap-Up` over an *Exit Ticket* banner coherent rather than inconsistent — **they are not supposed to match**, and a later session must not 'fix' the divergence. **PILL 7 WAS RULED ON THE WORD, NOT THE SHAPE.** `Commission It` is the precise engineering term for what §7 does and was DECLINED on measurement: the word appears **nowhere else in the book**, so it would reach a student as a nav pill with no definition behind it anywhere. `Diagnose` was rejected for §7 because **§7 and §8 are opposites** — §7 is entered with a WORKING robot and unknown numbers, §8 with a BROKEN one — and naming both the same tells a student they are one errand. `Verify` was taken because it is equally true of L01's *press A and watch* and L16's three-run A/B protocol, which is the whole span one word had to cover. **§7's MATERIAL WAS READ BEFORE THE LABEL WAS APPLIED**, and the six bare §7s are the STRONGEST testing content in the book, not the weakest: L10's 7A–7E primitive ladder, L11's robot-as-a-ruler, L13's motors-never-run Surface Meter, L14's *Break It, On Purpose*, L15's three rules of a fair test, L16's baseline discipline. **PILL 10 HAS ONE EXCEPTION, MEASURED BEFORE IT WAS GRANTED.** A read of all sixteen §10 sections found exactly TWO that are not a Brain Check exit ticket — L14 and L16 — so fifteen take `Wrap-Up` and **L14 keeps `Match Day`**, its §10 being `10.1 Morning Routine` / `10.2 Pre-Match Routine`, a competition-day procedure. **All sixteen §10 BANNERS already read `Section 10 / Exit Ticket`**, so the pill diverges everywhere by design. **AND THE HALF NO INSTRUMENT COULD HAVE DONE: A CONTENT READ OF L01 FOUND A PROMISE THE COURSE NEVER KEEPS.** §3.0 taught closed-loop control with *“when your Zumo fights a sumo match later in this course”* — and the course is Rescue Line. **The book already contradicted itself twice**: L03 says *“our non-sumo RoboCup bots”* and L14 lists sumo under *“beyond these pages”*. A scan of every course-scope promise in all sixteen lessons returned **exactly one hit**, and it was this one; rewritten to line following, which is what L08 teaches. **THE 69 GATES MEASURE STRUCTURE AND A FALSE CLAIM IN PROSE IS WELL-FORMED** — §24.13's *re-derive, do not re-read* has no purchase on a sentence, which is why the error survived 135 sessions and a suite that has never been greener. **The general shape: an instrument suite grows to cover what instruments can see, and the uncovered region is invisible precisely because nothing reports on it.** **RECORDED, NOT RULED:** the `for` loop has THREE homes and no owner — introduced in L01's INSIGHT, dissected in L04 §8A.6 with GRAPHIC 4.6, and the objective *write a for loop and trace it* owned by L05 §5.15 — and **L04 §8A.6 opens *“nobody explained it”***, true only if L01's INSIGHT is skipped. L03 points a student at L05 for it. **CANON DEBT, RECORDED NOT FIXED:** the pill vocabulary and the pills-are-handles pattern live ONLY in this entry, the same shape as §3.1a, §24.14 and §21.1 before their sections were written — **a rule whose home is a changelog entry holds only where someone happens to look** — and **no gate holds a pill**, so the drift this session repaired can recur tomorrow undetected. Applied S136, all sixteen MINOR (labels and one sentence; no structure moved, so the visible §5b banners correctly stay put): L01 **v03.28.1** · L02 **v03.21.1** · L03 **v03.40.1** · L04 **v04.28.1** · L05 **v04.28.1** · L06 **v04.31.1** · L07 **v04.31.1** · L08 **v04.30.1** · L09 **v05.26.1** · L10 **v02.27.1** · L11 **v02.28.2** · L12 **v01.31.2** · L13 **v02.28.2** · L14 **v02.33.1** · L15 **v02.30.1** · L16 **v02.22.1**. No stylesheet movement: `build_css --check` current at 574 rules, family map unmoved at 1119, `callout_id --audit` 1119 with 0 problems, `keyterm_prefix --audit` 238, `image_audit --check` current at 16 of 146, `gate_payload_match` PASS, census unchanged at **40,455** [lesson_inventory.py]. `book_gates` unchanged at **v1.64**, 69 gates.

v8.128, S135, moderate — **§10 NEW GATE 69: A FIGURE IS PLANNED BY ITS TAG, AND THE TAG DOES NOT LIVE WHERE THE FIGURE DOES. AND §18.1's "1-2" IS A FLOOR, NOT A CEILING (DJ ruling: *"floor. No ceiling."*).** Three §1 hook GRAPHICs landed - 11.5, 12.4, 13.3 - and swapping the `[TAG]` placeholder for the asset made `image_audit` report **planned 146 to 143 with LANDED UNMOVED at 127**. The three had not moved from outstanding to landed; **they had left the population.** A landed figure's tag lives in the lesson's **FIGURES INDEX TABLE**; the body carries only the `<img>`. Deleting the placeholder deleted the tag's only occurrence and shrank the **DENOMINATOR**. **NOTHING FAILED.** The only signal was `image_audit --check` printing DIFFERS, which reads as *re-run me* (S131 rule 22) - and `--write` then accepts the smaller population and prints a smaller `outstanding` **that looks exactly like progress**. Caught because the count was ASSERTED against its own arithmetic rather than read: *16 outstanding* alone is indistinguishable from success (§24.13). **GATE 63 IS STRUCTURALLY BLIND AND THE COMPLEMENTARITY WAS MEASURED (the S106 shape).** Gate 63 walks TAG to asset, so a tag that no longer exists is never iterated (§24.8). Control-run by reproducing the defect end to end: with the index row deleted gate 63 fires, but only on its **CURRENCY** arm - *IMAGE_WORKLIST.md does not match what image_audit emits*. **Regenerate the worklist, as any real workflow does next, and gate 63 GOES GREEN while gate 69 stays red**, the worklist then reading *16 outstanding of 145 planned*: outstanding unchanged, denominator one smaller. **THE PIN IS THE PLANNED TOTAL, NOT THE OUTSTANDING COUNT**, and that distinction is the rule - outstanding is MEANT to fall as art lands, so pinning it would fire on every success; `planned` moves only when a figure is ruled into or out of the book, which is a DJ ruling and never a side effect of an edit. The gate seats **ABOVE the summary print** (S112: where the block sits is part of the gate) and loads `image_audit` **by module name**, the `lesson_inventory` convention already in the file - a first draft invented a `ROOT` symbol this file has never carried and failed on the clean tree, which is the control working before the gate shipped. Restored from an explicit snapshot rather than `git checkout --`, which reverts to HEAD and destroys uncommitted work (S130 rule 15); all three touched files byte-identical afterwards, untouched tree passing at **both ends, 69 of 69**. **THE FIGURE BLOCK HAS FOUR SPELLINGS AND EACH LESSON'S OWN WAS MATCHED RATHER THAN ONE IMPOSED:** L11 `div-m-24px0`/`img-ddd`/`div-c-666-2`, L12 semantic `<figure>`/`<figcaption>`, L13 `div-m-25px0`/`img-br-8px`, L15 `div-m-26px0`/`img-cdd9e1` with a bracketed caption. Choosing between them is a ruling; matching needs none. **CLASS NAMES CHECKED FOR RANK BEFORE REUSE (S133 rule 25):** `img-h-auto` carries five ranked siblings and `div-c-666` three, so adding uses can SWAP a pair - measured, margins are **12 to 40** against an increase of 3, so no rank could move. **§27.11 MOVED BY DIGEST ONLY:** rules and declarations UNCHANGED at **574 / 2,033**, zero born, died or altered; the 122 changed lines are usage-count comments and a rank reorder, `td` overtaking `div` at 796 against 795. **The acceptance test was the RESOLVED STYLING, not the rule count (S134 rule 24): all 22,165 resolved declaration strings byte-identical across the cycle in all sixteen lessons.** Five entities authored into text nodes were swept literal by `entity_sweep`, decoded text byte-identical. **§18.1 IS RE-RULED IN THE SAME SESSION AND THE MEASUREMENT IS WHY.** The queue carried an impression that the Saxon spiral *stops at Lesson 11*. It does not - L13 has the HIGHEST coverage in the book and L11 is second. **It never started:** only **13 of 171 challenge units carry a star, 8%**, and half the 26 star references sit outside challenge cards. Five lessons carry none - **L04, L05, L06, L12, L15** - while L01 correctly has none and L16 has no challenges. **THE FIRST DENOMINATOR WAS WRONG AND WAS DISCARDED:** counting backward *Lesson N* mentions gives **509 against 26** and reads as catastrophe, but L15 §1 saying *"back in Lesson 8 you wrote four lines"* is narrative, not a spiral - §18.2 puts the star on a CHALLENGE CARD. **A SCAN FOR CARDS NAMING A PRIOR LESSON RETURNS 29, OF WHICH 17 ARE UNSTARRED - AND IT IS BLIND TO THE REST:** only one of five spirals found by READING (4.4) appears in it; 5.1's boolean-memory trick, 6.7's ramp, 12.2's encoder-versus-gyro comparison and 15.2's noise-as-a-rate never say *Lesson N* aloud. The true population is **21 at minimum and not yet enumerated**, and the enumeration is a READ of all 171 cards, not a query. **DJ ruled the "1-2 prior concepts" a FLOOR**, on the §25.8 precedent that made Knowledge Check a floor after a flat cap of 5 was dodged four times. **THE ARC IS DELIBERATELY PARKED, NOT STARTED**, because under a floor ruling partial application is worse than none: a half-starred book is what produced the *stops at Lesson 11* reading, and starring nine lessons while leaving five bare rebuilds that misreading at a new boundary. Applied S135, all three MINOR (a figure landing does not move page structure, so the visible §5b banners correctly stay put): L11 **v02.28.1** · L12 **v01.31.1** · L13 **v02.28.1**. `book_gates` **v1.64**, 69 gates. Census 40,450 to **40,455**, the +5 reconciling exactly: L11 +1, L12 +2 for its three-line `<figure>`, L13 +1, plus three index rows. `image_audit --check` moves to **16 outstanding of 146 planned**.

v8.127.1, S134, minor — **A COUNT IN THE v8.127 ENTRY WAS WRONG AND THE TRIPLE-CHECK FOUND IT.** It stated 19 body KEY TERM blocks carry `id="term-*"`. **18 do.** The nineteenth, L03 `3.44`, carries `id="glossary-trim"` — a glossary-prefixed id on a BODY block, which is an unruled lead in its own right and is queued. **The error is §24.13 exactly:** the first measurement counted head `<strong>` elements carrying ANY id, and the sentence written from it reported them as one spelling; every later restatement re-read that sentence instead of re-deriving the number, so it propagated into four files including this Bible. **What caught it was a DIFFERENT METHOD** — a before/after census of every `id="term-*"` in all sixteen lessons, run to prove the anchors survived, which returned **169 before and 169 after, zero lost, zero gained**, and disagreed with the body/glossary split by exactly one. Doc-only: no behaviour, no gate, no lesson file and no rule changed; `keyterm_prefix` reaches **v1.0.1**.

v8.127, S134, moderate — **§24.14d NEW: A CALLOUT FAMILY NAMES ITSELF IN THE BODY, AND THE REGION
DECIDES.** DJ ruling, taken on rendered specimens of both glossaries side by side: *"the lesson
should have the KEY TERM callout and the glossary just has the word and def."* **THE MEASUREMENT
REVERSED THE SESSION'S DIRECTION, WHICH IS THE ENTRY'S POINT.** S133 left 22 body KEY TERM blocks in
three unruled shapes and the obvious reading was that a literal `KEY TERM:` prefix was redundant with
the `key` mark — S129's *"we don't want Note - Note"*. Measured across all 1,119 live callouts the
opposite is true: **NOTE 113/133, CHECKPOINT 102/112, TIP 79/85, DO THIS NOW 55/58, WARNING 67/80,
LEARN 38/47, and BRAIN CHECK / THE GOAL / MY PLAN / BUILDS ON at 100%.** KEY TERM stood at **13 of
238**, and those 13 were **exactly the 10 prefix blocks and 3 L15 outliers about to be stripped** —
the only large family that did not name itself was one pass from being made unanimous the wrong way.
**S129 is not contradicted:** those six blocks read their family word TWICE; KEY TERM shows only as
`key.svg`, so the block named itself ZERO times in words. **THE REGION SPLIT IS LOAD-BEARING, NOT A
CONCESSION** — the glossary banner already reads *Glossary*, so 151 repetitions name the family at
section scope twice and push the term, the thing a reader scans a glossary FOR, off the left edge of
every line; the prefix marks *taught here* and its absence *looked up here*, and §24.14b's region
tier already makes that a property rather than an exception. **83 BLOCKS CONVERTED** across 13
lessons by new instrument `keyterm_prefix.py` **v1.0**: 65 canon gained the prefix, L04's five
`Key Term:` took the caps spelling, L14/L16's five gained the `<strong>`, L10's five plain terms
gained both, and L15's three inline `<b>` heads became the head div every other block already had.
**THE PREFIX SITS OUTSIDE THE `<strong>`** because 19 body head anchors sit there — 18 spelled `id="term-*"` and one, L03 `3.44`, spelled `id="glossary-trim"` — as the
targets of the ruled body→glossary link direction §27.14 enforces, and that span is what the
end-of-book glossary harvest extracts as the term. **THE LOCATOR IS THE AUTHORED ATTRIBUTE:** the
first draft matched the head by its style declarations, which exist only in the EXPANDED source
since the file carries `class="div-fs-105em"` — and keying on that class would have been worse than
a bug, because S133's rule 25 means the `-N` name could be handed to another rule by an unrelated
edit and the tool would go silently blind. It walks out from `data-mark` instead. **AN INLINE HEAD
BECOMING ITS OWN LINE TAKES THE PROSE WITH IT:** L15's three definitions were lowercase because they
were grammatically continuing `<b>KEY TERM — Term:</b>`, so the converter's span extends past the
head to capitalise, asserting both that a letter follows and that it was lower case. **THE PIN FALLS
125 → 55 AND THE GATE RULED IT, NOT THE SESSION** — `canon_of` now names 70 previously-unnameable
blocks from CONTENT and gate 62's coverage arm reported the holds expired without being told to,
S132's behaviour and rule 20 for the second time; retired BY THE PROPERTY, the gate's own `_need`
derivation, never a hand list, reconciling as 83 converted less the 13 already self-naming and
therefore never pinned. **The coupling is recorded rather than hidden:** those 70 depend on the
prefix staying, so removing one fires §24.14a, §24.14c and §24.14d together — three gates agreeing.
**FOUR BODY BLOCKS HELD BY NAME (§25.2a):** `3.31` a provenance question, `3.101` an operator
announcement, `6.24` a formula box, `14.28` a procedural list — none a term followed by a definition.
DJ held constrain explicitly and the other three are the same shape; **whether they belong to this
family at all is UNRULED**, and two wear another family's canon ground (`3.31` on `#d1ecf1`,
`3.101` on `#e3f2fd`/`#2196f3`). **GATE 68 NEW**, written in the same pass as the ruling, asserting
BOTH halves because a body-only gate would certify a glossary drifting into the prefix, and
IMPORTING `head_of()` and `PREFIX` from the converter rather than keeping a fourth copy of a matcher
(S133 found three copies of one in a single file). **ITS FIRST DRAFT FAILED ITS OWN CONTROL AND THAT
IS THE REUSABLE PART:** adding a fifth id to `HELD` excepted a real conformant block while every arm
stayed green, because **a coverage count measures blocks SCANNED, not blocks ASSERTED**. A gate that
carries exceptions owes a HOLD ARM — the set pinned by NAME, and each held block asserted to still
genuinely lack the prefix, so a hold that has quietly become unnecessary expires loudly instead of
sitting there certified. Control-run seven ways from a snapshot of the FIXED tree (rule 21), each
firing gate 68 with the untouched tree passing at both ends. **§27.11 MOVES BY DIGEST ONLY** to
`2cf7bbd1225543d4`, rules and declarations UNCHANGED at 574/2,033, zero born, died or altered when
diffed by selector; the entire sheet delta is FOUR lines, the header count and `.div-fs-105em`
×792 → ×795, which is L15's three new heads. **The acceptance test is the RESOLVED STYLING, not the
rule count (rule 24): all 22,142 pre-existing resolved declaration strings are byte-identical across
the restore/regenerate/apply cycle in all sixteen lessons**, and §27.15b could not fire because
nothing left a class. **NOT RULED, DELIBERATELY UNTOUCHED:** the head text colour, 16 body heads on
`color: #6a1b9a` in clean strata (L04 5/5, L09 6/6, L10 5/5, 0 of 59 elsewhere) — a paint question
parked at S133 with the five KEY TERM grounds. `book_gates` **v1.63**, 68 gates; `session_versions`
**v1.24.0** registering and emitting the new tool, its Control G naming it registered-but-not-emitted
on the first run for the fifth consecutive tool-adding session. Applied S134, all thirteen MODERATE
(the pages render differently, so the §5b homes move): L01 **v03.28.0** · L02 **v03.21.0** ·
L03 **v03.40.0** · L04 **v04.28.0** · L05 **v04.28.0** · L06 **v04.31.0** · L07 **v04.31.0** ·
L08 **v04.30.0** · L09 **v05.26.0** · L10 **v02.27.0** · L14 **v02.33.0** · L15 **v02.30.0** ·
L16 **v02.22.0**; L11, L12 and L13 carry no body KEY TERM blocks and are unchanged.

v8.126, S133, moderate — **§2.1 AND §2.2 NEW, AND §27.15b GAINS THE FINDING THAT COST TWO
REVERTS: A GENERATED CLASS NAME IS NOT A HANDLE.** DJ asked for something three bytes wide — an
objective list should not draw a bullet on top of its checkbox. Assigning the existing class
`ul-ls-none-3` to the four offending lists silently re-resolved **twelve lessons nobody
touched**: L03's lists went `padding-left: 0` to `5px`, L06–L13 went `5px` to `0`. Reverted,
re-authored as **inline** — the pipeline's own canonical input — and **it happened again**,
because `.ul-ls-none` and `.ul-ls-none-3` are distinguished only by a rank-assigned `-N` suffix,
so adding uses to either **SWAPS them**. §27.15b fired twice in one hour, and the rule now
states plainly that it fires through inline authoring too, which the S124 entry did not say.
**IT WAS CAUGHT BOTH TIMES BY COMPARING RESOLVED STYLING AGAINST A SNAPSHOT. No gate saw it and
no gate can** — §27.13 asserts the sheet regenerates from the lessons, and a swap regenerates
perfectly. **DJ ruled `css/semantic.css`**, which worked first time with **zero collateral: all
22,142 resolved declaration strings byte-identical in all sixteen lessons**. **The selector is
an ATTRIBUTE, not a class** — S128's lesson, where `class="mark"` was re-emitted as `.img-fs-0`
because a semantic class going INTO markup runs the generator backwards. The general rule: **if
a construct needs a name that means something, the name lives in the preserved layer and the
markup carries an attribute; never reach for a name `build_css` emitted.** **RECORDED, NOT
FIXED: `build_css` IS NOT IDEMPOTENT ON THIS TREE.** The same sixteen lesson files produce two
different stylesheets depending on which sheet the run starts from, `.ul-ls-none-2` and
`.ul-ls-none-3` alternating. §27.13 assumes a single fixed point; there are two. **THE EM DASH
LEAVES THE BODY KEY TERMS — 24 blocks, and the first count of 41 was wrong because it measured a
PROXY.** S132 made all 151 glossary entries the canon term card with no em dash; the body half of
the same family was never touched. Counting blocks missing `<strong>` conflated four shapes;
measured against the separator itself the population is **41 already correct, 24 em-dash, 10
carrying a literal `KEY TERM:` prefix, 9 plain, 3 outliers**. The 24 converted (L02×11, L03×7,
L09×6), and L03's seven `<span id="term-*">` became `<strong id="term-*">` so the anchors
survived. **The other 22 shapes are unruled and parked**, and the 3 L15 outliers carry real em
dashes inside their definition prose, so a dash rule would eat punctuation. **EVERY BACK-TO-TOP
LINK IN THE BOOK IS ONE SPELLING.** 237 links take `<p class="p-ta-right">`, where five wrappers
had been in use and **L15 and L16's 28 rendered flush LEFT** with no `text-align` at all —
verified three ways sharing no code: a markup census, a DOM ancestor walk, and OCR of a render
putting *Back* at x=890 on L14 against x=122 on L15. Three rules died carrying two declarations
each, and **§27.15b did NOT fire, because all three were used by back-to-top links and nothing
else** — checked before deletion rather than after. **`Learning Objectives` COST A TAXONOMY FIX,
AND THE FIX FOUND THREE COPIES OF ONE RULE.** Renaming L02's `🎯 OBJECTIVES` re-familied the
block LEARN, because the CANON tier matched a bare prefix and `LEARNING` starts with `LEARN`.
The prefix must now end on a **word boundary** — **measured inert before it was applied: across
all 1,119 live labels the result changes for ZERO of them**, so it cannot move the map and only
stops a future label being claimed by a shorter name. The matcher had been written out **three
times**, twice in `book_gates` and once in `family_tag`; all three now import the exported
`canon_of()` (§S83). The OBJECTIVES rule names the **noun** rather than the string start, and
its reach was controlled: it can match five other labels, all of which CANON claims first.
**§26 GAINS NOTHING AND §14.1 IS CORRECTED** — it named `ZUMO_TDP_Template.md`, a file that does
not exist; the live file is `ZUMO_TDP_Template_v3.md` (v3.1, A5 Lab Log present), so that queue
item is closed. **THE SESSION'S PROCESS FINDING, AND IT IS THE ONE TO CARRY: MEASURE THE
PROPERTY THE RULING NAMES, NOT A PROXY FOR IT.** Five wrong counts reached DJ and every one came
from measuring something adjacent — `<strong>` presence for *has a readable term* (41, truly
24), a path substring for *is a figure* (a false 16 of 16 with six lessons still empty), a
400-byte window for *what follows the glossary* (reported a structural difference that was a
window length), a regex requiring a period for *has a lead-in* (four false negatives; the
lead-ins end in a colon), and `start`+`bytes` for a block whose length is measured in the
EXPANDED source. **Every one looked like a finding and none was.** `book_gates` **v1.62**
(67 gates), `build_family_map` **v1.6.0**, `family_tag` **v1.2.1**. Applied S133, all MODERATE
(every page renders differently, so both §5b homes move): L01 **v03.27.0** · L02 **v03.20.0** ·
L03 **v03.39.0** · L04 **v04.27.0** · L05 **v04.27.0** · L09 **v05.25.0** · L10 **v02.26.0** ·
L11 **v02.28.0** · L12 **v01.31.0** · L13 **v02.28.0** · L14 **v02.32.0** · L15 **v02.29.0** ·
L16 **v02.21.0**. `image_audit --check` moves to **19 outstanding of 146 planned**.

v8.125, S132, moderate — **§27.15f NEW: THE REVEAL BOX GRADUATES, AND THE ONE DEFECT IN IT WAS NOT DRIFT.** DJ, on being shown the split: *"Let's get rid of the split once and for all."* The click-to-reveal `<details>` shipped in **THIRTEEN generated class names across 453 elements** — and **55 more carried no class at all**, falling to the browser default. **THOSE 55 HAD NO `cursor: pointer`**, so the mouse did not change over the clickable line and nothing told a student the box opened: 17 in L07, 14 in L08, 11 in L09, and fifteen of them the very *Check your work* blocks renamed hours earlier. Every other reveal in the book declared it. **That is the only part of this ruling a reader would have FELT**, and it is what the gate asserts first. **THE SPLIT WAS REAL AND SAT AT THE FAMILIAR SEAM:** standalone reveals ran `.details-dee2e6-2` in L01–L09 (83, bordered, 8px) against `.details-bg-f8f9fa` in L10–L16 (79, borderless, 6px) — the same L09/L10 seam as the §6.5 Box (S108), the mono stack (S123) and the `<pre>` border (S126). **THE BORDER WAS ALREADY DECIDED AND THIS SESSION ASKED ANYWAY, TWICE.** §27.15e ruled *a border that earns its place* and **CORRECTED** an invisible value rather than deleting it; `#dee2e6` on `#f8f9fa` is **1.24:1**, LOWER than the `#333` that ruling retired, so the answer was `#909090` — the lowest grey clearing 3:1 on this fill, **derived by computation**, with `#919191` one step lighter at 2.9900:1. DJ named the repeat: *"That's twice you suggested the border that was already decided on."* **§24.13 on a DESIGN rather than a measurement, which is exactly what S131 recorded and this session had already written into its own handoff.** **THE SUMMARY IS THE OTHER HALF AND NOBODY HAD COUNTED IT:** twelve spellings — six text colours, two weights, three paddings, and 16 summaries carrying their own background and radius, a box inside the box. Ruled to `#5a6872` (304 of 453, 5.45:1 on the fill), `bold` (452 of 453) and no padding (382 of 453), which also satisfies §6.12c's stacked-sibling rule by agreement at zero. **THE ONE CANDIDATE EXCEPTION WAS PUT TO DJ AND HE RULED IT OUT, TWICE OVER.** Stripping the classes dropped the family map 1123 → 1119 because L11's four Mystery-answer reveals wear a CALLOUT skin and are **the entire ANSWER family** — the only reveals in the book that do. Priced both ways and put to DJ rather than absorbed by moving a constant. **His first ruling, *"Keep those as answers"*, was read as keeping the SKIN and was implemented that way, complete with a `border: none` cascade fix; he corrected it on sight:** *"I wanted the word answer to stay, but the pill should be consistent with the others."* **The word was never the box.** The four take this rule like every other reveal, their summaries still read *Answer*, and **the ANSWER family retires** — four members, one lesson, on an element no other callout used, doing the job L08's six Mystery answers already did as ordinary reveals. Their `data-family` and `data-callout` come off with the skin, because a reveal is not a callout and a dead attribute is a claim nothing checks. **The family delta is CONTROLLED at an identical generator version: exactly ONE family moves, ANSWER 4 → 0, and the other 29 are byte-identical.** Gate 65's hold arm is **INVERTED rather than deleted** (S108): it now fails if that skin returns, or if any `<details>` ever carries `data-family` again. **THE OTHER PLAUSIBLE EXCEPTION WAS CHECKED AND IS NOT ONE:** L02's three translucent reveals read as a dark-ground case and sit inside a GREEN `#e8f3ec` callout, so the S124 trap does not fire. **§27.15b's RENAME TRAP FIRED AGAIN** — 21 rules die, one is born (`.div-dee2e6-6`, a `<div>` that had been sharing a name with a reveal), and five surviving spellings changed declarations — paid by restore → regenerate → apply `--include-held`. **THE ACCEPTANCE TEST IS NOT THE RULE COUNT: every element in the book that is NOT a reveal resolves byte-identically across the cycle, all 22,137 of them, in all sixteen lessons.** **THE BUILD WAS RE-DERIVED FROM THE PUSHED CLONE RATHER THAN PATCHED FORWARD** once the hold was ruled (S124/S126). §27.11 moves to **577 / 2,039**. **GATE 65 NEW**, written in the same pass as the ruling, control-run five ways from a snapshot of the FIXED tree (rule 21): the old `#dee2e6` restored, `#919191` at one step under threshold, `cursor: auto`, the hold's `border: none` removed, and a stray class on one summary — all five fire it, and the untouched tree passes at 65 of 65 at both ends. `book_gates` **v1.60**, 65 gates. Applied S132, all sixteen MODERATE (every page renders differently, so both §5b homes move): L01 **v03.26.0** · L02 **v03.19.0** · L03 **v03.38.0** · L04 **v04.26.0** · L05 **v04.26.0** · L06 **v04.30.0** · L07 **v04.30.0** · L08 **v04.29.0** · L09 **v05.24.0** · L10 **v02.25.0** · L11 **v02.26.0** · L12 **v01.29.0** · L13 **v02.26.0** · L14 **v02.31.0** · L15 **v02.27.0** · L16 **v02.20.0**.

v8.124, S132, moderate — **THE GLOSSARY IS ONE COMPONENT: §6.9's ENTRY FORMAT AND §8's 11-TYPE TABLE BOTH BECOME POINTERS TO `BookComponentStandard` §7.4 AND §7.** DJ ruling: *"Just make them ALL the same… IF glossary is better use it, if it's key term use it."* **151 entries shipped in FIVE schemas** — 97 KEY TERM callouts in four sub-shapes, 14 bare `div-9b59b6` in L04, 15 `<dl>/<dt>` pairs in L13/L14, 25 table rows in L11/L15 — and **54 of them carried no `data-family` and no `data-callout` at all**, invisible to `build_family_map`, `family_tag`, `callout_id`, the pin and every gate reading through them. **A THIRD OF THE BOOK'S VOCABULARY COULD NOT FAIL A GATE.** All 151 are now the canon term card, and **every part of it is DERIVED from the majority of the live tree rather than chosen**: wrapper `.callout-9b59b6-bg-e7d4ff` 83 of 97, head `div-fs-105em` 88 of 97, `key` mark 87 of 97. **NO EM DASH** (DJ, S131) — 43 cards carried one, in TWO positions, and the separator is stripped only in LEADING position because L04's *"just beyond red — invisible to human eyes"* would otherwise lose its punctuation. **THE FAMILY IS KEY TERM AND `GLOSSARY` IS DELIBERATELY NOT ADOPTED**, though §7 offers it with a `journal-bookmark` mark and zero uses: **a family names CONTENT, not LOCATION**, the glossary entry and the body callout define the same thing (the dual is DESIGN, ruled S128), and *where a block sits* is what the REGION carries. **THE HARVEST KEY IS THE REGION, WHICH IS THE WHOLE POINT AND IS NOT THE FAMILY:** `data-family="KEY TERM"` returns **184** blocks where the glossary is **151**. **§24.14b NEW — THE STRUCTURE TIER, AND IT IS THE FIRST NON-CONTENT TIER SINCE S112, SO ITS DISTINCTION IS STATED:** a callout inside the glossary region is a KEY TERM, **97 of 97, zero exceptions**. **A SECTION ID IS STRUCTURE, NOT DECORATION** — colour died at S112 and the glyph at S130 because both were presentation that merely correlated with family and either could be repainted without the family changing; a banner id cannot be repainted, and moving a block out of the glossary is a MOVE, after which it SHOULD stop being a KEY TERM. **PROVEN AGAINST `family_tag`, THE AUDITOR THAT CAN FAIL, NEVER AGAINST `build_family_map`, WHICH READS THE ATTRIBUTE FIRST:** with the 87 glossary-side pin rows stripped the audit reports **1069 correct, 0 unnamed**; with the tier BLINDED on the same tree it reports **exactly 87 unnamed**. Ten blocks are named by both a tier above `struct` and by `struct`, **0 disagreements**. **THE PIN FALLS 212 → 125 AND THE GATE RULED IT, NOT THE SESSION:** taught the tier, gate 62's coverage arm reported 87 holds EXPIRED without being told to — S130 rule 20, a hold something else satisfies is not a hold — and the rows were retired BY THE PROPERTY, never by a list of ids. **This is NOT a regeneration from `data-family`**, which that file forbids: the value it exists to check was never consulted, a tier replaced the need, which is exactly how the glyph tier's death created the file. Convert with no tier and the pin would have gone to **266** — a 141-row swing. **`lesson_inventory` v1.3.5 GAINS `region`, AND THE EXISTING `section` COULD NOT HAVE CARRIED IT:** §10's anchor opens Exit Ticket, Glossary, Quick Reference and Figures alike, so `section` reads §10 for the 97 term cards AND for 95 blocks that are not glossary — a tier keyed on it would have looked sound and been wrong (§24.8). **THE DETECTOR READS A PROPERTY, NOT A SPELLING (S130 rule 19), AND THE FALSE CLEAN HAPPENED BEFORE THE RULE SAVED IT:** the first probe keyed the region on the `div-bg-*` wrapper, **which L04's glossary banner does not wear**, and returned ZERO regions for L04 — clean by omission. **GATE 64 NEW**, written in the same pass as the ruling. It asserts the PREDICATE rather than the tier's output, because asking the tier whether it returns KEY TERM is circular: it BLINDS the tier and fails if any OTHER tier resolves a glossary-region block to something else, with silence legal since most resolve to nothing once the pin rows retire. Arm 2 pins the banner ROSTER. Control-run both ways from a snapshot of the FIXED tree (S131 rule 21): a NOTE relocated into L10's glossary fires the contradiction arm, breaking L04's banner shape fires the roster arm, each **ALONE** at 63 of 63 others green, untouched tree passing at both ends. **§27.15b's RENAME TRAP FIRED FOR REAL AND WAS PAID:** six rules died (`.div-9b59b6`, `.dl`, `.dt`, `.dd`, `.td-ddd-12`, `.callout-2e86ab-bg-fff`) and **12 SURVIVING spellings changed declarations** — `.th-17496a-2` and `-3` swapping grounds outright — so the sequence was restore → regenerate → apply `--include-held`, and **the acceptance test is not the rule count: all 22,933 RESOLVED declaration strings are byte-identical across the cycle in all sixteen lessons.** **NO TEXT WAS LOST, AND THE FIRST CONTROL THAT SAID OTHERWISE WAS WRONG:** it fired on 8 of 16 regions and every hit was a missing SPACE at a tag boundary — a control that cannot tell a lost word from a lost space is not a control (§24.6c). Re-run on words: **0 of 16 changed a word.** The converter is IDEMPOTENT, measured: a second pass rewrites 0 of 16 files. **`glossary_convert.py` MINTS NO IDENTITY** — the 54 new blocks are written without `data-callout` and `callout_id --apply` authors them at the next free ordinal, because a second minter is the third-copy defect (S83). **THE ID IS UNIVERSAL AND THE `-gloss` SUFFIX IS LOAD-BEARING**, checked rather than assumed: L01 carries seven pairs where the same term is defined in the body and the glossary (`term-platformio` / `term-platformio-gloss`), ids must be unique per page, and six ids repeat ACROSS pages, which is legal. **`regex_audit` FLAGGED THIS SESSION'S OWN NEW FILE** — a dead `<em>` branch in the term matcher, verified dead against the converted tree before narrowing. **BASELINES MOVED, EACH DERIVED:** family map **1069 → 1123** (= 1069 + 54); §21 coverage **1,134 → 1,198** (= 151 − 87 already-marked); gate 59 to 1123; §27.11 **604/2,141 → 598/2,123**; the pin md5. `book_gates` **v1.59**, 64 gates; `lesson_inventory` **v1.3.5**; `build_family_map` **v1.5.0**; `family_tag` **v1.2**; new instrument `glossary_convert.py` **v1.0**; `BookComponentStandard` **v01.13.0** with §7.4 new. **§8's 11-TYPE TABLE IS ALSO SUPERSEDED**, ruled S129 and done here: it named 11 of the 30 families `build_family_map` resolves, and **a LOCKED table naming a third of the population is worse than no table because it reads as complete**; its opening clause *"All callouts use inline `style=` only"* was retired by §27. Applied S132, all sixteen MODERATE (the glossary renders differently on every page, so both §5b homes move): L01 **v03.25.0** · L02 **v03.18.0** · L03 **v03.37.0** · L04 **v04.25.0** · L05 **v04.25.0** · L06 **v04.29.0** · L07 **v04.28.0** · L08 **v04.27.0** · L09 **v05.22.0** · L10 **v02.23.0** · L11 **v02.24.0** · L12 **v01.27.0** · L13 **v02.24.0** · L14 **v02.29.0** · L15 **v02.25.0** · L16 **v02.18.0**. **OPEN, NOT RULED:** whether the 25 body key terms with no same-lesson glossary entry get authored ones; and §6.5's nav-pill rule still says 12–14 where the live range is 10 to 19.

v8.123, S131, moderate — **§10 NEW GATE 63: A FIGURE IS LANDED BY AN ASSET AND NEVER BY A DECORATION, AND S130's OWN LESSON ARRIVED ONE INSTRUMENT LATER.** `image_audit` has a NEIGHBOUR arm: when a filename does not match, an `<img>` inside the tag's own paragraph lands the figure anyway. It exists for a real case — L10 prints *"this is the same photo you met in Lesson 5"* and wires L05's file directly, so counting that as missing would send DJ out to re-shoot a photograph the book already ships. **S130 then put 884 `<img data-mark>` marks into exactly that prose, and the arm could not tell a lightbulb from a photograph.** Six real shots — L03 3.2 / 3.5 / 3.6, L12 12.1, L14 14.1, L16 16.1 — reported LANDED with nothing taken, each satisfied by `marks/` alone and by no figure asset at all, verified per tag rather than inferred from the count. **NO GATE FAILED.** The only signal was `image_audit --check` printing DIFFERS, and the obvious reading of that is *re-run me* — which would have written **8 over 14** and silently retired six entries from the shot list four weeks out from the course. **THE ARC WAS MEASURED ON THE CONSUMERS THAT WERE WATCHED**, S130's own finding repeating on an instrument nobody thought to point at it. **THE PREDICATE IS THE AUTHORED ATTRIBUTE, NOT THE DIRECTORY (S126 rule 18, S130 rule 19).** Keyed on `images/marks/` this would read a SPELLING: move the files and the audit silently reverts. `data-mark` is authored, which is the whole reason S130 authored it. The two sets coincide at **884 today and that is measured, not assumed** — and CONTROL F2 fails if the predicate ever starts reading the path, testing a mark moved OUT of `marks/` and an unmarked file left inside it. **THE SPANS ARE COMPUTED OVER THE WHOLE FILE, NEVER OVER THE WINDOW:** `data-mark` can sit outside a window that contains the tag's `src=`, so a tag classified on a slice is classified on whatever the slice happened to include. **THE FIX'S PROOF IS THAT IT RESTORES EXACTLY THE PRE-MARKS ANSWER AND NO MORE** — the regenerated worklist is byte-identical to the repo copy but for the version line, all fourteen outstanding rows unchanged. **GATE 63 SHARES NO CODE WITH THE FIX FOR THE QUESTION THAT MATTERS.** Its independent arm re-derives landing from FILENAMES ON DISK with its own regexes and never asks `image_audit`, so it fails on whatever a future arc decorates the prose with rather than on `data-mark` specifically; a currency arm does import the generator, because *the artefact matches its generator* cannot be asked without it, and that is not the independence claim. Disagreements are a **NAMED set with a reason each** (§25.2a), and **arm 4 fails a name that has stopped needing its exemption** (S130 rule 20). Coverage arm included (S117/S118). **THE INDEPENDENT ARM FOUND A SECOND CASE NOBODY HAD COUNTED, AND IT IS RECORDED UNRULED RATHER THAN DECIDED:** L07's `[IMAGE 7.3]` is landed by `L07_GRAPHIC_7-15_platformio_file_tree.svg` — a GRAPHIC standing in for an IMAGE across the two number spaces (§10), with no prose declaring the substitution, where L10's case says so in the lesson. Same subject, so it is plausibly deliberate; nothing in the book says so, and a gate does not get to decide it. **Control-run four ways from a snapshot of the FIXED state**, each firing gate 63 **ALONE** at 62 of 62 others green and exit 1, untouched tree passing at BOTH ends, all three files restored byte-identical: the S130 defect reproduced end to end (old `image_audit` plus the worklist it emits) names all six; a deleted worklist row; `IMAGE 10.1` dropped from `REUSE`; a non-load-bearing name added. **A PROCESS FAILURE, MINE, AND IT IS S130 RULE 15 REACHED BY A NEW ROAD:** the first control run restored the BEFORE snapshot with no snapshot of the AFTER, and silently destroyed the fix under test — **a snapshot of the state you are leaving is not a restore path for the state you are in.** Redone with the fixed state snapshotted first. `image_audit` **v1.2**, gaining CONTROL F in three arms, the third of which fails if the live book ever stops containing the shape F guards, because an arm guarding an absent shape passes forever. `book_gates` **v1.58**, 63 gates. No lesson file changed; census unchanged at **40,683** [lesson_inventory.py]; family map unmoved at 1069; `build_css --check` current at 604 rules; `site_parity` PARITY.

v8.122.1, S130, minor — **§12.6 RECORDS THE CENSUS DEFINITION CHANGE, BECAUSE THE DISCONTINUITY IS A TRAP.** The S130 census fix left every historical figure in this CHANGELOG running high by exactly the lesson count, and those entries are **not retro-edited** (§24.9). The hazard is specific rather than tidy: a census delta is this book's arithmetic check that *nothing else moved*, so a reader comparing S129's **40,698** with S130's **40,683** computes a phantom **−15** and goes hunting for fifteen deleted lines. **S69 already lost a session to a false finding of this exact shape**, which is why one line is cheaper than the alternative. Seated in **§12.6 rather than §24.10** on a discoverability argument, not a topical one: **gate 62 is labelled `§12.6`**, so anyone investigating a census discrepancy lands there first — put the answer where the investigator arrives. **The conversion is DERIVED, not a magic number — the offset IS the lesson count**, because the old formula added one per file regardless of whether the file ended in a newline. Anything from S130 on needs no conversion at all: `cat lessons/Lesson_*.html | wc -l` now agrees to the line. **DELIBERATELY NOT DONE:** annotating the emitted census string so it carries its own definition. It was considered and rejected as belt-and-braces — it churns the format of two regenerated documents to solve a problem the fix already eliminated, since a doubted figure is now re-derivable in one command. **Also settled this session and recorded as a NON-action:** gate 62's trailing-newline arm stays scoped to the sixteen lessons. All five non-lesson pages were measured and **already end in a newline**, so extending the arm would assert something already true, on files outside the census, protecting nothing — the shape by which gates accumulate weight without value. No lesson, gate or tool version changed; census unchanged at **40,683**.

v8.122, S130, moderate — **THE MARKS ARC LANDS, AND THE SECOND DECORATION-KEYED TIER DIES.** DJ ruled the arc, then ruled it uniform: *"I want the book to be uniform."* **884 leading emoji become `<img data-mark>` marks** across all sixteen lessons; `mark_wire` reconciled at 884 SWAP + 13 NO_GLYPH + 56 HELD + 116 NO_MARK = 1,069, and every mark was verified to LEAD its own label. §21's coverage lands at **1,134**, derived exactly as 250 + 884.

**THE ARC HAD A PRICE NOBODY HAD PRICED, AND IT SURFACED ONLY BY RUNNING IT.** `build_family_map` returned 1069/1069 after the swap and looked clean. `family_tag` — the AUDITOR behind gate 59, which resolves from CONTENT so it is not checking the attribute against itself — reported **208 unnamed**. The S129 acceptance in `glyph_scan` read *"BLINDABLE. Proved S129: `GLYPH={}` still returns 1069/1069, so no family depends on it."* **That statement is true and the conclusion drawn from it is false:** `build_family_map` reads `data-family` FIRST by design (v1.3.8), so it cannot fail that test. **The control was run against the one consumer that structurally could not fail.** Reproduced on the pre-swap tree with the tier emptied: generator 1069/1069, auditor **212 unnamed**. S129's own headline rule — *a control that depends on the state of what it audits is not a control* — one layer down, inside the acceptance written to enforce it.

**§24.14c NEW, AND THIS IS THE SECOND OCCURRENCE, NOT THE FIRST.** `ZUMO_FAMILY_MAP.md` already carried the S112 annotation in its own words: *the repaint did not break the assignment, it revealed that the assignment was never earned.* S112 deleted a COLOUR-keyed tier and replaced it with a GLYPH table — decoration standing in for content a second time. **A stopgap that resolves real blocks is not a stopgap, it is a dependency.** The rule now forbids keying any tier on paint, emoji, icon or class name, extending §24.14's mark prohibition to every decorative property.

**WHAT WAS LOST AND WHAT WAS NOT, AND CONFLATING THEM PRODUCED A WRONG RECOMMENDATION.** The first proposal to DJ was to accept the loss and scope gate 59 to a named 861. **DJ pushed back — *"Why are they unverifiable? Shouldn't we verify them then there is no chance of loss?"* — and he was right.** Two things had been collapsed: *re-derivable from content*, which is genuinely gone, and *protected from silent change*, which is not. Only the first is lost. **All 212 were then verified against the pre-swap glyph with ZERO mismatches** — a real one-time verification, banked, not hypothetical.

**§20.5 NEW: THE PIN NEEDED AN IDENTITY AND THE PARSER DID NOT EMIT ONE.** Three DERIVED keys were tried and all three failed: a content hash at a 1,200-char window re-found **87 of 212**, at 3,000 chars **199 of 212**, and the label text **14 of 193** — with **zero family disagreements in all three**, so the families were never in doubt and the IDENTITY was. Every locator in the suite is a file offset, which any edit above it invalidates. New tool **`callout_id.py` v1.0** writes `data-callout="L.n"` on all 1,069 in §20.2's machine-marker form; `lesson_inventory` **v1.3.3** emits it. **The ordinal is AUTHORED and never recomputed** — an insertion takes the next free number and nothing below renumbers. Proof the write was inert: stripping the attribute back out returns all sixteen files **byte-identical**.

**`ZUMO_FAMILY_PINS.md` IS A PRESERVED LAYER ON THE `css/semantic.css` MODEL.** 212 blocks, generated once from the verified state, read-only input thereafter. **GATE 60 NEW**, written in the same pass as the ruling (S126 rule 16), and it pins by **md5 rather than by row count** for a specific reason: **a pin regenerated from `data-family` has exactly the same 212 keys**, so only a fingerprint can see it. Its coverage arm asserts the PROPERTY — the pinned set must equal the set content cannot name, so a block gaining or losing a signal fails loudly. **The GLYPH tier is deleted outright rather than kept for the rump of 4** blocks whose families have no §7 mark row; keeping it would have preserved exactly the tier this entry rules against.

**TWO PROCESS FAILURES, BOTH MINE, BOTH CAUGHT BY ASSERTS AND NEITHER BY A GATE.** (1) **A control corrupted the pin file and there was no restore path** — it was untracked, so `git checkout` silently did nothing while reporting success. **A control run against an untracked file has no restore path**; snapshot it like any lesson. (2) **The first pin generator read the POST-swap tree**, so the *retired glyph* column — the one fact the record most needed — was empty for all 208 swapped blocks. Regenerated from the pre-apply snapshot after proving per-lesson callout counts identical.

**AND A THIRD, WHICH IS THE WORST SHAPE IN THIS BOOK: A LIBRARY THAT EXITS.** `load_pins()` first raised `SystemExit` on a missing file. Because `build_family_map` is imported by the gate suite, a missing pin **killed the whole run mid-suite and exited 0** — stopping before gates 59 and 60 and reporting success by the very exit code the session ritual says to trust. Fixed to return empty and let gate 60 name it; re-controlled, the missing file now exits 1 with all 60 gates run and TWO gates naming it.

**S130 ADDENDUM 2 — THE CENSUS RAN ONE HIGH PER FILE FOR ITS ENTIRE LIFE, AND §24.6c GAINS A FIFTH PART.** A double check requested by DJ re-derived five load-bearing numbers by methods sharing no code with the originals — 884 marks, 1,069 callouts, 212 pins with 0 family mismatches, and 1,134 image references all reproduced exactly on the pushed clone. **Two figures did not, and neither was an error: both were DEFINITIONAL.** The referenced-asset count differs because `site_parity`'s scope is `images/` + `css/` while an extension-matched regex catches `favicon.ico` and no stylesheet — 157 assets common to both. **The census was the real one:** `lesson_inventory` computed `count('\n') + 1` unconditionally, which counts the empty string AFTER a trailing newline as a line, so it ran **exactly one high per file, 16 high book-wide**, and `wc -l` had disagreed for its whole life. Offered a handoff note explaining the gap, **DJ ruled the other way — *"Why do we need it. Can't we fix the problem?"*** — and that is right: **a number that needs a footnote to avoid being misread is a defect, not a convention.** The `+ 1` is now conditional on the file NOT ending in a newline, which is the only case with a genuine unterminated final line; **L03 was normalised to end in one, so all sixteen now do** and both definitions agree. Census reaches **40,683** (from 40,698). **GATE 62 NEW** is the SECOND METHOD rather than a restatement of the first — it counts newlines directly without importing the parser, and asserts every lesson ends in one, that being the condition under which the two definitions are equal. Control-run both ways, each arm firing alone and naming its own cause: a stripped trailing newline FAILS naming the file, and reverting the counter to the old `+ 1` FAILS naming **40,699 against 40,683**. **§24.6c GAINS PART 5** on DJ's ruling, and the evidence for it is this session's own error catalogue: **nine of nine wrong claims came from a one-off code path and zero from a versioned instrument**, and the only two that reached DJ were the two with no assert at all. Both were one query scoped to a single source reporting a conclusion about all of it. The rule therefore changes what is BUILT rather than what is recalled — a count no instrument produces is a signal to write the instrument — and its compliance is visible in the commit. `book_gates` **v1.57** (62 gates) · `lesson_inventory` **v1.3.4** · `build_mark_index` **v1.1.0** · `glyph_scan` **v1.1**. Applied: L03 **v03.36.1**, MINOR — one byte, nothing renders differently, so the visible banner correctly stays at 03.36.

**S130 ADDENDUM — THE MARK ROSTER RECONCILES, AND BOTH HALVES OF THE CARRIED DEBT WERE WRONG IN OPPOSITE DIRECTIONS.** A hand reconciliation scoped to §7.1's `FAMILY` table reported **14 marks on disk in no §7 row** and declared the carried debt *"§7.2 names seven marks not on disk"* FALSE. **Both readings were wrong and the first was dangerous:** all 14 are named in §7.2, so the list was one ruling away from DELETING eleven correctly-indexed files. §24.8 in the instrument — **the standard has TWO tables and the instrument knew one.** Re-run against both: **zero on-disk marks are unindexed**, and the carried debt is TRUE — seven marks named in §7 are genuinely absent. **But they are not a debt either**, which is the second correction: they are §7.2's SYSTEMS group, and **§7.2's own Grounds table already rules that group *"in scope: no"*** — it sits on a filled band rather than the page tint. The standard accounted for them completely; two sessions of handoffs had been carrying a resolved question as an open one. `build_mark_index` **v1.1.0** now PARSES §7.2 rather than restating it (S83: import the definition, never write a second copy), and `supporting()` is **empty-loud**, because a table that silently parsed to nothing would reproduce the same false orphan list. **GATE 61 NEW**, holding the seven **BY NAME** rather than by count — a count arm passes when one mark is drawn and a different one goes missing. Control-run four ways, each arm firing alone: an unindexed mark on disk FAILS naming it · an indexed non-held mark removed FAILS naming it · **a HELD mark landing FAILS, so the hold expires by itself** · the clean tree is silent. Also this addendum: `glyph_scan` **v1.1** — its post-run line named `mark_wire --apply`, an arc that has now RUN, so a message pinned to one arc had begun quietly misinstructing whoever read it next; rewritten as the durable rule. **Its D1 detector then caught gate 60, written this same session to retire the glyph tier, still reading the glyph field** — accepted by name after a BEHAVIOURAL proof that the RULE lambdas ignore it: blanking the glyph changes the resolution of **0 of 1,069** callouts. **And the S129 acceptance that misled this session is REPLACED rather than left on disk**, since it read *"no family depends on it"* about a module that reads `data-family` first and could never have failed that test.

Controls, each from a snapshot: a removed mark FAILS §21 at 1,133 · a silently re-familied pinned block FAILS §24.14a and `family_tag` reports 1 drifted · an edited pin row FAILS gate 60 on the fingerprint · a deleted pin file FAILS two gates · the untouched tree passes at 60 of 60. `book_gates` **v1.56** (60 gates) · `build_family_map` **v1.4.0** · `family_tag` **v1.1** · `lesson_inventory` **v1.3.3** · `session_versions` **v1.22.0**, whose Control G again named the new tool as registered-but-not-emitted on the first run — the control working as designed for the fourth consecutive tool-adding session. `regex_audit` 1 lead, the known `entity_sweep.py:70` false positive. Applied S130, all sixteen MODERATE (the pages render differently, so both §5b homes move): L01 **v03.24.0** · L02 **v03.17.0** · L03 **v03.36.0** · L04 **v04.24.0** · L05 **v04.24.0** · L06 **v04.28.0** · L07 **v04.27.0** · L08 **v04.26.0** · L09 **v05.21.0** · L10 **v02.22.0** · L11 **v02.23.0** · L12 **v01.26.0** · L13 **v02.23.0** · L14 **v02.28.0** · L15 **v02.24.0** · L16 **v02.17.0**.

v8.121, S129, moderate — **§5.1 STOPS READING THE GLYPH, AND THE GLYPH DETECTOR LEARNS TO SEE A MARK IT WAS BUILT BLIND TO.** S127's rule 19 — scan every instrument by AST when a spelling is ruled — was S129's first job and it is now a versioned instrument rather than a one-off: new tool **`glyph_scan.py` v1.0**, two detectors whose independence is proven by control, keyed acceptance by NAME with a recorded reason. **D1 reads the PROPERTY** — any read of `lesson_inventory`'s `glyph` field, whatever emoji it holds. **D2 reads a glyph LITERAL reaching a locator**, directly or through a module-level table, and catches pins that never touch the parser. Neither subsumes the other, measured. **THE FEAR WAS LARGER THAN THE FINDING: §5.1 is the only GATE pinned on the glyph**, and an independent sweep of HTML, CSS and JS found **zero glyph-pinned locators outside Python**, so the marks arc's blast radius is one gate. **§5.1 RE-SEATED IN BOTH PLACES THE GLYPH SAT.** Its scope pin becomes scheme-agrees-with-`data-family` — S92's reasoning stands, the glyph was merely the only content signal 2026-S92 had — and its label pin becomes decoration-agnostic: the extractor already strips every tag, so an `<img data-mark>` contributes no text and a marked label reduces to the same string a glyphed one does. The gate asserts the property and cannot certify a spelling (S126 rule 18); decoration is owned by §24.14b. **THE RE-SEAT WIDENED COVERAGE 251 → 255 AND THE FIVE THAT ENTERED WERE HIDDEN ONLY BY WEARING A NON-CANONICAL EMOJI ON CANONICAL PAINT** — one passes, four were real §5.1 violations. **One block correctly LEFT scope**: L05's is painted NOTE and resolves INSIGHT, so the old gate was wrong to inspect it. **THE DECISIVE PROOF IS THE S128 EXPERIMENT RE-RUN:** 34 marks applied to L04 — the exact input that measured coverage falling 251 → 240 — leaves the re-seated gate at **255**. **DJ RULED A** on rendered specimens (*"B looks stupid"*): canonical family glyph in the label, old glyph dropped. The four split, and reading BELOW the label then found the class rather than the instance — **8 of 183 two-div blocks opened their title with their own family word**, six carrying nothing but it. Those six are sentence lead-ins S92's own sweep block-split; the redundant div is deleted and the label promoted to `div-fs-105em`, the class the book's other 35 label-alone blocks already use. DJ: *"we don't want Note - Note."* **AND THE RESIDUE HAS ONE CAUSE: `sweep_option_c.py` SCOPED ON `FAMILY_GLYPHS` AND §5.1 SCOPED ON `_FAMGLYPH`** — the S92 sweep and the S92 gate shared one blind spot, so the blocks it missed are exactly the blocks the gate could not see. **THE S92 ONE-SHOT IS DELETED** (DJ ruling), its ruling verified recorded in v8.79 and `BookComponentStandard` §5.1 first; it would not have fixed these either. **THE MAPPING HAS FOUR HOMES AND THE BIBLE'S IS THE STALE ONE** — §8's LOCKED table names 11 types against `BookComponentStandard` §7's DJ-approved 26 and `build_family_map`'s live 30, and opens *"All callouts use inline `style=` only"*, which §27 retired. Measured rather than argued: `BookComponentStandard` is parsed by **six** instruments and this Bible by **one**, and it is 30 KB against 694 KB. **The roster stays where the parsers are**; §8's table becomes a pointer, recorded not yet done. **QUEUE ITEM 2, AND IT NEEDED THREE FIXES WHERE THE HANDOFF PREDICTED ONE.** `lesson_inventory` **v1.3.2** gains a second glyph arm for the four LEARN blocks writing `</>`; **the arm requires TRAILING WHITESPACE, and that property is the whole design**, separating a standalone token from a sentence merely opening with punctuation — L11:173's `"If the robot can tell...` must never read as decoration. Measured across all 1,069: fires on exactly four, refuses the quote; the emoji arm runs first so the widening is additive by construction. **That alone left the four in NOT_LEADING, not SWAP.** `mark_wire`'s test read `label[:1] != glyph` — **two wrong assumptions in one line**: it compares a SINGLE character against what is now a three-character token, and it compares `label`, which preserves entities by design, against `glyph`, which is unescaped. **S127's `_LAND` in a new costume**, latent for emoji only because §27.16 made them literal, and live the moment a glyph has no literal form — `<` and `>` never do. `_swap_one` carried the same assumption on the write side. **PROVED BY DOING IT, NOT BY THE TALLY:** `&lt;/&gt; The five data types` becomes an `<img data-mark>` with the token and its space both consumed. `mark_wire` **v1.0.2** now reconciles at **884 SWAP + 13 NO_GLYPH + 56 HELD**. **QUEUE ITEM 3 DERIVED, AND THREE PROJECTIONS GAVE THREE ANSWERS.** §21's coverage lands at **1,134**, delta **exactly 884**, reconciling against a per-lesson count — against the handoff's 1,134 reached by wrong arithmetic and this session's own 1,147, which wrongly added the 13 insertions `mark_wire` reports but never writes. **Only the run settles it, which is what §24.14 means by derive rather than project.** **THE MEASUREMENT FOUND A DEFECT NO GATE COULD SEE:** `mark_wire`'s own read-back assert refused L16, where two WARNING blocks carry a **doubled variation selector** (`U+26A0 FE0F FE0F`, and `U+26A0 FE0F SPACE FE0F`). A lone selector renders as nothing, so it was invisible on the page, invisible to every gate, and `entity_sweep` passes it because DECODED text is unchanged. Censused book-wide at **exactly two, both L16**; the book is corrected and the consumer is now a RUN rather than a fixed shape, control-run on five shapes. **THREE PROCESS FAILURES, ALL MINE, EACH CAUGHT BY AN ASSERT AND NONE BY A GATE.** (1) A restore used the SESSION-OPEN snapshot after later edits, silently reverting L02's fixes and its bump — S126's rule 17 reached by a new road: **a harness must snapshot every file it can touch, and a snapshot taken before the work is not a snapshot of the work.** (2) Editing L03 top-down moved every target below the first deletion; the callout lookup returned zero and stopped before writing. **§6.12c extended: an edit that changes line count invalidates every line-keyed target below it in the same file — go DESCENDING per file.** (3) A held label was hand-typed and wrong; the coverage arm named it and the set was EMITTED from the tree instead. **`glyph_scan`'s own Control F fired twice on my edits**, both times because an acceptance named a site I had just changed — and its Control A had to be rebuilt against a fixture, because it asserted §5.1's pins EXIST and §5.1 was the thing being fixed: **a control that depends on the state of what it audits is not a control.** `book_gates` **v1.55** unchanged in count at 59 gates, digest baseline moved twice by DERIVATION and re-controlled against a dropped `color: white;` each time. §27.11 holds at **604 / 2,141** across both regenerations — zero born, zero died, zero altered, S113's shape twice — with `.div-fs-09em` moving 156 → 160 → 154 and the attribute count tracking it exactly; `strip_inline --verify` 0 dead class names, so §27.15b never fired. Applied S129: MODERATE (the pages render differently, so both §5b homes move) L02 **v03.16.0** · L03 **v03.35.0** · L05 **v04.23.0** · L08 **v04.25.0** · L12 **v01.25.0**; MINOR (an invisible character, page identical, banner correctly unchanged) L16 **v02.16.1**. **L03 and L08 bump TWICE this session** because their first version was stated before further work landed and §9 makes a version unique per delivery. Census 40,700 → **40,698** [lesson_inventory.py] — +4 for the splits, −6 for the deleted divs, which is the arithmetic check that nothing else moved. Family map unmoved at 1069; `build_css --check` current at 604 rules; `site_parity` PARITY.

v8.120, S128, moderate — **§24.14a NEW: THE FAMILY LIVES IN THE MARKUP, BECAUSE THE MARKS ARC CANNOT START UNTIL IT DOES.** DJ ruled the marks arc next, then its scope: *"Let's do the 953 first."* **THE BLOCKER WAS NOT THE MAPPING — IT WAS THAT REMOVING THE EMOJI ERASES THE FAMILY.** `build_family_map` resolves 1,069 callouts through three tiers, and the last, GLYPH, resolved **209 of the 953 mapped blocks by the decorative emoji alone** — KEY TERM 159 of 184, THE WALL 17 of 17, GOING DEEPER 7 of 7. S112 shipped that tier calling itself a stopgap and predicting this exact moment in its own comment: *"when the emoji are replaced the family must already be known from content."* **`data-family` now sits on all 1,069 blocks**, written by new tool `family_tag.py` **v1.0** FROM `build_family_map`'s own IMPORTED tiers, never a second copy (§83). **THE CORRECTNESS PROOF IS AGREEMENT, NOT INSPECTION (S124):** the generator reading the attribute reproduces its 30-family table **byte-identically**, and blinding the GLYPH tier entirely still returns **1069/1069**. **DIRECTION IS RULED, NOT ASSUMED:** §24.14 makes the mark an OUTPUT of the family, so nothing may ever read a mark filename back to recover one — that closes a loop the canon forbids and would let an icon swap silently re-family the book, the S112 colour-table failure in a new costume. **THE MAPPING WAS ALREADY RULED AND THIS SESSION'S FIRST REPORT MISSTATED IT.** DJ: *"I thought we had the new marks all designated by callout names?"* Correct — `BookComponentStandard` §7 carries the 26-family roster he approved at S91 and amended at S93. The join is **family to mark**, covering **20 families / 953 blocks, 89%**, all EXTRACTED, against the handoff's INFERRED name-match at 58%. The claim that 14 marks carry no role was also wrong: **§7.2 covers 11 of them**, only `battery`/`battery-full`/`battery-half` sit in no table, and §7.2 names **seven marks not on disk**. **A NAME-MATCH MAPPING WAS MEASURED AND REJECTED, WITH A NUMBER ON IT:** the bullseye carries **thirteen distinct labels across 125 uses** (THE GOAL 100, TRY IT 12, FINISHED EARLY? 2, OBJECTIVES 1, nine more) while NOTE's 133 blocks wear thirteen different emoji — not a function in either direction, and matching bullseye to `bullseye.svg` by name would be right 100 times and wrong 25. **TWO DEFECTS IN THIS SESSION'S OWN WORK, BOTH CAUGHT BY ASSERTS WRITTEN FOR THEM.** (1) **`lesson_inventory`'s node offsets are not file offsets** — `build()` runs `expand_classes` first, so every offset indexes the EXPANDED source where each `class` became a longer `style`; the drift grows with position, so a writer lands mid-prose and lands PLAUSIBLY. §24.8 on an offset: real, usable-looking, measured against a different document. Closed with `expand_classes_mapped()`; all 1,069 verified to bracket a clean opening tag. (2) A dry run reported **zero work to do** because the counter only incremented inside the apply branch — a report that cannot tell *nothing to do* from *not looking* is not evidence. **GATE 59 NEW**, same pass as the ruling (S126 rule 16), asserting the PROPERTY (rule 18): it re-derives the family from CONTENT and requires the attribute to AGREE, so a hand-typed attribute contradicting its own content fails like a missing one. Coverage arm included (S117/S118). Control-run four ways with read-back and restore asserts — deleted, wrong family, nonexistent family, emptied — each firing gate 59 **ALONE** at 58 of 58 green, untouched tree passing at BOTH ends, file restored byte-identical. **§24.14b THE MARK RULE IS WRITTEN AND DELIBERATELY HAS NO USERS YET**, which §27.15's model explicitly permits (*stoppable at any point*). `css/semantic.css` gains `img[data-mark]`, and **the selector is an ATTRIBUTE because §27.15b fired again in a NEW direction**: `class="mark"` was promptly re-emitted by `build_css` as `.img-fs-0`, which would have killed `.mark` on the next strip. The graduation model assumes a class LEAVES the markup when a rule arrives; a new semantic class going INTO markup runs the generator backwards. **THE SWAP IS BUILT AND NOT APPLIED.** `mark_wire.py` **v1.0** reconciles exactly: **880 SWAP + 17 NO_GLYPH + 56 BRAIN CHECK = 953**. **BRAIN CHECK is HELD BY NAME** (§25.2a) — all 56 already carry the two-state BrainGear emblem, precisely the behaviour §7.1 specifies, so a mark would be their SECOND; holding it also disposes of all 22 non-leading-glyph blocks, every one of which is a BRAIN CHECK. **ITS OWN CONTROL E CAUGHT THE HOLD BEING FAKE:** §7.1's keys read `BRAIN CHECK · open`, so the family was falling out of scope on a middot mismatch rather than on the hold — delete `HELD_FAMILIES` and behaviour was identical. Normalised, so the hold is real and provable. **DJ RULED THE 17 NO-GLYPH BLOCKS IN** (*"Do we know what the blocks are Named? If so, yes"*) — all 17 carry a named header — and reading them found a fourth shape the instrument cannot see: **four LEARN blocks carry a literal `</>` marker**, and the glyph field takes the first character above U+2100, so an ASCII pseudo-glyph is invisible to it. §24.8 again, the same shape as `_LAND` missing the literal left arrow at S127. True population: **884 swaps and 13 insertions**. **AND THE L04 CONTROL PROVED S127's RULE 19 STILL LIVE: §5.1's coverage fell 251 → 240** on one lesson's marks, because that gate finds a label THROUGH the glyph. The AST scan for glyph-pinned locators is the next act and is NOT yet run; §21's coverage also moves (250 → 284 for one lesson) and needs a controlled per-file delta. Both are why no lesson carries a mark in this push. **A CAPABILITY LANDED THAT DJ SPOTTED:** asked whether this eases an end-of-book glossary, the answer is yes — and it is `data-family` that does it, not the mark. **184 KEY TERM blocks harvest in one pass, 172 with an extractable term, 55 already carrying an `id="term-*"` anchor**, yielding **106 distinct terms with 49 defined more than once** — and showing the Tier-3 census item *"glossary reported absent in all sixteen"* is FALSE: the glossaries are built from this same family and the probe read only `h2`/`h3`. Whether the 49 repeats are deliberate spiral restatements or drift is a READ, not a count, and is unruled. `book_gates` **v1.55**, 59 gates; `lesson_inventory` **v1.3.1**; `build_family_map` **v1.3.8**, baseline UNMOVED at 1069; `session_versions` **v1.21.0**, whose Control G named both new tools as registered-but-not-emitted on the first run — the control working as designed for the third consecutive tool-adding session. `regex_audit` 1 lead, pre-existing in `entity_sweep.py:70` and a match-and-discard false positive: **the S127 handoff's claim of 0 leads does not reproduce, and both files are byte-unchanged since that push**. Applied S128, all sixteen MINOR — **attributes do not render, so the visible §5b banners correctly stay put**: L01 **v03.23.2** · L02 **v03.15.2** · L03 **v03.33.2** · L04 **v04.23.2** · L05 **v04.22.2** · L06 **v04.27.2** · L07 **v04.26.2** · L08 **v04.23.2** · L09 **v05.20.2** · L10 **v02.21.2** · L11 **v02.22.2** · L12 **v01.24.2** · L13 **v02.22.2** · L14 **v02.27.2** · L15 **v02.23.2** · L16 **v02.15.2**. Census unchanged at **40,700** [lesson_inventory.py]; family map unmoved at 1069; `build_css --check` current at 604 rules.

v8.119, S127, moderate — **§27.16 NEW: ONE SPELLING PER CHARACTER, AND THE RULE IS A PROPERTY RATHER THAN A LIST.** DJ ruling: *"Ok, Yes change them to the symbols."* The book spelled the same character two ways — **5,935 non-ASCII characters were already literal while 4,827 were entities** — so every byte-reading instrument counted a fraction of each population. **The discrepancy the handoff carried is now settled and its cause is bigger than the two numbers.** S126's glyph census of *95 distinct / 3,258 occurrences* is reproducible at exactly one scope out of an eight-way grid — literal bytes of the sixteen lesson files, Unicode symbol categories — which yields **95 / 3,273**, and the 15-occurrence gap **IS `≈` U+2248**, the only character in the census with a count of 15. **But that scope is itself the defect:** decoding entities, the true figure is **110 distinct / 3,984**. **34 characters were written BOTH ways and 15 existed ONLY as entities** — 🏠×16, ©×16, 🕵×8, ⭐×3, plus 🎓 and the three medals — and those fifteen appear in **no census ever taken of this book**. Every glyph count in the handoff was therefore low: 💡 114 is **118**, 🔑 158 is **174**, 🎯 113 is **125**. §24.8 again, on a denominator: the population you can enumerate by spelling is not the population that renders. **THE OPPOSITE DIRECTION WAS CONSIDERED AND FAILS ON ITS OWN TERMS.** DJ asked whether writing everything as an entity would be safer. It would not: entity form is not one spelling but three, and **the book already carries 14 characters spelled in more than one entity form** — em dash `&mdash;` 2,129 against `&#8212;` 117, and the apostrophe at `&#39;` 27 against `&#x27;` 44 with no literal at all. An all-entity rule needs a SECOND rule for named vs decimal vs hex, and that rule has already drifted three ways unruled. **Literal is the only direction that collapses to exactly one spelling**, and the only legible one: **103 of the book's 143 non-ASCII characters have no named entity**, so an all-entity book prints `&#128161;` where an editor needs to see a lightbulb. **THE RULE: write the character however it can be SEEN in the source.** Literal when the literal form is distinguishable; an entity when it is not. **The second clause is a PROPERTY, and it reaches exactly three characters** — no-break space, narrow no-break space, and **the non-breaking hyphen U+2011, which was missed on the first pass and is the sharper case of the three**: literal U+2011 renders and reads identically to `-`, so normalising it would silently convert 60 deliberate no-break decisions into characters nobody can see or verify. Named where a name exists, decimal where none does — `&nbsp;`, `&#8209;`, `&#8239;` — and **the gate DERIVES the no-name claim from `html.entities` rather than asserting it**. **SCOPE IS TEXT NODES, AND THE TWO EXCLUSIONS ARE PROPERTIES NOT TASTES:** `<script>` and `<style>` are RAW TEXT the parser never decodes, so `&#39;` there is six literal characters of JavaScript and rewriting it would change what the program says — **the Maker's payloads live exactly there**; and attribute interiors stay untouched, where `&quot;` inside a double-quoted value is mandatory. **4,789 conversions across 20 files, 239 held, 357 protected occurrences left alone.** **THE PROOF IS NOT THE DIFF.** The decoded text of all 21 files is **byte-identical before and after**, asserted inside the tool and again independently against a full-tree snapshot: the page renders exactly as it did and only the bytes on disk moved. **THE COUPLING PREDICTED AS DANGEROUS WAS THE SAFE ONE, WHICH IS WHY IT WAS MEASURED.** 439 entities sit inside `<pre>` and `gate_payload_match` byte-derives Maker payloads from those blocks — but `decode_pres()` calls `H.unescape`, so it compares DECODED text and was always invariant to this. It passed untouched. **WHAT DID BREAK IS S126's RULE 18 ARRIVING AS A LIVE DEFECT: FOUR PREDICATES PINNED A SPELLING.** Gates §25.6, §25.2 and §4.5 searched the source for the literal string `&mdash;`, and `next_pointer` EMITTED it. **`book_gates.py:396` already carried the correct dual form and its broken twin sat eleven lines away at 701** — one assertion, written both ways, in the same file. **AND THE SAME DEFECT HID IN A LOCATOR, WHERE IT IS DANGEROUS RATHER THAN LOUD:** `strip_inline`'s hero-version and footer spans are found by the same pinned strings, and with them blind **L01's held attributes fell 39 → 32** — seven attributes silently unprotected, which is the `--include-held` hazard reached by a different road. Caught by its own Control H, not by a gate. An AST scan of every instrument then enumerated the rest rather than waiting for the next failure, and found **`_LAND`, the landing-zone detector, listing `&larr;` and `&#8592;` but not the literal `←` — already blind to 33 of the book's 36 left-arrows BEFORE this session**. Widened; the book is clean behind it. **TWO DEAD CONSTRUCTS FOUND AND PROVED PRE-EXISTING RATHER THAN BLAMED ON THE SWEEP:** `BONUS_MARK` in `book_gates.py` and `MARK` in `gen_bonus_banner.py` are defined and indexed nowhere, and two stray-checks cannot fire — the bonus banner carries no mark at all, having lost it to S108's no-icons ruling. Verified against the pre-sweep snapshot: **zero marks in any banner before the sweep. That is a live lead for the marks arc, since the banner is where a mark would go.** **GATE 58 NEW**, written in the same pass as the ruling per S126. It asserts the property, not the spelling: it re-runs the sweep over every page and fails on any convertible occurrence, checks that no held character appears literally, and **guards its own definition** — adding a fourth character to `HOLD` fails. Control-run seven ways from a full-tree snapshot with read-back asserts: four defect shapes each fire gate 58 **ALONE** at 57 of 57 others green; a fourth-character `HOLD` drift fires alone; **an entity injected into a `<script>` body leaves gate 58 correctly silent**; and the untouched tree passes at 58 of 58 at BOTH ends with a full-tree diff proving the restore real. `regex_audit` 0 leads. `book_gates` **v1.54**, `strip_inline` **v1.2**, `next_pointer` **v1.2**, `session_versions` **v1.20.0** — whose Control G named `entity_sweep` as registered-but-not-emitted on the first run, the control working as designed. New instrument `entity_sweep.py` **v1.0**, registered. Applied S127, all MINOR — **the rendered output is byte-identical, so the visible §5b banners correctly stay put**: L01 **v03.23.1** · L02 **v03.15.1** · L03 **v03.33.1** · L04 **v04.23.1** · L05 **v04.22.1** · L06 **v04.27.1** · L07 **v04.26.1** · L08 **v04.23.1** · L09 **v05.20.1** · L10 **v02.21.1** · L11 **v02.22.1** · L12 **v01.24.1** · L13 **v02.22.1** · L14 **v02.27.1** · L15 **v02.23.1** · L16 **v02.15.1**; `going_deeper` **v01.6.1**, Maker **v2.45.4**, Timer **v1.3.2**. No stylesheet movement: `build_css --check` current at 604 rules, family map unmoved at 1069, census unchanged at **40,700** [lesson_inventory.py].

v8.118, S126, moderate — **§27.15e NEW: THE DARK CODE BLOCK GRADUATES, AND A BORDER 398 BLOCKS CARRIED FOR TWO YEARS TURNED OUT TO BE INVISIBLE.** The third and largest graduate: **802 `<pre>` elements** that **fourteen** generated class names stood for, and the fourteen disagreed. Padding was unanimous at 15px; colour, line-height, font-size and overflow-x were each 98–99% one value, so those are drift and normalise. **TWO declarations were a real split and DJ's eye is what resolved the first one.** Border: 398 blocks carried `1px solid #333`, 289 carried none — and that is **the L09/L10 SEAM, not a preference** (L02–L09 bordered 365 of 372; L10–L16 bordered 29 of 281), the same seam S108 found for the §6.5 Box and S123 for the mono stack. **Shown four rendered specimens, DJ said they all looked the same — and the measurement agrees: `#333` on `#1e1e1e` is 1.32:1 against a 3:1 WCAG minimum for a non-text boundary.** The aggregate 57/42 was a true number telling the wrong story; there was never a majority to defer to, only two conventions. Ruled to a border that earns its place. **THE FIRST VALUE PROPOSED WAS WRONG AND ITS OWN GATE CAUGHT IT: `#686868` was computed by hand as "3:1" and is 2.992:1.** Gate 57 asserts the RATIO rather than the spelling, so it failed before the push; a gate checking for the literal `#686868` would have certified an invisible border as the ruled one. **`#696969`** is the lowest grey that genuinely clears 3:1 and is indistinguishable by eye from the value it corrects. Radius ruled **6px** (389 against 265 at 8px). **NO OPT-OUT AND NO EXCEPTION LIST.** The 30 light `#f8f9fa` blocks need none — they declare background, border, radius and padding themselves, so their class beats the element rule on every property it sets, checked rather than assumed. **The 109 blocks that sat inside a dark WRAPPER `<div>` in L02/L03 were UNWRAPPED rather than excepted**, because an opt-out is an exception someone must remember forever; all 109 held exactly one `<pre>` and nothing else, verified across all **eight** wrapper spellings — a population first sampled at ONE and reported as two spellings, §24.8 again. Six bare `<pre>` in L02 join the rule too: three carry `tok-*` syntax spans and are simply code, and three are expected serial output, where **the dark block was already the book's convention**, measured rather than ruled from the three in hand. **§27.15b FIRED FOR REAL, WHICH IS THE ENTRY'S FIRST PROCESS FINDING.** 176 `<pre>` joined `.p-m-0`'s 120 users, `<pre>` became the dominant tag, the rule renamed to `.pre-m-0`, and **119 `<p>` plus one `<ul>` went dead** — caught by `strip_inline --verify`, invisible to every gate, exactly as §27.15b predicts. **The tree was REVERTED to the pushed clone and re-derived with the survivor rename in its correct slot, not patched forward**, and the rename mapped survivors by **identical declarations**, never by name arithmetic — which is what caught the second one, `.div-bg-1e1e1e-2` taking the name `.div-bg-1e1e1e` after the original holder died in the unwrap. **AND THE CONTROL HARNESS COMMITTED S121'S ERROR A SECOND TIME, IN A NEW PLACE.** It snapshotted the stylesheets but not the lesson file one injection touched, and the cleanup then ran `git checkout --` on `Lesson_05.html` — which reverts to **HEAD**, and none of this work was committed, so it silently destroyed that lesson's share of the build. **The untouched-tree control is what exposed it, by failing — the one control whose only job is to have nothing wrong with it.** §12's rule restated on harnesses: **a harness must snapshot every file it can touch, and version control is not a snapshot for work that has never been committed.** Redone from a full-tree snapshot, six ways, each firing gate 57 **ALONE** once the stylesheet is regenerated — including on `#686868`. **§27.11 MOVES TO 604 / 2,141 AND RECONCILES EXACTLY:** 23 rules die carrying 148 declarations, 3 are born carrying 3, one is altered gaining 4, and −148 + 3 + 4 = −141. **PROOF NOTHING ELSE MOVED:** every lesson expanded against its own stylesheet before and after, and **every changed line is a `<pre>`, a removed wrapper `<div>`, or whitespace — ZERO others across all sixteen.** `book_gates` **v1.53**, 57 gates; `regex_audit` 0 leads. Applied S126, all sixteen MODERATE (every page renders differently, so both §5b homes move): L01 **v03.23.0** · L02 **v03.15.0** · L03 **v03.33.0** · L04 **v04.23.0** · L05 **v04.22.0** · L06 **v04.27.0** · L07 **v04.26.0** · L08 **v04.23.0** · L09 **v05.20.0** · L10 **v02.21.0** · L11 **v02.22.0** · L12 **v01.24.0** · L13 **v02.22.0** · L14 **v02.27.0** · L15 **v02.23.0** · L16 **v02.15.0**. Census 41,128 → **40,700** [lesson_inventory.py] — the 428-line fall IS the unwrap, 109 wrappers at four lines each minus the blanks it closed. Family map unmoved at 1069.

v8.117, S126, moderate — **§27.12 EXTENDED AND §27.15d NEW: A PAGE CAN STYLE ITSELF AND STILL BE PART OF THE BOOK, BUT ONLY IF THE RESOLVER CAN SEE IT.** Three jobs, and each one found the next. **FIRST, §27.15a's *no exception list* was not true of the tree.** Five inline `<code>` elements in L03 still carried a class — the last five in the book — invisible to S124's strip because that sweep enumerated the class names it knew and these three are spelled differently (§24.8). Two of them, `.code-inline-bg-f5c6cb` and `-f5c6c0`, are the SAME pink one digit apart on the same construct: a typo, not two colours. DJ: *"Fix the typo, all should be consistent in the entire book."* Fixing the SPELLING would have preserved an exception the ruling forbids, so all five classes were deleted and the elements take the ruled wash. §27.11 moves to **624 / 2,282**, and the −15 declarations IS those three rules at five each — zero born, zero altered, so §27.15b's rename trap does not fire. **SECOND, §27.15d NEW: THE RESOLVER KNEW EXACTLY ONE STYLESHEET.** `expand_classes` read `css/book.css` and nothing else, so a page styling itself from its own `<style>` block was OPAQUE to every gate that reads through it — the class stayed a class and resolved to nothing. **The consequence was concrete and nobody had seen it: `going_deeper.html`'s hero satisfied §25.6 ONLY BECAUSE IT WAS WRITTEN INLINE.** Inline and expanded-from-`book.css` produce the same string; a class the resolver cannot see produces neither. So the seven inline `style` attributes S125 recorded as a scope question were not leftovers at all — six of them were load-bearing for a gate. **DJ ruled A: teach the resolver the page's own `<style>` block.** `lesson_inventory` **v1.3.0** gains `page_css()`, merged with the page winning any collision, which is the seating §27.15c already ruled. **It is INERT for the sixteen lessons, measured rather than assumed: not one carries a `<style>` block at all.** going_deeper's hero and footer convert to classes whose declarations are byte-faithful restatements of the lessons', which is normally the two-spellings drift §27.15c rules against — **it is safe here, and only here, because §25.6 is the watcher: it compares all seventeen heroes by RESOLVED styling, so a drift in either copy fires it.** Proven by injection: one declaration moved from `padding: 30px` to `31px` FAILS §25.6 alone. The page also adopts **Inter**, closing S125's second recorded find — it was loading the webfont and rendering Arial. Four dead `.hero` rules deleted, zero uses, verified before deletion. **THIRD, AND IT IS THE ENTRY'S REAL FINDING: TWO GATES KEYED THEIR SCOPE ON A BARE SUBSTRING, AND THE REPAIR FOR THAT INTRODUCED A WORSE BUG THAT ONLY A COVERAGE ARM COULD SEE.** A COMMENT added to going_deeper's own `<style>` block, reading *…reach through `css/book.css`…*, pulled the page into §27's scope and reported eleven of its own classes as unresolved — because the predicate was `'css/book.css' in src`. **A page that merely MENTIONS a path is not a page that LINKS it.** Replaced with a `<link…href>` match — which demanded the bare path and therefore matched **ZERO of the sixteen lessons**, because they live in `lessons/` and link `../css/book.css` (§27.10, the book does not name its own host). **§27 then PASSED on an empty scope and looked exactly like a gate that was working.** It was caught ONLY because gate 44 had been given a COVERAGE arm in the same pass and §27 had none — **S117/S118 for the third time, and the first time the empty scope was one I had just written.** Both predicates now allow the relative prefix; §27 gains its coverage arm. **§27.12's SCOPE now DERIVES from EITHER stylesheet** (DJ: *"Yes watch pages that link"*), so a page joining the layer by either pipe enters automatically — S125's lesson applied one level up, since §27.15c's new delivery path had left going_deeper PARTLY converted with gate 44 structurally unable to see it. **`index.html` links NEITHER sheet and is held BY NAME** (§25.2a), because DJ ruled its one inline `style` out and **a fix with no gate is an unguarded fix**; the tool pages stay out by §25.6a. **COMPLEMENTARITY MEASURED, NOT ASSERTED (the S106 shape):** an inline style pasted into going_deeper's preamble — a span no other gate covers — FAILS §27.12 **alone** under the new scope and is **completely invisible** under the old one. Control-run from a pristine snapshot with read-back asserts, seven ways, untouched tree passing at both ends. `regex_audit` 0 leads. `book_gates` **v1.52**, `lesson_inventory` **v1.3.0**. Applied S126: L03 reaches **v03.32.0** (moderate — four pills stop being pink, so both §5b homes move); `going_deeper` reaches **v01.6.0** (moderate — body face and hero markup both change). Census unchanged at **41,128** [lesson_inventory.py]; family map unmoved at 1069.

v8.116.1, S125, minor — **§27.15c IS NOW HELD BY GATE 56, AND IT SHIPPED UNGATED FOR ONE COMMIT.** The ruling was made and applied, and **nothing guarded the path it created.** The sixteen lessons reach the semantic layer through `css/book.css`, which gate 54 holds byte-for-byte; `going_deeper.html` reaches it through a direct `<link>` that no instrument in the tree could see. **MEASURED BY INJECTION BEFORE THE GATE WAS WRITTEN, and each of the four left ALL 55 PRECEDING GATES GREEN AND EXIT 0:** delete the `<link>` and 71 inline `<code>` silently lose the ruled pill; restate `code { background: … }` in the page's own `<style>` and one ruled construct has two spellings again, which is the exact drift §27.15c ruled against; point the `href` at a path that does not exist and the pipe is decorative; seat the `<link>` BELOW the `<style>` block and the layer starts overriding the page instead of the other way round. **THE TWO INSTRUMENTS THAT LOOK LIKE THEY SHOULD HAVE COVERED IT ARE BOTH STRUCTURALLY BLIND, §24.8 TWICE:** `site_parity` compares what the site SERVES against the repo, and a page that stops REFERENCING a file still matches; `font_stack_sweep` rewrites non-compliant stacks, and an ABSENT stack is not non-compliant. **The consumer set is NAMED, not counted** (§25.2a), and the gate carries a COVERAGE arm because a gate that scans zero pages passes (S117/S118) — emptying `_SEM_CONSUMERS` FAILS naming *scanned ZERO consumer pages*. Control-run six ways from a pristine snapshot with read-back asserts (S117/S121), each of the five defect shapes firing gate 56 **ALONE** at 55 of 55 others green, and the untouched tree passing at 56 of 56. `regex_audit` run after the gate was written per S124 and returned 0 leads. **THE GENERAL SHAPE, AND IT IS THE ENTRY'S POINT: A RULING THAT CREATES A NEW DELIVERY PATH CREATES A NEW UNGUARDED PATH, AND THE SESSION THAT RULES IT IS THE SESSION THAT OWES THE GATE.** The coupling cost of §27.15c was written into the handoff as a known risk and no gate was written for it in the same pass — §24.2's shape exactly, one scale down: **write the gate in the same pass as the ruling, not in the handoff that describes it.** `book_gates` reaches **v1.51** (from v1.50), 56 gates. No lesson file changed, `going_deeper.html` unchanged and byte-verified against the pushed clone, `build_css --check` current at 627 rules, census unchanged at **41,128** [lesson_inventory.py], family map unmoved at 1069.

v8.116, S125, moderate — **§27.15c NEW: `going_deeper.html` JOINS THE SEMANTIC LAYER, AND ONE BOOK MEANS ONE CONVENTION.** DJ ruling C: *"I want the book to be the same throughout."* S124 recorded the page as carrying a THIRD inline-code treatment — a DARK pill, `#1e1e1e` ground, `#9cdcfe` text, a border, `.88em` — and deliberately left it unruled. Measured this session before proposing anything: **77 `<code>` elements, 71 inline and 6 inside `<pre>`**, and **not one of the 71 carries a class attribute**, the same bare-element population that was invisible to S124's first sweep. **THE S124 TRAP DOES NOT FIRE HERE AND THAT WAS CHECKED RATHER THAN ASSUMED:** the grounds are **62 on `#ffffff` and 9 on `#e3f0e5`**, zero dark, zero `<th>` ancestors, and no container in any chain declares `color: white`. **The content is the lesson construct**, not a different one — 22 identifiers, keywords and types, 19 calls and signatures, 9 expressions, 7 compiler-error strings, 5 literals, 5 preprocessor directives, 4 filenames — and the one shape that could have been a divergence is not: **compiler-error text marked as `<code>` matches lesson practice at 23 bare occurrences across L01–L16**, `'calibrateSensors' was not declared in this scope` appearing verbatim in both places. **THE ARGUMENT AGAINST A DELIBERATELY DIFFERENT CONTEXT IS THAT THE BOOK ALREADY SHIPS THE SPLIT IT WOULD HAVE DEFENDED.** `css/book.css` carries six `.code-block-bg-1e1e1e` variants and nine bare `<code>` live inside L01's `#1e1e1e` blocks under the light pill, so **dark block plus light inline is already canon in sixteen lessons**; going_deeper was the only page where inline matched the block, and it is linked from all sixteen lessons and from `index.html`, so a student crosses that seam constantly. **A CLAIM CARRIED IN FROM THE PRIOR SESSION WAS FALSE AND DJ'S QUESTION IS WHAT TESTED IT.** Asked whether S124 had delivered the pill *by linking*, the answer measured against the tree is **no: nothing in the repo links `css/semantic.css` at all**. The lessons link `css/book.css`, which carries the semantic layer preserved verbatim at byte offset 973 — the pill arrived through a pipe that already existed. §24.6c: **a mechanism inherited from a changelog is a lead, not a finding.** **THE MECHANISM WAS RULED ON WHICH FAILURE IT LEAVES BEHIND, NOT ON EFFORT.** Rewriting the page's own `code` rule in place spells the pill in TWO files and **nothing watches the second** — gate 54 asserts the layer appears verbatim inside `css/book.css` and is structurally blind to a third copy inside a page's `<style>` block, so that option is only safe with a gate written alongside it and is therefore the MORE expensive one. Linking `css/book.css` instead was measured and rejected: **84,541 bytes and 627 lesson-specific rules pulled into a page that uses none of them, to obtain four declarations.** So going_deeper becomes the **FIRST DIRECT CONSUMER of `css/semantic.css`**, and the layer now has two delivery paths and one source. **THE COUPLING COST IS REAL AND IS RECORDED RATHER THAN WAVED OFF:** the page will inherit whatever graduates into the layer next, uninvited, and it defines its own `.callout` while callout families are a queued graduation candidate. The `<link>` is seated ABOVE the page's `<style>` block on purpose, so the page wins any collision on a declaration it sets — but only on the declarations it sets. **FOUR DECLARATIONS LEFT THE PAGE AND NOTHING WAS ADDED:** the local `code` rule, its mono `font-family`, the `pre` rule's mono `font-family`, and the page's own `pre code` reset, which the layer already holds. Six changed lines total, each edit asserted at exactly one occurrence with post-asserts that `#9cdcfe` and every local `ui-monospace` string are gone. **NO STYLESHEET MOVEMENT AND NO CASCADE:** going_deeper is not a `build_css` source, so §27.8b's three-step sequence never ran, `build_css --check` stays current at **627 rules**, §27.11 holds at 627 / 2,297 and the family map is unmoved at 1069. `site_parity` moves 139 → 140 referenced assets and still reports PARITY, because the site already serves the file. **GATE 44 STILL DOES NOT REACH THIS PAGE AND THAT IS NOW A SCOPE QUESTION RATHER THAN AN OVERSIGHT:** §27.12 keys on `css/book.css` appearing in the file, so going_deeper's seven inline `style` attributes stay outside it while the page is now PARTLY converted — recorded, not ruled. **THE VISIBLE §5b BANNER WAS MISSED AND THE GATE CAUGHT IT**, failing with *hidden=01.5 but visible=01.4* before anything was presented; the first sweep for version homes grepped for `version:` and `Version 1` and the banner reads `Version 01.4`. **A SEPARATE FIND, RECORDED NOT RULED:** going_deeper preconnects to Google Fonts and loads **Inter**, and never uses it — its only `font-family` declarations are Arial for `body` and the ruled mono stack; every lesson body is Inter, so the page is off-family in body text and pays for a webfont it discards. Applied S125: `going_deeper` reaches **v01.5.0** (from v01.4.2), MODERATE — 71 elements render differently, so both §5b homes move. Census unchanged at **41,128** [lesson_inventory.py]; `book_gates` unchanged at v1.50, 55 gates, all passing.

v8.115, S124, moderate — **§27.15a NEW: THE PILL IS RULED IN, AND AN ELEMENT RULE REACHES ELEMENTS NO AUTHOR EVER LISTED.** DJ ruling: *"Pill everywhere."* The second graduate into `css/semantic.css`, and the first to leave by DELETION — **2,132 class attributes stripped from all sixteen lessons**, ten rules dead, one born, net −9. **The ruling was taken TWICE on purpose**: once for inline code generally, and again explicitly for the 28 `.code-ff-uimonosp` uses that had drifted to mono-without-pill, on the evidence that **36 operator `<code>` elements elsewhere in the book were ALREADY pilled against 12 that were not** — the same operator appearing both ways, the 12 confined to L02 and L03. There was no operator convention to preserve, only local drift, so the rule ships with no exception list and no exception a gate must encode. **THE L09/L10 SEAM THE HANDOFF DESCRIBED IS NOT CLEAN**: re-measured without inheriting S123's figures, every back-half lesson still carried pills (L10 27, L12 27, L13 20, L14 16, L16 9) and every front-half lesson carried bare ones (L02 24, L08 23, L07 18); L01 is 52% bare, not the 49% carried forward. **SIX CLASSES STOOD FOR ONE CONSTRUCT AND DISAGREED ABOUT IT** — `.code-inline-bg-e8e8e8` through `-6` shipped three font sizes (0.9em, 0.85em, and none, which renders 1em), one added `color:#333`, one added `white-space:pre-wrap`. All six dropped; the normalisations are the POINT of one pill, and the pre-wrap was **verified inert before it went** — its string has no repeated spaces and no newline. **THE GROUND IS `rgba(0, 0, 0, 0.08)`, NOT AN OPAQUE GREY, DJ RULING B.** Inline code sits on **43 distinct backgrounds** and only 46% of it is on the cream page; **686 pills already sit on 27 coloured grounds**, so neutral-grey-on-coloured was already the status quo. **Eight `<code>` elements sit on DARK grounds whose containers each declare `color:white`** — seven bare ones in dark table headers (L04 `#a04a5f`, L05 `#352405` ×2, L10 `#00393d` ×2, L12 `#433014` ×2) and L14's score formula in a `#1f2a3d` banner — and an opaque pill renders white text on light grey in all eight, silently. **THE SEVEN WERE INVISIBLE TO THE FIRST SWEEP BECAUSE THEY CARRY NO CLASS**: the audit looked at the elements that still had one, found the two that did, and reported two. **§24.8 again — the population you can enumerate is not the population an element rule reaches** — and DJ found the hole by asking *"why would I want to change from code to span?"* about a proposal that was reaching for the MARKUP on a sample of one. With eight cases the markup was obviously innocent and the rule was obviously unscoped. **THE OPTIONS WERE ALSO MISPRICED TO DJ AND HE CAUGHT THAT TOO.** A translucent wash was first recommended as *the conservative choice* and it is the opposite — it repaints all 3,336 elements including 2,097 already ruled, where an opaque pill plus a scoped opt-out changes 1,233. Then, told the wash was *more work*, the correct answer was that it is a ONE-LINE value change, identical labour: **3,336 is a count of what CHANGES, not of what it COSTS**, and conflating the two nearly lost DJ the option he preferred. **THE `pre code` RESET IS PART OF THE RULE.** 15 bare `<code>` live inside `<pre>`, nine in L01's `#1e1e1e` blocks; an unscoped background paints a pill inside every dark code block in the book. **Gate 53 checks declarations that EXIST, §27.13 checks the GENERATED block, and gate 54 checks the layer is preserved VERBATIM — a verbatim copy of a wrong rule passes all three.** **GATE 55 NEW**, holding the reset and asserting the ground is still a wash; control-run three ways, each firing gate 55 ALONE: reset deleted FAILS, ground made opaque FAILS, reset that does not clear the background FAILS. **`going_deeper.html` REACHED THE SAME RESET INDEPENDENTLY** and is recorded as carrying a THIRD inline-code treatment nobody had counted — a DARK pill, `#1e1e1e` ground, `#9cdcfe` text, a border, `.88em`, across 71 inline uses — **not ruled**, because that page does not link this stylesheet. **THE GENERATOR PROPERTY THAT COST A FULL REDO, AND IT IS NEW CANON (§27.15b): STRIPPING A CLASS FROM SOME OF ITS USERS RENAMES IT FOR THE SURVIVORS.** `build_css` names a rule after its DOMINANT TAG, so when four `<code>` elements left `.code-ff-uimonosp-2`, the three compiler-error `<span>`/`<strong>` elements sharing it were renamed to `.span-ff-uimonosp` and went dead. **No gate saw it; `strip_inline --verify` did.** The recovery order is load-bearing: **strip → regenerate → move the survivors onto the name pass 1 emitted → regenerate again.** Renaming after a SECOND pass is unrecoverable, because the generator can only preserve a class it can still resolve — that mistake was made, and the fix was to revert and re-derive from a clean clone rather than to patch forward. **§27.11 MOVED TO 627 / 2,297**, and the −9 is fully accounted: ten rules die (six pill variants, three mono, `.code-c-white`), one is born (`.span-ff-uimonosp`). Declarations fall further than rules because the six disagreed. **THE WHOLE BUILD WAS RE-DERIVED FROM A CLEAN CLONE AND DIFFED AGAINST THE FIRST ATTEMPT**: all sixteen lesson files and the entire generated block came back **byte-identical**, which is the strongest available evidence that the strip, the survivor rename and the thirty-two version homes are right — two independent runs agreeing beats either run verifying itself. `book_gates` **v1.50** (55 gates); `regex_audit` caught a match-and-discard alternation in gate 55 on its first run and it was made non-capturing, restoring 0 leads. Applied S124, all sixteen MODERATE (every page renders differently, so both §5b homes move): L01 **v03.22.0** · L02 **v03.14.0** · L03 **v03.31.0** · L04 **v04.22.0** · L05 **v04.21.0** · L06 **v04.26.0** · L07 **v04.25.0** · L08 **v04.22.0** · L09 **v05.19.0** · L10 **v02.20.0** · L11 **v02.21.0** · L12 **v01.23.0** · L13 **v02.21.0** · L14 **v02.26.0** · L15 **v02.22.0** · L16 **v02.14.0**. Census unchanged at **41,128** [lesson_inventory.py]; family map unmoved at 1069.

v8.114, S123, moderate — **§27.15 NEW: THE SEMANTIC LAYER. THE STYLESHEET COULD NOT SAY WHAT ANYTHING IS.** DJ, cutting through a ruling I had framed as a paint choice: *"I thought since we weren't using canvas we didn't need the inline code."* **Correct, and measured — the lessons carry ZERO inline `style` attributes and 25,752 `class` attributes.** The inline representation `build_css` round-trips through exists only inside one run of the generator; §27 retired the constraint that produced the model and the model outlived it. **THE CONSEQUENCE IS ARCHITECTURAL, NOT COSMETIC:** because names are re-derived from VALUES every run, the generated block can only reproduce the status quo — `.code-inline-bg-e8e8e8` can never become `.code`, and no element selector, semantic class or custom property can survive in it. **It is a fixed point of the migration, not a design system**, which is why there was nothing a second book could inherit and why `ZUMO_COLOR_LEDGER.md`'s first item is not a colour but whether custom properties can be emitted at all. One disease, two symptoms. Measured: **540 of 636 class names encode a value**, covering 22,816 of 25,752 uses. **`css/semantic.css` IS THE ONE STYLESHEET FILE HAND-EDITED ON PURPOSE**, preserved verbatim at the top of `css/book.css` and never re-derived; `book.css` stays DO-NOT-HAND-EDIT (§24.12). **THE CHANGE IS ADDITIVE BY CONSTRUCTION: zero lesson edits, all 540 value-named classes keep working, and a tree without `semantic.css` still builds.** Retiring the round-trip outright is explicitly NOT ruled — it would touch 540 class names, seven instruments reading `expand_classes` and 25 gate references — so the door is open and walking through it is optional, incremental and stoppable. **FIRST GRADUATE: `code, pre`.** 1,199 bare `<code>` elements across L10–L16 were reached by NO author rule, so their typeface was the browser's rather than the book's, while 2,136 styled ones in L01–L09 carried the ruled stack via a class — one construct, two conventions, **split at the L09/L10 seam, the same seam S108 found for the §6.5 Box**. The rule now reaches **3,336 inline `<code>` and 832 `<pre>`**. It declares `font-family` and nothing else: **the grey pill is unruled and deliberately deferred**, and when ruled it is ruled in ONE line for every book. **GATE 53 COULD NOT HAVE FOUND THIS AND SAID SO BY PASSING** — it checks declarations that EXIST, and an unstyled element has none (§24.8, on a gate shipped four hours earlier in the same session). **GATE 54 EXISTS BECAUSE THE FAILURE MODE IS SILENT:** measured by injection before the layer was built, an element rule pasted into `css/book.css` did not error and did not warn — the next regeneration DELETED it and every gate stayed green. It asserts presence, byte-equality with the source, both markers, at least one element selector, and no value-named class. **Its independence is MEASURED:** adding a value-named class to `semantic.css` and regenerating leaves `build_css --check` reporting *current* and §27.13 green, and gate 54 alone names it — the S106 complementarity shape. **§27.11 AND CONTROL D ARE NOW SCOPED TO THE GENERATED BLOCK, SCOPED RATHER THAN WEAKENED.** Scoping closed a mixed population this session created: the baseline counted class rules but every declaration, so one added element rule read as *"a rule gained or lost a property"* — the precisely wrong diagnosis in the gate's own words. **Rules and declarations hold UNCHANGED at 636 / 2,332**, which is the proof that adding the layer moved nothing generated; only the digest moves, because its span now begins at the GENERATED BLOCK marker. `build_css` **v1.3.0**, `book_gates` **v1.49** (54 gates). No lesson file changed; census unchanged at **41,128** [lesson_inventory.py]; family map unmoved at 1069. **OPEN, NOT RULED:** whether `css/semantic.css` should carry a version home of its own, as `BookComponentStandard.md` does — it is canon and nothing versions it.

v8.113, S123, moderate — **§6.5a-T EXTENDED: THE BOOK HAS ONE MONO STACK, AND THE LEAD THAT OPENED THE ARC NAMED THE MINORITY.** The queue carried a *Consolas* lead: 15 declarations in `css/book.css`. Measured, it was **422 declarations across 12 lessons** consolidating into those 15 rules, in **five spellings resolving to THREE different fallbacks off Windows and ONE face on Windows** — which is why it survived, because the only readers who could see it were the ones who could not report it. On a Mac the dark code block rendered Monaco while inline code rendered the browser default: two code faces on one page. **THEN GATE 53 RESIZED THE PROBLEM ON ITS FIRST RUN.** The book's DOMINANT code type was never Consolas at all — `'Courier New', monospace` held **21 rules and 2,825 uses** against Consolas's 14 and 459, and `font_stack_sweep` is structurally blind to it because its first face is not a named substitution risk. **§24.6c one level up: a lead names a SYMPTOM, and scoping the instrument to the lead's own vocabulary guarantees you measure only what you were already told.** The right question was never *where is Consolas* but *how many mono stacks does this book have*. That error reached DJ as a priced ruling — the A/B/C specimen was costed on 459 uses when the population was 3,294, and A was described as a downgrade when for 86% of the book it was already the status quo. Re-priced and re-ruled **B-FULL**. **THE RULED STACK:** `ui-monospace, SFMono-Regular, Menlo, Consolas, 'Liberation Mono', 'Courier New', monospace`, now on all 38 declarations. **GATE 53 NEW, AND ITS INDEPENDENCE IS MEASURED RATHER THAN ASSERTED:** injecting `ui-monospace, monospace` — safe first face, wrong tail — leaves the sweep reporting **0 rewrites** while gate 53 names the file; a third control on `timer.html` fires gate 53 and NOT §27.11, because the stylesheet baseline cannot see a tool page. **THE TOOL PAGES JOIN THE SWEEP'S SCAN**, for the reason S108 put `css/book.css` in it: four declarations in `going_deeper.html`, `newproject.html` and `timer.html` that NO instrument in the tree could see, the sweep never opening them and `build_css` not modelling them (§25.6a). The web-served exemption is now DERIVED per file — going_deeper serves Inter, the other two do not, so a blanket `.html` rule would have been wrong in one direction — and the mono target is CONTEXT-dependent, since an SVG cannot resolve the `ui-monospace` keyword. **§27.11 MOVED TWICE AND THE SECOND MOVE IS THE ONE THAT COUNTS:** 641/2,350 → 636/2,332, diffed by SELECTOR — **39 mono rules before, 10 DIED and 29 ALTERED, 39 = 10 + 29, and nothing non-mono moved.** Five of the ten are font-named classes replaced by `.code-ff-uimonosp*`; the other two, `.code-block-333-6` and `.code-inline-bg-e8e8e8-7`, **COLLAPSED into surviving twins** — proved by comparing their declarations with `font-family` removed, not inferred from the rule disappearing. `strip_inline --verify` 0 dead class names. Re-controlled after the move against a dropped `color: white;`, which still FAILS. **A REGISTRATION GAP THE SESSION EXPOSED BY WALKING INTO IT:** `timer.html` carries a version home and was registered in NOTHING — `session_versions` had never tracked it, so its version could only ever have been hand-typed, the exact §12.6 shape. Registered, and its Control G confirmed it reaches both emitted blocks. `book_gates` **v1.48** (53 gates), `font_stack_sweep` **v1.3.0** with new controls I/J/K, because every pre-existing control called `rewrite()` without a path and could therefore see none of this behaviour — an assert that cannot fail is not evidence. Applied S123, all MINOR (style only; no content or structure moved, so the visible §5b banners correctly stay put): L02 **v03.13.1** · L03 **v03.30.1** · L04 **v04.21.1** · L05 **v04.20.2** · L06 **v04.25.1** · L10 **v02.19.2** · L14 **v02.25.1** · L16 **v02.13.1**; `going_deeper` **v01.4.2**, Maker **v2.45.3**, Timer **v1.3.1**. Census unchanged at **41,128** [lesson_inventory.py]; family map unmoved at 1069.

v8.112, S123, moderate — **§3.1b EXTENDED: THE TITLE IS FED, THE OPENER IS UNIQUE, AND THE APOSTROPHE IS RULED.** DJ asked for the title to be fed rather than typed, and the reason is that gate 51 already caught the drift while nothing closed it — the gate named a mismatch and a human retyped the title, which is the hand-typing the gate exists to catch, one step later. **New tool `title_feed.py` v1.0** rewrites the `<p>In <strong>Lesson N: Title</strong>,` span from the §6.5a strip, section-scoped, at an absolute offset with a byte-delta assert. **Its correctness proof is that applying it to a conforming book rewrites NOTHING**, which doubles as the cross-check against gate 51: the two derive the title separately, so a divergence turns that control red. **THE HANDOFF'S UNIQUENESS CLAIM WAS TRUE ONLY UNDER TWO CONDITIONS, AND IT NAMED ONE.** Page-wide, `In <strong>Lesson` occurs **3 times in L03, 2 in L09 and 3 in L12** — backward references. Under the colon form AND the section scope it occurs **exactly once in every lesson 01–15 and zero times in L16**. A page-wide rewriter would have overwritten L12's *"In Lesson 4 you calibrated…"* with the wrong lesson's title, which is S122's scope lesson arriving as a live hazard rather than as a maxim. **GATE 52 NEW, AND IT HOLDS A HOLE GATE 51 CANNOT SEE.** Gate 51 counts occurrences of the CORRECT opener and requires one; a section holding the correct opener PLUS a stale duplicate satisfies that count. Measured by injection — a duplicate opener spelling *Line Following* pushed into L07 left **all 51 preceding gates green, exit 0**. **Not a synthetic shape: S122 committed exactly this defect to L05**, by reading an audit result instead of the file. The gate IMPORTS the target regex from `title_feed` rather than re-implementing it (S83), so its spelling cannot drift away from the generator's. Control-run four ways from a pristine snapshot with read-back asserts (S121): clean PASSES, a duplicate FAILS naming **07: 2 §3.1b opener(s) inside one section**, a wrong target FAILS naming *points at Lesson 9, expected 8*, and a removed section FAILS. **THE APOSTROPHE IS RULED B AND THE DERIVATION IS NOW BYTE-FAITHFUL.** Three owners — gate 51's matching expression, `next_pointer.esc()` and `title_feed.to_prose()` — each rewrote an ASCII apostrophe to `&rsquo;`, so a title called *derived* was transformed in one character; the lossy step had been recorded and deliberately parked since S121. Exactly one of sixteen titles carries an apostrophe. **Measured before ruling: L11's *Time Lies, Distance Doesn't* was spelled STRAIGHT in 21 places** — the strip attribute in all sixteen lessons, plus L11's own `<title>`, `<h1>` and §5b footer, L16's table cell, and a JS string in `newproject.html` — **and CURLY in exactly 3, all inside `Lesson_10.html`**. The transform was making the book's dominant spelling the minority one on the single page that generated it. Priced against the alternative: curly everywhere touches 17 files including a `<title>` tag and a JS string literal where the entity renders as its own source text. DJ ruled the straight form; **all three owners moved in the same commit**, because one left behind re-creates the drift on the next apply. One spelling now stands at **24 of 24**, verified by re-measuring the whole book rather than by inspecting the three sites edited. The ampersand branch is untouched and still required, since the strip attribute already carries `&amp;`. **THE THREE SITES HAD THREE DIFFERENT OWNERS**, which is the practical finding: the opener belongs to `title_feed`, the link block to `next_pointer`, and the NEXT LESSON callout to nobody — authored prose, hand-edited. A generator arc that assumes every occurrence of a derived string is generated will leave the authored one behind. **NO STYLESHEET MOVEMENT:** the edit is prose-only, no class changed, so §27.8b's three-step sequence never ran and `build_css --check` stayed current at 641 rules; family map unmoved at 1069. `book_gates` **v1.47** (52 gates), `next_pointer` **v1.1**, `session_versions` **v1.19.0** — its Control G named `title_feed` as registered-but-not-emitted on the first run, which is the control working as designed. Applied S123: L10 **v02.19.1**, MINOR — three characters change and the page structure does not, so the visible §5b banner correctly stays at 02.19. Census unchanged at **41,128** [lesson_inventory.py].

v8.111, S122, moderate — **§3.1a AND §3.1b NOW HAVE NUMBERED SECTION BODIES, AND §3.1b IS NEW.** v8.110 recorded as canon debt that *§3.1 itself has no numbered section body*; **measured against the file, that was false** — §3.1's body has sat under §3 since v8.109, and only §3.1a was bodiless. §24.6c: a claim inherited from a changelog is a lead, not a finding, and this one survived a full session as a queue item. **§3.1b NEW: every lesson 01–15 carries the canonical *What's Next?* section.** The census that opened it found the book ending **four different ways** — seven lessons with a heading in THREE spellings (*What's Next*, *What's Next?*, *What's Next: Preview of Lesson 4*), five with a bare `<p><strong>Next:</strong>`, one stacking both, and four just stopping. **DJ ruled A-INVERTED** after being shown that retiring the seven headings would delete 752 words of authored prose and reverse S120's own L14 ruling: build the other eight UP rather than level the seven DOWN. **The shape is a FLOOR** (§25.8's precedent) — heading, a bold opener naming the next lesson, at least one list, seated immediately above Engineer's Log — and **nothing was deleted to reach conformance**: the five `Next:` paragraphs were MOVED inside their section and L01's block was RELOCATED past *Before You Go*, asserted by tag multiset rather than by eye. DJ, on being offered a no-callout canonical shape: *"I like the next example you have above, but I do not want to lose any data or prose"* — which is what turned a ceiling into a floor. **THE TITLE IS DERIVED FROM THE §6.5a STRIP AND NEVER TYPED**, after asserting the strip byte-identical across all sixteen. That caught a defect nothing else could: L07's prose said *Line Following* and L08's said *Intersections and Dead Ends* while the generated §3.1a link block **on the same page, ninety lines below**, said *Line Following with P-Control* and *Intersections & Dead Ends*. Two spellings of one title, invisible because no instrument compared prose against the strip. DJ ruled the strip's spelling both times and asked for the title to be FED rather than typed so it self-corrects — recorded as the next tool, with the target measured and confirmed unique: `<p>In <strong>Lesson N: Title</strong>,` occurs exactly once per lesson. **RE-ESCAPE BEFORE EMITTING** — the first emission of *Intersections & Dead Ends* shipped a bare `&` where the strip carries `&amp;`, because the title was `html.unescape`d and re-emitted raw. S94's decode-before-matching rule, inverted, and invisible to all 51 gates. **THE SESSION'S MOST REUSABLE FINDING IS SCOPE, AND IT COST THREE FALSE RESULTS:** a duplicate opener added to L05 where one already existed (the audit had reported only *no id*, which MEANT the paragraph was already there); a stray-`Next:` flag on L14 that could not tell *moved inside the section* from *left outside it*; and a read-back assert that counted L12's BACKWARD references (*"In Lesson 4 you calibrated…"*). **Every one was a SECTION question asked with a PAGE-wide instrument.** Gate 51 is section-scoped throughout, and it immediately caught L09's opener buried mid-paragraph after a hand-rolled audit had passed it — the gate finding what the sweep that motivated it could not. Control-run from a pristine snapshot of the working tree (S121's rule): a deleted heading FAILS naming the lesson, a drifted title FAILS naming the expected string, a section on L16 FAILS, and **removing ONE list item of three correctly PASSES** — a floor that fails on a shortened list is not the floor that was ruled, and that is recorded so a later session does not read it as a hole. **TIER-1 NORMALIZATIONS from the same census, all DJ-ruled:** 17 blocks in L08–L10 to the single spelling `🧠 THE LOGIC (Pseudocode)`; **89 of 237 back-to-top links** to one markup, literal ↑ and `link-c-2e86ab-2`, from six competing shapes crossing three arrow encodings with four classes; and **98 `&#9744;` entities to the literal ☐** per v8.104, L13 having carried BOTH within one lesson. **§27.11 MOVED TWICE, AND THE SECOND MOVE DIED RULES** — the first time in this arc's history that S113's benign shape did not hold. 643/2,357 → **641/2,350**: `.link-c-2e86ab-3` and `.link-c-2e86ab-4` are gone and **the −7 declarations ARE those two rules**. Both existed only to shrink the back-to-top link to 0.9em and 0.85em, and normalizing left them with zero uses — **verified by grep across every page, not inferred from the rule disappearing.** Zero born. Re-controlled after both moves against a dropped `color: white;`, which still FAILS. **§26.10 NEW: THE COLOUR LEDGER.** DJ, when offered a ruling on the Engineer's Log stripe: *"We haven't gone through and done all the colors have you?"* — correct, and the answer is measured: **169 distinct colour values, 708 tokens, ZERO custom properties**, and the blocking fact that **155 hexes appear inside 562 class names** (`#ddd` alone owns 38), so **a repaint is a RENAME, not a substitution.** `ZUMO_COLOR_LEDGER.md` opens with 16 items, three of them marked *re-measure before ruling* because they were recorded at S94 against a tree the §27 migration has since replaced. The stripe split (`#0e1a2c` L01–L11, `#6f7582` L12–L16) is **PARKED, not ruled**: deciding one value out of an unsettled palette is the S91 failure §26 exists to document. `book_gates` **v1.46.2** (51 gates), `next_pointer` **v1.0.2** — its docstring banner had gone stale at v1.0 against a v1.0.1 home, caught by `session_versions` grep_trap Control D and invisible to all 50 gates, because `--check` reads the ANCHORED home and never the banner. Applied S122, all MODERATE except L05 (id-only, does not render, MINOR): L01 **v03.21.0** · L02 **v03.13.0** · L03 **v03.30.0** · L04 **v04.21.0** · L05 **v04.20.1** · L06 **v04.25.0** · L07 **v04.24.0** · L08 **v04.21.0** · L09 **v05.18.0** · L10 **v02.19.0** · L11 **v02.20.0** · L12 **v01.22.0** · L13 **v02.20.0** · L14 **v02.25.0** · L15 **v02.21.0** · L16 **v02.13.0**. Census 41,028 → **41,128** [lesson_inventory.py]. Family map unmoved at 1069.

v8.110, S121, moderate — **§3.1a NEW: EVERY LESSON 01-15 ENDS WITH A WORKING LINK TO THE NEXT LESSON (DJ ruling: *"Yes, fix it completely"*).** S120 left the forward pointer as an open ruling and the measurement that reopened it found something the construct census could not see. **Counting CONSTRUCTS, seven lessons carried neither a *What's Next* heading nor a `Next:` paragraph. Counting CONTENT, thirteen of fifteen already named the next lesson in their final quarter** — the pointer was nearly universal and nearly invisible, living in the last sentence of whatever block happened to end the lesson. Only **L11 and L12** had no forward reference of any kind, L11's sole tail mention pointing *backward* to L10. So the ruling is not *add a pointer to seven lessons*; it is *the practice is already the norm, and two lessons are missing it.* DJ: *"keep the 2 new ones like the other 13."* **THE REAL DEFECT WAS NOT THE POINTER BUT THE DEAD AFFORDANCE.** Measured by parse: **zero of sixteen lessons contained a single `<a href>` to any lesson file anywhere in their tail.** A student reading *"In **Lesson 7: Code Organization**, you'll learn…"* sees bolded text shaped exactly like a link, clicks it, and nothing happens. **THE SEAT IS THE RULING'S OTHER HALF, AND IT IS A PEDAGOGICAL CHOICE RATHER THAN A TYPOGRAPHICAL ONE.** The forward-pointer PROSE sits at **84% of the page**, above Engineer's Log — the entry that feeds the TDP, which `ZUMO_Syllabus_WORKING.md` weights at 25% of the grade — and above the bonus block, glossary and quick reference. A link there routes a student out of the lesson *before the graded work*, which is §3.1's failure one scale down: a lesson ending itself early instead of the book ending early. **The block therefore sits immediately above the §5b footer**, the last element on the page and the only seat that exists, exactly once, in all sixteen lessons (the `end Image Index wrapper` comment was the obvious alternative and is present in only nine). **GENERATED, NOT REPAIRED**, on the §6.8a precedent: new tool **`next_pointer.py` v1.0**, whose titles are DERIVED from the §6.5a lesson strip after asserting the strip is byte-identical across all sixteen — no lesson title is typed anywhere in this session's work. **L16 generates no block and the gate fails if it acquires one** (§3.1: it ends the book). **THE ANCHOR IS A TEACHER AFFORDANCE AND IS RECORDED AS ONE.** DJ ruled *"we need an anchor link in all books"*; the block carries `id="next-lesson"` in all fifteen, which gives a stable Canvas address. It is honestly labelled: the §6.5a strip is inside `<nav class="nav">` at `position: sticky; top: 0`, so all sixteen lesson links are on screen at the bottom of every page and the anchor buys navigation nothing a student lacked. **L03's orphan `id="whats-next"` is the counter-example that made the point**: the string `whats-next` occurs exactly once in the entire repo, in L03's own heading, targeted by no link on any page — and §27.14 cannot see it, because that gate asserts every LINK resolves to an id and never that every ID is reached. **§27.11's DIGEST MOVED AND THE RULES DID NOT**: 643/2,357 unchanged, zero born, zero died, zero altered, S113's shape for the fourth time. Cause is benign and worth naming because it will recur — `build_css` orders rules by usage RANK, so fifteen new uses of `.link-c-2e86ab` (398→413) and `.p-mt-22px` (29→44) relocated one rule and changed two count comments, 20 changed lines, all of them a comment or that relocation. **Every class in the new block already existed**, which is why no lesson was re-classed and §27.8b's three-step sequence never ran. `book_gates` **v1.46**, 50 gates, gate 50 control-run four ways — a deleted block FAILS naming **07**, a wrong target FAILS naming *points at Lesson 9, expected 6*, a block on L16 FAILS, and a block moved below the footer FAILS — plus the moved digest re-controlled against a dropped `color: white;`. **THE PROCESS FAILURE IS THE ENTRY'S MOST REUSABLE PART, AND IT IS §24.6b IN A PLACE THE RULE DID NOT REACH.** The first control harness restored each injection with `git checkout --`, which reverts to the **COMMITTED** tree — and none of this session's work was committed. So every *restore* silently DELETED the block instead of replacing it, and three of the four controls ran against a tree already failing for an unrelated reason. All four printed FAIL and all four looked like evidence. **What exposed it was the untouched-tree control, which also failed** — the one control whose only job is to have nothing wrong with it. Redone from a pristine snapshot of the working tree, each control isolating cleanly. **Version control is not a control harness when the thing under test has never been committed**, and a harness needs its own snapshot. **CANON DEBT, RECORDED NOT FIXED:** §3.1 itself has no numbered section body — it exists only in this version line and in the CHANGELOG, so §3.1a is seated the same way. That is precisely the shape v8.103 closed for §24.14 and v8.101 logged for §21.1: **a rule whose home is a changelog entry holds only where someone happens to look.** Applied S121, all fifteen MODERATE (the pages render differently, so both §5b homes move): L01 **v03.20.0** · L02 **v03.12.0** · L03 **v03.29.0** · L04 **v04.20.0** · L05 **v04.20.0** · L06 **v04.24.0** · L07 **v04.23.0** · L08 **v04.20.0** · L09 **v05.17.0** · L10 **v02.18.0** · L11 **v02.19.0** · L12 **v01.21.0** · L13 **v02.19.0** · L14 **v02.24.0** · L15 **v02.20.0**; L16 unchanged. Census 40,979 → **41,028** [lesson_inventory.py] — 45 lines for fifteen blocks plus 4 for the two authored pointers, which is the arithmetic check that nothing else moved. Family map unmoved at 1069.

v8.109, S120, moderate — **§3.1 NEW: THE BOOK ENDS AT LESSON 16, FOR EVERY READER (DJ ruling).** The L14 flow review DJ queued at S115 found **L14 closing the course two lessons early.** Its *What's Next?* offered L15 and L16 as post-competition ***options*** in a three-bullet menu whose third bullet was *"Different competitions: sumo wrestling, maze solving, soccer"* — an after-the-book list in peer position with the capstone. **The byte chain said the opposite:** L16 §4.2 audits *"where YOUR 28,034 bytes are"*, which IS L15's finished build, so L16 hard-requires the lesson L14 called optional. Prose and code disagreed and the prose was wrong. **A SECOND ENDING WAS OFFERED AND DJ RULED IT OUT** — the calendar stops most students at L13 Step 3, and the obvious move was to author a Fall-terminal close there. DJ: *"I don't think we need to make the book tell them they are done at 13… if this book is used by others, then 13 won't be the end. It would be 16."* **The corollary is that this Bible carries NO calendar canon and must not acquire any** — measured **against the file as it stood before this entry**: `trimester`, `Fall`, `Winter`, `calendar` and `class period` each occurred **zero** times. **They occur now, in this entry and in §3.1, and that is the trap worth recording** — the first draft of this sentence asserted the zero as a present-tense fact about a file the sentence itself had just falsified, and it survived a full gate pass because no gate counts words. A word count was the wrong instrument in any case: §3.1 forbids a scheduling FACT that a lesson depends on, not the vocabulary for discussing the boundary. The term split lives in `ZUMO_Teacher_Daily_Grid_WORKING.md`, which rules its own mid-L13 break *"a calendar seam, not a book seam."* That separation is exactly what lets an adopter run a different length. **THE GATE CAUGHT THE FIX BEFORE IT SHIPPED, AND THE OLD TEXT HAD BEEN PASSING ON LUCK.** The rewritten bullets carried no terminal punctuation, so *full PID* ran on into the Lesson 16 sentence and §24's promise gate FAILED naming `L14 -> L16: promises ['PID']`. **The ORIGINAL bullets had the same run-on** and passed only because the distance to the next period exceeded the gate's 110-character window — a gate satisfied by the geometry of the defect rather than by its absence. Closed by punctuating each bullet as its own sentence, not by touching the gate. **Ungated by design:** *no lesson closes the course early* has no machine predicate, the §24.6b/§24.6c shape. **Recorded as an open ruling, measured not guessed:** the `<strong>Next:</strong>` construct exists in exactly **three** lessons (13, 14, 15) and a *What's Next?* heading in **seven** (01, 03, 05, 06, 07, 08, 14), overlapping only in L14 — whether every lesson 1–15 must carry a forward pointer, and which construct, is unruled. Applied S120, both MODERATE (the pages render differently, so both §5b homes move): L14 **v02.23.0**, L15 **v02.19.0** — L15 gaining the `Next:` pointer it had never carried, seated on L13's live placement. Census 40,978 → **40,979** [lesson_inventory.py]. No stylesheet change: `build_css --check` current at 643 rules, family map unmoved at 1069.

v8.108.1, S119, minor — **§25.10l IS NOW HELD BY GATE 49, and it is a CONSTANT rather than a baseline.** The rule was ruled in v8.108 and nothing enforced it. BC01 measured at five items in 13 of 13 converted lessons before L15 and 14 of 14 after, and no gate held the number — so S119 came within one ruling of breaking a 14/14 norm silently, on a question asked from a false premise. **What makes this different from §21's coverage or the family map's total is that it can never legitimately move again:** the conversion arc closed at S119 and `BC_PENDING` is empty, so there is no future edit entitled to change it. If it moves, something was edited that nobody ruled on. The gate carries a COVERAGE arm as well as a count arm, because a gate that scans zero lessons passes (S117, S118). Control-run four ways: an item deleted FAILS naming **4**, an item added FAILS naming **6**, the anchor removed FAILS on COVERAGE naming 13 of 14, and the unperturbed book passes. `book_gates` **v1.45**, 49 gates. **Canon debt, recorded not fixed:** §25.10l's constant now lives ONLY in `book_gates.py`, the same shape as §21.1's thresholds.

v8.108, S119, moderate — **§25.10l NEW: WHICH EXIT BLOCK EXERCISES AN OBJECTIVE IS DECIDED BY THE OBJECTIVE'S VERB, NOT BY A COUNT.** L15 is the FOURTEENTH AND LAST conversion; `BC_PENDING` is now empty. The ruling arose because a question was asked with a wrong premise and DJ ruled past it: on being told *"objective 7 has no BC01 item"* he ruled that one be added — and the premise was false, because **four of L15's seven objectives had no BC01 item**, BC01 having never been an objective map at all (BC02 is). Measured before acting: **BC01 = 5 items in 13 of 13 converted lessons**, a norm no gate holds. Three options were priced — swap at five, add to six, or one item per objective at seven — and **DJ ruled a fourth, D, on the teaching rather than the structure**: objective 7's verb is *Diagnose*, BC01 sits before §6 and asks what the BUILD depends on, and diagnostic vocabulary is what a student reaches for at the BENCH in §7 when something is already twitching. So objective 7 is exercised in **BC03**, BC01 stays at five and 14/14, and **BC03 goes to six** — legal without a ruling because §25.8 made Knowledge Check a FLOOR and L02 already ships seven. The general rule extends §25.10j from checklist rungs to OBJECTIVES: a capability claim is BC02's, an *understand why* or *diagnose* claim is BC03's, and displacing a BC01 item to make room for one is paying the wrong price. **§25.5 ON A GLYPH-LESS §2, AND L01 RULED LEGACY.** L15 and L01 are the only two lessons whose §2 carries no `☐`, and the S118 survey read that as *zero objectives* — **it was counting the GLYPH, not the objectives**. L15 has seven, verb-first, under the standard lead-in, so §25.10k's reword was unnecessary and §25.5 applied unchanged; BC02 supplies the literal ☐ and the equality assert normalises it away, which is L01's shape and not L13's. **L01's own BC02 does NOT carry L01's objectives** — six against ten, sourced from a technical-skills list §2 never states — and **DJ ruled L01 LEGACY**, pre-§25.5 rather than a competing precedent. **BC03's ANCESTOR WAS INVISIBLE TO THE NAME SWEEP, WHICH IS §25.10e REPEATING.** S117 predicted L15 as all-authoring; it is not. §10's *What you should be able to say out loud, without notes* is a five-item block doing BC03's job under a name no retired list carries, found by READING §10. Five migrated, one authored, answers and §-citations authored throughout — §7B verified to define both K_u and T_u, including that the robot supplies T_u = 2 ÷ WEAVE. **§25.10i RAN AND RETURNED NOTHING, WHICH IS ALSO A RESULT.** All 5 × 7 pairs scored: matrix max **0.182**, against L11's duplicates at 0.53–0.86 and its KEEPERS at 0.36 on the same scorer. No item folds. The scorer was control-run first against L11's four independently recorded pairings and reproduced all four RANKINGS while its magnitudes differ from S116's, **so the ranking was trusted and the S117 absolute threshold was not** — a calibration limit stated rather than assumed. Recorded and deliberately not tuned away: reading says say-2 restates Objective 3 and say-3 restates Objective 6, and the scorer sees 0.154 and 0.111 because the sentences share one content token. **BC02 CLAIM PLUS BC03 QUESTION ON ONE FACT IS THE DESIGN, NOT A DEFECT** — §25.10j's B ruling exactly: the checkbox collects the claim, the question tests it. **THE PROCESS FAILURE IS THE ENTRY'S REAL FINDING, AND IT IS AN INSTRUMENT THAT RESOLVES THE DIFFERENCE IT IS ASKED TO DETECT.** §27.8b's third step was run as `strip_inline --apply` without `--include-held` — the flag is written in the handoff in those words — which reverted the four byte-exact-across-lesson constructs from `class=` to inline `style=` in **all fifteen** other lessons. **The expansion proof PASSED on that broken tree.** Expansion resolves a class back to its declarations, so a lesson left inline expands byte-identically to one correctly classed: the test proves RENDERING equivalence and cannot see FILE-STATE divergence, and it is the very test S118 canonized for proving a rename innocent. Only the line diff saw it, at 570 changed lines. **§24.8 on a proof rather than on a gate: an instrument whose transform erases the distinction under test reports agreement no matter which way the answer falls.** Redone correctly, only **ONE** file differs from the pushed clone plus the stylesheet — S117's shape, not S118's — proved by both instruments agreeing at zero. **FOUR BASELINES MOVED, EACH CONTROLLED IN THE FAILING DIRECTION:** §21 coverage 245 → **250**, sole delta the four block icons plus the column emblem; family map 1065 → **1069**, controlled at an identical generator version with exactly ONE family moving, BRAIN CHECK 52 → 56 and the other 29 byte-identical; §27.11 digest to `f852cf656a9bda51` with rules and declarations **UNCHANGED** at 643/2,357, zero born, zero died, zero altered — S113's shape; and `'15'` leaving `BC_PENDING`, which is what §25.2a's NAMED sets buy, since it fails naming **L15**. After the moves a deleted callout FAILS naming **1068/1069**, `'15'` restored FAILS naming L15, a deleted `color: white;` FAILS, and a broken image ref FAILS. `book_gates` **v1.44.5**, `build_family_map` **v1.3.7**. Applied S119: L15 **v02.18.0**, moderate — the page renders differently, so both §5b homes move. Census 40,780 → **40,978** [lesson_inventory.py].

v8.107, S118, moderate — **§25.10k NEW: OBJECTIVES THAT ARE NOUN PHRASES ARE REWORDED IN SECTION 2, NOT IN BC02.** L13 is the THIRTEENTH conversion, and it is the first lesson whose §2 objectives could not migrate character-exact into BC02's **I can…** group, because they are NOUN PHRASES — *"The lawnmower sweep — covering an unknown room with rows"* reads, under that heading, as *"I can… ☐ The lawnmower sweep."* **Measured before it was ruled, across all sixteen lessons: L13 is the only one.** L02–L12, L14 and L16 all open verb-first under *"By the end of this lesson you will be able to:"*; L13 carries neither the verbs nor the lead-in; L01 and L15 carry no ☐ list at all. **DJ ruling S118, presented as three priced options and ruled A** — reword §2 verb-first and migrate that, so §25.5 holds by construction. B (migrate the nouns and relabel the group) dies on §25.10j, which had just ruled that an *understand* claim belongs in BC03 rather than behind a checkbox; C (reword only inside BC02) dies on principle, because the two lists drift apart while both still render correctly, which is the exact defect §25.5 exists to prevent. **The general rule: BC02 is a COPY, and editing a copy to read better is how two documents stop agreeing** — when the objectives do not fit the label, the objectives are what change. Only the opening verb moved on each of the six; every noun, em-dash clause and `<code>` span is preserved, and the equality is ASSERTED in the builder, not eyeballed. **The corollary is a trap in the assert itself:** §2 and BC02 differ in the box glyph BY DESIGN (`&#9744;` versus the literal ☐, the S115 trap hit again at S117), so an equality assert between them must normalise the glyph away or it fails on a required difference. **L13 HAD NO ANCESTOR OF ANY KIND** — zero pre-§6 reveals of any type, no checklist, no Knowledge Check, no Reflection Questions, its §10 being a narrative *Inventory* plus a *Next:* pointer — so BC01 (5), BC03 (5) and BC04 (3) are all authored from nothing, worse than L12's ratio and exactly as S117 predicted. BC01 is §-ordered §3 → §4.1 → §4.2 → §4.3 → §5.1 (L13's §3 is FLAT, with no subsections, unlike L11's and L12's five §3.x), every citation read and verified to CONTAIN its answer, and deliberately non-overlapping with BC03: BC01 asks why the clamp destroys silver, BC03 asks how you decide which channel a question belongs to. The *Inventory* and *Next:* prose stay OUTSIDE the family and reseat after BC04, on L10's arrangement, with the Engineer's Log surviving as its own block. **THIS IS A CLASS RENAME, S115's SHAPE AND NOT S117's, AND THE RENAME IS THE ENTRY'S REAL FINDING.** `.ul-ls-none-2` and `.ul-ls-none-3` **SWAPPED DECLARATION SETS** — same spelling, different meaning — because `build_css` ranks by usage count and BC02's list changed the counts. So **TEN lessons differ from the pushed clone, not one**, and which nine were carried along is a MEASUREMENT: L05–L12 and L14 changed by NAME ONLY, proved two ways rather than asserted — a line diff showing all 44 changed lines touch only those two class names, and an expansion through each tree's own stylesheet returning byte-identical for all nine. **A rename that changes no pixel still changes every file that spells it**, which is precisely the shape S115's incomplete push was made of. **THREE BASELINES MOVED, EACH CONTROLLED IN BOTH DIRECTIONS:** §21 coverage 240 → **245**, sole delta `BrainGear_Incomplete.png` 60 → 65; family map 1061 → **1065**, at an identical generator version with exactly ONE family moving, BRAIN CHECK 48 → 52 and the other 29 byte-identical; §27.11 digest to `ee0c8ac2039bc90e` with rules and declarations UNCHANGED at 643/2,357, zero born and zero died, the whole delta being the two swapped selectors. After the moves a deleted `color: white;` FAILS, a broken image ref FAILS, `'13'` left in `BC_PENDING` FAILS **naming L13**, and a deleted callout FAILS naming **1064/1065**. **THE CONTROL MISFIRED TWICE AND BOTH MISFIRES ARE KEPT, BECAUSE THEY ARE DIFFERENT FAILURES.** The first deleted a `<details data-reveal="quiz">`: the injection LANDED, the read-back confirmed it, and §24.14 stayed silent anyway — because a reveal is not a callout the family map counts. That is S117's third attempt repeating, and the read-back does not catch it, since the read-back proves only that something moved. **§24.6b's assert must be that the injection landed IN THE SHAPE THE GATE MEASURES, which means asking the generator what it counts rather than assuming.** The second misfire is worse and newer: the assert `'brain-check-04' not in back` is ALWAYS false, because the Brain Check column carries nav links to that anchor — so a correct injection would have reported as a failed one. **An assert that cannot succeed is the mirror of an assert that cannot fail, and both read as evidence.** Only the third attempt — asserting `id="brain-check-04"` and a parsed callout count of 22 → 21 — was a real control. `book_gates` **v1.44.4** (from v1.44.3), `build_family_map` **v1.3.6** (from v1.3.5). Applied S118: L13 **v02.18.0**, moderate — the page renders differently, so both §5b homes move; plus NINE minor bumps carrying the class rename with every visible banner asserted unmoved (L05, L06, L07, L08, L09, L10, L11, L12, L14). Census 40,579 → **40,780** [lesson_inventory.py].

v8.106, S117, moderate — **§25.10j NEW: A CHECKLIST RUNG THAT ASKS A *WHY* IS A KNOWLEDGE-CHECK ITEM WEARING A CHECKBOX.** L12 is the TWELFTH conversion. Its ten-rung *Exit Ticket* held four rungs no §2 objective covers and that are not bench observations either — each a knowledge claim carrying its own §-citation, and each with nowhere legal to go, since §25.5 makes BC02's **I can…** group §2's objectives character-exact and a third labelled group would be a construct invented to hold four items. **DJ ruling S117, presented as two options and ruled B:** *A, the student ticks a box claiming they know; B, the student is asked, and finds out whether they actually do.* The rung supplies the stem and the citation; the session authors the interrogative form and the answer. **The test is the VERB, not the tense** — a capability claim is BC02's, an observation claim is BC02's **I have…** group, and an *understand why* claim is BC03's, because a checkbox cannot tell a student who knows from a student who is willing to tick. This runs OPPOSITE to L07's and L09's precedent of moving an item *out of* BC03, and that is its value: L12's BC03 had **no ancestor at all** and would have been four items authored from nothing; it is now four authored from four stems the lesson wrote itself. **§25.10i APPLIED, AND IT RULED ONLY ONE PAIR.** A scored pairwise diff of all **10 × 6** combinations separates cleanly on exactly one: rung 1 scores **0.78** against Objective 1, and the next-best pair in the entire matrix scores **0.42**, so `min(dup) > max(keep)` holds on a single NAMED pair. **Unlike L11, none of L12's past-tense rungs is a duplicate** — rungs 2, 3, 4, 6 and 10 are bench observations and outcome claims, folded into **I have…** per §25.10a/b with no ruling needed, so S116's open question about *did you do it* versus *can you do it* **stays open** and recurs on L13/L15 only if either carries a past-tense checklist. **AN INSTRUMENT-VS-READING DISAGREEMENT IS RECORDED AND DELIBERATELY NOT RESOLVED:** the scorer ranks rung 2 against Objective 6 highest of the keepers at 0.42 on the shared words *robot / hands / turn / gyro*, while reading says rung 2 is a §7A bench observation and Objective 6's content lands on rungs 4 and 5 at 0.27 and 0.21. **The normaliser was not tuned to produce that pairing**; only the pair above the separation was acted on. **BC01 had NO ancestor** — zero pre-§6 `check` reveals, zero `quiz`, zero TRY IT, all thirteen pre-§9 reveals being `catchup` — so five items were authored and §-ordered §3.1→§3.2→§3.3→§3.4→§3.5, each citation read and verified to CONTAIN its answer (v8.58.1), and each deliberately non-overlapping with BC03: BC01 asks why NO encoder can ever detect slip, BC03 what the two disagreeing numbers on the OLED tell you that neither instrument could alone. **BC02 is L12's own six §2 objectives migrated character-exact** (§25.5), asserted equal in the builder, with the literal ☐ and not `&#9744;`. **BC04 has no ancestor either**; three prompts authored, no reveals, the Engineer's Log surviving as its own block after it (S80), and the 🏆 *one sentence to walk out with* plus the *Next lesson* prose kept OUTSIDE the family and reseated after BC04 on L10's arrangement. **§25.10c ACHIEVABILITY, MEASURED AGAINST §7's OWN PROSE:** §7's opening TIP supplies a no-materials fallback — *"No delrin? Hold the robot… guaranteed slip, zero materials"* — which covers 7A, 7B and 7C, all motors-off or turn-in-place. **It cannot cover 7E**, which drives four 30 cm sides and four corners. That rung is now behind the Mark-done lock; its wording was generalised from *on the delrin* to *on a slick floor*, which does not remove the dependency. **OPEN FOR DJ, and it is a fact about the room rather than the book:** if every student gets floor time the lock is earnable, and if access is rationed the rung comes out of the locked list. **THREE BASELINES MOVED, EACH CONTROLLED IN BOTH DIRECTIONS.** §27.11 to **643 rules / 2,357 declarations**, diffed by SELECTOR: exactly one gone (`.div-ddd-3`, the consumed Exit-Ticket checklist box, 5 declarations — the −5 IS that rule), **zero born, zero altered, so no class RENAME**, S116's shape rather than S115's. §21 coverage 235 → **240**, sole delta `BrainGear_Incomplete.png` 55 → 60, which is four block icons plus the column emblem. Family map 1057 → **1061**, controlled at an identical generator version: exactly ONE family moves, BRAIN CHECK 44 → 48, the other 29 byte-identical. After the moves a deleted `color: white;` still FAILS, a deleted callout FAILS naming **1060/1061**, a broken image ref FAILS, and leaving `'12'` in `BC_PENDING` FAILS **naming L12**. **ONLY TWO FILES DIFFER FROM THE PUSHED CLONE** — `Lesson_12.html` and `css/book.css`; the other fifteen lessons returned byte-identical through restore → regenerate → apply, which is measured and not assumed (S115). **THE PROCESS FAILURE, AND IT IS S116's REPEATING:** the §24.14 deletion control **misfired three times** — a regex that matched nothing, then a class absent from L04, then a class the family map does not count as a callout — and on two of those the script died before writing while the gate then printed `exit=0` against an untouched tree. Caught only by reading what the control DID rather than what the gate said afterwards. The fourth attempt added a **disk read-back assert before the gate runs**, and it fired correctly at 1060/1061. **Knowing §24.6b confers no immunity; the read-back does.** **AND A SECOND, WORSE ONE, IN THIS VERY FILE:** the first attempt at this entry used `open(p,'w')` and hit a `UnicodeEncodeError` mid-write, **truncating the Bible to 0 bytes**. Recovered from the clone in one command, and the rule §12 already carried is now paid for twice: **encode the whole blob FIRST, then temp-file + `os.replace`** — a truncating `open()` must never be reachable from a call that can raise. `book_gates` **v1.44.3** (from v1.44.2), `build_family_map` **v1.3.5** (from v1.3.4). Applied S117: L12 **v01.20.0**, moderate — the page renders differently, so both §5b homes move. Census 40,391 → **40,579** [lesson_inventory.py].
...
### 2.1 EVERY LESSON OPENS SECTION 1 WITH A FIGURE (v8.126 — NEW, S133, DJ ruling)

Section 1 is the hook — *The Messy Room Problem*, *The Crooked Robot Problem*, *The Bug You
Have Not Met Yet* — and its whole job is to make a student care before any code appears. DJ,
S133: *"All lessons should have a hook, if we can find an appropriate images/graphics to put
there."*

**At S133 open, EIGHT of sixteen carried a real figure, three carried a placeholder, and FIVE
carried nothing at all** — L04, L11, L12, L13, L15. The five were invisible to `image_audit`,
**and that tool was right**: it reports figures the book DECLARES it wants, and nobody had ever
declared these. An absence nobody wrote down is an absence no instrument can find (§24.8). The
worklist rose 14 of 141 to **19 of 146** the moment they were declared, and that rise is the
point rather than a regression.

**A DECLARED PLACEHOLDER SATISFIES THIS RULE.** The ruling is that a hook needs something to
look at; a declared figure is on `IMAGE_WORKLIST.md` and is therefore visible work. A gate
demanding a landed ASSET would be a to-do list wearing a gate — red for months, and tuned away
by the first reader who wants a green run.

**THE PREDICATE IS THE AUTHORED NAME, NOT A PATH** (rule 19). A figure is a file named
`L##_IMAGE_` or `L##_GRAPHIC_` per §10/§17. Keying on the directory instead counts every
decorative mark in `images/marks/` and every `spiral_star_NN.svg` as a figure — and it did, on
the first draft of this very check, which reported a false **16 of 16 with six lessons still
empty**. The naming convention is a property of the asset; the directory is where it happens to
live.

**Gated** by `book_gates` §2.1, with a coverage arm, control-run three ways — placeholder
deleted, and a decorative mark substituted for one, both FAIL.

**Placement is authorial and deliberately unruled**: the eight live figures sit between 7% and
87% of §1.

### 2.2 SECTION 2 STATES OBJECTIVES, AND EACH ONE IS A BOX (v8.126 — NEW, S133, DJ ruling)

DJ, S133: *"Can you make it a rule that lessons have to have objectives and there are
squares/boxes not dots."* Three arms, and the third is the one nothing in the tree could see.

1. **Every lesson's Section 2 states at least one objective.**
2. **Every objective carries the literal box `☐`.** L01 and L15 carried **none** — 13 objectives
   with no box — and v8.108 had recorded exactly that as an observation with no gate behind it.
   The box is the LITERAL character, never an entity (§27.16); asserting on the entity spelling
   would pass a book that renders no boxes at all.
3. **The list must not ALSO draw a bullet.** Four lessons left the `<ul>` at the browser
   default, so each objective rendered as a **dot followed by a box**. Twelve suppressed it and
   four did not, and **no instrument in the tree looked at `list-style`** — it is resolved
   styling, not markup, so no grep of a lesson can answer it.

**SCOPE IS THE SECTION, NOT THE PAGE** (§S122, repeatedly): a page-wide count of the box glyph
is satisfied by BC02, which carries its own boxes by §25.5 and would mask an empty §2 entirely.

**THE LEAD-IN IS ONE SPELLING.** *"By the end of this lesson, you will be able to:"*, in 15 of
16. It had drifted by a single comma, split at the **L09/L10 seam** — the same boundary that
produced the §6.5 Box split, the mono stack, the dark-code border and the reveal-box pointer.
**L16 is held out by ruling** and reads *"By the end of this multi-week project…"*, because the
project genuinely spans weeks. L16 also keeps numbered `1.x` subsections under §1, ruled
intentional.

**HOW ARM 3 IS ENFORCED IS THE ENTRY'S REAL CONTENT — see §27.15b.** The fix is three bytes of
markup and it was attempted twice through the generated layer and reverted both times. It is
carried by `ul[data-objectives]` in `css/semantic.css`, and the gate accepts either that
attribute or a class that already suppresses the bullet, so the twelve conforming lessons are
not made non-compliant by a rule written for the four that were not.
...
### 3.1 The book ends at Lesson 16, for every reader (S120, DJ ruling)

**No lesson before 16 closes the course.** L16 is the capstone; every lesson 01–15 points forward, and none of them delivers a valediction. Where a particular section’s calendar stops is a fact about that calendar, not about the book — DJ, S120: *“I don’t think we need to make the book tell them they are done at 13. That’s something I can do, and if this book is used by others, then 13 won’t be the end. It would be 16.”*

**This Bible therefore carries no calendar canon, and must not acquire any.** The term split, the period count and the mid-lesson Fall/Winter seam live in `ZUMO_Teacher_Daily_Grid_WORKING.md`, which rules its own break *“a calendar seam, not a book seam.”* Keeping scheduling out of the book is what lets an adopter run a different length without editing a lesson.

**The test a lesson has to pass** is not whether it congratulates the student — L14’s competition send-off is legitimate and stays — but whether it tells them the work is over. A retrospective is fine; a menu of *options after this* in place of the road ahead is not.

**Ungated by design** (§24.6b/§24.6c shape): there is no machine predicate for *closes the course*. **The measurable neighbours are now RULED and gated — see §3.1a for the LINK and §3.1b for the SECTION that holds it.** This paragraph previously recorded the forward pointer as unruled; it was ruled at S121 and S122, and the sentence is corrected rather than left to describe its own successor as unwritten (§24.9).

### 3.1a Every lesson 01–15 ends with a working link to the next lesson (S121, DJ ruling)

**The rule is the LINK, not the prose above it.** Each lesson 01–15 carries exactly one
generated block containing an `<a href>` to the next lesson's file. L16 carries none, and gate 50
fails if it acquires one — §3.1: it ends the book.

**Before S121 the pointer was nearly universal and entirely dead.** Thirteen of fifteen lessons
already named the next lesson in their final quarter, and **zero of sixteen contained a single
`<a href>` to a lesson file anywhere in their tail.** A student reading *"In **Lesson 7: Code
Organization**, you'll learn…"* sees bolded text shaped exactly like a link, clicks it, and
nothing happens. Counting CONSTRUCTS said seven lessons had no pointer; counting CONTENT said
two. The construct census was measuring the wrong thing.

**The seat is the other half of the rule and it is pedagogical.** The block sits immediately
above the §5b footer — the last element on the page, and the only seat present exactly once in
all sixteen lessons. It is deliberately NOT seated with the pointer prose at 84% of the page,
which sits above Engineer's Log (the entry feeding the TDP, weighted 25% of the grade), above the
bonus block and above the quick reference. A door there routes a student past the graded work:
§3.1's failure one scale down.

**Generated, not repaired** (§6.8a): `next_pointer.py`. Titles are DERIVED from the §6.5a strip
after asserting it byte-identical across all sixteen; no lesson title is typed. Known lossy step,
recorded in that tool's docstring: `esc()` rewrites an apostrophe to `&rsquo;`, so one emitted
title is transformed rather than copied.

**The anchor is a teacher affordance and is labelled as one.** `id="next-lesson"` in all fifteen
gives a stable Canvas address. It buys students nothing — the §6.5a strip is `position: sticky`,
so all sixteen links are already on screen. **L03's orphan `id="whats-next"` was the
counter-example**: targeted by no link anywhere, and §27.14 could not see it, because that gate
asserts every LINK resolves to an id and never that every ID is reached. §3.1b resolved it by
giving the id a purpose in all fifteen rather than deleting it.

### 3.1b Every lesson 01–15 carries the canonical What's Next? section (S122, DJ ruling)

**Sixteen lessons ended four different ways.** Seven carried a *What's Next* heading in **three
spellings**, five carried a bare `<p><strong>Next:</strong>`, one stacked both, and four just
stopped. The link block below was uniform because it is generated; everything above it was
authored per lesson and drifted — §6.8a's shape, one construct over.

**The canonical shape:**

```
<h3 id="whats-next" class="h3-c-6f7582">What's Next?</h3>
<p>In <strong>Lesson N: Title</strong>, …</p>
<ul> … </ul>
```

seated immediately above Engineer's Log. L16 carries none, and gate 51 fails if it acquires one.

**The shape is a FLOOR, not a ceiling** (§25.8's precedent). Required: the heading, at least one
paragraph naming the next lesson in bold, at least one list. A lesson carrying more keeps it —
L03's and L05's forward-preview callouts and L14's competition send-off all survive untouched.
**Nothing was deleted to reach conformance**: the five `Next:` paragraphs were MOVED inside the
section, and L01's block was relocated rather than rewritten.
...
*Intersections & Dead Ends*. Two spellings of one title, and no instrument could see it because
nothing compared prose against the strip. Re-escape before emitting: the first emission of
*Intersections & Dead Ends* into L08 shipped a bare `&` where the strip carries `&amp;` — the S94
decode-before-matching rule, inverted.

**EVERY CHECK IS SCOPED TO THE SECTION, NEVER TO THE PAGE.** Learned three times in one session:
a page-wide search for the opener matches L12's BACKWARD references (*"In Lesson 4 you
calibrated…"*); a page-wide search for the `Next:` paragraph cannot distinguish *moved inside*
from *left outside*; and a section that merely contains a bold lesson name passes a check that a
paragraph-initial opener would fail — which is how gate 51 caught L09's opener buried
mid-paragraph after a hand-rolled audit had passed it.

**THE TITLE IS FED, NOT TYPED (S123, DJ: *"feed the title, don't type it"*).** Gate 51 already
failed when an opener disagreed with the strip; what did not exist was the thing that made it
agree again, so the gate named the drift and a human retyped the title — the same hand-typing the
gate was written to catch, one step later. **`title_feed.py` rewrites the title span from the
§6.5a strip**, section-scoped, at an absolute offset with a byte-delta assert. Its correctness
proof is that applying it to a conforming book rewrites **nothing**, which is also the cross-check
against gate 51: the two derive the title separately, so any divergence turns that control red.
The uniqueness of the target is what makes the rewrite safe, and it holds only under **both**
conditions — the colon form **and** the section scope. Measured S123: page-wide,
`In <strong>Lesson` occurs 3 times in L03, 2 in L09 and 3 in L12; section-scoped and colon-formed,
it occurs **exactly once in every lesson 01–15 and zero times in L16**.

**THE OPENER IS UNIQUE INSIDE ITS SECTION, AND GATE 51 CANNOT SEE THAT (S123, gate 52).** Gate 51
counts occurrences of the CORRECT opener and requires one. A section holding the correct opener
**plus a second one carrying a stale title** satisfies that count and passes. Measured by
injection: a duplicate opener spelling *Line Following* pushed into L07 left all 51 preceding
gates green, exit 0. **This is not a synthetic shape** — S122 committed exactly this defect to
L05, by reading an audit result instead of the file. Gate 52 holds it, and imports the target
regex from `title_feed` rather than re-implementing it (S83), so the gate's spelling cannot drift
away from the generator's.

**THE APOSTROPHE PASSES THROUGH UNTRANSFORMED (S123, DJ ruling B).** Three separate owners —
gate 51's matching expression, `next_pointer.esc()` and `title_feed.to_prose()` — each rewrote an
ASCII apostrophe to `&rsquo;`, so a title claimed to be *derived* was transformed in one character.
Exactly one of sixteen titles carries an apostrophe: L11's *Time Lies, Distance Doesn't*. Measured
before the ruling, that title was spelled **straight in 21 places** — the strip attribute in all
sixteen lessons, plus L11's own `<title>`, `<h1>` and §5b footer, L16's table cell, and a JS string
in `newproject.html` — and **curly in exactly 3, all inside `Lesson_10.html`**: the §3.1b opener,
the generated next-lesson link, and an authored NEXT LESSON callout. The transform was turning the
book's dominant spelling into a minority one on the single page that generated it. The alternative
— curly everywhere — would have touched 17 files including a `<title>` tag and a JS string literal,
where the entity would render as its own source text. **All three owners moved in the same commit**,
because any one left behind re-creates the drift on the next apply. The derivation is now
byte-faithful, and one spelling stands at **24 of 24**. The ampersand branch is unaffected and
still required: the strip attribute already carries `&amp;`.

**Held by gate 51.** Control-run from a pristine snapshot of the working tree: a deleted heading
FAILS naming the lesson, a title drifted off the strip FAILS naming the expected string, a
section on L16 FAILS. Removing ONE list item of three correctly PASSES — the rule is *at least
one list*, and a floor that fails on a shortened list is not the floor that was ruled.
...
**Glossary entry format — SUPERSEDED S132; the canon is `BookComponentStandard` §7.4.**
DJ ruling, S132: *"Just make them ALL the same."* All **151** entries in all sixteen lessons
are now one component, and this Bible no longer holds a second copy of its spelling — a rule
written in two places is a rule that will disagree with itself (§24.2).

**What the entry above got wrong, and each one is why the pointer replaces it rather than
amends it:** the inline `style=` was retired by §27 (the book is a website); the `<span>🔑</span>`
was retired by S130 (marks, not emoji); the em dash was ruled OUT by DJ at S131; and the
`<strong id="term-...">` shape was true of only 37 of the 97 cards that existed. **Three of
its four clauses had been superseded by rulings this document itself carries.**

**What still holds and is restated here because it is BOOK canon rather than component canon:**
the glossary is END MATTER, seated Glossary → Quick Reference → Image Index, and the term card
is the radius exception at `8px` (§6.10). **The body Key Term callout is a DIFFERENT element**
and stays — per-lesson dual entries are DESIGN, ruled S128, tested and not drift.

**The family is KEY TERM, not GLOSSARY.** A family names CONTENT, not LOCATION; the glossary
entry and the body callout define the same thing, so they carry the same family. **Where an
entry SITS is carried structurally by its REGION** (`lesson_inventory` v1.3.5), which is what a
harvest keys on — `data-family` alone returns 184 blocks where the glossary is 151.
...
**THE BOOK HAS ONE MONO STACK (S123, DJ ruling B), AND IT IS HELD BY GATE 53.**

```
ui-monospace, SFMono-Regular, Menlo, Consolas, 'Liberation Mono', 'Courier New', monospace
```

This is the S108 finding above, one property over and four times larger. **The prose stack was
Windows-only; the mono stacks were Windows-INVISIBLE**, which is worse — five Consolas spellings
resolved to a single face on Windows and to **three different fallbacks** everywhere else, so the
drift could only be seen by a reader who could not report it. Measured before the ruling:
`'Monaco','Courier New',monospace` (311 uses), bare `monospace` (143), `'Monaco',monospace` (5).
On a Mac the main dark code block rendered Monaco while inline code rendered the browser default
— **two code faces on one page.**

**THE SESSION'S REUSABLE FINDING IS THAT THE LEAD NAMED THE MINORITY.** The queue item said
*Consolas*, so the instrument was scoped to Consolas, and it could not see that the book's
dominant code type was `'Courier New', monospace` at **21 rules and 2,825 uses** against
Consolas's 14 and 459. §24.6c one level up: a lead names a SYMPTOM, and scoping the instrument to
the lead's own vocabulary guarantees you measure only what you were already told. The right
question was never *where is Consolas* but *how many mono stacks does this book have*. Final
totals across all 38: one stack.

**WHY THE SWEEP CANNOT HOLD THIS AND A GATE MUST.** `font_stack_sweep` rewrites a stack whose
FIRST face is a named substitution risk. That is a different question from whether a stack equals
the ruled one, and the gap is real rather than theoretical: injecting `ui-monospace, monospace`
— safe first face, wrong tail — leaves the sweep reporting **0 rewrites** while gate 53 names the
file. Measured by injection, both directions, plus a third control on `timer.html` that fires
gate 53 and NOT §27.11, because the stylesheet baseline cannot see a tool page at all.

**THE TOOL PAGES JOIN THE SCAN**, for the reason S108 put `css/book.css` in it. Four Consolas
declarations in `going_deeper.html`, `newproject.html` and `timer.html` were invisible to every
instrument in the tree: the sweep never opened them, and `build_css` does not model them because
§25.6a keeps tool pages out of the class migration. The web-served exemption is now **DERIVED per
file** rather than blanket — `going_deeper.html` links `fonts.googleapis.com` and declares Inter,
while `newproject.html` and `timer.html` link nothing, so the same Inter declaration is safe in
the first and a real risk in the other two. The mono target is likewise **context-dependent**: an
SVG keeps the plain system stack, because `ui-monospace` is a CSS keyword no SVG consumer resolves.
...
**The 11-type callout table is SUPERSEDED — the roster is `BookComponentStandard` §7.**
Ruled S129, done S132. That standard carries **30 families**; the table below held **11**, and
the eleven were a subset that had stopped being a roster years ago — `build_family_map` resolves
thirty. **A LOCKED table naming a third of the population is worse than no table**, because it
reads as complete.

**Its opening clause was also false:** *"All callouts use inline `style=` only"* was retired by
§27 — the lessons carry ZERO inline styles. **The FAMILY is the unit, and the mark and the colour
are OUTPUTS of it** (§24.14); reading a table backwards from a colour is exactly the defect S112
and S130 each cost a session to remove.

**Kept below for the historical record only. Do not author from it.**
...
### 15.6 A STEP PAYLOAD IS THE FILE AS IT STANDS AT THAT STEP, NOT AS IT ENDS UP (v8.133 — NEW, S141)

`after_step_N` is the project a student holds at the START of step N+1. It is a **snapshot of a build in progress**, not an abbreviation of `finished` — so where a lesson builds a defect ON PURPOSE and repairs it later, every payload before the repair still carries the defect.

**The case.** L06 Steps 7 and 10 write `driveDistance()` and `turnDegrees()` in their BROKEN form by design (S24 ruling): both wheels commanded the same number, and a gate reading one encoder. **Step 13 — *The Square That Won't Close* — is the whole payoff**: the student drives a square, watches it fail, and is walked through the two bugs. Its prose quotes their own file back at them — *"Look at what `driveDistance()` actually commands: `motors.setSpeeds(speed, speed);`"* — and then adds TRIM and `averageCounts()`.

`after_step_7` through `after_step_11` shipped the **Step 13 versions**, so the five DISCOVERIES entries covering Step 8, Discovery 6.7, Step 10, Discovery 6.8 and Step 12 handed a catching-up student a file in which **both bugs were already fixed**. Step 13 then quoted two lines that were not in their file, and told them to run a square that would close. Cause: Maker **v2.14**'s TRIM pass regenerated `PAYLOADS[6..9]` wholesale from the corrected states rather than only the states after the repair — its own changelog line says so.

**Why nothing saw it.** `gate_payload_match` asserts that a payload byte-derives from **SOME** lesson `<pre>`, never from the `<pre>` belonging to the step the payload is named for (§24.8). A payload carrying a LATER step's code satisfies it perfectly, and did for 117 sessions.

**GATE 70** (`§15 a step payload is the file as it stands AT that step`). Its predicate is **DERIVED, not pinned** (S126 rule 19): the expected bodies are EXTRACTED from the lesson's own Step 7 and Step 10 reveals, and the expected `finished` body is those same bodies with Step 13's two substitutions applied — both of which Step 13 publishes as standalone single-line `<pre>` blocks. Rewording a comment therefore moves both sides together; only a payload drifting off its own step fires.

**And the far side of that gate needed a blinding control to be worth anything** (S139 rule 17). Its first draft asserted only that `finished` did NOT hold the broken body and that the words `TRIM` and `averageCounts` appeared somewhere in the file. Removing `+ TRIM` from `finished`'s `driveDistance()` left it **silent**: the block no longer matched the broken one either, and both words survived elsewhere. **A gate that checks for a word is checking for a word.** It now compares CODE LINES against the derived expectation, and fires.
...
**THE CENSUS CHANGED DEFINITION AT S130, AND EVERY FIGURE BEFORE IT RUNS HIGH BY EXACTLY THE
LESSON COUNT.** (v8.122.1 — S130.) `lesson_inventory` computed `count('\n') + 1` unconditionally,
which counts the empty string AFTER a trailing newline as a line — so the census ran **+1 per file,
+16 book-wide**, and disagreed with `wc -l` for its entire life. Fixed at S130 (the `+ 1` is now
conditional on a genuinely unterminated final line, and all sixteen lessons were normalised to end
in a newline), and **gate 62 holds the two definitions equal by counting newlines without importing
the parser.**

**This is recorded here rather than fixed in the CHANGELOG because historical entries are not
retro-edited** (§24.9), and it is recorded at all because the discontinuity is a trap: the census
delta is this book's arithmetic check that *nothing else moved*, so a reader comparing S129's
**40,698** against S130's **40,683** computes a phantom **−15** and goes hunting for fifteen deleted
lines. S69 already lost a session to a false finding of this exact shape.

**The conversion is exact and DERIVED, not a magic number: the offset IS the lesson count**, because
the old formula added one per file regardless. To compare any pre-S130 census figure with a current
one, subtract the number of lessons. Anything from S130 on can simply be re-derived — `cat
lessons/Lesson_*.html | wc -l` now agrees to the line, which is the whole point of the fix.
...
### 16.10 WHEEL AND TRACK GEOMETRY — 39 mm IS THE DIAMETER **OVER THE TRACK** (v8.134 — LOCKED, S142)

Every driven distance in this book is computed from these numbers, and the one that gets measured
wrong is the diameter. **The sprocket alone is 35 mm; the diameter that matters is 39 mm, measured
over the rubber track**, because the track is what touches the floor and therefore what sets travel
per revolution.

| Quantity | Value | Where it comes from |
|---|---|---|
| Sprocket diameter | **35 mm** | Pololu product pages |
| **Diameter over the track** | **39 mm** | Pololu product pages — **this is `WHEEL_DIAMETER_MM`** |
| Sprocket spacing (wheel base) | **85 mm** | Pololu product pages — `WHEEL_BASE_MM` |
| Encoder, motor shaft | **12 CPR** | Zumo 32U4 User's Guide — family-wide, true at any ratio |
| Gear ratio (fleet) | **75.81 : 1** | §16.1 — blue sticker |
| Counts per sprocket revolution | **909.7** | 12 × 75.81, confirmed in the User's Guide |

- **`WHEEL_BASE_MM = 85.0` is a STARTING value, not a finished one.** It is the true sprocket spacing,
  but a tracked robot scrubs when it turns, so the tuned value runs higher — the book gives 98–115 as
  the typical tuned range. The 85 is where you start; the four-turn test is how you leave it.
- **Measure with calipers, not a ruler** (L06 TIP 6.68): zero them first, outside jaws through the
  axle, gentle on the silicone, three readings rotating the wheel between. Tread depth is ~0.1 mm,
  below the noise floor of the measurement, and is deliberately not modelled.
- **OPEN, not settled:** whether travel per revolution follows the outer surface (39 mm) or the
  track's pitch line (somewhere between 35 and 39). If nearer 37, true counts/cm is ~78 rather than
  74.3 and a commanded 30 cm lands ~1.6 cm short — **inside §7's own ±2 cm tolerance**, and L08's
  calibration step absorbs it. A bench question, not a correction.

### 16.11 Kp RANGE — 0.1 TO 0.3 ON THE 75:1 FLEET (v8.134 — LOCKED, S142)

**Canon: Kp runs 0.1 – 0.3, starting at 0.10.** DJ ruling, S142. This is a **fleet-dependent** number
and the fleet is 75:1 (§16.1), which has the balanced speed-and-torque mix these gains assume.

**The number is DERIVED, not chosen, and the lesson proves it.** L08 §3.1 states the correction range
as *"roughly −600 to +600"* — and **600 = 0.3 × 2000**, the maximum error on the 0–4000 position scale.
That sentence can only have been written against a 0.3 ceiling. When §7.3 and the Quick Reference had
drifted to 0.08–0.25 they were contradicting a sentence that derives from the number; **§3.1 was right
and the other two were the drift.** Restored S142, along with L08 Challenge 5's three adaptive gains,
which had been sitting on the old range's endpoints (0.08 / 0.15 / 0.25 → **0.10 / 0.20 / 0.30**).

- **Any lesson that names a Kp range says 0.1 – 0.3.** Same rule as §16.1's ratio: the book speaks to
  the robot in front of the student.
- **A ratio other than 75:1 is NOT book content.** A suggested 0.08–0.25 for a 100:1 fleet is recorded
  in the teacher-facing resource doc, flagged **unverified** — no 100:1 robot has ever been tested for
  this book, and the two physical arguments point in opposite directions. Settle it with L08's
  Challenge 2 Wiggle Test before it ever reaches a lesson.
- **Kd belongs to Lesson 15 and nowhere earlier.** L15 derives it from the student's own measured `Ku`
  and `Tu` via `Kd = Kp × Tu / 8`; that derivation *is* the lesson. L08 stops at P and says so.
...
### 16.12 THE AVOIDANCE BOX IS SEVEN PHASES, ONE JOB EACH (v8.135 — LOCKED, S143)

**Canon: the obstacle maneuver is a FOUR-LEG box, expressed as seven phases, each doing ONE job.**
DJ ruling, S143.

    PHASE_TURN_AWAY    turnDegrees(+AVOID_TURN_DEGREES)   90 degrees off the line
    PHASE_DRIVE_OUT    driveDistance(AVOID_OUT_CM)        15 cm sideways, clear of the line
    PHASE_TURN_ALONG   turnDegrees(-AVOID_TURN_DEGREES)   back to the original heading
    PHASE_DRIVE_PAST   driveDistance(AVOID_PAST_CM)       20 cm parallel, clearing the block
    PHASE_TURN_BACK    turnDegrees(-AVOID_TURN_DEGREES)   now FACING the line
    PHASE_SEEK_LINE    start the clock                    moves nothing
    PHASE_COMPLETE     hand back to the state machine     moves nothing

**Why seven and not five.** §8A's own argument is one job per step; a phase that turns AND drives is
two. The graphic's phase panel was re-flowed to match the argument rather than the argument bent to
fit the panel.

**THE GEOMETRIC INVARIANT, AND IT IS THE PART TO CHECK:** the three turns must NOT sum to zero.
`+90 −90 −90` leaves the robot **facing** the line, and the hunt crosses it perpendicular within a few
centimetres. A maneuver whose turns cancel finishes on its approach heading — parallel to the line —
and **cannot** return to it, whatever the distances are. That was the shipped defect for the whole
life of the lesson, and no gate could see it, because 202 payload files copied it faithfully.

- **A diagnosis that blames turn ACCURACY for a failure to find the line is only correct once the
  turns already sum correctly.** Before S143, §8.3 sent the student to make the turns *more* accurate,
  which makes a cancelling maneuver fail *more* reliably.
- **The wedge is the sanctioned alternative and it is a CHALLENGE, not the base build** (L10
  Challenge 6). It drops the parallel leg and turns back PAST the original heading — `AVOID_RETURN_DEGREES`
  larger than `AVOID_TURN_DEGREES`, not smaller — closing on the line at a slant. Fewer moves, longer
  open-loop run. **The trade is the teaching.**

### 16.13 BYTE COUNTS ARE CHECKABLE — THE AVR TOOLCHAIN IS ONE COMMAND (v8.135 — NEW, S143)

**Any byte figure printed in the book can be verified, and therefore must be.**

    apt-get install -y gcc-avr avr-libc          # avr-gcc 7.3.0+Atmel3.7.0
    git clone --branch 2.0.1 .../pololu/zumo-32u4-arduino-library
    # + its seven declared depends= repos, + arduino/ArduinoCore-avr
    # symlink all nine beside pio_harness.sh, then run the harness

- **`pololu/Zumo32U4@2.0.1` and `zumo-32u4-arduino-library` are the SAME code**, verified from the
  tag's own `library.properties` (`name=Zumo32U4`). The other seven LIBDIRS are that library's
  declared `depends=`, which PlatformIO resolves transitively and the harness cannot.
- **`platformio.ini` is never touched by a verification build.** The clone lives in a scratch folder.
- **The control is the book's own printed figure.** The pre-change L10 finished build compiles to
  **20,364 bytes**, byte-for-byte what the lesson printed — which is what licenses trusting every
  other number the harness returns. **Run that control before quoting a new absolute.**
- The seven `depends=` carry **no version constraints** and the Arduino core is the framework rather
  than a `lib_dep`, so an absolute total is reproducible here but is not guaranteed to match a
...
Lesson 3 from every page in the book.**

- **The split was never a disagreement between lessons. It was PROSE against CODE COMMENTS.**
  Derived from the tree at `6f5a276`: every *Lesson 3* claim sat in lesson prose; every *Lesson 6*
  claim sat inside a syntax-highlighted comment span. One prose leak existed, in L13.
- **TWO CLAIMS CARRIED FORWARD FROM S144 WERE FALSE, AND BOTH WERE TEXT MATCHES READ AS ANSWERS
  (§24.10, rule 38).** *L03 says both* is wrong — L03's two Lesson-6 hits are FORWARD pointers to
  closed-loop control (*"TRIM is the open-loop stepping stone — Lesson 6's encoders are your first
  taste of the real thing"*, and an `AUTO-TRIM ALGORITHM (Preview of Lesson 6)` block that is an
  encoder technique). *L09 says Lesson 6* is a false positive — *"the TRIM-independent,
  battery-independent turn you built in Lesson 6"* describes `turnDegrees()`. **Neither lesson ever
  claimed a home.**
- **THE 342 PAYLOAD COMMENTS ARE TWO SHAPES AND ONLY ONE WAS WRONG.** 148 carry a four-line block
  whose fourth line already reads *"(It was born in Lesson 3, spent in Lesson 6, and filed here.)"* —
  a RELOCATION narrative in L07's reorg lesson, naming the file the student is holding when L07
  opens, which per S51 is the Lesson 6 finished build byte-for-byte. **It is true and it is L07's own
  thesis.** The other 194 hang a bare `(Lesson 6)` on TRIM itself with no Lesson 3 credit anywhere
  near them — checked at 400 characters on all 194, zero hits.
- **THE RULED FIX REMOVES A CITATION RATHER THAN FLIPPING ONE.** Group A's line 1 becomes
  *"// Remember TRIM? It lived at the top of main.cpp,"* — no lesson number — and line 4 carries the
  whole history. Group B's 194 become `(Lesson 3)`. Flipping Group A to Lesson 3 instead was
  measured and REJECTED: it is not false (L03's finished `main.cpp` is 227 lines against L06's 215,
  so *buried in a 200-line file* holds either way) but it makes line 4 redundant, and deleting line 4
  deletes the only sentence in the book that says TRIM MOVED. **Uniformity bought by deleting a true
  fact is the wrong trade** (§24.13's shape applied to prose rather than to a count).
- **NOT EVERY SPLIT IS DRIFT, AND THIS ONE WAS BOTH.** S144 stopped at 342 on the correct instinct
  that a book-wide sweep on one operator's judgment is a ruling. The instinct was right and the
  population was mixed: 194 sites of drift and 148 sites carrying information, indistinguishable
  until the comments were READ IN FULL rather than counted. **A census that reports one number for
  two shapes has already lost the ruling.**
- **`gate_payload_match` IS THE HOLD AND IT WAS BLINDING-CONTROLLED** (rule 17): reverting exactly
  ONE of the 342 payload sites fails the gate naming 1. No new gate is owed — the lesson `<pre>` and
  the payload must move together by construction, so a half-swept fix cannot ship.
- **THE RESIDUE IS FIVE SITES AND ALL FIVE ARE CORRECT.** Two in L13 and two in L03 attach *(Lesson
  6)* to ENCODERS, not to TRIM; one in L09 attaches it to `turnDegrees()`. **A co-occurrence is not
  a claim.**

### 16.16 A SUPERLATIVE IS A CROSS-LESSON CLAIM, AND AN EXPLANATION IS CHECKABLE SEPARATELY FROM ITS NUMBER (v8.136 — LOCKED, S146)

**Two defect shapes, both found in Lesson 13, neither visible to any of the seventy gates.**

**A. A SUPERLATIVE RANKS THIS LESSON AGAINST THE OTHER FIFTEEN.** *First*, *only*, *never*, *the
last time*, *the first lesson that* — each asserts a fact about every other lesson in the book, and
each costs exactly one sweep to test. Do not ship one without reading the lessons it ranks against.

- **The case:** L13 §4.2 and BRAIN CHECK 01's answer 3 both read *“Lesson 13 is the first lesson in
  this book that adds zero hardware.”* **L09 §4 already reads “no new hardware required” and L12
  §4.1 reads “You do not have to plug anything in. You do not have to move a jumper.”**
- **The fix is to narrow, not to delete.** *Not one wire moves, not one jumper changes — every
  sensor was already aboard* is true, is the interesting half, and loses nothing.
- This is §11's dependency rule (a cross-lesson *never…* claim needs a full-tree sweep) stated for
  the case where the claim is a RANK rather than a fact.

**B. AN EXPLANATION CAN BE FALSE WHILE ITS NUMBER IS TRUE, AND THE REVERSE.** A sentence that gives
a figure AND a reason carries two claims. Check both.

- **The case:** §7E said two builds *“differ by just 64 bytes — the price of keeping both turn
  functions alive in the binary.”* Both turn functions are alive in **both** builds, which differ by
  one call site, so the reason names something they SHARE. **Had the 64 been correct, the sentence
  would still have been false.** Measured, the builds are byte-identical.
- **When the correction makes the point stronger, take it.** Two identical builds teach *the size of
  the file will never tell you which line* better than two that differ by 64.

**C. A DECOMPOSITION THAT DOES NOT SUM IS DELETED, NOT ADJUSTED.** Rule 50 offers *derive it or
delete it*; the illegal third option is *replace it with a number that nearly works*.

- **The case:** Step 6's *“120 bytes tighter … bookkeeping cost only 26”* nets −94 against a measured
  −42, and the true symbol-level split (−118 against +81, with 5 bytes of section padding) cannot be
  written for a student without teaching linker padding mid-lesson. Both sub-numbers came out; the
  net and the reasoning stayed.

### 16.17 A CITATION IS A CLAIM ABOUT AN EDITION, NOT ABOUT A NUMBER (v8.137 — NEW, S147)

**When two citations of the SAME rule disagree, neither is a typo. They are two editions in one
document, and no amount of reading either section will tell you which is current.**

- **The case:** L14 §4.1 cites **§4.2.8** for the Lack-of-Progress procedure; L14's own Engineer's
  Log cites **§4.3.7** for the same requirement. The RCJ Rescue Line 2025 changes list carries
  *Added "Terms and Definitions"* — a new **§4.1** that pushed all of Section 4 down by one, so
  `4.2.7 → 4.3.6` (handle) and `4.2.8 → 4.3.7` (single binary switch, and the LoP declaration that
  rides on it). **Every §4.x citation in this book is a 2024 number.** §4.3.7 is correct.
- **The method: go to the rulebook's CHANGES list, not to the more common spelling.** A majority
  vote among citations counts how often the old edition was copied, which is the opposite of
  evidence. Confirmed independently here — RCAP Junior Rescue Line U19 v2026.0, derived separately
  from RCJ 2025, numbers both rules identically.
- **CITE THE EDITION INLINE** — *"§4.3.7 (RCJ Rescue Line 2025)"* — so a citation carries its own
  expiry date. This defect class survived two editions precisely because a bare section number
  cannot be dated by the reader.
- **A FILE NAMED FOR A YEAR IS NOT EVIDENCE OF THAT YEAR.** `ROBOCUP_RESCUE_LINE_2026.md` is the
  **2025** ruleset throughout, and it is the file the book checks itself against, so every claim it
  certified inherited its edition. **Name the edition INSIDE the file.**
- **CHECK THE LEAGUE BEFORE THE EDITION.** A document can be the newest, be genuinely from this
  year's championship, and still be a different competition — the *RoboCup Rescue Robot League* is
  the major league (80 kg teleoperated robots, stepfields, GeoTIFF map submissions) and shares no
  rule with Junior Rescue Line. **Read the league name and the scenario before the section numbers.**
- **A REGIONAL VARIANT IS A DIFFERENT GAME, NOT A REPRINT.** RCAP U19 is *based on* RCJ 2025 and
  still deletes the black victim, the red evacuation point and ramps entirely, capping the
  multiplier at 1.96. **Agreement on one section's numbering is not agreement about the field.**
- **Corollary, and it is what §4.1's inspection bullets cost:** a spec that appears in NO edition is
  not a stale citation but an invention. Neither RCJ 2025 nor RCAP carries a robot size, height or
  weight limit; the 25 cm is **bridge** geometry (RCJ §3.2.3, tile entrance and floor-to-ceiling),
  split into two wrong inspection bullets, one of which then told the student to fit UNDER an
  obstacle the book's own Lesson 10 teaches them to drive AROUND.

**DEBT, RECORDED NOT FIXED: §16.14 still has no numbered section body** — v8.135.3 announces it and
...
### 20.5 `data-callout` — THE STABLE CALLOUT IDENTITY (v8.122 — NEW, S130, DJ ruling)
Every callout block carries `data-callout="L.n"` — lesson number, then an ordinal — in §20.2's form,
written by `callout_id.py`. **It is a MACHINE MARKER, not an anchor**, which is why it takes
`data-challenge`'s shape rather than `id="kind-slug"`: nothing reads it but instruments, and minting
1,069 `id` anchors nothing links to would put reader-facing furniture in the markup to serve a gate.

**WHY IT EXISTS: every other locator in the suite is a FILE OFFSET, and any edit above it is
invalidated** — the rule S129 learned the hard way. Three DERIVED keys were tried at S130 before
this one (a content hash at two window sizes, and the label text) and the best re-found only **199
of 212** blocks across a single markup change. A derived key cannot be an identity.

**THE ORDINAL IS AUTHORED, NEVER RECOMPUTED, AND THAT IS THE WHOLE PROPERTY.** It is assigned once
in document order; inserting a callout takes the next free number in that lesson and **nothing below
it renumbers**. `--apply` refuses to touch a block that already carries one. A positional scheme
would drift exactly as line-keyed targets do.

**GATED** by `callout_id.py --audit` and by gate 60's coverage arm: every callout marked, every
marker unique book-wide, every marker naming its own lesson.
...
**(5) A COUNT NOBODY'S INSTRUMENT PRODUCES IS A SIGNAL TO WRITE THE INSTRUMENT, NOT TO WRITE THE
NUMBER.** (v8.122 — S130, DJ ruling.) Added because parts 1–4 did not stop it, and neither did §24.8
or §24.10. S130 catalogued every wrong claim it made: **nine of nine came from a one-off code path
and zero from a versioned instrument.** The two that reached DJ were the two with no assert of any
kind — every other one failed loudly and privately against a control that already existed. Both had
the identical shape: **a query scoped to one source, reporting a conclusion about all of it.** §7.1
read as all of §7, producing "14 orphans" that came one ruling from deleting eleven correctly-indexed
files; and *"verifiable"* read as one thing when it is two, producing a recommendation to accept a
loss that was never necessary.

This is deliberately NOT a fifth restatement of "be careful with your grep". §24.10 already records
why that fails: *a rule that must be remembered at the moment of temptation is not working; what
actually closed these defects was TOOLING.* So the rule changes what is BUILT, not what is recalled,
and it is checkable after the fact by looking at the commit: **did an instrument ship beside the
claim, or not?** S130's own correction is the pattern — the bad one-off became
`build_mark_index.roster()` plus gate 61, and the census gap became gate 62. Weaker than a gate,
stronger than a maxim, and its compliance leaves an artefact.
...
### 24.10 THE PARSER IS THE DEFAULT INSTRUMENT; A TEXT MATCH READS ONE KNOWN LINE (v8.78 — S91, DJ ruling; AMENDED v8.130.1 — S137, DJ ruling: *"Grep just seems to be a waste and you miss so much when you grep. When possible use other methods."*)
...
- **S137: this rule was violated twice in the session that amended it, and both times the
  operator believed he was complying, because he had not typed the word `grep`.** A Python
  `if 'for loop' in line` scan over L03 returned two hits; those two were reported to DJ as a
  finding with two rulings attached. **The population was never known** — the scan cannot see
  a challenge that needs a loop and does not say the word, which is the majority case. Worse,
  the underlying question had ALREADY BEEN RULED in S57 and shipped; the scan could not see
  that either, because a ruling does not live in the lesson text.

**The rule.**

0. **THE RULE NAMES A PROPERTY, NOT A TOOL (v8.130.1).** A **text match** is any instrument
   whose predicate is "does this string occur in this line": `grep`, `rg`, an editor's find,
   a Python `in` / `str.find` / `re.search` walk over lines, a list comprehension over
   `open().split('\n')`, or a `--audit` flag that does any of those internally. **They are one
   instrument wearing different clothes.** S137 spells this out because the S91 wording said
   *grep* and an operator therefore wrote the same defect in Python twice in one session while
   believing the rule did not reach him. **Pin the property, not the spelling (S126 Rule 19,
   applied to a canon entry rather than a gate).**
1. **A structural question goes to a parser.** Counts, censuses, inventories, placement,
   nesting, "how many lessons carry X", "which construct owns this reveal", "which challenges
   hand the student a construct" — these are `lesson_inventory.py`'s job. Where it cannot
   answer, **EXTEND IT**; do not scan around it.
2. **A text match is legal for exactly two things.** (a) **Reading a single line whose format
   is fixed and known** — a version banner, a stamp. This use has never failed us and §12.6
   depends on it. (b) **LOCATING candidates to go and READ.** In this second use its output
   is a set of line numbers and nothing else.
3. **A TEXT MATCH LOCATES; IT NEVER ANSWERS (v8.130.1).** Its result is a set of candidate
   lines **whose population is unknown by construction**, because the one thing it cannot
   return is the case that does not contain the string. Therefore: **a hit is read in full
   before it is reported, and the absence of hits is never reported as absence.** This is
   §24.6c applied to the locator rather than to the gate, and §34's *grep the figure
   book-wide and read every sentence it appears in* is this clause's positive form — the
   scan is step one of two, and stopping after step one is the defect.
4. **Every count AND EVERY CLAIM presented to DJ names the instrument that produced it
   (v8.130.1 extends this from counts).** A number with no named source is a lead, not a
   finding — and so is a SENTENCE with no named source. **A lead is presented as a lead, and
   a lead never carries a ruling request**, because attaching a decision to an unverified
   premise asks DJ to rule on something that may not be true, which is the §12.5
   wrong-answer-costs-3× shape arriving as a question instead of an answer.
5. **BEFORE SCANNING, ASK WHETHER THE ANSWER IS IN THE SESSION RECORD.** S137's scan
   re-opened a question DJ had already ruled in S57 and shipped. **No instrument that reads
   the tree can see a ruling**, because a ruling lives in this Bible, in the changelog and in
   the session history — never in a lesson file. Search past sessions before measuring the
   book.
...
against, so §24.2 owes no gate here. The check is whether a presented number OR CLAIM can
name its parser.*

**AND THE AMENDMENT OWES TOOLING, NOT WORDS (v8.130.1).** This entry's own thesis is that a
rule which must be recalled at the moment of temptation does not work, and S137 proved it by
breaking this rule inside the session that amended it. **The standing obligation is therefore
on `lesson_inventory.py`:** every time a text match is reached for because the parser cannot
answer a structural question, that is a **feature request against the parser**, and it is
recorded rather than absorbed. S137's open one: *enumerate, per challenge card, the C++
constructs the student is asked to WRITE* — the question §11 (v8.36.1) requires an answer to
before any lesson's §9 can be certified, and which no instrument in the tree can currently
answer.
...
#### 25.10j A CHECKLIST RUNG THAT ASKS A *WHY* CAN BE RESHAPED INTO A BC03 QUESTION (v8.106 — NEW, S117, DJ ruling)

**§25.10a folds an extra checkbox block into the Brain Check it most resembles, as a labelled
group. This is the case §25.10a cannot take.** L12's ten-rung *Exit Ticket* held four rungs that
no §2 objective covers and that are not bench observations either — *"I know why `gyroSetup()`
must run **before** the line-sensor calibration"*, *"I know why `turnDegreesGyro()` has **no
TRIM**"*, *"I understand why Bonus B4 compiles to exactly the same number of bytes"*, *"I can say,
out loud, which of those two numbers was wrong"*. Each is a **knowledge claim carrying its own
§-citation**, and each has nowhere legal to go: §25.5 makes BC02's **I can…** group §2's
objectives character-exact, so an extra claim cannot join that list without breaking the
*asked back* contract, and a third labelled group would be a new construct invented to hold four
items.

**DJ RULING S117 — THEY BECOME QUESTIONS.** Presented as two options: **A**, the student ticks a
box claiming they know; **B**, the student is asked, and finds out whether they actually do.
DJ ruled **B**. The rung supplies the stem and the citation; the session authors the interrogative
form and the answer.

**THE TEST IS THE VERB, NOT THE TENSE.** A rung that claims a capability (*Convert…*, *Write…*)
is BC02's business. A rung that claims to have **observed** something at the bench (*I turned…*,
*I deleted…*) is BC02's **I have…** group per §25.10a/b. A rung that claims to **understand why**
is a Knowledge Check item wearing a checkbox, and a checkbox cannot tell the difference between a
student who knows and a student who is willing to tick.

**THIS RUNS THE OPPOSITE WAY TO THE EXISTING PRECEDENT AND THAT IS THE POINT.** L07 and L09 both
moved an item *out of* BC03 — L09's practical question reshaped into BC04, L07's likewise. S117
is the first move *into* BC03, and it is worth more than it looks: it converts an ancestor a
lesson already had into seeds for a block that otherwise has **no ancestor at all**. L12's BC03
would have been four items authored from nothing; it is four items authored from four stems the
lesson wrote itself.

**THE COST, STATED PLAINLY.** The student loses four ticks and gains four reveals, so BC02's
lock gets easier to reach while BC03 gets longer. Where a lesson's checklist is its ONLY exit
construct, reshaping it wholesale would leave BC02 with nothing but §2's objectives — check that
BC02 still carries an **I have…** group before reaching for this.

#### 25.10k OBJECTIVES THAT ARE NOUN PHRASES ARE REWORDED IN SECTION 2, NOT IN BC02 (v8.107 — NEW, S118, DJ ruling)

**§25.5 makes BC02's *I can…* group the lesson's own §2 objectives, migrated character-exact.
L13 is the case where character-exact migration produces a sentence that is not English.**
Its six objectives are NOUN PHRASES — *"The lawnmower sweep — covering an unknown room with
rows"*, *"An honest limitation — why the black victim ball is nearly invisible"* — so under an
*I can…* heading they read *"I can… ☐ The lawnmower sweep."*

**MEASURED BEFORE IT WAS RULED, AND L13 IS THE ONLY ONE.** All sixteen lessons were surveyed:
L02–L12, L14 and L16 open verb-first (*Explain…*, *Calibrate…*, *Write…*) under the lead-in
*"By the end of this lesson you will be able to:"*. L13 carries neither the verbs nor the
lead-in, and is the sole outlier in the book. L01 and L15 carry no ☐ objective list at all.

**THREE OPTIONS WERE PRICED AND DJ RULED A.**

- **A — reword §2 verb-first and migrate that.** §25.5 then holds by construction.
- **B — migrate the nouns verbatim** and relabel the group. Rejected: no label fits both *"the
  raw channel"* and a capability claim, and *I understand…* runs straight into §25.10j, which
  had just ruled that an *understand* claim belongs in BC03 rather than behind a checkbox.
- **C — reword only inside BC02.** Rejected on principle: the two lists drift apart, which is
  the exact defect §25.5 exists to prevent, and the drift is invisible because both lists still
  render correctly.

**THE RULE. When a lesson's §2 objectives do not fit BC02's group label, the objectives are
wrong and §2 is what changes.** BC02 is never the place to fix the wording, because BC02 is
downstream: it is a copy, and editing a copy to read better is how two documents stop agreeing.
Only the opening verb moves — every noun, every em-dash clause and every `<code>` span is
preserved, and the equality is ASSERTED in the builder rather than eyeballed, comparing the six
§2 items against the six BC02 items with markup stripped and the box glyph removed.

**A COROLLARY THAT COST NOTHING HERE AND COULD COST A LOT LATER.** §2 and BC02 differ in the box
glyph BY DESIGN — §2 ships `&#9744;` and BC02 must ship the literal ☐, because the §25.10 gate
counts the literal (the S115 trap, hit again at S117). Any equality assert between the two lists
must therefore normalise the glyph away, or it will fail on a difference that is required.
...
#### 24.14d A CALLOUT FAMILY NAMES ITSELF IN THE BODY, AND THE REGION DECIDES (v8.127 — NEW, S134, DJ ruling)

**DJ ruling S134, taken on rendered specimens of both glossaries side by side:** *"the lesson should
have the KEY TERM callout and the glossary just has the word and def."* A KEY TERM callout sitting in
the LESSON BODY opens its head with the literal `KEY TERM: ` followed by the term in a `<strong>`;
a KEY TERM entry sitting in the GLOSSARY region does not.

**THE MEASUREMENT REVERSED THE SESSION'S DIRECTION, AND THAT IS THE ENTRY'S POINT.** S133 left 22
body KEY TERM blocks in three unruled shapes and the obvious reading was that a literal `KEY TERM:`
prefix was redundant with the `key` mark — the S129 *"we don't want Note - Note"* shape. Measured
across all 1,119 live callouts, the opposite is true: **NOTE 113/133, CHECKPOINT 102/112, TIP 79/85,
DO THIS NOW 55/58, WARNING 67/80, LEARN 38/47, and BRAIN CHECK / THE GOAL / MY PLAN / BUILDS ON at
100%.** KEY TERM stood at **13 of 238** — and those 13 were **exactly the 10 prefix blocks and the 3
L15 outliers a normalisation pass was about to strip.** The only large family that did not name
itself was about to be made unanimous in the wrong direction. **§24.13 on a convention: the shape
that looks like drift may be the shape that is right, and only the whole population can say which.**

**S129 IS NOT CONTRADICTED, AND THE DISTINCTION IS THE FAMILY'S VISIBLE NAME.** Those six blocks read
their family word TWICE — a text label *and* a repeat. KEY TERM shows only as `key.svg`, so the block
named itself ZERO times in words. A prefix is not a second naming; it is the first one.

**THE REGION IS THE OTHER HALF OF THE RULE AND IS NOT A CONCESSION.** The glossary sits under a
banner that already reads *Glossary*, so 151 repetitions would name the family at section scope
twice — S129 one level up — and would push the term, the thing a reader scans a glossary FOR, off
the left edge of every line. The split is therefore load-bearing and legible: **the prefix marks
*taught here*, its absence marks *looked up here*.** It costs nothing to enforce, because §24.14b
already gave `lesson_inventory` the glossary `region` tier; this is not an exception a human must
remember but a property a gate reads.

**THE PREFIX SITS OUTSIDE THE `<strong>`, AND THAT IS STRUCTURAL RATHER THAN COSMETIC.** **19 body head anchors sit on that `<strong>` — 18 spelled `id="term-*"` and one, L03 `3.44`,
spelled `id="glossary-trim"`** and those ids are the targets of the ruled
body→glossary link direction that §27.14 enforces; the `<strong>` is also the span the end-of-book
glossary harvest extracts as the term. Wrapping the prefix would put the family name inside the
term. It renders identically either way, because the head div is already bold.

**FOUR BODY BLOCKS ARE HELD BY NAME (§25.2a), NOT BY A COUNT.** `3.31` is a provenance question,
`3.101` an operator announcement, `6.24` a formula box, `14.28` a procedural list. None is a term
followed by a definition, so a `KEY TERM:` prefix would label them as something they are not. DJ held
constrain explicitly — *"I don't know what to do with Constrain. Leave it for now"* — and the other
three are the same shape. **Whether they belong in this family at all is UNRULED**, and two of them
wear another family's canon ground (`3.31` on `#d1ecf1`, WHAT YOU SHOULD SEE's; `3.101` on
`#e3f2fd`/`#2196f3`, §18.4's type-explainer). That is a family question, not a shape question.

**THE PIN FELL 125 → 55 AND THE GATE RULED IT, NOT THE SESSION.** Writing the family into the markup
means `canon_of` names 70 previously-unnameable blocks from CONTENT, and gate 62's coverage arm
reported those holds expired without being told to — S132's behaviour exactly, rule 20 for the second
time. **The rows were retired BY THE PROPERTY**, the gate's own `_need` derivation, never a hand
list, and the count reconciles: 83 converted, less the 13 that already named themselves and were
therefore never pinned, leaves 70. **The coupling this creates is real and is recorded rather than
hidden:** those 70 blocks now depend on the prefix staying, so removing one fires §24.14a, §24.14c
and §24.14d together — which is three gates agreeing, not three gates failing.

**GATE 68, written in the same pass as the ruling.** It asserts BOTH halves, because a gate checking
only the body would certify a glossary drifting into the prefix. It IMPORTS `head_of()` and `PREFIX`
from `keyterm_prefix.py` rather than keeping a fourth copy of a matcher — the defect S133 found three
times in one file. **Its first draft failed its own control and the fix is canon here too:** adding a
fifth id to `HELD` excepted a real conformant block while every arm stayed green, because the
coverage count measures blocks SCANNED, not blocks ASSERTED. **A hold arm is not optional on any gate
that carries exceptions** — the set is pinned by NAME, and each held block must still genuinely lack
the prefix, so a hold that has quietly become unnecessary expires loudly instead of sitting there
certified. Control-run seven ways from a snapshot of the FIXED tree: a body prefix removed, a
glossary entry given one, a term un-wrapped, `HELD` emptied, a held id swapped, a held block given
the prefix, and the untouched tree passing at both ends.

**THE HEAD LOCATOR IS THE AUTHORED ATTRIBUTE, NEVER A CLASS NAME.** The converter's first draft
matched the head by its style declarations, which exist only in the EXPANDED source — the file
carries `class="div-fs-105em"`. Keying on that class instead would have been worse than a bug: S133's
rule 25 means the `-N` name it depended on could be handed to a different rule by an unrelated edit
and the tool would go silently blind. `data-mark` is authored, so the locator walks out from the mark
to its enclosing element and reads the same for a `<div>` head and for L15's inline `<b>`.

**AN INLINE HEAD BECOMING ITS OWN LINE TAKES THE PROSE WITH IT.** L15's three read
`<b>KEY TERM — Term:</b> a control algorithm that…`, lowercase because the definition was
grammatically continuing the head's sentence. Lifting the head onto its own line makes that lowercase
letter a defect the reader sees, so the converter's span extends past the head to capitalise the
first letter and asserts both that a letter is there and that it was lower case. **A structural
change that ends a sentence early owes the sentence a look.**

**NOT RULED, AND DELIBERATELY UNTOUCHED:** the head text colour. 16 body heads carry `color: #6a1b9a`
in clean lesson strata — L04 5/5, L09 6/6, L10 5/5, and 0 of 59 elsewhere. That is a paint question,
parked at S133 alongside the five KEY TERM grounds, and a repaint does not spend this rule.

#### 24.14c NO TIER MAY BE KEYED ON DECORATION (v8.122 — NEW, S130, DJ ruling)

**This failure has now happened twice, one tier apart, and the second time was caused by the fix
for the first.** S112 deleted a COLOUR-keyed fallback after a repaint silently orphaned a block —
recorded in `ZUMO_FAMILY_MAP.md` in its own words: *the repaint did not break the assignment, it
revealed that the assignment was never earned.* The replacement was a GLYPH table. At S130 the
marks arc replaced the emoji with an `<img>` and **212 blocks lost their only family signal** — the
same failure, one decoration later.

**THE RULE:** a resolution tier may key on CONTENT or on an AUTHORED marker, never on paint, emoji,
icon, class name or any other output of the family. §24.14 already makes the mark an output and
forbids reading it back; this extends that to every other decorative property, because a tier keyed
on decoration is a tier waiting for the next repaint or reskin. **A stopgap that resolves real
blocks is not a stopgap, it is a dependency** — the GLYPH tier shipped calling itself temporary and
was load-bearing for 212 blocks eleven sessions later.

**The replacement is `ZUMO_FAMILY_PINS.md`, a PRESERVED LAYER** on the `css/semantic.css` model: it
is read-only input, written once from a verified state, and must never be regenerated from
`data-family`. **A pin rebuilt from the value it exists to check agrees with any drift by
construction** — so gate 60 pins the table by md5 rather than by row count, since a regenerated pin
has exactly the same keys. Its coverage arm asserts the PROPERTY: the pinned set must equal the set
of callouts the content tiers cannot name, so a block gaining or losing a content signal fails
loudly and a human rules.

**WHAT WAS AND WAS NOT LOST, STATED PRECISELY**, because conflating these two produced a wrong
recommendation before DJ pushed back on it: the family is NOT lost — it lives in `data-family`, and
all 212 were verified against the pre-swap glyph with **zero mismatches** before the tier died. What
was lost is the ability to RE-DERIVE it from content. Those come apart, and only the second one is
gone; **protection from silent change is recoverable, and the pin recovers it.**

**A LIBRARY MAY NOT EXIT.** `load_pins()` first raised `SystemExit` on a missing file, and because
`build_family_map` is imported by the gate suite, an absent pin killed the whole run **mid-suite and
still exited 0** — the run stopped before two gates and reported success by exit code. An absent
input is reported BY THE GATE, which can name it; a library cannot.
...
#### §26.10 — THE COLOUR LEDGER IS `ZUMO_COLOR_LEDGER.md` (S122, DJ ruling)

**Every outstanding colour decision in the book lives in `ZUMO_COLOR_LEDGER.md` in the repo
root, not scattered across session handoffs.** Opened S122 with 16 items, each carrying a
measured count and the instrument that produced it (§24.10). This section names it so the file
is not an ungated orphan in the root — the §12.2 shape.

**The ledger's blocking fact, recorded here because it changes how every colour item is
priced:** `css/book.css` is generated and has **no value-substitution layer**. Class names are
derived from the values they carry, so at S122 **155 distinct hexes appear inside 562 class
names**. `#ddd` alone owns 38 of them. **A repaint is therefore a RENAME, not a substitution**,
and the ledger's first item is not a colour at all — it is whether `build_css` emits custom
properties, so a repaint becomes one file instead of 562 renames.

**Do not rule a single colour because an unrelated arc surfaced it.** S122's Tier-1 sweep found
the Engineer's Log stripe split by era — `#0e1a2c` in L01–L11, `#6f7582` in L12–L16 — and it was
PARKED rather than ruled, precisely because deciding one value out of an unsettled palette is
the S91 failure this section exists to document.
...
### 27.15a Inline `<code>` carries the pill, and an element rule needs its own scope (S124, DJ ruling)

**The rule.** Every inline `<code>` in a lesson carries the pill. There is no exception list, no
carve-out by lesson, and no class to remember. It lives in `css/semantic.css` as `code { … }` and it
is ruled once for every book.

**The ground is a translucent wash, `rgba(0, 0, 0, 0.08)`, not an opaque colour.** This is not a
taste ruling, it is a scope ruling wearing a value. Inline code sits on **43 distinct backgrounds**
in this book and only 46% of it is on the cream page; eight elements sit on DARK grounds whose
containers each declare `color: white`. An opaque light pill renders white text on light grey in all
eight, silently. A wash inherits its ground instead of fighting it, so one rule is correct on cream,
on the forty callout and table colours, and on the eight dark headers. The price, ruled knowingly:
the pill is no longer a fixed neutral grey — on a coloured callout it tints to that colour.

**The `pre code` reset is part of the rule, not an optimisation.** 15 bare `<code>` elements live
inside `<pre>`. An unscoped background paints a pill inside every dark code block in the book.

**Why this needed a new gate.** An element rule reaches elements no author ever listed — that is the
whole point of graduating one and it is the whole risk. Gate 53 checks declarations that EXIST,
§27.13 checks the GENERATED block, and gate 54 checks the layer is preserved VERBATIM. **A verbatim
copy of a wrong rule passes all three.** Gate 55 holds the reset and asserts the ground is still a
wash, and its independence is measured by injection in three directions, each firing gate 55 alone.

**The audit rule this arc restates, and it is §24.8 in its most expensive form.** The first sweep
found the dark-context elements that still carried a CLASS — two — and reported two. The seven that
carry no class at all were invisible to it, because the instrument enumerated the styled population
while the rule reaches the whole population. **When an element selector is the subject, the audit
must enumerate ELEMENTS, never the classes on them.**

**A count of what changes is not a count of what it costs.** Three options were priced for DJ and
two of the three prices were wrong on first telling: the wash was recommended as the conservative
choice when it repaints 3,336 elements against 1,233, and was then described as more work when it is
a one-line value change. Both errors pointed the same way — away from the option DJ preferred. State
the two numbers separately, always.

### 27.15b Stripping a class from some of its users renames it for the survivors (S124)

`build_css` names a rule after its **dominant tag**. Remove a class from the elements that made that
tag dominant and the rule is re-named on the next regeneration, while the surviving elements still
carry the old spelling in their markup — and go dead. It happened to `.code-ff-uimonosp-2`, which
became `.span-ff-uimonosp` when its four `<code>` users left and three `<span>`/`<strong>` compiler-
error elements remained. **No gate saw it. `strip_inline --verify` did.**

**The order is load-bearing:**

1. strip the class from its departing users
2. regenerate — pass 1 emits the new name while the old definition is still resolvable
3. move the survivors onto the name pass 1 emitted
4. regenerate again — it converges

**Renaming after a second pass is unrecoverable**, because the generator can only preserve a class it
can still resolve; by then the definition is gone and no rename can bring it back. The only remedy is
to revert to the pushed tree and re-derive. Do not patch forward.

### 27.15c A page outside the generated block can still consume the semantic layer (S125, DJ ruling)

**DJ:** *"I want the book to be the same throughout."* The rule: **a construct ruled in the semantic
layer is ruled for the whole book, not for the pages that happen to link `css/book.css`.**

`going_deeper.html` is not a `build_css` source and never will be — it carries its own `<style>`
block, its own design language and its own inline attributes. That made it invisible to the §27
migration, and it grew a third inline-code treatment nobody had counted. **Being outside the
generated block is not a reason to hold a different convention; it is only a reason to be reached by
a different pipe.**

**The pipe is a direct `<link rel="stylesheet" href="css/semantic.css">`, and this page is the first
direct consumer.** The layer now has TWO delivery paths and ONE source: preserved verbatim inside
`css/book.css` for the sixteen lessons, and served as a file to any page that links it. One source is
the part that matters; gate 54 guards it.

**Seat the `<link>` ABOVE the page's own `<style>` block.** Equal specificity resolves by source
order, so the page wins every collision on a declaration it sets. It does NOT win on declarations it
does not set — which is the standing cost of this arrangement, and it is accepted knowingly: the page
will inherit whatever graduates into the layer next, uninvited.

**Before converting a page, measure its grounds, not its markup (§27.15a).** The question is never
*what are these elements called* but *what will the element rule land on*. Here: 71 inline `<code>`,
none carrying a class, 62 on `#ffffff` and 9 on `#e3f0e5`, zero dark grounds and no container
declaring `color: white`.

**Delete the page's local rule; do not restate the ruled values in it.** A second spelling of a ruled
construct is drift waiting to happen, and no gate watches a `<style>` block for a copy of the layer.

**GATE 56 HOLDS THIS SECTION (S125).** The four defect shapes it catches were each measured by
injection first, and each left all 55 preceding gates green:

| injected defect | what actually breaks |
|---|---|
| the `<link>` is deleted | 71 inline `<code>` lose the ruled pill, silently |
| the page restates `code { background: … }` | one ruled construct, two spellings — the drift this section rules against |
| the `href` resolves to nothing | the pipe is decorative |
| the `<link>` is seated below the `<style>` | the layer overrides the page instead of the reverse |

**`site_parity` and `font_stack_sweep` are both blind to the first two (§24.8).** Parity compares
what the site *serves* against the repo, and a page that stops *referencing* a file still matches;
the sweep rewrites non-compliant stacks, and an absent stack is not non-compliant.

**The consumer set is NAMED, never counted** (§25.2a). `_SEM_CONSUMERS` in `book_gates.py` is the
list; a page joining it is a deliberate act. The gate's coverage arm fails if that list is empty.

### 27.16 One spelling per character — literal unless invisible in source (S127, DJ ruling)

**The rule.** A character is written however it can be **seen** in the source file. If the literal
form is distinguishable when you look at the file, write it literally. If it is not — if it renders
and reads identically to an ordinary space or hyphen — write it as an entity, because otherwise
nobody, human or instrument, can verify it is there.

**Why it is a property and not a list.** The second clause reaches exactly three characters today
(U+00A0, U+2011, U+202F), and gate 58 derives that set from the property rather than trusting it,
so a fourth invisible character cannot be smuggled in. Their spelling is **named where a name
exists, decimal where none does** — `&nbsp;`, `&#8209;`, `&#8239;` — and the no-name claim is
derived from `html.entities`, not asserted.

**Why literal and not entity.** Entity form is three spellings, not one, and the book already
carried 14 characters spelled two ways inside entity form alone. An all-entity rule needs a second
rule nobody has ruled, and 103 of the book's 143 non-ASCII characters have no named entity, so it
would print `&#128161;` where an editor needs to see a lightbulb.

**Scope is text nodes.** `<script>` and `<style>` are raw-text elements the parser never decodes —
an entity there is literal program text, and the Maker's payloads live in one. Attribute interiors
are left alone, where `&quot;` inside a double-quoted value is mandatory. `&`, `<`, `>` are
mandatory everywhere.

**The proof of a sweep is render-equivalence, not the diff.** `entity_sweep.verify()` asserts the
HTML-decoded text of every file is byte-identical before and after; a sweep that changes what the
page says fails before it writes.

**The corollary, and it is S126's rule 18 restated: a gate that pins a spelling certifies whatever
it was given.** Four predicates and one generator broke on this ruling, and one of them was a
`strip_inline` locator whose failure was silent — seven held attributes lost their protection with
no gate saying so. When a spelling is ruled, **scan every instrument by AST for the old one**;
waiting for the next failure finds only the loud ones.

### 27.15 The semantic layer — where meaning lives, and why the generated block cannot hold it (S123, DJ ruling)

**DJ:** *"I thought since we weren't using canvas we didn't need the inline code."* Correct, and
the file agrees: the lessons carry **zero** inline `style=""` attributes and **25,752** `class=`
attributes. The inline representation `build_css` works in exists only **inside one run of the
generator** — it expands every class back into the declarations it stands for, deduplicates, and
re-derives a name. §27 retired the constraint that produced that model; the model outlived it.

**THE CONSEQUENCE IS NOT COSMETIC.** Because names are re-derived from **values** every run, the
generated block can only ever reproduce the status quo. `.code-inline-bg-e8e8e8` will never become
`.code`, no matter how many times it runs. It cannot hold an element selector, a semantic class or
a custom property. It is a **fixed point of the migration, not a design system** — which is why
there was nothing a second book could inherit, and why `ZUMO_COLOR_LEDGER.md`'s first item is not
a colour but whether custom properties can be emitted at all. One disease, two symptoms. Measured:
**540 of 636 class names encode a value** (a hex, a px, an em), covering 22,816 of 25,752 uses.

**`css/semantic.css` IS THE ONE STYLESHEET FILE THAT IS HAND-EDITED ON PURPOSE.** It is preserved
verbatim at the top of `css/book.css`, under a marker, and never re-derived. `css/book.css` remains
DO-NOT-HAND-EDIT (§24.12): edit the source and regenerate.

**HOW A CONSTRUCT GRADUATES** — one at a time, gated, stoppable at any point:

1. Write the semantic rule in `css/semantic.css`.
2. Drop the value-named class from the elements it replaces.
3. Regenerate; the generated block shrinks by one rule.

Constructs that never graduate keep working exactly as they do now. **Nothing in this section
obliges the rest of the migration to happen**, and retiring the round-trip outright is explicitly
NOT ruled: it would touch 540 class names, seven instruments that read `expand_classes`, and 25
gate references. The door is open; walking through it is optional and incremental.

**THE FIRST GRADUATE IS `code, pre` (S123).** 1,199 bare `<code>` elements across L10–L16 were
reached by no author rule at all, so their typeface was the browser's choice rather than the
book's, while 2,136 styled ones in L01–L09 carried the ruled stack through a class — one construct,
two conventions, split at the **L09/L10 seam**, the same seam S108 found for the §6.5 Box. The rule
now reaches **3,336 inline `<code>` and 832 `<pre>`**. It declares `font-family` and nothing else:
**whether inline code keeps its grey pill is unruled and deliberately deferred**, and when it is
ruled it is ruled here, in one line, for every book.

**THE FAILURE MODE IS SILENT, SO IT NEEDS A GATE AND NOT A CONVENTION.** Measured by injection
before the layer existed: an element rule pasted into `css/book.css` did not error and did not
warn — the next regeneration simply **deleted it**, and every gate stayed green. **Gate 54**
asserts the layer is present in the stylesheet and byte-equal to its source, that both markers
exist, that the layer carries at least one element selector (a layer with nothing in it that the
generated block could not express is decoration), and that it carries no value-named class.
**Its independence is measured, not claimed:** adding a value-named class to `semantic.css` and
regenerating leaves `build_css --check` reporting *current* and §27.13 green, and gate 54 alone
names it.

**§27.11 IS NOW SCOPED TO THE GENERATED BLOCK**, and so is `build_css`'s Control D. Control D was
**scoped rather than weakened** — the generated block must still be class-scoped, which is the
property it was written to hold. Scoping also closed a mixed population this same session created:
the baseline counted class rules but every declaration, so one added element rule read as *"a rule
gained or lost a property"* — the precisely wrong diagnosis, printed in the gate's own words.
Rules and declarations hold **unchanged at 636 / 2,332** across the whole change, which is the
proof that adding the layer moved nothing generated; only the digest moves, because the span it
covers now begins at the GENERATED BLOCK marker.
