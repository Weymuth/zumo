...
**Date:** August 9, 2026 (Session 137 — **A TEXT MATCH LOCATES; IT NEVER ANSWERS, AND NAMING THE TOOL LET ME ROUTE AROUND THE RULE.** §24.10 has said *the parser is the default* since **S91**, off DJ's own words — *"Grep has caused most of the issues we have faced in the book"* — and **it was violated twice in the session that amended it**, both times by an operator who believed he was complying because he had not typed `grep`. A Python `if 'for loop' in line` walk over L03 returned two hits, and those two reached DJ as a **finding with two rulings attached**: a population of unknown size reported as a measurement. **The rule now pins a PROPERTY** — `grep`, `rg`, editor find, `str.find`, `in`, `re.search`, or an `--audit` flag doing any of those internally are **one instrument in different clothes** — and its output is candidate lines whose population is unknown by construction, because the one case it cannot return is the one lacking the string. **A hit is read in full before it is reported; an empty result is never reported as absence; a lead never carries a ruling request.** **And no instrument that reads the tree can see a ruling** — the `for`-loop question the scan "found" had **already been ruled in S57 and shipped**. The same error was paid a second time when `IMAGE 3.16` vs `GRAPHIC 3.16` was called a collision: a census across all sixteen lessons shows **eleven lessons reuse a number across figure types**, L04 carrying `4.1` as an IMAGE, a GRAPHIC *and* a VIDEO. DJ ruled it in one line — *they teach two different things.* **Then L03 was read end to end, 4,409 lines, and it carries SIXTEEN fixes no gate could see.** Challenge 2's reveal printed **`Replace batteries`** in a book whose §3.6 says *recharge — draining NiMH below 4200 mV damages the cells*. Challenge 3's reveal sent students to place code *after `waitForButtonA()`* — **a function not in the library, an object not in their file, and a gate that did not exist.** §1's Physics Corner explained the curve with **F=ma and "1% less acceleration"** where §3.11 explains the same thing as a **speed** difference. **And the safety one was found by pulling the PAYLOAD rather than reading the card:** both L03 challenge-template starters carried **no button object and no gate**, so the robot drove the instant the upload finished — and the `ramp` starter never told the student to stop the motors, in a lesson whose §3.8 warning is *"the robot drives off the table."* Both now run inside `if (buttonB.getSingleDebouncedPress())`, L03's own pattern; `waitForButton()` was rejected on measurement — **it does not appear before L04.** Also fixed: C4's stale **Lesson 5** pointer for the `for` loop, which S57 gave to L04 §8A.6, plus a second copy of it inside the `ramp` payload; C5's goal contradicting its own `speedIndex = 1`; a dead `<p>Differential drive diagram:</p>` with nothing under it; Step 4 citing a banner Step 2 had deleted; §8A.4's settle-time challenge whose stated reason was not true; Bonus 4's unnamed descending loop; and **L04 §8A.6's "nobody explained it"** — L01 had. Plus the *robot's* left/right rule and why TRIM always hits the left motor. **Checked and left alone:** sticker colours, TRIM polarity, `constrain()`'s rationale and the NiMH bands all match Bible §16.1–16.3, and **`1000 ms = 1 second` is taught in §3.7 prose** — that queue item retires after standing since S40.)
**Status:** ✅ **69 GATES PASS · L03 READ END TO END, 16 FIXES · BOTH L03 STARTERS NOW GATED ON BUTTON B.** **THE ONE TO CARRY: a text match locates, it never answers** — its population is unknown by construction, so read every hit in full and never report an empty result as absence. **A lead never carries a ruling request.** **And no instrument that reads the tree can see a ruling** — search the session record before measuring the book. **L04–L16 ARE STILL UNREAD; L04 IS NEXT**, and it is the lesson S57 gave the `for` tutorial to. **THE ORDER STAYS CANON: READ → FIX → QUIZ** — L03's bank is now unblocked, 1 of 12 written, status DERIVED from `quizzes/quiz_bank.py --status`. **DEBT, MEASURED NOT ARGUED: no gate ties a challenge card to its starter payload** — reverting `buttonB` out of a payload while leaving the card gated passes all 69 gates at exit 0, and that hole is how `RUN_MS` came to sit in the payload and not in the card. Bible **v8.130.1** · `book_gates` **v1.64.1** · Maker **v2.45.5**.

**Versions:** L01 v03.28.2 · L02 v03.21.2 · L03 v03.40.2 · L04 v04.28.2 · L05 v04.28.1 · L06 v04.31.1 · L07 v04.31.1 · L08 v04.30.1 · L09 v05.26.1 · L10 v02.27.1 · L11 v02.28.2 · L12 v01.31.2 · L13 v02.28.2 · L14 v02.33.1 · L15 v02.30.1 · L16 v02.22.1 · going_deeper v01.6.1 — census **40,467** · Bible **v8.130.1** · BookComponentStandard v01.13.0 · gen_component v1.6.1 · Maker v2.45.5 · **book_gates v1.64.1** · lesson_inventory v1.3.5 · pill_sweep v1.1 · gate_payload_match v1.7 · build_family_map v1.6.0 · callout_id v1.0 · keyterm_prefix v1.0.1 · build_mark_index v1.1.0 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.24.0 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.3.0 · image_audit v1.2 · strip_inline v1.2 · build_worklist v1.1 · regex_audit v1.0 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · entity_sweep v1.0 · font_stack_sweep v1.3.0 · next_pointer v1.2 · family_tag v1.2.1 · glossary_convert v1.0 · mark_wire v1.0.2 · glyph_scan v1.1 · title_feed v1.0 · Timer v1.3.2 · `ZUMO_Syllabus_WORKING.md` v1.2 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `e27d50b`.**
