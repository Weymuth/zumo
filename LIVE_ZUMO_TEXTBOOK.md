...
**What shipped.** **`KINDS[1]` + `PAYLOADS["1"]` — Lesson 1 has a Maker registry for the first time** (13 kinds, 11 payload bodies, one shared code body per Bible §18.3). `mainCpp()` is now **lesson-aware for MY PLAN**: L01 ships WITHOUT the pseudo-code block, because **L01 mentions pseudocode zero times** (verified L01=0, L02=13, L03=20 — used-before-taught, same class as the L03 modulo and L04 `for` finds). `DL_ALL_LESSONS` += 1. Verification: `node --check` PASS on both script blocks · **Maker output vs. the 11 disk files = 11/11 BYTE-IDENTICAL** · all 16 lessons intact in KINDS+PAYLOADS (212 keys) · **book-wide payload gate PASS for L02–L16** · all 11 files compile at 10,208 B. Maker md5 after push: `3c06a7c0dc36db1e7d1dee9ee3893e79`.

**🔴 BLOCKING INCONSISTENCY — read before touching L01.** **L01 is still v03.2.7.** The Maker now serves eleven L01 challenge files that `Lesson_01.html` never mentions, so the **payload gate FAILS 148 checks on L01** — every one the same cause: the payload code does not appear in the lesson. This is the gate refusing to let the Maker and the book disagree, not a defect. Nothing student-facing is broken (no L01 card links to the Maker yet), so it waits safely — but **the §9 rewrite is now mandatory, not optional polish.**

**The design reversal that shaped this session.** S53's approved "one file, all 11 challenges as commented blocks" design was **killed mid-build by DJ** and replaced with **eleven separate per-challenge files sharing ONE code body**, each with its own header block. Ch1 alone ran ~60 lines of commented instruction; eleven of those is a wall a Lesson 1 student scrolls past to find their own program. Proof the body is genuinely shared: **all eleven compile to exactly 10,208 B.** `ZUMO_L01_CHALLENGE_FILE_SPEC.md` was deleted as superseded.

**Bench-verified on DJ's robot this session.** The *Close Encounters* five-note sequence (G4 392 · A4 440 · F4 349 · **F3 175** · C4 262) — all audible at even volume, DJ: *"Sounded exactly like the movie."* **Ship the real sequence, no octave substitution.** Also: EEPROM name read works · the 800 ms beep is audible where 200 ms was not · **volume 30 does NOT error** (library caps at 15; compiles, uploads, makes sound — queued for the syntax-gap pass as *it compiles, it runs, it's wrong*).

**Three defects caught by testing, not by review.** (1) The robot name printed in the first milliseconds of `setup()`, before the 32U4's USB port finished re-enumerating — invisible. Moved to print immediately after `Hello, Robot!`. **Never ask a student to catch output printed before the USB port settles.** (2) The 200 ms beep was unevaluable under the LED blink that follows → **800 ms book-wide** (§5.5 listing, §5 physics prose, Ch2 card, §10 Knowledge Check, GRAPHIC 1-19, all 11 files). (3) MY PLAN in a lesson that never teaches pseudocode. **A fourth was caught by the gate:** my first Maker injection failed 11/11 because my generator invented its own header (`Hello, Robot!` + hyphen) instead of the Maker's real format (`TITLES[1]`=`Hello Robot` + em-dash). **Never hand-write a header the Maker also generates.**

**Also added to L01 (staged, NOT yet inserted):** a §8 **butterfly-error** troubleshooting row + callout — DJ hits `butterfly_recv(): programmer is not responding` recurringly. Web-verified against Pololu 0J66/8.1: **double-tap reset within 750 ms enters bootloader mode; the bootloader exits after 8 seconds**. DJ's Clean-then-upload workaround works because it *buys time* for the port to re-enumerate, not because it fixes the build — the callout teaches both, double-tap first. ⚠️ Pololu documents a macOS bug that broke this bootloader outright (10.15 Catalina); long superseded, but DJ's error came off a Mac — **if butterfly errors cluster on Macs and not Windows in September, that is a platform pattern, not student error.**

**Currently working on (S55 NEXT — land ONE complete L01 as v03.3.0):** do all of it in a single pass so the file bumps once: (1) **§9 rewrite** — all 11 cards rewritten to match the built files + a "📁 Work in:" bar on each linking `newproject.html?lesson=1&kind=cNN` (L08 pattern); ⚠️ C01's card still says "Change the Message," which is now PART 3 of five; (2)–(5) the four approved prose blocks, **verbatim HTML in `ZUMO_L01_PROSE_BLOCKS.md`, do NOT re-draft** — §5 Jim Reekes · §9 Williams/Spielberg · §3.1 AI-autocomplete hard callout · §1 "How You Ask Is Part of the Answer"; (6) Quick Reference note chart (C4–C6 naturals + F3 175, plus `playNote()` / `play()` pointers and the background-play caution); (7) GRAPHIC 1-19 `<img>` wiring + image-index row; (8) C05 wording — add *"Why might the Serial Monitor be a challenge **at that point?**"*; (9) **re-run the gate: L01 must go FAIL(148) → PASS**; (10) §5b version-in-two-homes — visible banner AND hidden comment.

**⚠️ TEST C06 BEFORE WRITING §9 CARDS.** C06 (Does It Come Home?) and C11 (Battery Check) are the only two of eleven not yet bench-tested. **If C06's robot comes home too accurately the drive needs lengthening — which regenerates the shared body, all 11 files AND the payloads**, invalidating any cards written against the current versions.

**Bench status of the eleven:** APPROVED — C01 (**LOCKED**) · C02 · C03 · C04 · C05 · C07 · C08 · C09 · C10. NOT YET TESTED — C06 (needs tape + ruler) · C11 (needs a tired battery pack).

**Rulings locked this session:** everything lives in git (**option B** — the eleven challenge files live in the repo as authored source AND as Maker payloads, with the gate proving they agree; ⚠️ **this makes extending the gate to L01 mandatory, not optional**) · robot names in **EEPROM addr 512**, magic `0x5A`, **22-name roster** (L16 uses addr 0 — no collision, but L16's "this book has never touched it" line is now false and needs the 512 address-map note) · **L04 C03 = option (b)**, a `for` primer framed as a callback to L01 §5.5, where `for` IS explained (**the S53 log's "unexplained code" claim is wrong and must be corrected**) · the AI ranking stays teacher-side in `TEACHER_NOTE_AI_tools.md`.

**🔴 SEPTEMBER DEADLINE:** **name-flash every robot before students open L01**, or the whole class sees `(unnamed -- see your teacher)` on day one. ~1 min/robot with `ZUMO_NAME_WRITER_main.cpp`. Add to the Period 1 handout-day row of the Teacher Daily Grid.

**Still queued:** L16 EEPROM prose fix · **L04 `setLayout21x8` — the ONLY lesson of 16 missing it** (L01=1, L02=11, L03=3, **L04=0**, L05=7 … L16=3) · L04 C03 primer + `L04_LEARNMODE_LOG.md` correction · extend the gate to L01 bodies · the 6 syntax-gap prose candidates + "out-of-range values don't error." **BENCH:** Q017 L09 six numbers · Q044 calibration-spin · Q046 gyro-bias · L02 §5 green-LED · Constrain RUN_MS. **PARKED:** solution-disclosure · monetization/ebook · "Know Your Zumo" · day-by-day grid + syllabus · TDP template v3 (A5 Lab Log) · §9 difficulty grouping · L06 card pattern. **⚠️ AI Tutor** — students get API access, the syllabus has no entry for it, `tutor.html` is stale with no L12+ content, term starts Sept 8.

---
_↓ S53 and earlier history below, unchanged. ↓_

**Date:** July 19, 2026 (Session 53 — LEARNER-MODE session; book NOT touched).
**Status (S53):** ✅ **L04_C02 Line Counter COMPLETE & hardware-verified.** L04_C03 Position Pointer PAUSED at the `for`-loop wall. Live versions UNCHANGED (no lesson/Maker/Bible edits): all 16 lessons as at S51 · Bible **v8.33.1** · Maker **v2.36** · Gate v1.3 · Harness v3.0. Verified by fresh clone, commit `84d255b`. New root files (records/specs only): `ZUMO_L01_CHALLENGE_FILE_SPEC.md`, `ZUMO_S54_HANDOFF.md`; `L04_LEARNMODE_LOG.md` appended.
**Currently working on (S54 NEXT — BOOK-WORK session, DJ's call "fix the book, then go back and test it"):** PRIMARY = build the **L01 challenge file** per `ZUMO_L01_CHALLENGE_FILE_SPEC.md` (one file, all 11 challenges as commented blocks, uncomment→fix→test→recomment; needs NEW `KINDS[1]` + `PAYLOADS["1"]` — L01 currently has ZERO Maker integration, grep-verified; bumps Maker + L01, must pass the payload gate). THEN the **syntax-gap prose pass** (6 S53 candidates: `=` vs `==` both directions · three increment spellings · stray `if(...);` · `;` vs `}` · C02 display collision **kept and taught** per DJ ruling · slot-ambiguity 3rd recurrence). ALSO still queued: the 5 unapplied S52 L04 candidates. ⭐ **S53 findings:** (a) **C03 requires a `for` loop but `for` isn't taught until L05 §5.13** — missing prerequisite, NEEDS A DJ RULING (move it / add a primer / restructure loop-free / leave as stretch); (b) **L01 has no Maker registry at all** — only lesson without one. ⚠️ DJ stated every student will eventually have learner-mode access for every lesson — that makes the **AI Tutor rebuild a dependency of the classroom model**, not a parked nice-to-have (Sept 8 start). Detail: `ZUMO_S54_HANDOFF.md` + `L04_LEARNMODE_LOG.md` (S53 entry).

---
_↓ S49–S51 history below. NOTE: the next `**Date:**` line's parenthetical describes S49; the `**Status:**` / `**Currently working on:**` lines below are S51-current. The S52/S53 banner above supersedes them. ↓_

**Date:** July 18, 2026 (Session 49 — SIX tasks, ALL PUSHED & LIVE. (1) **L03 challenge-card dark-block anomaly fixed** (commit bb10c28): L03 was the only lesson of 16 wrapping challenge Goal PROSE in a dark `#1e1e1e` block; 8 goal-blocks unwrapped to the white-body norm — root cause of invisible chips + washed-out callout. L03 v03.4.4→v03.4.5. (2) **Maker C07/C08 payloadRef fix** (bb10c28): `drive_a_square`+`auto_trim` had payloadRef `null`; set to `finished` (DJ ruling: C01–C06 stay finished-preload). Maker v2.31→v2.32. (3) **`ZUMO_L03_TEMPLATES.md` completed** (bb10c28): 6→all 8 challenges, term "challenge template", reference record not payload source. (4) **raw→Pages image URLs, BOOK-WIDE** (commit 05247d4): all 114 `<img src>` refs across 16 lessons moved from `raw.githubusercontent.com` (rate-limited, intermittent blank images) to `weymuth.github.io/zumo/images/` (same-origin, unthrottled, Canvas-compatible). Fixes the L04 4.x-missing symptom for the whole book. Every lesson +1 minor. (5) **L04 jumper photos wired** (05247d4): IMAGE 4.2 (factory jumper close-up) + new IMAGE 4.4 (5-sensor jumper position) added, EXIF/GPS stripped; IMAGE 4.1 filled with a temporary L11-diagram stand-in until the underside photo is shot (4.3 still needed). L04 → v04.0.11. Bible v8.32→**v8.33** (§10 image-URL canon + EXIF-strip rule + §11 no-dark-prose checklist item). (6) **S49 close batch** (commit a8e2130): L03 prose — prototype explainer [C01], 3 build Coach's Tips [C04], zero-index clause [C05], global-vs-local scope note [C06] → L03 v03.4.6→**v03.4.7**. L04 API-crutch — each of the 5 challenge cards gains a "🔧 Functions you'll need" signature line at point-of-use → L04 v04.0.11→**v04.0.12**. `L04_LEARNMODE_LOG.md` added. Gate PASS all 16 throughout.)
**Status:** ✅ **S51 COMPLETE — ALL PUSHED & LIVE (final commit 8346b24, Jul 19)** · L01 v03.2.7 · L02 v02.2.4 · L03 v03.4.7 · L04 **v04.0.12** · L05 v04.1.9 · L06 v04.5.9 · L07 **v04.3.10** · L08 v04.1.7 · L09 v05.0.9 · L10 v02.1.12 · L11 v02.2.2 · L12 v01.2.3 · L13 v02.2.2 · L14 v02.4.2 · L15 v02.2.3 · L16 v02.2.2 · Bible **v8.33.1** · Maker **v2.36** · Gate v1.3 · Harness v3.0 · 🖼️ **All images now served from Pages (no more raw rate-limit blanks).** L04 jumper photos live. · ✅ **Verified by fresh clone (S51 close, Jul 19): commit 8346b24 — Maker v2.36 (L05 challenges `finished`, L07 `after_step_1` present, per-lesson vocab in), Lesson_07 v04.3.10 (note present), Bible v8.33.1; LIVE tokens match the files.**
**Currently working on:** ✅ **S51 CLOSED — book-work session, all pushed & live (commit 8346b24, verified fresh clone Jul 19).** **Done:** Maker **v2.32→v2.36** — (1) L04 blank-starter missing `GLOBAL VARIABLES` added (book-wide); (2) L05 challenges C1/C4/C5 `payloadRef null→finished`; (3) per-lesson blank-starter vocab (L05/L06 `CONFIGURATION`, correct globals) via lesson-aware `mainCpp`; (4) L07 catch-up base — new `after_step_1` = byte-exact L06 `finished` + `step_1` kind, `step_2` repointed (mirrors L08–L16). Bible **v8.33→v8.33.1** (§18.3 section-list reconciled lesson-agnostic + v8.33 changelog backfilled). L07 **v04.3.9→v04.3.10** (Step 8 note: L06's `displayEncoderCounts()` diagnostic isn't carried into the organized build). Gate PASS throughout. **The L04 learner-mode build was NOT touched this session** (DJ had no line surface at home). **S52 NEXT — PRIMARY: resume learner-mode L04 build** (paused mid-Step-4; full detail in `ZUMO_S51_LEARNMODE_L04_HANDOFF.md`): three discoveries (wave/numbering/honesty) → record raw white + black-tape values → Step 5 calibration (deliberate RED-build rep) → Step 6 → Act Two → §7 Resolution → restore ritual. **BLOCKER to clear first:** robot + white surface + black electrical tape (the exact thing that blocked S51). **IMAGE QUEUE (DJ still to shoot):** L04 4.1 underside (temp stand-in live) · L04 4.3 test surface · the rest of the 22-photo IMAGE_SHOT_LIST. **NEXT queued:** finish L04 learner-mode (C02–C05, after DJ does the discoveries) OR pending L03_C05 Variable Speed · C06 reorder to #1 · whole-template starters L08/L09/L10 · Maker batch (bulk DL · `?lesson=N` gate · C## labels) · L01 VS Code multi-root step. **L04 LEARNER-MODE FINDING (S49, C01 walked):** logic is solid; the wall is Zumo/OLED API recall — put a small 'functions you'll need' crutch at point-of-use in each challenge card. See `L04_LEARNMODE_LOG.md`. **BENCH (need robot):** Q017 L09 six numbers · Q044 calibration-spin · Q046 gyro-bias · L02 §5 green-LED · Constrain RUN_MS. **PARKED:** solution-disclosure · monetization/ebook · "Know Your Zumo" · AI Tutor (LAST) · day-by-day grid + syllabus · TDP template v3 (A5 Lab Log) · §9 difficulty grouping · L06 card pattern.
