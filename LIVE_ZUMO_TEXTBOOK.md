...
**Date:** August 9, 2026 (Session 138 — **A FIGURE THAT IS PUSHED BUT UNREFERENCED IS A RED TREE, NOT A DELIVERY.** `L04_GRAPHIC_4-07_five_sensor_windows.svg` landed on its own and took the suite from 69/69 to two failures — §17.3c's raster census and §10's worklist — because an asset nothing consumes still moves every counter that watches the tree. It is now wired into L04 twice: §1 as the *there they are* reveal, retiring the `[IMAGE 4.5]` placeholder, and §4.1 as the numbered working reference, retiring a **borrowed Lesson 11 diagram** that had been standing in with a *temporary* caption and array-slot labels. **`IMAGE 4.1` retires with it** — the shot list, the Figures table and the reserved filename all described the same photograph the new figure now carries, so a shot came off DJ's list without being taken. **THE ORIENTATION WAS THE HARD PART AND IT WAS SETTLED BY MEASUREMENT, NOT BY LOOKING.** Pololu's own manual (0J63 §3.5) gives the convention — sensor 1 is the robot's left-most — but the photograph is an underside view with the blade pointing *away*, which mirrors it. Three independent signals agreed: the `FRONT` arrow, the silkscreen (`RGT`/`DN4` on the viewer's left, `DN2`/`LFT` on the right, matching §3.2's own pin table), and the moulded chassis logo reading reversed. DJ confirmed on a real board. The chips run **5 4 3 2 1** left to right and the margin labels say **ROBOT'S RIGHT** and **ROBOT'S LEFT**, so the figure now *demonstrates* the mirror §4.1 spends a paragraph warning about instead of falling into it. **AN ILLUSTRATOR ROUND-TRIP IS NOT AN EDIT.** Four passes through Illustrator resampled the embedded photograph every single time — JPEG 1200×1148 209 KB became PNG RGBA 1208×1152 at 1.36 MB, three pixels wider per save — dropped `role`/`<title>`/`<desc>`, flattened hand-written attributes into `.st0` classes, baked text centring into `translate()`, and on one pass **injected a `Lorem ipsum` textPath** where the footer rule had been. Every label edit was ultimately made in text against the original file; the shipped photo is byte-identical to the first upload. **AND A GENERATED BOARD IS NOT A PHOTOGRAPH** — a blueprint-styled redraw was declined because its silkscreen read `OJ8696` where the real board reads `0J8696`; one provable wrong glyph means all fifteen labels are model output rather than evidence, and §14 forbids exactly that.)
**Status:** ✅ **69/69 GATES · L04 READ END TO END · GRAPHIC 4.7 WIRED IN, IMAGE 4.1 AND 4.5 BOTH RETIRED.** **THE ONE TO CARRY: an asset is not a delivery until something references it.** Three counters moved for a file no page pointed at. **THE FALL FIGURE DEBT DROPS 11 → 9** and the shot list goes 22 photos → 21. **`gate_payload_match` went PASS → FAIL inside the S137 close push and the handoff still claimed PASS** — it is not one of the 69, so nothing runs it unless a session opener does; its EXEMPT keys pin literal starter LINES, and S137 rewriting those lines orphaned three of them. Fixed at **v1.7.1**, control-run loud. **L04's read found TWELVE defects, none yet applied — they are the next thing.** The largest: `Challenge 4's TEMPLATE does not compile` (`LINE_SEEN` used, never declared — compiler-confirmed), §8A.8 saying a crossing counts *hundreds* when its own arithmetic derives **five**, a `Lesson 2 §3.2b` citation to a section L02 does not have, and arrays introduced as new in §5.5 when **L03 §8A.5 already teaches them**. **THE ORDER STAYS CANON: READ → FIX → QUIZ** — 1 of 12 banks written. Bible **v8.130.2** · `book_gates` **v1.64.2** · Maker **v2.45.5**.

**Versions:** L01 v03.28.2 · L02 v03.21.2 · L03 v03.40.2 · L04 v04.28.3 · L05 v04.28.1 · L06 v04.31.1 · L07 v04.31.1 · L08 v04.30.1 · L09 v05.26.1 · L10 v02.27.1 · L11 v02.28.2 · L12 v01.31.2 · L13 v02.28.2 · L14 v02.33.1 · L15 v02.30.1 · L16 v02.22.1 · going_deeper v01.6.1 — census **40,468** · Bible **v8.130.2** · BookComponentStandard v01.13.0 · gen_component v1.6.1 · Maker v2.45.5 · **book_gates v1.64.2** · lesson_inventory v1.3.5 · pill_sweep v1.1 · gate_payload_match v1.7.1 · build_family_map v1.6.0 · callout_id v1.0 · keyterm_prefix v1.0.1 · build_mark_index v1.1.0 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.24.0 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.3.0 · image_audit v1.2 · strip_inline v1.2 · build_worklist v1.1 · regex_audit v1.0 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · entity_sweep v1.0 · font_stack_sweep v1.3.0 · next_pointer v1.2 · family_tag v1.2.1 · glossary_convert v1.0 · mark_wire v1.0.2 · glyph_scan v1.1 · title_feed v1.0 · Timer v1.3.2 · `ZUMO_Syllabus_WORKING.md` v1.2 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `442f68d`.**
