...
**Date:** July 30, 2026 (Session 94 — **NAMING IS FINISHED: ALL 1,048 CALLOUT BLOCKS ARE NOW ASSIGNED TO 30 FAMILIES, AND PAINT WAS DELIBERATELY NOT DECIDED.** `ZUMO_S94_FAMILY_MAP.md` places every block, reproducibly, via the new `build_family_map.py`. **Nine new rows are required** (WHAT YOU SHOULD SEE 25 · TRY THIS 22 · STILL GREEN 17 · WHAT YOU NEED 13 · REAL-WORLD CONNECTION 7 · THINK ABOUT IT 7 · MYSTERY 5 · YOU MIGHT WONDER 4 · OBJECTIVES 1), and **four roster families that had ZERO blocks now have work** — IF YOU'RE STUCK 0→9, GOING DEEPER 0→7, HOW THIS SECTION WORKS 0→2, COMMON PITFALLS 0→1. The challenge-card *Work in / Where to look* header stays a §7.2 supporting mark, **not** a family, on the same §4.1 grounds that killed the 🛑 row at S93. **THE SESSION'S SHARPEST FINDING IS AGAINST ITS OWN ANALYSIS: `ENGINEER'S LOG` WAS REPORTED AS ZERO BLOCKS AND IT HAS 17** — the labels carry `&rsquo;`, not a straight apostrophe, so the matcher missed every one. That is **§24.11 verbatim, an entity is not the character it encodes**, committed by the instrument eleven sessions after the same mistake shipped a live L12 regression. Three further numbers from earlier in the session are therefore corrected or void: **the "5 collisions / 121 repaints" figures are VOID** (pre-entity-fix), **`WHAT YOU NEED` is 13 blocks not 6** (four schemes, two glyphs), and **"104 blocks remaining" understated the job** — 420 blocks lacked a family label; the 104 were only those on schemes where nothing at all matched. **THREE STANDING QUEUE ITEMS WERE CLOSED AS PHANTOMS, ALL ON FALSE PREMISES:** the L03/L08/L09/L10 `finished`-payload debt (all four lessons HAVE a `finished` payload, every row resolves, and DJ's own S49 ruling already put C01–C06 on finished-preload), the L03 "1000 ms = 1 second" explainer (**already written and live**), and the L03 modulo explainer (**already written and live**). **Before doing any queue item, verify it still exists.** — **ALSO SHIPPED: `lesson_inventory` v1.1.0→v1.1.1**, banner expectation corrected 2→1, stale since S89, **control-run first** by seeding a real second banner and confirming the check fired; the anomalies list now holds only the Brain Check norm line, **so any new entry in it is a real lead** — which was not true at S94 open. **AND `ZUMO_TDP_Template` v2→v3**, adding **A5 Lab Log**: the syllabus grades Outside Work at **5%** on a log the template gave students nowhere to record. Four assert-guarded edits, diff **+24/−3**, and `ZUMO_TDP_Template_v2.md` **deleted** so two student-facing templates do not coexist. **NO LESSON FILE WAS TOUCHED — census byte-identical at 39,970.**)
**Status:** ⚠️ **ONE GATE IS FAILING AND IT IS THE FIRST JOB OF S95: `ZUMO_MARK_INDEX.html` SITS IN REPO ROOT AND `§12/§23 site layout` REPORTS `STRAY page: ZUMO_MARK_INDEX.html (not a canonical location)`. The book is 34/35, not 35/35.** It is a workshop reference sheet, not a book page, and in root it is also published at `weymuth.github.io/zumo/ZUMO_MARK_INDEX.html` where a student can reach it. **Fix: delete it from root** — `build_mark_index.py` regenerates it from `images/marks/` in seconds, which is why it was written as a generator; committing it instead needs a non-published home **and** a §12/§23 canon entry, which is a Bible change, not a file move. **PAINT REMAINS ENTIRELY UNDECIDED, DELIBERATELY** — naming finished, colour did not start, and nothing was invented at hour thirteen of a session in which two prior sweeps had each shipped a regression. **Five open paint questions, in the order they should be taken:** (1) **LEARN and INSIGHT both sit dominant on `#e3f2fd`/`#2196f3`** and one must move — 37 vs 38 blocks, so it is a meaning call, not arithmetic; (2) **KEY TERM spans three purples**, `#9b59b6` ×136, `#9c27b0` ×33, `#9b6a9e` ×1, and the third is **MY PLAN's own colour**, so a KEY TERM block and a MY PLAN block already share paint; (3) **12 of the 51 schemes are one-offs**, several one hex from a neighbour (`#ffb300` vs `#ffc107`, `#fff9e6` vs `#fef9e7`, `#e8f3ec` vs `#e3f2ed`) — these look like typos and are **the safest possible first repaint batch, 12 blocks, proving the method before anything with 136 on it**; (4) **46 distinct glyphs book-wide, 12 used exactly once** — 🔎 🍽 📐 🔋 📚 🔢 📏 🆕 ≈ 🍳 🎉 🔌; (5) **THE BIG ONE — the label convention for KEY TERM's 184 blocks**: does the label carry the words "KEY TERM", or does the 🔑 mark alone identify the family once marks are wired? That governs more blocks than every other open question combined. **THE MEASURED GROUND UNDER ALL OF IT: 51 distinct bg/border schemes and 32 distinct border colours for 30 families**, top 10 schemes covering 72% of blocks; **the mark library is still entirely unwired** — every attribute of every element across all 21 pages parsed, **zero** references to `images/marks/` or `images/icons/` against a control of 196 other `images/` references, 89 SVGs none used, every live block on emoji; and **every generated mark's baked fill disagrees with its family's live paint**, worst case INSIGHT's bronze `#725637` mark on a blue `#2196f3` callout, because the marks were coloured for Heritage Blue role values that render on **zero pixels** of the book. **THE ROUTE THAT NEEDS NO §26 RULING, AND DJ APPROVED A RENDERED 14-FAMILY PREVIEW OF IT: derive each mark's fill from that family's OWN border hue, darkened until it clears 4.5:1 against its own panel.** All 14 tested families clear it, no new hue is invented, and marks can then ship as plain `<img src>` with no inline-SVG byte cost. **§26 STAYS PARKED AND MUST NOT BE REOPENED: RoboLore has not changed since DJ handed over the zip, it is uncommitted, and it is private, so a session cannot check the remote** — a ruling now would be made against a document that cannot be cited. **DJ ruled explicitly in S94 that the colour and icon work continues and is not to be deferred; that ruling stands.** What changed is that the remaining work is now bounded and measured rather than open-ended.

**Versions:** L01 **v03.14.0** · L02 **v03.5.0** · L03 **v03.19.0** · L04 **v04.14.0** · L05 **v04.14.0** · L06 **v04.18.0** · L07 **v04.14.0** · L08 **v04.13.0** · L09 **v05.11.0** · L10 **v02.10.0** · L11 **v02.12.0** · L12 **v01.14.0** · L13 **v02.12.0** · L14 **v02.14.0** · L15 **v02.11.0** · L16 **v02.7.0** · going_deeper v01.1.1 — **no lesson file was touched in S94; census byte-identical at 39,970** · Bible **v8.81** · **BookComponentStandard v01.10.0 (26 families in the roster; the S94 map requires 9 more)** · gen_component v1.5 (ALL CHECKS PASS) · Maker v2.45.1 · **book_gates v1.26 (35 gates, ⚠️ 34/35 — `§12/§23` FAILS on the `ZUMO_MARK_INDEX.html` root stray)** · **lesson_inventory v1.1.1** · pill_sweep v1.0 · gen_bonus_banner v1.2 · gen_part_banners v1.0 · gate_payload_match v1.6 · Harness v3.0 · **`build_family_map.py` (new, no version string yet — give it one)** · `build_mark_index.py` **(NOT in the repo yet)** · web tools: timer v1.3.0 · tutor v1.1.0 · index v1.3.0 · `images/icons/` 48 + LICENSE · `images/marks/` **41** · **`ZUMO_TDP_Template_v3.md` v3 (A5 Lab Log); v2 DELETED** · **`ZUMO_S94_FAMILY_MAP.md` + `ZUMO_S94_FAMILY_RULINGS.md` are live.** **Verified by fresh clone at `b2ee980`; five of six S94 files byte-identical by md5.**
