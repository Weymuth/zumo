...
**Date:** August 20, 2026 (Session 179 — **DJ RULED FOUR ROOM FACTS AND ALL FOUR ARE NOW CANON RATHER THAN QUEUE ITEMS, WHICH IS THE SESSION'S MOST DURABLE OUTPUT.** Each had been re-asked across three or more sessions because it lived in a bench tracker or a worklist row instead of in the Bible. **§16.48 NEW: GIT IS REQUIRED ON A MAC BECAUSE IT INSTALLS APPLE'S XCODE COMMAND LINE TOOLS — THE SOFTWARE MOST STUDENTS ARE MISSING**, not because PlatformIO fetches the Zumo library over git, which it does not; **GPT was right about the stated reason and its implied fix, *drop the step*, would have broken every Mac in the room**, which is why S177 rekeyed `L01_B37` instead. Closes `L01-03`, open since S137, and bench row `L01-B9`. **AND THE USB CABLE POWERS THE CHIP WHILE THE SWITCH FEEDS THE MOTORS**, so an upload succeeds with the robot off — closes `L01-B2`. **§16.49 NEW: THE PROJECT PATH IS `Documents/PlatformIO/Projects/<FolderName>`, AND THE SVG THAT MADE IT LOOK EXPENSIVE WAS NOT.** S178 fixed L01 only; eleven sites disagreed, so L02 said `ZUMO_Template` lived one directory above where L01 said it lived — two lessons naming different homes for one folder, in week 1. **THE POPULATION WAS MEASURED TWICE AND THE FIRST MEASUREMENT WAS SHORT:** scoped to the lessons it returned eleven, scoped to the whole tree it is **FOURTEEN**, including **two present-tense canon lines in §11 of the Bible itself** (§24.6c). **The SVG was priced as a graphics-chat pass and is live text, 6 KB, one `<text>` node** — the only real cost was layout, 29 Courier characters at 33 px running to 748 against a panel edge at 726, so `mono-root` drops to 30 px, **controlled rather than asserted: at 33 px `svg_layout_audit` reports the overflow and names 748 against 726, at 30 px it is clean** (rule 59). **THE REUSABLE HALF: *coupled to an SVG* IS NOT A COST, IT IS A CATEGORY** — `L02-06` needs an eighth BAND drawn and stays parked; this needed a longer word. Price the artefact, not the category (rule 70). **AND LESSON 1's LAST CONTENT DEFECT WAS FOUND BY READING, PAST A GREEN GATE 76:** §8's Common Pitfalls read *verify you're targeting the **A-Star 32U4 / Zumo 32U4***, a SLASH equating the build target with the board — the exact conflation §16.25 retired — and **gate 76 passes because it forbids the retired SLOGAN, not the slash-equation**, so S178's claim that all six surviving `A-Star` mentions were legitimate was true of five (§24.8). **The bank was right while the lesson contradicted it, a third time in three sessions:** `L01_B14` cites §5.0 AND §8, keys the build-profile answer, and carries *it names the exact board you are holding* as its declared trap — **the old §8 line corroborated the trap.** **LESSON 3's ARC SHIPPED, ELEVEN EDITS, AND STEPS 2–4 WERE ONE COUPLED DEFECT:** Step 2's checkpoint claimed the file held *nothing else* when the Maker also ships the include and the MY PLAN block and **Step 4 depends on that include**; Step 3 told the student to paste a SECOND header into a file the Maker had already headed; Step 4's checkpoint denied the `setup()`/`loop()` Step 2 had just told them to keep, one callout above a CHECKPOINT that builds green. **TWO UNSOURCED NUMBERS CAME OUT AND THE LESSON CONVICTED ONE ITSELF** — a 30-second cooldown rule with no pedigree, replaced by the current-under-load mechanism, and a ±10% battery figure that would make §3.6's own 5400/4800/4200 bands unusable. **`L03_B35` WAS FALSIFIED BY A CORRECT EDIT**, its keyed answer being that ±10% figure, authored faithfully from a lesson that was wrong — §16.47's shape again. **DJ ALSO RULED THE SIX FEET OUT AND THE FOURTH SITE WAS A DIFFERENT DEFECT:** the curve test stays on the floor, students simply do not need 6+ feet, and §4.5's *a 6-foot test run makes drift very obvious* describes the RUN rather than the ROOM — **the run is `TEST_DURATION` 2000 ms at `BASE_SPEED` 200, nothing like six feet, so the lesson was overstating its own test.** L06 asks for ~1 meter, L07 for a meter, L10 for 30 cm: **the lesson demanding the most floor had the shortest run in it.** **AND A DEAD/LIVE PROBE MANUFACTURED A FALSE DEAD, WITH THE RECOMMENDATION WITHDRAWN IN THE SAME SESSION IT WAS MADE:** `L03-01` was reported DEAD on a grep for `no setup()/loop() exists` where the file writes `no <code>setup()</code>/<code>loop()</code> exists`. **A false DEAD is invisible because nobody re-checks a retired row**, so a mechanical pass over the 48-row bucket needs a probe that normalises inline-code markup and fails loud. `L03-05` stays dead because gate §16.31 says so — the difference between a dead row and a row a grep missed. **`ZUMO_GPT_REVIEW_WORKLIST.md` v1.3 GAINS PART 0 — CLOSED ROWS**, marked and never deleted per DJ's ruling: 36 closed, 2 parked, each carrying HOW it closed, because `L02-04` and `L02-05` died unnoticed and cost a whole read at S178.)
**Status:** ✅ **S179 — FOUR ROOM FACTS RULED INTO CANON, LESSON 3's ARC SHIPPED, AND LESSON 1 OWES ONLY A FLOOR.** **78/78 gates**, `gate_payload_match` **PASS**, `quiz_bank` 16 banks at **1,246** questions, `build_css --check` current, `build_worklist --check` current **after regeneration**, `image_audit --check` current, `callout_id` 1127/0, `next_pointer` clean, census **41,010**. **`site_parity` WAS PARITY ON TWO CONSECUTIVE RUNS AT SESSION OPEN**, closing S178's owed item; the tree now differs from the published site by design and it **MUST RUN TWICE AFTER THIS PUSH (§16.42)**. **`byte_audit` DID NOT RUN AND DID NOT NEED TO** — no payload moved, and the Maker changed by ONE STRING in a display hint. **§27.8b's CYCLE WAS NOT OWED AND A SCRATCH-COPY CONTROL SAYS SO:** regenerated in a COPY and compared name by name — **574 rules both ends, ZERO names added, dropped or repointed, and the two files md5-IDENTICAL.** **SEVEN lessons, TWENTY-ONE bank bumps, the Maker, one SVG, TWO new Bible sections and one new document.**

**Versions:** L01 v03.31.3 · L02 v03.22.1 · L03 v03.44.3 · L04 v04.29.3 · L05 v04.29.2 · L06 v04.32.5 · L07 v04.31.6 · L08 v04.32.1 · L09 v05.27.4 · L10 v02.30.3 · L11 v02.31.1 · L12 v01.33.1 · L13 v02.35.0 · L14 v02.36.0 · L15 v02.32.0 · L16 v02.28.0 · going_deeper v01.6.1 — census **41,010** · Bible **v8.173** · BookComponentStandard v01.13.0 · gen_component v1.6.1 · Maker v2.63 · **book_gates v1.72.9** · lesson_inventory v1.3.5 · pill_sweep v1.1 · gate_payload_match v1.9.0 · build_family_map v1.6.6.1 · callout_id v1.0 · keyterm_prefix v1.0.1 · build_mark_index v1.1.0 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.30.0 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.2 · build_css v1.4.0 · image_audit v1.3 · strip_inline v1.2 · build_worklist v1.2 · regex_audit v1.0 · byte_audit v1.9.1 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · entity_sweep v1.0 · font_stack_sweep v1.3.0 · next_pointer v1.2 · family_tag v1.2.1 · glossary_convert v1.0 · mark_wire v1.0.2 · glyph_scan v1.1 · title_feed v1.0 · quiz_bank v1.6.1 · Timer v1.3.2 · harness_setup v1.1 · pio_harness v3.1 · `ZUMO_Syllabus_WORKING.md` v1.3 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `b1986ac`.**
