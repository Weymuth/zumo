...
**Date:** July 25, 2026 (Session 70 — **§25 THE FOUR EXIT-REGION CONSTRUCTS** canonized · L01 built as the reference lesson · header/footer/hidden-banner made identical across all **17** pages · DEEPER pill added to the strip).
**Status:** **SIX NAMES FOR ONE JOB.** A §10 audit found six differently-named written-response blocks — *STOP & PROCESS — Explain It in Writing* · *STOP & PROCESS — Answer From Your Head* · *Conceptual Understanding* · *Knowledge Check* · *Check Your Understanding* · *Reflection Questions* — unevenly spread, two of them sharing a name while running opposite pedagogies, and **L13 and L15 carrying none at all**. Same disease as §4.1's three meanings of "Challenge". DJ ruled **four** constructs: **Mental Knowledge Check** (3–5 recall items, before hands-on work, feeds the Canvas reading quiz) · **Knowledge Check** (§10, apply/predict/trace) · **Technical Skills ☐** (§10) · **Reflection** (§10, no reveal, feeds the Notebook/TDP). The split is **recall vs apply, not section number** — L01 runs read→do→read→do, so any placement rule keyed to section numbers breaks on lesson one; that error is what wrongly pushed `setup()`/`loop()` out of L01's Mental block mid-session. **L01 is built as the reference** (v03.8.1) with five banked `QUIZVARIANT` stems for the Canvas quizzes, which **do not exist yet** — the flipped design's 20% gate has had no content behind it since the Bible logged *quiz feature deferred*. Separately, the page furniture was made uniform: seven distinct footer shapes collapsed to one, the second visible version banner moved into a **hidden build banner** (DJ: the footer belongs to the reader), and `going_deeper.html` — which had no shared chrome at all — was brought onto the same header and footer. Verified by **markup-skeleton hash**, not by eye: hero `4fdedafb` ×17, footer `aff5311e` ×17. §5b's "both visible homes" rule is superseded; the gate needed no edit because it greps raw source and raw source includes comments. **book_gates v1.3 → v1.4** adds §25.6 and §25.2, control-run per §24.6b in both directions — and the first control run was itself faulty (it injected a string that did not exist and passed), §24.6c landing on my own verification. **All 19 gates PASS.**

**Versions:** L01 **v03.8.1** · L02 **v02.15.2** · L03 **v03.13.2** · L04 **v04.6.2** · L05 **v04.8.2** · L06 **v04.11.2** · L07 **v04.7.2** · L08 **v04.6.2** · L09 **v05.4.2** · L10 **v02.5.2** · L11 **v02.7.2** · L12 **v01.7.2** · L13 **v02.6.2** · L14 **v02.8.2** · L15 **v02.6.2** · L16 **v02.5.2** · Bible **v8.54** · Maker v2.45 · Gate v1.6 · Harness v3.0 · pill_sweep v1.0 · book_gates **v1.5 (21 gates)** · going_deeper **v01.1.0** · **web tools (now version-lined in-file): timer v1.3.0 · Maker v2.45.1 · tutor v1.0.0 · index v1.3.0**.
