...
**Date:** August 11, 2026 (Session 144 — **L11 SAYS A VOID READS WHITE. L04 ALREADY SAYS IT READS BLACK, AND POLOLU'S SOURCE NAMES THE CASE.** §3.5's whole cliff argument rests on *no reflection reads like the brightest possible white*; QTRSensors' own header says larger values are lower reflectance, “*a black surface OR A VOID*”, and L04's troubleshooting table already says *air counts as black*. Traced through the book's own code the truth is worse than §3.5 describes: over a cliff all five sensors read black, `isLineVisible()` returns TRUE, `readLine()` returns dead centre, error is zero — **the robot drives off the edge at full speed believing it is perfectly centred on a wide line**, and the gap machinery never engages at all. §8A.4 therefore prices an alarm that cannot fire, and its boom payoff fails too: **a reflectance sensor on a boom reads the void as black as well. The fix is sensor TYPE, not POSITION.** UNRULED, not rewritten — the conclusion *use a barrier, not software* is unchanged, so L13 and L14 are unaffected and **the cascade is contained to L11**.)
**Status:** ✅ **70/70 GATES · L11 READ AND FIX-PASSED → v02.29.0 · §3.5 OPEN FOR A RULING · L11 UNBANKED.** Three defects closed: **Step 6 broke the build** (it said delete `gapStartTime` claiming *nothing reads it* — true, and two lines still WRITE it, so §6's *every step compiles* promise failed and §8 blamed the student using the same wrong word); **7E's `handleGap()` was truncated mid-function**, unbalanced brace, in the experiment the lesson was built for; and **Step 6's byte claim was backwards** — measured on the toolchain against an exact control (`after_step_1` reproduces S143's 20,516 byte-for-byte), the finished build is **20,702, i.e. 186 bytes BIGGER**, not smaller. **THE ONE TO CARRY: a callout is never a free edit** — a seventh INSIGHT in L11 failed SIX gates at once, every one a baseline pinned at six, and the fact became prose instead. **And a content tier that pins a spelling owes an edit when the spelling IS the defect:** `build_family_map` matched the literal headline that was false, so fixing the prose made the block unnameable.

**Versions:** L01 v03.28.3 · L02 v03.21.2 · L03 v03.41.0 · L04 v04.29.0 · L05 v04.29.0 · L06 v04.32.1 · L07 v04.31.3 · L08 v04.31.0 · L09 v05.27.0 · L10 v02.29.1 · L11 v02.29.0 · L12 v01.31.2 · L13 v02.28.2 · L14 v02.33.1 · L15 v02.30.1 · L16 v02.22.1 · going_deeper v01.6.1 — census **40,616** · Bible **v8.135.2** · BookComponentStandard v01.13.0 · gen_component v1.6.1 · Maker v2.49.1 · **book_gates v1.65.9** · lesson_inventory v1.3.5 · pill_sweep v1.1 · gate_payload_match v1.8.0 · build_family_map v1.6.2 · callout_id v1.0 · keyterm_prefix v1.0.1 · build_mark_index v1.1.0 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.24.1 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.3.0 · image_audit v1.2 · strip_inline v1.2 · build_worklist v1.1 · regex_audit v1.0 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · entity_sweep v1.0 · font_stack_sweep v1.3.0 · next_pointer v1.2 · family_tag v1.2.1 · glossary_convert v1.0 · mark_wire v1.0.2 · glyph_scan v1.1 · title_feed v1.0 · Timer v1.3.2 · `ZUMO_Syllabus_WORKING.md` v1.2 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `f392173`.**

**Quiz banks:** derive with `python3 quizzes/quiz_bank.py --status` — do not hand-count, and do not keep a list here. **L11 is read and fixed but UNBANKED: §3.5 must be ruled first, because six of its homes would key answers.** L12–L16 remain unread and unwritten.
