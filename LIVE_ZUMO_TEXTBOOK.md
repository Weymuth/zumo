...
**Date:** August 20, 2026 (Session 178 — **THE FIX THAT WAS PRICED DID NOT WORK ON ITS FIRST BUILD, AND THE REASON IT FAILED WAS ITS OWN RATIONALE.** Gate 78's claim form required the digits ADJACENT to the words, so `**12** discards over **7**` — this project's ordinary house style, live in three documents — slipped a green gate. **S175 HAD ALREADY WIDENED THIS ARM ONCE and the adjacency requirement survived that widening untouched: a widening is evidence about the case it was aimed at, never about the class.** S177 priced the fix as *blank `**` at EQUAL LENGTH, the way `_CODE78` blanks backticks, so reported offsets stay true to the source.* Built exactly as specified, **control 2 — the bold plant, the entire point of the fix — stayed SILENT**, because equal-length blanking turns `**12** discards` into `  12   discards`, three spaces where the predicate demanded one. **The rationale for the mechanism caused the failure of the mechanism, and only a control written to FIRE where the arm was silent could have said so (rule 59); the four controls that passed proved nothing about it.** Widened to `\s+`, which also reaches a figure broken across a LINE WRAP. **AND THE ORDERING TRAP THE DESIGN WARNED ABOUT DOES NOT EXIST** — measured on six shapes, both orders behave identically, because `**` and a backtick are disjoint characters; **a hazard reasoned out and written into a design is a lead and not a finding (§24.6c), including when this book is the author of the lead.** **THE SESSION'S PRIMARY WORK WAS LESSON 02's GPT ARC, WHERE THE COMPILER SETTLED WHAT READING COULD NOT.** §3.1's *Three Builds That Fail* claimed Build 3 yields a LINKER *undefined reference*; compiled with `avr-g++` it is a **COMPILE** error, *too many arguments to function 'void showCount()'*, because the definition sits below `loop()` and the only `showCount` in scope at the call is the argument-less promise. Builds 1 and 2 measured CORRECT as printed, so **GPT was right about the place and wrong about the diagnosis a FIFTH time**. The section now teaches three cases instead of two. **§8A's TWO CODE BLOCKS WERE WRITTEN IN A FOREIGN HOUSE STYLE AND THE LESSON HAD ALREADY CONVICTED THEM** — introduced as *the `blinkLED()` function you read earlier*, they used `ledRed`, Allman braces thirty lines after §3 declares *this book uses K&R everywhere*, and 4-space indent, while Step 5's own checkpoint says *Yellow is the only LED that's fully yours.* **AND `L02_B05` IS S177's INVERSION REVERSED:** there the bank was right and the lesson wrong; here the bank INHERITED the lesson's error, keying a distractor `why` that reads *the failure arrives later, from the linker* — **a correct edit can falsify a bank that was authored faithfully.** The other three banks pinning `lesson_02` were read and their pins EARNED. **TWO WORKLIST ROWS WERE ALREADY DEAD IN THE LIVE TREE**, found by reading rather than assumed. **TWO WERE DELIBERATELY NOT SHIPPED:** `L02-09`'s baud comment appears **4× inside L02's own payload**, so a lesson-only fix breaks `gate_payload_match`; and `L02-06`'s missing `GLOBAL VARIABLES` section is real — **0 occurrences in the lesson and 0 in its payloads**, against a Challenge 3 that says *declare three counters up top* — but its fix moves an SVG, a color key, *The Seven Sections* and probably Bible §18.3. **TEN MORE PROSE DEFECTS SHIPPED PAST 78 GREEN GATES — §16.18 from the content side, second session running.** Bible **v8.171**, §16.47 NEW and seated with a numbered body.)
**Status:** ✅ **S178 — LESSON 2's ARC IS SHIPPED AND GATE 78 REACHES A BOLDED FIGURE.** **78/78 gates**, `quiz_bank` 16 banks at **1,246** questions, `build_css --check` current, `build_worklist --check` current **after regeneration**, `image_audit --check` current, `callout_id` 1127/0, `next_pointer` clean, census **41,006**. **`site_parity` WAS PARITY ON TWO CONSECUTIVE RUNS AT SESSION OPEN**, which closes S177's owed item; the tree now differs from the published site by design again and it **MUST RUN TWICE AFTER THIS PUSH (§16.42)**. **`byte_audit` DID NOT RUN AND DID NOT NEED TO** — no payload moved and there was no Maker change. **ONE lesson, FOUR banks, ONE instrument and ONE document changed.**

**Versions:** L01 v03.31.2 · L02 v03.22.0 · L03 v03.43.2 · L04 v04.29.2 · L05 v04.29.1 · L06 v04.32.4 · L07 v04.31.5 · L08 v04.32.1 · L09 v05.27.4 · L10 v02.30.3 · L11 v02.31.1 · L12 v01.33.1 · L13 v02.35.0 · L14 v02.36.0 · L15 v02.32.0 · L16 v02.28.0 · going_deeper v01.6.1 — census **41,006** · Bible **v8.171** · BookComponentStandard v01.13.0 · gen_component v1.6.1 · Maker v2.62 · **book_gates v1.72.9** · lesson_inventory v1.3.5 · pill_sweep v1.1 · gate_payload_match v1.9.0 · build_family_map v1.6.6.1 · callout_id v1.0 · keyterm_prefix v1.0.1 · build_mark_index v1.1.0 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.30.0 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.2 · build_css v1.4.0 · image_audit v1.3 · strip_inline v1.2 · build_worklist v1.2 · regex_audit v1.0 · byte_audit v1.9.1 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · entity_sweep v1.0 · font_stack_sweep v1.3.0 · next_pointer v1.2 · family_tag v1.2.1 · glossary_convert v1.0 · mark_wire v1.0.2 · glyph_scan v1.1 · title_feed v1.0 · quiz_bank v1.6.1 · Timer v1.3.2 · harness_setup v1.1 · pio_harness v3.1 · `ZUMO_Syllabus_WORKING.md` v1.3 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `7bf5bef`.**
