...
**Status:** ✅ **S49 COMPLETE — ALL PUSHED & LIVE (commits bb10c28 + 05247d4 + a8e2130)** · L01 v03.2.7 · L02 v02.2.4 · L03 v03.4.7 · L04 **v04.0.12** · L05 v04.1.9 · L06 v04.5.9 · L07 **v04.3.10** · L08 v04.1.7 · L09 v05.0.9 · L10 v02.1.12 · L11 v02.2.2 · L12 v01.2.3 · L13 v02.2.2 · L14 v02.4.2 · L15 v02.2.3 · L16 v02.2.2 · Bible **v8.33.1** · Maker **v2.36** · Gate v1.3 · Harness v3.0 · 🖼️ **All images now served from Pages (no more raw rate-limit blanks).** L04 jumper photos live. · ✅ **Verified by fresh clone (S50 open, Jul 18): latest commit 0fbf194, L03 v03.4.7, L04 v04.0.12, Bible v8.33, Maker v2.32, `L04_LEARNMODE_LOG.md` present.**
**Currently working on:** 🔧 **S51:** Fixed the S50-found L04 template gap — the ≥L4 blank starter (`mainCpp()`) was missing `// ===== GLOBAL VARIABLES =====`, the exact download L04 "Main build" + "Discovery 4.1 (Step 2)" use (both payloadRef `null`). Added it between FUNCTION PROTOTYPES and `setup()`; **book-wide** (all L4–16 Main builds), gate PASS. **Maker v2.32→v2.33.** Root-caused to Bible §18.3: its section-list (line 859) said "all five" but named four in L03 vocab that omitted `GLOBAL VARIABLES` — rewrote it lesson-agnostic + backfilled the missing v8.33 changelog entry. **Bible v8.33→v8.33.1.** (The learner-mode L04 build itself is unchanged — DJ paused mid-discoveries; no robot/line surface at home.) **+ L05 challenge fix (Maker v2.33→v2.34):** C1/C4/C5 (`detection_counter`/`social_distancing`/`obstacle_avoidance`) had payloadRef `null` → downloaded a blank scaffold instead of the finished L05 program; set to `finished` to match C2/C3 + S49 canon (searched repo + project docs, no prior decision to leave them blank). **Light step-1 audit (L02/L03/L05/L06/L07):** no other L04-class *missing-section* bug — L02/L03 first-step starters cover their reveals; L05's `GLOBAL VARIABLES` is now provided by the scaffold fix. Two lower-priority finds worked: (a) **FIXED (Maker v2.34→v2.35):** L05/L06 use `CONFIGURATION` (+ L05 puts `GLOBAL VARIABLES` before `FUNCTION PROTOTYPES`; L06 has no globals) but the blank ≥4 starter hardcoded `CONSTANTS` + globals for every lesson — made `mainCpp` lesson-aware so each single-file ≥4 lesson's blank "Main build" / first-step download matches its own section set/order/vocab (L04 + multi-file fallback unchanged; **verified generated scaffold headers == each lesson's payload headers** for L04/L05/L06). (b) **FIXED (Maker v2.35→v2.36, DJ ruling S51):** L07 catch-up had no `step_1` and `step_2`=`null` → blank scaffold. DJ confirmed L07 Step 1 starts from the **Lesson 6 finished build**, so `PAYLOADS["7"]["after_step_1"]` was set to a **byte-exact copy of `PAYLOADS["6"]["finished"]`** — extracted from the registry per §11, NOT reconstructed (the L07-only `RobotConfig.h` consts like `TURN_SPEED`/`BATTERY_GOOD`/`BATTERY_LOW` are introduced in Step 2's refactor, so they are not part of the Step-1 base). Added a "Step 1 — No Lesson 6 project? Start here (Lesson 6 finished build)" CATCH-UP kind → `after_step_1` and repointed `step_2` → `after_step_1`, mirroring the L08–L16 pattern. **Verified:** gate PASS (L07 now 9 keys), `after_step_1` byte-identical to L06 `finished`, refs step_1/step_2→`after_step_1`, step_3→`after_step_2`. **+ L07 lesson note (v04.3.9→v04.3.10, DJ ruling S51 = option b1):** Step 8 now acknowledges L06's `displayEncoderCounts()` diagnostic isn't carried into the organized build — the ONLY L06 function silently dropped in the L07 reorg (verified by a lesson-prose-vs-payload trace: born L06 Step 3, lived through L06 `finished`, dropped at L07 Step 2 under the build-up model; all other L06 functions + all 9 constants carry over; the loop rewrite was already covered by Step 8). Prose-only, gate PASS. · ✅ **S49 CLOSE.** Book-wide image serving fixed + L04 photos in + L03 card pattern conformed + C07/C08 payloads fixed + L03 prose (v03.4.7) + L04 API-crutch cards (v04.0.12) + Bible v8.33. **IMAGE QUEUE (DJ still to shoot):** L04 4.1 underside (temp stand-in live) · L04 4.3 test surface · the rest of the 22-photo IMAGE_SHOT_LIST. **NEXT queued:** finish L04 learner-mode (C02–C05, after DJ does the discoveries) OR pending L03_C05 Variable Speed · C06 reorder to #1 · whole-template starters L08/L09/L10 · Maker batch (bulk DL · `?lesson=N` gate · C## labels) · L01 VS Code multi-root step. **L04 LEARNER-MODE FINDING (S49, C01 walked):** logic is solid; the wall is Zumo/OLED API recall — put a small 'functions you'll need' crutch at point-of-use in each challenge card. See `L04_LEARNMODE_LOG.md`. **BENCH (need robot):** Q017 L09 six numbers · Q044 calibration-spin · Q046 gyro-bias · L02 §5 green-LED · Constrain RUN_MS. **PARKED:** solution-disclosure · monetization/ebook · "Know Your Zumo" · AI Tutor (LAST) · day-by-day grid + syllabus · TDP template v3 (A5 Lab Log) · §9 difficulty grouping · L06 card pattern.
