...
**Date:** July 29, 2026 (Session 91 — **THE CALLOUT TITLE IS NOW ONE FORM BOOK-WIDE, AND THE STANDARD'S OWN CLAIM ABOUT IT WAS FALSE.** DJ: *"Why would i want a div bold?"* — the answer is §5.1's three properties, which a bare `<strong>` carries none of: the 8px gap, 1.05em, and block display so the body needs no `<br>`. Counting found the live book had it backwards — **794 titles as `<strong>` against 55 in §5.1's form** — while §5.1 asserted *"Geometry is unchanged from prior practice."* That sentence was approved the same morning without being checked against the book. Swept on DJ's ruling: **794 titles converted, 119 redundant `<br>` removed**, structure only, every colour and attribute carried over. **BookComponentStandard v01.6.0 → v01.7.0** records the rule, the correction, and the cost — `<strong>` is semantic and a bold div is not, so 794 titles lost their screen-reader emphasis cue; the title is still the block's first text, so nothing became unreachable. **THE SWEEP'S VERIFICATION TOOK THREE ASSERTS, EACH EARNED BY A CONTROL THAT DEFEATED THE LAST.** (1) Rendered-text-identical — defeated by a transform that ate a following `<p>`, invisible because `flat()` strips tags either way. (2) Tag-multiset-by-exact-delta — defeated by a transform that dropped every non-style attribute, which **shipped into the tree and was caught by the CONSTRUCT CENSUS at 171→169**, tags balancing perfectly while `data-challenge` markers vanished. (3) data-* attribute multiset. All three now control-run. **THE SWEEP THEN EXPOSED A LATENT GATE DEFECT AND THE BOOK WAS INNOCENT:** §25.2 bounded the Brain Check with `rfind('<div')` and worked only because the title happened to be a `<strong>`; once titles became divs it landed on the title, read **0 items in all nine lessons**, and the lessons were untouched — 5 quiz reveals present throughout. Same defect the Bible already recorded for §20.1(5) at S83, one gate over. Re-bounded on the `id="brain-check-01"` anchor §25.10h canonizes; **old bounding produces 9 false failures on the tree the fixed gate passes.** **book_gates v1.22 → v1.24, gate 34** holds the title form, control-run three ways — and against the pre-sweep tree it fails with **exactly 794**, matching the sweep's own edit count. **A control of mine misfired and is recorded as such:** the first old-bounding run reported zero failures because executing from `/tmp` put `/tmp` on `sys.path` and `lesson_inventory` never imported — §24.6b, verify the control landed before trusting it. Census byte-identical: 39,792 lines · 1,025 headings · 174 · 174 · 64 · 171 · 56 · 403. **ALL 16 LESSONS BUMPED MODERATE** — 1.05em and the 8px gap change rendering. **THEN A TRIPLE-CHECK FOUND THAT DJ'S SAFETY RULING HAD NEVER REACHED THE STANDARD.** The retirement was applied to six lessons, written into LIVE.md and into the Bible changelog, and `BookComponentStandard` was left saying **26 families on eight roles with SAFETY on red** — so the standard and LIVE.md contradicted each other, the exact drift class this session spent its length on. **`--selftest` passed 22/22 throughout**, because the standard was internally consistent with itself and the generator cannot see that a ruling never arrived (§24.8). Completed at **v01.7.0 → v01.8.0**: SAFETY row out of §7, red row out of §5, §5.0 now reads *seven roles, three semantic*, §7 header **25 families / 48 marks**, §7.3 arithmetic **26 + 22 = 48**, §8's warning-triangle collision down to 2, and `shield-exclamation` **deleted from BOTH `icons/` and `marks/`** because §6/§7.3 require the folder to be assertable against the table in both directions — an icon with no family is not a spare, it is an unasserted file. **`gen_component` v1.4 → v1.5, and retiring one role broke it twice on hardcoded counts:** the §5 palette boundary was anchored on the literal string `'Eight roles.'` and a `len(palette) == 8` assert sat beside it. **A count has no business inside a parsing anchor.** Both now derive from the document — the anchor by regex, the count via the existing `word_to_int` so §5's own prose cross-checks its own table — and both control-run: prose saying eight against a seven-row table FAILS, prose saying twenty-six against twenty-five FAILS. This is S90's hardcoded-count lesson recurring inside the tool written to avoid it. `images/icons/` 49→48 + LICENSE, `images/marks/` 41→40. **THEN THE 794-BLOCK SWEEP TURNED OUT TO HAVE COVERED ONE SHAPE OF THREE.** Gate 34 rejected only a bare `<strong>`, so **120 `<span>`-led and 44 `<b>` titles walked straight through the gate shipped two hours earlier** — one construct in three shapes, the §6.8a/§6.6a disease again. Second sweep: **133 converted (111 span-shape + 22 title-shape)**. **31 `<b>` blocks were deliberately NOT converted**: a `<b>` not followed by `<br>` or a block element is a sentence SUBJECT, and a block title would split the sentence — *"The #1 project killer"* as a heading with a body opening *"is not a bug."* That discriminator is a judgement encoded as a rule, and **no assert verifies it** — recorded as such. **Two bugs caught by the text assert, neither visible to tag counting:** callouts sharing a start line emitted the same edit twice (the `Bootloader` duplication), and the sentence-subject cases. **All 52 `term-` ids preserved verbatim — 7 are live `href="#term-…"` targets — asserted by an id multiset. The inline em-dash between term and definition was retired with the shape, 111 of them, the same class as the 119 `<br>`. **book_gates v1.24 → v1.25**, gate 34 now holds all three shapes; against the pre-sweep tree it reports **142 where the sweep converted 133** — both correct: **L16 carries exactly 9 callouts sharing a start offset**, and since offsets are derived from LINE numbers the gate counts those blocks twice. Logged: a line-derived offset cannot separate two callouts on one line. Titles now **991 canonical block · 0 inline · 22 sentence-lead · 35 no title**. All 16 lessons bumped MODERATE again.)
**Status:** **THE AMBER SCHEME WAS DOING SIX JOBS, AND §6.6a HAD BEEN LOCKED AND UNSWEPT FOR THIRTY SESSIONS.** DJ retired the **SAFETY family** (*"get rid of safety and make them all warning"*), which **empties the red role — the semantic palette is a semantic THREE**, green/amber/purple. All seven callouts carrying SAFETY now read WARNING; L04's jumper promise moved amber → NOTE. Then DJ ruled labels **bare** (*"get rid of the second part of the title because it makes us have more categories"*), which is not a new preference but §6.6a compliance, LOCKED since S61. **69 labels still violate it — 28 NOTE, 27 TIP, 14 WARNING** — and they split three ways: **8 carry only a stray colon** (drop it, nothing moves), **16 hold a sentence in the label** (moves to the body verbatim), and **45 hold a TITLE** whose removal dangles a fragment and needs a read per block. **Two live glyph inversions found en route** — L03 1180 is NOTE wearing TIP's lightbulb, L03 1285 is TIP wearing NOTE's book — the exact inversion §6.6a was written to fix at S61. **STILL HOLDING THE REPAINT: the semantic palette**, now three colours. **Nothing of Heritage Blue renders on the site yet** — zero of its values appear in any of the 18 pages and the 41 marks are referenced nowhere.

**Versions:** L01 **v03.13.0** · L02 **v03.4.0** · L03 **v03.18.0** · L04 **v04.12.0** · L05 **v04.13.0** · L06 **v04.17.0** · L07 **v04.13.0** · L08 **v04.12.0** · L09 **v05.10.0** · L10 **v02.9.0** · L11 **v02.11.0** · L12 **v01.12.0** · L13 **v02.11.0** · L14 **v02.13.0** · L15 **v02.10.0** · L16 **v02.7.0** · going_deeper **v01.1.1** — **ALL 16 lessons changed in S91** · Bible **v8.78** · **BookComponentStandard v01.8.0** · **gen_component v1.5 (22 checks, ALL PASS)** · Maker **v2.45.1** · book_gates **v1.25 (34 gates, 34/34 PASS)** · pill_sweep v1.0 · lesson_inventory **v1.1.0** · gen_bonus_banner **v1.2** · gen_part_banners v1.0 · Gate v1.6 · Harness v3.0 · `images/icons/` **48 marks + LICENSE (generator input)** · `images/marks/` **40 generated marks, all regenerated S91** · **web tools: timer v1.3.0 · Maker v2.45.1 · tutor v1.1.0 · index v1.3.0**.
