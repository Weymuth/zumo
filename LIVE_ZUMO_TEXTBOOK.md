...
**Date:** August 3, 2026 (Session 114 — **A RULE CANONIZED WITHOUT ITS GATE HOLDS ONLY WHERE SOMEONE HAPPENS TO LOOK, AND FOR TWO SESSIONS NOBODY LOOKED.** §12.3 step 4 has said *write `ZUMO_S<N+1>_HANDOFF.md`* since v8.24 and §12.2's `git rm ZUMO_S<N-1>_HANDOFF.md` line agrees with it, so the handoff numbering was never in doubt — and it was violated in two consecutive sessions anyway. **S112 wrote its OUTGOING handoff into the INCOMING handoff's filename**, editing `ZUMO_S112_HANDOFF.md` in place across three commits with its STATE block updated each time (`893b8b6` → `4558257` → `8ae3857`) while its title stayed *"paste at top of Session 112"*, so **the correctly-named incoming handoff never existed** and the text pasted at S114 open — state `b4d5559` — matches no committed version of any file. **S114 then read the convention off that one defective example rather than off the Bible** and pushed S114's content as the correctly-named incoming handoff. **DJ caught both by reading the numbers.** Gate 28 could not: it asserted only that exactly ONE handoff exists, which a file renamed `ZUMO_S999_HANDOFF.md` satisfies — measured, and it PASSED. **§24.8 exactly: if the answer were the opposite, the old gate looked identical.** Prior, S112 — going_deeper on the book's settings, §27.14 every link and id resolves, and the family map closed at 1048/1048.)
**Status:** ✅ **47/47 GATES PASS · THE HANDOFF NUMBER IS GATED, AND FOUR FIGURE DEBTS CLEARED WITHOUT A CAMERA.** Outstanding images **18 → 14**, planned **144 → 140** [`image_audit.py`]. **`book_gates` v1.42.1 → v1.43.1 — gate 28 now checks the NUMBER**, parsing it out of the filename, out of the `# ZUMO — SNN HANDOFF` title and out of the *"paste at top of Session N"* clause, failing when any two disagree and failing **loudly rather than skipping** when the title shape is gone. **Control-run four ways, and two of the four are the real historical defects rather than synthetic ones.** Bible **v8.101 → v8.102** records it at §12.2. **FOUR FIGURES RETIRED, EACH BECAUSE ITS CONTENT WAS ALREADY LIVE OR COULD BE WRITTEN.** `[IMAGE 3.4]` became a real `👀 WHAT YOU SHOULD SEE` callout carrying a §22 terminal block — L03 Step 12 told the student *"Look for SUCCESS in the terminal output"* and then never showed it, and L01's build-result block is the live precedent, copied in shape **with no numbers invented**. `IMAGE 3.14`'s brackets dropped, because the BRACKETED tag is what marks a figure PLANNED and that row was already ❌ removed. **`[IMAGE 7.13]` retired: it asked for a *"diagram showing the final project structure"* while GRAPHIC 7.15 (the project tree) and GRAPHIC 7.16 (the eight-file architecture) are both already live in that lesson** — and its row was typed *Photo / screenshot* for a thing its own description calls a diagram. **`[IMAGE 14.2]` retired: the twelve-item Competition Day Kit list sits directly above it and names every item a photo would show.** The S111 `IMAGE 2.5` retirement is the precedent for all of them. **`[IMAGE 3.6]` REFUSED and staying refused** — a Serial Monitor TRIM log needs real numbers from a real run, and a fabricated log is the same defect as a fabricated screenshot. Also S114: **the family map's denominator is a FROZEN BASELINE, not a count** — adding one callout made it print `assigned 1049 / 1048`, and the literal moved to 1049 only after a control proved the delta was that one block and the other **29 family counts were byte-identical**. **§27.11's digest moved TWICE, and both times the safe way: 646 rules and 2,367 declarations never moved**, only rule ORDER, because `build_css` sorts by usage count — diffed in full each time before the baseline was touched and control-run after, with a deleted `color: white;` still FAILING. **`GPT_BRIEF_FORMAT.md` → Bible §24.15 and the file is deleted.** **`IMAGE 3.14` RULED at close** &mdash; DJ: *"3-18 replaced 3-14."* The replacement claim in that row is true, so it takes the S111 retirement form. **AND THEN DJ SUPPLIED THE PHOTO, WHICH REVERSED IT AGAIN.** The row's ORIGINAL description was correct all along &mdash; *"Top view of the A-Star 32U4 board"* &mdash; and `L03_IMAGE_3-14_astar_board.jpg` is now WIRED IN at &sect;4.2, right after the sentence that introduces the board, giving a board &rarr; motor &rarr; gearbox ladder. **The PARENTHETICAL was the stray, not the subject:** `&#129504; LEARN: Inside the little silver can` is real, at L03:1142, and GRAPHIC 3.18 sits INSIDE it at 1150 next to IMAGE 3.1 &mdash; so 3.18 replaced a MOTOR figure in that block, and that clause had been appended to the board photo's row. One row carrying two figures' histories. **Three readings were offered before the right one and nothing had to be un-written, because none of them put a guess into canon:** the disputed subject was DELETED rather than rewritten, and the one guess that did get written was overwritten by an artefact a message later. **A wrong entry costs three times a blank one, and a blank one is what a guess should leave behind.** Applied S114: **L03 v03.28.0** · **L07 v04.22.0** · **L14 v02.22.0**; `book_gates` reaches **v1.43.2** after FOUR baselines moved this session &mdash; the family map's 1048&rarr;1049, &sect;21's image-reference coverage 224&rarr;225, and the &sect;27.11 digest twice &mdash; every one of them diffed in full first and control-run after, because a baseline moved without its diff is a spent gate. **§24.14 IS WRITTEN AT LAST (v8.103).** **The rule was never missing — only its home was.** Gate 47 enforced it, `book_gates.py` carried the reasoning, and `ZUMO_FAMILY_MAP.md` recorded the S112 derivation in full. What did not exist was the numbered section: §24 ran 24.13 straight to 24.15, and v8.101 logged that as a debt rather than closing it — the §24.2 failure, a rule enforced everywhere and homed nowhere. The section states the ORDER (**family from CONTENT; mark and colour are OUTPUTS**), records that **252 of 1,048 blocks were hex-resolved until S112** and that a correct STILL GREEN ruling was **overridden by paint**, and says plainly that the printed denominator is a **frozen baseline, not a count** — so `assigned 1049 / 1048` reads like a count and is not one, and moving that literal is legal only after a control run proves the per-family delta. Transcription of an enforced rule: no ruling changed, no lesson changed, census unchanged at **40,015**.

**Versions:** L01 v03.19.1 · L02 v03.11.1 · L03 v03.28.0 · L04 v04.19.1 · L05 v04.19.1 · L06 v04.23.1 · L07 v04.22.0 · L08 v04.19.1 · L09 v05.16.1 · L10 v02.16.1 · L11 v02.17.1 · L12 v01.19.1 · L13 v02.17.1 · L14 v02.22.0 · L15 v02.17.1 · L16 v02.12.1 · going_deeper v01.4.1 — census **40,015** · Bible **v8.103** · BookComponentStandard v01.12.1 · gen_component v1.6.1 · Maker v2.45.2 · **book_gates v1.43.2** · lesson_inventory v1.2.0 · pill_sweep v1.1 · gate_payload_match v1.7 · build_family_map v1.3.2 · build_mark_index v1.0.2 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.17.0 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.2.1 · image_audit v1.1 · strip_inline v1.1 · build_worklist v1.1 · regex_audit v1.0 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · font_stack_sweep v1.2.0 · `ZUMO_Syllabus_WORKING.md` v1.2 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `0bc66da`.**
