...
**Date:** August 9, 2026 (Session 139 — **AN ENTRY FILED IN ONE HOME IS NOT FILED, AND THIS TIME THE READER WAS RIGHT AND THE BOOK WAS WRONG.** S138 bumped the Bible version home to v8.130.2 and wrote its entry into line 17's `Current:` chain, but never inserted the line-start changelog entry. The diff proves the convention — S136 and S137 each show `@@ -17 +17 @@` *and* `@@ -97,0 +98,2 @@`; S138 shows only the first. `bible_consistency()` reads the line-start list, so it believed S137 had just run and reported LIVE.md and the handoff as wrong when **both were right**. Note the two homes have never been byte-identical — S137's filed entry runs 2,607 characters against 2,816 in the chain, differently worded and differently tailed — so filing is authoring, not copying. **AND THE CLOSE PUSH IS WHERE THINGS BREAK, BECAUSE NOTHING RUNS AFTER IT**: `session_versions --check` was exit 0 at `442f68d` and exit 1 at `2f33655`, the second session running that a defect landed inside a close push and survived into the next opener. **THE HANDOFF READ IS NOW SCOPED.** `check()` fed the WHOLE handoff to `_versions_in`, which builds its dict with `re.findall` — so the LAST match won, and a prose aside eighty lines below the STATE block overwrote the block's own claim. LIVE.md never had this defect because its read is already scoped to line 6; the handoff was the outlier and now takes the same discipline, a window anchored on the VERSION BLOCK marker — **which is now actually emitted, having claimed since S138 that it was.** **AND A SPEC'S OWN NUMBERS CAN BE ARITHMETICALLY IMPOSSIBLE**: QUIZ_SPEC asked for ~70% multiple choice under caps whose floor was 76%, so L02 met it only by running true/false at 22% — over its own cap, unnoticed, because nothing validates a mix.)
**Status:** ✅ **69/69 GATES · L04 FIXED AND ITS QUIZ BANK WRITTEN · BANKS 1 → 2 OF 12.** **THE ONE TO CARRY: an entry filed in one home is not filed — and when a reader disagrees with a document, read the disagreement, because the reader may be right.** S138's missing Bible changelog entry is filed. **`session_versions` v1.24.1** scopes the handoff read to a marker-anchored window, emits the marker, refuses a whole-file fallback, and reports rather than skips a key absent from the window; **CONTROL H runs three directions plus a blinding pass — ALL EIGHT CONTROLS PASS.** **L04: 11 of 13 read findings applied.** Challenge 4's template now compiles (three blanks, because §8A.2 says choosing thresholds from real data IS the skill). §8A.8's *hundreds per crossing* now reads *every loop, not every crossing* — true at any tape speed, where the section's derived *five* assumes one example dwell. **THRESHOLD CANON: 600 BOOK-WIDE IN L04 WITH THE MARGIN STATED** — the census split six teaching 500s against six practice 600s, so a student applying the rule just taught wrote 500 and met 600 in every reveal; **eight sites were swapped and TWO were REWRITTEN**, because substituting 600 into *"500 is the natural darker-than-halfway line"* manufactures a false sentence. **THE *SEVEN-SECTION* FINDING WAS REVERSED BY MEASUREMENT** — L02 §3.1's Color Key lists seven numbered sections, and the banner census that contradicted it counted `// ===== =====` banners, which Header and Include never carry; the instrument could not return the answer the claim asserted. `waitForStart()` and `checkBattery()` moved into a new **Startup & safety** Quick Reference group. **QUIZ_SPEC v1.1.0** — the matching allowance moved to ~10% rather than the true/false cap, **because true/false is 50% guessable and buying a ratio with it lowers the signal the gate exists to produce**; L02 is deliberately NOT rebalanced and the spec says so. **L04 BANK v1.1.0, 73 QUESTIONS**, authored after the read and the fixes — five would have been keyed wrong against v04.28.3. **L03 IS NEXT AND IS UNBLOCKED.** Bible **v8.131** · `book_gates` **v1.64.3** · Maker **v2.45.5**.

**Versions:** L01 v03.28.2 · L02 v03.21.2 · L03 v03.40.2 · L04 v04.29.0 · L05 v04.28.1 · L06 v04.31.1 · L07 v04.31.1 · L08 v04.30.1 · L09 v05.26.1 · L10 v02.27.1 · L11 v02.28.2 · L12 v01.31.2 · L13 v02.28.2 · L14 v02.33.1 · L15 v02.30.1 · L16 v02.22.1 · going_deeper v01.6.1 — census **40,479** · Bible **v8.131** · BookComponentStandard v01.13.0 · gen_component v1.6.1 · Maker v2.45.5 · **book_gates v1.64.3** · lesson_inventory v1.3.5 · pill_sweep v1.1 · gate_payload_match v1.7.1 · build_family_map v1.6.0 · callout_id v1.0 · keyterm_prefix v1.0.1 · build_mark_index v1.1.0 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.24.1 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.3.0 · image_audit v1.2 · strip_inline v1.2 · build_worklist v1.1 · regex_audit v1.0 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · entity_sweep v1.0 · font_stack_sweep v1.3.0 · next_pointer v1.2 · family_tag v1.2.1 · glossary_convert v1.0 · mark_wire v1.0.2 · glyph_scan v1.1 · title_feed v1.0 · Timer v1.3.2 · `ZUMO_Syllabus_WORKING.md` v1.2 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `2f33655`.**
