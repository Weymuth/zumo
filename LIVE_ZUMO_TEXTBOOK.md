...
**Date:** July 25, 2026 (Session 71 — **BRAIN CHECK**: the four §25 exit constructs get one family name, one Type 10 indigo livery, a fixed nav column with per-browser check-off, and a two-state icon pair; L01 nesting/white-text defect fixed).
**Status:** **THE FOUR BLOCKS BECAME A FAMILY.** S70 gave the exit region four constructs; S71 gave them ONE name — **Brain Check 01–04** (01 Mental · 02 Technical Skills · 03 Knowledge Check · 04 Reflection) — and one livery: **§8 Type 10 Knowledge** (bg `#e8eaf6` / border `#3f51b5` / title `#283593`, indigo picked by ΔE audit against every locked color, min ΔE 32.3). Before this pass the four blocks had NO identity of their own: Mental and Knowledge Check wore §9 Challenges plum and Technical Skills + Reflection borrowed Checkpoint green — the color said the opposite of §25.7 (§9 is the hands, §10 is the head). Found and fixed en route: **the L01 Mental block had been nested INSIDE the §6 banner div since it was built** — the banner rendered below the block, and the banner's `color: white` made the five reveal ANSWERS white-on-white (questions visible, answers invisible); three gates were structurally blind to it (tag balance passed BECAUSE opens/closes balanced, the skeleton hash stops at the hero, §25.2 only counted items). The block is now standalone, the banner rebuilt canonical. **New: a fixed right-edge Brain Check column** (emblem + pills 01–04 → anchors), shipping ONLY in converted lessons; per-block "Mark done" buttons persist to `localStorage` (`bc_LNN`), painting pill/icon/button green and flipping the emblem at 4/4 — a personal tracker, per-browser, NOT a grade (the Canvas quiz stays the real gate). **Icon pair** (DJ-authored, flood-fill cut per §21): gray `#454545` incomplete / green `#24911b` + check complete; gray NOT red because §22 owns red for ERROR; state never color-alone (check glyph carries it — colorblind-safe); dark backings forbidden; the gray was rebuilt single-color-plus-alpha so both icons' interiors are transparent (opaque light-interior px = 0 in both, matching). **book_gates v1.5→v1.6**: §25.2 gate extended (anchors 01–04, Type 10 wrapper, column presence), control-run FOUR ways with landed-injection asserts per the S70 addendum. **All 21 gates PASS.**

**Versions:** L01 **v03.9.0** · L02 **v02.15.2** · L03 **v03.13.2** · L04 **v04.6.2** · L05 **v04.8.2** · L06 **v04.11.2** · L07 **v04.7.2** · L08 **v04.6.2** · L09 **v05.4.2** · L10 **v02.5.2** · L11 **v02.7.2** · L12 **v01.7.2** · L13 **v02.6.2** · L14 **v02.8.2** · L15 **v02.6.2** · L16 **v02.5.2** · Bible **v8.55** · Maker **v2.45.1** · Gate v1.6 · Harness v3.0 · pill_sweep v1.0 · book_gates **v1.6 (21 gates)** · going_deeper **v01.1.0** · **web tools (version-lined in-file): timer v1.3.0 · Maker v2.45.1 · tutor v1.0.0 · index v1.3.0**.
