...
**Date:** September 2, 2026 (Session 200 — **THE SESSION THAT BUILT THE INSTRUMENT THAT REFUSES AN OUT-OF-SCOPE QUIZ QUESTION, AND FOUND THE RETIRED SELECTION STILL LIVE IN A SECOND COPY.**)
**Status (S200 close, Sep 2):** S199's thirteen-entry batch is PUSHED and verified at `a1b267b` — the handoff's "NOTHING IS PUSHED" was true when written and false at open, for the third consecutive session. Talking points written for periods 5–8. `quizzes/reading_quiz.py` **v1.0** builds a reading quiz from an explicit bank-id selection and **refuses an id citing anything outside §1–§5**; L03 and L04 quizzes and QTI packages built with it, and the rule is canon at **Bible §25.3a (v8.198)**. A second copy of L01's keyed quiz still carried the RETIRED S194 selection and was deleted. `session_versions` **v1.36.0** registers the talking-points and reading-quiz families. All 82 gates pass, 16 banks valid, `GPT_WORKLIST.md` regenerated, site_parity twice.

**Versions:** L01 v03.32.2 · L02 v03.26.2 · L03 v03.47.4 · L04 v04.29.7 · L05 v04.30.2 · L06 v04.37.3 · L07 v04.33.1 · L08 v04.34.4 · L09 v05.28.0 · L10 v02.30.7 · L11 v02.31.4 · L12 v01.35.4 · L13 v02.39.0 · L14 v02.36.3 · L15 v02.32.2 · L16 v02.28.1 · going_deeper v01.6.1 — census **41,848** · Bible **v8.198** · BookComponentStandard v01.13.0 · gen_component v1.6.1 · Maker v2.72 · **book_gates v1.76.7** · lesson_inventory v1.4.1 · pill_sweep v1.1 · gate_payload_match v1.9.6 · build_family_map v1.6.6.8 · callout_id v1.0 · keyterm_prefix v1.0.1 · build_mark_index v1.1.0 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.36.0 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.23 · site_parity v1.2.1 · build_css v1.4.0 · build_syllabus_html 1.1 · image_audit v1.3 · strip_inline v1.2 · build_worklist v1.2 · qti_export 1.2 · reading_quiz v1.0 · prose_canon v1.4.0 · retired_claims v1.3.1 · census v1.3.0 · regex_audit v1.0 · byte_audit v1.9.1 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · entity_sweep v1.0 · font_stack_sweep v1.3.0 · next_pointer v1.2 · family_tag v1.2.1 · glossary_convert v1.0 · mark_wire v1.0.2 · glyph_scan v1.1 · title_feed v1.0 · quiz_bank v1.6.1 · Timer v1.3.2 · harness_setup v1.1 · pio_harness v3.1 · `ZUMO_Syllabus_WORKING.md` v1.5 · `ZUMO_Teacher_Daily_Grid_F26.md` v2.1 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `a1b267b`.**
...
## WHAT SHIPPED IN S200

**Worklist tally, derived by `census.worklist()` and unmoved this session: 103 closed / 96 fixed / 2 parked / 140 open of 245.** S200 touched no worklist row, no lesson, no payload and no byte of the book.

**THE S199 BATCH IS LIVE.** Thirteen entries pushed as `a1b267b` — both deletions ticked, the grid rename scored, `newproject.html` and `lessons/` correctly absent. **That is the third consecutive session whose handoff opened "NOTHING IS PUSHED" and was wrong by the time it was read.** The artefact is the answer (S197); the diff was read, not the sentence. The pattern is now stable enough to state plainly: **a handoff's push claim is written before the push and is therefore never evidence of anything.**

**THE HANDOFF ALSO PLACED S200 AFTER FRIDAY AND IT IS BEFORE IT.** S200 opened Tuesday September 1; the first class is Friday September 4. Every pre-Friday item the handoff had written up as retrospective was still live. DJ confirmed the Canvas import is done and a pre-quiz added, which is what actually closed it.

**THE THIRD HAND-BUILD IS THE ONE THAT BECOMES AN INSTRUMENT.** S194 built L01's Canvas package by hand, S199 rebuilt it by hand, and S200 was about to build L03's and L04's the same way. `quizzes/reading_quiz.py` **v1.0** takes an explicit list of bank ids and emits both the keyed fallback page and the QTI zip. **The §1–§5 scope rule is a REFUSAL, not a sentence**: a selection naming a question whose cite reaches into §6 Build, §7 Verify, §8 Diagnose, §8A Going Deeper or §9 Challenges does not build. **It caught its author on its first run** — the drafted L03 selection named `L03_B12`, which cites `§3.3, Glossary`, and the mixed-cite control refused it. `L03_B13` replaced it.

**THE DEFECT S199 FIXED IN L01 WAS MEASURED, UNSHIPPED, IN THE L03 AND L04 POOLS.** Of L03's 61 `before` questions **45** cite §1–§5 and nothing else; of L04's 51, **29** do. The remainder — 11 and 21 — reach outside the assigned reading. Nothing was broken, because no L03 or L04 quiz had been selected yet: **this is the window in which a rule that lives only in a sentence gets forgotten**, and it is now closed by refusal rather than by memory. The instrument independently reproduces S199's own figure for L01: **40 in-scope of 59**, which the rescoped page states in prose.

**A RETIRED SELECTION WAS STILL LIVE IN A SECOND COPY OF THE SAME FILE.** `quizzes/ZUMO_L01_Reading_Quiz.md` and `ZUMO_L01_Reading_Quiz.md` are not the same file. S199 rescoped the root one. The `quizzes/` one still named `L01_B43` (§6 Step 1) and `L01_B48` (§7.2) as the quiz, still justified them in the present tense — *"That is deliberate: Assignment 1 ends with a program running on the robot"* — and still instructed the reader to **keep at least one question from §6 or §7**, which is the retired ruling giving an order. It also claimed the QTI package was deliberately not committed, while the committed package sits in the repo root. **A claim fixed where it was quoted is not fixed where it was repeated (rule 72), and no instrument in the tree could see this**: `retired_claims` scopes to 122 pages and 16 banks, and a loose markdown file is neither. Deleted; the root copy is the one home.

**AND THE LIVE COPY HAD NO VERSION HOME AT ALL.** The `v1.0` header comment lived only in the stale copy, so S199's rescope of the file five students are graded against was asserted by nothing. The root page is now **v1.1** and `session_versions` **v1.36.0** registers the whole `ZUMO_L##_Reading_Quiz.md` family — one predicate, not one entry per lesson, **because a per-file entry is a registration that has to be remembered every time the family grows, and the thing that gets forgotten is the registration.** The same reasoning registers `ZUMO_TALKING_POINTS_F26_*`, which S199 left unasserted when it registered the grid. **A registration cannot assert on the commit that introduces it** — currency compares the working tree against HEAD and HEAD has no home to compare — so both were controlled against the tracked sibling instead: an unbumped edit is LOUD, a bumped edit is SILENT.

**IDENTS ARE DERIVED NOW, WHICH IS WHY L01 CANNOT BE REBUILT.** S194 generated L01's QTI item idents from a random source. That package is already imported into Canvas, and a rebuild would produce identical questions under different idents — a second quiz, not the same one. L01 is therefore registered for `--check` coverage only. L03's and L04's idents are a hash of (lesson, bank id) and the zip timestamps are pinned, so **a rebuild is byte-identical**, which is a control rather than a claim.

**NEW:** `ZUMO_TALKING_POINTS_F26_Wk3-4.md` **v1.0** (periods 5–8, every time block contiguous and summing to 65 minutes, verified by parser) · `quizzes/reading_quiz.py` **v1.0** (eleven controls, both directions) · `ZUMO_L03_Reading_Quiz.md` and `ZUMO_L04_Reading_Quiz.md` **v1.0** with their QTI packages.

**THE INSTRUMENT WAS NOT ENOUGH ON ITS OWN, AND `session_versions` SAID SO.** S200 first ruled against a Bible bump — a refusal in code is stronger than a sentence, and a bump cascades into the two homes on line 17 and a `GPT_WORKLIST` regeneration. **The SESSION arm then reported that LIVE.md said Session 200 while the newest Bible entry said S199**, which is the ledger breaking: a session that shipped work and wrote no entry. The Bible's own S199 text says it plainly — *a ruling reaches the canon or it did not happen*. **Bible v8.198, §25.3a NEW**, and `GPT_WORKLIST.md` regenerated as the obligation requires (S175).

**AND §25.3 ABOVE THE NEW RULE CARRIES THREE CLAIMS THAT NO LONGER MATCH THE COURSE.** It says *the quizzes do not exist yet* — sixteen banks exist and L01 is imported; it says **CLOSED BOOK**, against a working model that is open book and notes; and it prices the quizzes at **20%**, against a per-quiz weight nearer 1%. Two of the three are grading and course-scope decisions and belong to DJ under §24.17, so they are **flagged inside §25.3a rather than rewritten**. A rule written next to a contradiction is a rule that will be read wrongly.

**SIX OF THE FOURTEEN OUTSTANDING FIGURES SIT IN THE BLOCK JUST SCRIPTED**, and four of them are read at home on **September 15**: L03 VIDEO 3.1 and IMAGE 3.2 / 3.5 / 3.6, L04 VIDEO 4.1 and IMAGE 4.3. The shoot-or-cut ruling now has a date. **NEEDS DJ.** **AND THE CARRIED FIGURE FOR L01–L08 WAS WRONG: eight, not nine.** The S200 handoff printed nine and then listed eight, and S200's own opening report repeated the nine before deriving it — a count that disagrees with its own population survives exactly as long as nobody counts the population (§24.24).


## WHAT SHIPPED IN S199

**Worklist tally, derived by `census.worklist()` and unmoved this session: 103 closed / 96 fixed / 2 parked / 140 open of 245.** S199 touched no worklist row, no lesson, no payload and no byte.

**THE S198 BATCH IS LIVE.** Twenty entries pushed as `3fcdad2` — eighteen modified, `ZUMO_S198_HANDOFF.md` deleted, `ZUMO_S199_HANDOFF.md` new, and `newproject.html` correctly absent. The handoff's "NOTHING FROM S198 IS PUSHED" was true when written and false by session open; **the artefact is the answer** (S197), and the diff was read rather than the sentence.

**THE FIRST GRADED THING FIVE STUDENTS WILL DO WAS TESTING WORK NOBODY HAD ASSIGNED.** `ZUMO_L01_Reading_Quiz.md` and its QTI package drew Q7 from **§6 Step 1** and Q8 from **§7.2** — building and uploading — while Assignment 1 read *§1–§5, no build*, and explicitly parked §6–§7 for Wednesday. **25% of the points were unanswerable by a student who did exactly what was asked.** The quiz file called this deliberate, and under S194's model it was: DJ had ruled a full flip with the first upload at home. **DJ RULED THE MODEL AGAIN AT S199 AND IT MOVED** — the reading is flipped, everything else is class work — so the two questions were swapped for `L01_B17` (§5.1, the include) and `L01_B32` (§5.5, a loop trace, which is the can't-skim question the old pair used to be). The six survivors were proved content-identical by parsing both XML packages, not by eyeballing them. **THE BANKS ALREADY ENCODED DJ'S MODEL AND NOBODY HAD READ THEM:** every bank's `before` set is described *"Pre-class reading gate. Answerable from the text alone,"* its `after` set *"Post-build check."* The design was right; two questions were filed on the wrong side of it.

**A THIRD HOME HELD THE OLD MODEL AND THE TWO FILES WERE WRITTEN IN THE SAME SESSION.** `ZUMO_Teacher_Daily_Grid_WORKING.md` row 1 assigned **§1–§7 + first upload** and row 2 spent Wednesday on challenges; `ZUMO_DAY1_Sep4.md` assigned §1–§5 and spent Wednesday building. Both S194. **Rows 4–28 already scheduled practice in class**, so the cascade was three lines, not twenty-eight — a first reading called it a whole-term restructure and that was wrong.

**L02 IS THE STRESS POINT, NOT L01.** Measured §1–§5 across the Fall lessons: L02 about **8,300** words and L03 about **8,800**, against L08's ~1,900 — and L02 held **one period on a two-day turnaround**. DJ ruled it two. **The second period is paid for by the roster, not by a lesson:** period 6 was sixty-five minutes of M1 demos sized for a full class, and five students demonstrating *drives straight and a measured distance* is about twenty. L03 slid to Sep 16/18 and every pinned milestone date held. **The cost is named rather than buried** — L03 lost its cushion before M1, and if TRIM tuning drags on Sep 18 the demo is what slips.

**THREE PRINTED WORD COUNTS DID NOT REPRODUCE, ONE BY 41%** — and **the first correction was itself 2% high, because the three strippers that agreed were not independent where it mattered.** The Day-1 script stated 14,714 / 5,993 / 5,686. Three strippers returned identical figures of 14,552 / 5,736 / 4,032, and the agreement read as proof; all three called `get_text(' ')`, which inserts a separator at EVERY tag boundary and splits `<strong>TRIM</strong>ming` into two words. **They were independent on tag-stripping and IDENTICAL on tokenization, so they agreed on the step that was wrong.** Caught by the triple check, on an arm that used a different parser rather than a different stripper. Filed with a block-aware tokenizer: **about 14,300 / 5,618 / 3,927**, stated as *about*, because **word count is method-dependent to roughly 2% and printing four significant figures asserts a precision no tokenizer has.** The original §9 error was 41% and was never method noise. **No instrument in the tree derives a word count**, which is why all of this went unguarded.

**THE YEAR LAYER IS NOW NAMED FOR ITS YEAR.** Four files renamed to `_F26`, Bible §3.1's pointer moved with them and **names the predecessor rather than erasing it**. §3.1 has kept calendar canon out of the book since S120; it named the schedule file by a name carrying no year. **The rule the rename encodes: a file carrying dates, a period count or a ROSTER SIZE is rewritten every August.** Roster size is the one that hides — with eighteen students L02's second period silently disappears.

**AND THE SEAM THIS TURNED UP WAS FIXED IN THE SCHEDULE, NOT THE LESSON.** L02 §1's four warm-ups are timed mystery programs run under *work independently, no asking for help*, closing in a debrief — they do not survive being read at home. The obvious fix was a line in the lesson saying *we do these in class*, and **that is precisely the calendar canon §3.1 forbids**; the Sep 11 flip is now §2–§5 and the book is untouched. **Zero lesson edits, zero payloads, zero bytes this session.**

**NEW:** `ZUMO_DAY1_SIGNOUT_F26.md` (five roster rows plus two spares, a return table, and the agreement line read aloud before signing) · `ZUMO_TALKING_POINTS_F26_Wk1-2.md` v1.0 (periods 1–4, every time block controlled contiguous and summing to its period length).

## WHAT SHIPPED IN S198

**Worklist tally, derived by `census.worklist()` and unmoved this session: 103 closed / 96 fixed / 2 parked / 140 open of 245.** S198 touched no worklist row. The tally was re-derived a second way at close — 245 unique row ids across the file, and its 105 ledger rows reconcile exactly to the closed-plus-parked figures stated above.

**THE SEATING DEBT IS PAID, AND IT WAS NEVER TWENTY-SIX.** Measured by a heading walk against every `§16.x` the Bible references: **thirty-three unseated when the session opened, none at close.** The delta over the handoff's figure is exactly the seven rules born since S187 — §16.52a and §16.53–§16.58 — so the queue GREW for eleven sessions while being described as static. **Third time this list has been counted low, same cause every time: the window moved and the family did not.** Widened past the Bible as well — 22 files in the tree name a §16 rule, and every rule any of them names is now seated. Bodies are durable present-tense rules; the changelog entries they derive from are untouched, because a provenance record is history (§16.37, itself seated here). **The predicate was controlled both directions before its zero was believed**, and because a seating predicate proves a heading exists and never that what sits under it reads (§24.13, S192), the bodies were audited separately for substance — shortest 106 words, no stubs. Three matter outside process: **§16.53** (`TRACK_WIDTH_MM = 98.0` as a ruled calibration default, and an envelope is not a centreline separation), **§16.29** (RCJ §5.3.6, and the reason the first answer was wrong), **§16.14** (at a cliff the line does not vanish — it arrives).

**A FOURTH TALLY HOME WAS SITTING OUTSIDE GATE 81.** The worklist's colophon stated the closed figure again, outside the ledger boundary drawn to keep GPT's verbatim Part 2 out. **Controlled: a wrong figure planted there passed every gate.** It happened to be correct, which is why it survived sixteen sessions and why it would have rotted the moment a row closed. Scope widened to the colophon, resolved from the last horizontal rule; the figure was then **deleted rather than priced** (rule 50). `book_gates` **v1.76.7**.

**EIGHT HANDOFFS CALLED THE WORKLIST'S TWO VERSION HOMES A DISAGREEMENT. THEY AGREE.** What was missing was the comparator — S197's rule again, the artefact answers the instruction. Proved before building: rolling the colophon back several minor versions while the header stayed put left **every gate green, `--check` reporting every version agreed, and `--currency` silent**. Three instruments clean on a file stating two versions of itself. `session_versions` **v1.34.0** imports the first home from `CURRENCY_HOMES` rather than re-declaring it, and lives in `check()` because the disagreement exists whether or not the file changed. **Its first build asked `git ls-files` and went RED inside `--selftest`, whose fixture is a tempdir copy and not a repository** — an arm that only works inside a checkout has a hidden precondition. Population walked from disk; controls re-run after the change rather than trusted from before it.

**A TAG NAMED AS AN EXAMPLE IS NOT A PLAN.** L04's figures note typed a live figure tag to explain the two number spaces, so every instrument keying on tag syntax read the example as a planned figure. **IMAGE 4.1 has been reported outstanding since S138 — the session whose shot brief records it RETIRED.** The photography number on the critical path was one too high the whole time. **The instrument fix was built first and reverted**: an exclusion would have to be taught to every reader of that syntax and remembered forever (rule 20), while dropping the brackets makes all of them agree at once. Denominator 145 → 144 with the distinction stated — **a corrected population, not improved coverage; outstanding is unmoved at 14, which is how you tell** (S135).

**ONE RUN GIVES THE DIRECTION, NOT THE SIZE.** L03 gains NOTE 3.121, derived from DJ's S196 bench: eight TRIM-0 runs curved left **every time** while the distance off-centre spread more than 2×, on runs repeatable to about 1%. **The lesson's own tuning loop asks only for direction and was already correct** — the sweep's finding was where it was NOT. §9 Bonus 2's two figures are now named as averages over seven or eight runs; they had been presented flat beside a challenge asking the student for one run each. Five pinned population counters moved with reasons, including the CSS baseline, where **rules and declarations are unmoved at 574/2,033** — the check that says a callout was added and no style was edited. L03 **v03.47.4**, L04 **v04.29.7**, six bank pins across L03/L04/L05/L11/L12/L13, **all read before bumping** (rule 37), bank versions moved in both homes.

**THE BENCH SHEET SAID EIGHTEEN AND MEANT NINE.** S196 answered six rows and half-answered three, and the results were appended in a new section while **every Result cell in the station tables stayed blank** — so anyone working from the tables would have re-run six measured rows. Rebuilt: nine open rows first, in run order, then a closed-do-not-re-run table. **The closed rows were NOT deleted**, because `ZUMO_BENCH_TESTS.md` was checked and carries none of the measured numbers. The rule assumed the tracker keeps the record and it does not; **this sheet is the only home for that data.** `ZUMO_FLAGGED_CHECKS.md` **v1.6**.

**F5 RULED BY DJ — the L01–L03 battery bands stand as printed, adjustable later if found in error.** The contradiction is recorded and not withdrawn: measured fresh is ~201 mV below canon *fresh*, and a hard session ends ~292 mV above canon *working*. **The row is RULED, not CLOSED**, and the sheet now names what would settle it — a pack run flat in normal use, logged, not a bench sitting.

**L01–L08 CARRIES NO OPEN WORKLIST ROW** — eighty-four findings, all but two shut and those two parked, the only band in the book at zero. What stands between it and students is not content: F5's bands, nine unshot figures (five stills, four videos), nine unrun bench rows, and the unbuilt `(none needed)` rollout.

### THE TRIPLE CHECK (three arms, every control fires, and two arms found something)

**ARM 1 — LIVE-SITE DIFF.** Independent HTMLParser, no suite instrument. **Fourteen untouched lessons diff to ZERO** against the published site; L03 and L04 show only the intended lines. A sentence planted in `Lesson_09` **FIRES**; a whitespace-only change is **silent**. The zero is a verdict.

**ARM 2 — VERSION / PIN RE-DERIVATION.** Reads the tree directly, imports nothing. Every changed file's version re-derived against `HEAD`, all 16 bank two-homes compared, every source pin resolved against the live lesson. Three seeded defects — bank two homes disagreeing, a lesson edited without a bump, a stale source pin — **all FIRE**. **One of the four controls failed to plant and read exactly like a pass**, the same failure hit earlier in the session; the plant is now asserted before the result is believed.

**ARM 3 — CONTENT RE-DERIVATION.** Every figure filed this session re-derived a second way from the tree. Seating +33 and 0 unseated; callouts 1,136 → 1,137; image references +1; figure tags 145 → 144 with exactly `IMAGE 4.1` gone and nothing new. **It returned two findings and BOTH adjudicated to the arm's own scoping, not to the tree** (§16.15 — a hit is not a defect): its ledger regex was narrower than the file's row shape, and the one "lost" decimal was the retired version token. **But its loss detector was WRONG when first controlled** — a set difference cannot see one of six occurrences vanish. Rebuilt as a multiset, and then it found something real: **the S198 rewrite of the bench sheet had dropped the S189 scope clause** — the sentence saying this sheet's count is L01–L04 only and pointing at `L10-B1`/§16.12 as the most consequential row book-wide. **That clause exists because a figure read off this sheet was once generalised to the whole book, and it has now had to be restored twice.** Restored, and the measured regions were then asserted **byte-identical** to `HEAD` with zero numeric tokens lost.
...
Applied S186: Bible **v8.181.1** · L04 **v04.29.6** · `book_gates` **v1.74.0** (80 gates) ·
...
**§18.3b IN A THIRD REGISTER — TWELVE SITES.** S182 retired the ALL-CAPS banner names and S183 swept them; the retired words survived in **TITLE CASE**, where no predicate was looking. L03's Step 5 and Step 6 headings, its §3.2 prose, its Step-6 checkpoint, two Maker `KINDS` rows and `QUIZ_L03` B37 — **a student picked a dropdown row named one thing and opened a file bannering another.** And L03's Step-11 checkpoint printed the **RETIRED ORDER**, helpers above `setup()` with HARDWARE OBJECTS and FUNCTION PROTOTYPES missing while claiming *all 9*, four sections after §5.1 says the opposite. The replacement is **DERIVED from L02's own nine-section colour key**, so the two lessons cannot drift apart.

**THREE MORE CLASSES, THREE MORE DEFECTS IN L01–L02.** L02's objective promised **7 sections** where §3, §3.1, §5, the colour key and the anatomy SVG all say nine. L01's BC02 skill 4 disagreed with its **OBJECTIVE TWIN** (§25.5) — the objective was fixed in the S162 A-Star pass, the twin still asked students to tick *a project for the A-Star 32U4*, two screens after §3.3 says the robot contains none. L02's hardware table said motors = **Lesson 3** against four sources, **and the first fix was also wrong** (see `ZUMO_FIX_TRACKER.md` §6). L02's float claim carried a **false superlative** — *the first time you divide* — when L03 divides five times and L02's own Step 10 prints `millis() / 1000` (§16.16).

**CLEAN OVER ENUMERATED DENOMINATORS, NOT OVER GREP COUNTS (§24.22):** 145 `<pre>` blocks and 8 distinct banner sequences all in canon order · 31 placement claims all correct · 21 §-references all resolve · 3 BRAIN CHECK counts all match their own lists · 62 `KINDS` rows read through the real object in node and adjudicated one by one.

**GATE 79 EXISTS. §18.3a HAD NO INSTRUMENT FOR 151 SITES.** §16.25 had a gate and §16.31 had a gate; the LOCKED ruling that moved 140 sites at S183 and 11 at S184 had none, and **all three findings were made by hand.** `prose_canon.py` **v1.1.0** is built — arm 3 only — and imported by gate 79 rather than re-implemented, because a second regex would be a second thing to keep in step with the exemptions. **The gap it closes: all 79 gates run PAYLOAD → LESSON**, and `gate_payload_match` is a SUBSET test (§16.45), so stale prose was invisible to the whole suite by construction.

**IT SHIPS GREEN OVER A PINNED RESIDUE, AND THAT IS A DESIGN DECISION.** Seven real L05/L06 sites are pinned with a reason each — S162's `UNREAD_PINS` shape — because a permanently red gate trains its readers to ignore red (v8.130), while deleting the findings would lose them. The gate asserts **new drift** and **ORPHAN PINS**, never the pinned set, so **the list can only shrink.** Four of its fifteen controls guard the pin table itself: the residue absorbs exactly seven and nothing else · an orphan pin FIRES · an EMPTIED table re-reports all seven as drift · and a real new defect still fires with the table armed.

**THE ARM'S OWN GAP WAS FOUND BY A DOUBLE CHECK, NOT A RE-READ (§24.13).** Pointed at the PRE-FIX clone it named 11 of the 12 hand-fixed sites: `QUIZ_L03` B37 read *a STATE VARIABLE*, **singular**, and the plural-only pattern was silent. **The instrument built to end the guessing had the same shape of hole as the sweeps it replaced.** The same check caught an L07 false positive (§18.3a does not govern L07+) and `session_versions.py` edited-but-unbumped — **its own currency arm named its author.**

**A BIBLE ENTRY MUST BE FILED IN BOTH HOMES.** `current_session()` derives from a **LINE-START** entry; a version-line-only entry left the derivation reading S183 while `build_worklist --check` reported "current" — green, measuring the wrong thing.

**Applied S184:** Bible **v8.179** · Maker **v2.67.1** · L01 **v03.32.0** · L02 **v03.26.1** · L03 **v03.46.2** · `book_gates` **v1.73.0** (78 → 79 gates) · `session_versions` **v1.31.1** · `prose_canon` **v1.1.0** NEW · `ZUMO_FIX_TRACKER.md` **v1.1** NEW · seven banks bumped, every pin EARNED by reading it against the changed lines (rule 37). **No payload changed and no executable line moved, so `byte_audit` is owed nothing.**

---

## WHAT SHIPPED IN S183

**THE NAME, NOT THE SPELLING.** S182 retired `STATE VARIABLES`; `LINE FOLLOWING STATE` and `ROBOT STATE` ARE that section and contain none of the retired string, so the sweep could not see them. Both hold only mutable file-scope declarations in the pre-`SETUP` slot. **140 sites renamed** — 136 payload, 4 lesson — 0 leftover, Maker closing on 67 + 136 = **203**.

**BOOK-WIDE BY DJ RULING, WHICH OVERRIDES THE L13–L16 FREEZE FOR THIS PASS.** The defect breaks at L08: `GLOBAL VARIABLES` runs L01–L07 (64 sites), then flips. Only **8 of 136** sites are in the L01–L08 band; 71 are L09–L12 and **57 are in the frozen band**. A partial rename moves the inconsistency one lesson later rather than removing it, so it is all 136 or none.

**ZERO BYTES, MEASURED BOTH DIRECTIONS.** The edited Maker was reverted, the pre-edit baseline compiled, and the tables diffed: **0 flash and 0 status changes across all 221 payloads**, all eight standing controls exact with `11/after_step_1` at **20,592** first. The 219/221 gap is the two `data-nobuild` payloads and is not a defect.

**PIN BUMPS MACHINE-CLOSED.** Twelve stale `source:` pins across eight banks; the three lesson diffs are **seven changed lines total** and **zero banks assert either retired string**, so the bumps were earned rather than asserted.

**EVERY FILE NOW CARRIES ITS IDENTITY (DJ ruling).** `main.cpp` had LESSON/AUTHOR/DATE and no FILE; the seven support files had FILE and no LESSON/AUTHOR/DATE. Closed both ways by ONE generator change — `idFields()` + `stampBox()` in `extraFiles()` — not 1,295 payload edits. DJ ruled KEEP THE CURRENT BOX, so the stamp splices in and each file's own description stays put. **1,074 of 1,074** verified, idempotent, and the real functions were EXECUTED headlessly against the real payloads rather than a port of the logic.

**THREE FALSE FINDINGS, ALL KILLED BY A CONTROL RATHER THAN A RE-READ.** A box census reported 79 missing boxes — the predicate required the box at offset 0 and `RobotSensors.cpp` opens with `#include <Wire.h>`; re-predicated and controlled both ways the true figure is **0 of 1,074**. `INCLUDES` appearing once in the Maker is the GENERATOR, not 220 missing banners. L01's `┌───` box is the CHALLENGE INSTRUCTION box (§11, S56), not a second header. **A count is not a defect count in either direction.**

**AND A CORRECT MEASUREMENT WAS MISDIAGNOSED AT SESSION OPEN.** Nine lessons measured stale on the published site, controlled against seven unchanged lessons and against the prior commit. The measurement held; the inference did not — run #943 took **9m 14s** and both readings fell inside the build window. **S166's *run it twice* needs a TIME FLOOR, not a run count: two readings ninety seconds apart inside a nine-minute build are one reading.**

**WHAT S184 SHOULD DO FIRST:** the empty-section banners (`// ===== CONSTANTS ===== (moved to RobotConfig.h)`, DJ ruled S183) and the helper relocation below `loop()` in L08–L10 with the prototypes that become load-bearing. **That pass CAN move bytes and every intermediate state is red by construction** (S157). Then `prose_canon.py`, with a control per arm.

---

## WHAT SHIPPED IN S182
**36 CHANGED, 1 NEW, 1 DELETED — 38 ENTRIES, DERIVED FROM `git status`, NOT TYPED (S163/S170:
the number that is TYPED is the number that is wrong).**

**THE NINE SECTIONS ARE RULED (Bible §18.3a, v8.176).** `CONFIGURATION`, `STATE VARIABLES` and bare
`LOOP` retired; `INCLUDES`, `SETUP` and `MAIN LOOP` promoted to real banners; helpers below
`loop()`; all eight banners in every full-program starter AND every bonus challenge; MY PLAN
L02–L09 only. **63 payloads rebuilt** (Maker **v2.66**), five lessons re-cut, the anatomy SVG
redrawn at nine and the notebook card **generated from it** so the two can no longer drift.

**ZERO BYTES, MEASURED NOT ASSERTED:** baseline compiled before the pass, all 221 recompiled after,
**0 figures moved**, eight standing controls exact, `16/after_step_2` tightest at **28,648 / 24
spare**. `byte_audit --discards` **15 over 7 of 110, 7 adjudicated, 0 unexplained**.

**FOUR THINGS THE INSTRUMENTS CAUGHT:** `session_versions --currency` was **crashing on a binary
file and silencing itself for all 22 others** (→ **v1.30.2**); §27.8b's cycle was **owed for the
first time in five sessions** when two `<h4>`s made `.h4-c-433014` and `-2` trade their
`margin-top` (acceptance test: **22,963 elements, 0 resolved-styling changes**); the Maker badge
read **v2.64 against a file reading v2.65**; and **`L03_B36` keyed the exact sentence the ruling
reversed**, so a student reading the live lesson would have been marked wrong.

**Applied S182:** Bible **v8.177** · Maker **v2.66** · L01 **v03.31.4** · L02 **v03.25.0** ·
L03 **v03.46.1** · L04 **v04.29.5** · L05 **v04.29.3** · L06 **v04.34.0** · L07 **v04.31.8** · L14 **v02.36.2** ·
L16 **v02.28.1** · `book_gates` **v1.72.19** · `gate_payload_match` **v1.9.2** ·
`session_versions` **v1.30.2** · `build_family_map` **v1.6.6.3** · all 16 banks re-pinned ·
`css/book.css` and `GPT_WORKLIST.md` regenerated. Callout baseline **1132 → 1133**, image
references **1208 → 1209**, both homes each.

---

## WHAT SHIPPED IN S181
**29 CHANGED, 1 NEW, 1 DELETED — 31 ENTRIES, DERIVED FROM `git status`, NOT TYPED.**
**FIVE LESSONS, THE MAKER, THE BIBLE, FOUR INSTRUMENTS, THIRTEEN BANKS, THE STYLESHEET,
THE BENCH TRACKER, THE GPT LEDGER, LIVE.md AND THE HANDOFF SWAP.**

`Lesson_03.html` **v03.44.3 → v03.45.0** · `Lesson_06.html` **v04.32.5 → v04.33.0** ·
`Lesson_07.html` **v04.31.6 → v04.31.7** · `Lesson_13.html` **v02.38.0 → v02.39.0** ·
`Lesson_14.html` **v02.36.0 → v02.36.1** · `newproject.html` **v2.64 → v2.65** ·
`ZUMO_SUPER_BIBLE.md` **v8.174 → v8.175** (§24.21, filed in BOTH homes) ·
`book_gates.py` **v1.72.15 → v1.72.16** · `gate_payload_match.py` **v1.9.0 → v1.9.1** ·
`session_versions.py` **v1.30.0 → v1.30.1** · `site_parity.py` **v1.2 → v1.2.1** ·
`ZUMO_BENCH_TESTS.md` **v1.2 → v1.3** · `ZUMO_GPT_REVIEW_WORKLIST.md` **v1.3 → v1.4** ·
`css/book.css` and `GPT_WORKLIST.md` regenerated.

**THIRTEEN BANK BUMPS, AND FIVE QUESTIONS WERE REKEYED — TWO OF THEM CORRECT ANSWERS.**
`QUIZ_L03` **1.0.13** · `QUIZ_L06` **1.0.7** · `QUIZ_L13` **1.0.19** · `QUIZ_L14` **1.0.13**
carry content edits; the other nine are PIN-only bumps, each earned by reading the questions
that assert the changed material. **`L13_B43`'s CORRECT ANSWER was the retired DRV8838
driver-channel claim** and **`L14_B30`'s carried it too**; `L06_A04`'s CORRECT ANSWER quoted the
OLD code string `setSpeeds(speed + TRIM, speed)`; `L13_B42` and `L14_B04` had distractor
rationales resting on the retired mechanism, and `L03_A19`'s `why` was the retired asymmetry
explanation. **THREE UNEARNED BUMPS WERE REVERTED** — `QUIZ_L01`, `QUIZ_L02` and `QUIZ_L05` had
no pin change and their only diff was a version this session had just moved (rule 37).

**ELEVEN GPT ROWS CLOSED:** `L13-02` `L13-04` `L13-06` `L13-07` `L13-08` `L13-09` `L13-10`
`L13-13` `L13-14` `L13-20` `L06-01`, plus **`L13-17` REFUTED** (the constant is named correctly)
and **`L13-12` found already-done**. Ledger **57 closed · 2 parked · 186 open**, all sixteen
lessons reconciling with zero duplicates.

**`L06-01` REACHED 153 MAKER PAYLOAD SITES PLUS L06 ×6 AND L07 ×3**, with L12's planted
`leftSpeed + TRIM` sabotage excluded by name and asserted untouched after every write.
**`L13-02` moved `MAX_ROW_CM` 300.0 → 150.0 across 59 Maker sites and the lesson**, the constant
being inherited by L13–L16; census updated, line counts identical, key set unchanged.

**BYTE STATE: 221 payloads recompiled, ALL EIGHT STANDING CONTROLS REPRODUCED** — 20,592 ·
20,778 · 24,790 · 21,334 · 25,248 · 26,002 · 28,406 · 28,626 — **and all five S180 COMPILE CHECK
figures unmoved.** Zero payloads changed size, because `TRIM` ships as 0 and the new ternary
folds; the +10 bytes is paid on the student's robot once they enter their own number. Four
declared L16 overflows, tightest passing build `16/after_step_2` at **28,648 with 24 spare**.

**BASELINES MOVED WITH PROOF FIRST:** the CSS digest twice, **574 rules / 2,033 declarations
both ends every time**, and `PAYLOAD_CENSUS` twice, each spliced only after asserting that the
KEY SET and every LINE COUNT were unchanged and only hashes had moved.

## WHAT SHIPPED IN S180
**SEVENTEEN FILES. THREE LESSONS, THE MAKER, THREE INSTRUMENTS, THE STYLESHEET, THE BENCH
TRACKER AND EIGHT BANKS.**

`Lesson_02.html` **v03.22.1 → v03.24.1** · `Lesson_12.html` **v01.33.1 → v01.35.2** ·
`Lesson_13.html` **v02.35.0 → v02.38.0** · `newproject.html` **v2.63 → v2.64** ·
`book_gates.py` **v1.72.9 → v1.72.15** · `build_family_map.py` **v1.6.6.1 → v1.6.6.2** ·
`gate_payload_match.py` (PAYLOAD_CENSUS +5, re-emitted by the gate itself rather than
hand-typed) · `css/book.css` regenerated · `ZUMO_BENCH_TESTS.md` **v1.1 → v1.2**.

**EIGHT BANK BUMPS.** `QUIZ_L02` **1.0.12** · `QUIZ_L03` **1.0.12** · `QUIZ_L04` **1.1.14** ·
`QUIZ_L12` **1.0.12** · `QUIZ_L13` **1.0.18** · `QUIZ_L14` **1.0.12** · `QUIZ_L16` **1.0.19**.
**ONE question was rekeyed all session** — none in S180; every bump is a PIN, each earned by
reading the questions nearest the change. `L12_B46`/`B47` grade §8A.4's table and are
**strengthened** by 12.6; `L13_A17b` grades Step 6's three room assumptions and carries 13.4's
hazard as a distractor, and the assumption list did not move.

**FIVE NEW MAKER PAYLOADS, ALL COMPILED:** `c4_shortway` 20,968 (+22) · `c5_square` 21,742
(+774) · `c6_driftmeter` 20,728 · `c4_landmark` **25,248 (+0 — the guard is FREE)** ·
`c5_striplog` 25,366 (+118). **13.6 needs no build** and points at the existing `cal_7a` rung.

**AND THE FIVE NEW COMPILE CHECK FIGURES ARE UNPOLICED, MEASURED AT CLOSE:** a blinding control moved one 25,248 → 25,240 and **book_gates stayed 78/78 and `byte_audit --check` stayed PASS** — ARM 2 reads figures inside a declared band and a bolded number in a callout paragraph is prose. All five are correct today, each derived against the compiled table; nothing will catch them if they drift.

**CENSUS 41,043 → 41,465.** Six challenge cards, one prose section on MY PLAN, five COMPILE
CHECK boxes and the three corrected 7A-table sentences.

---

## WHAT SHIPPED IN S179
**SEVEN LESSONS, TWENTY-ONE BANK BUMPS, THE MAKER, ONE SVG, TWO NEW BIBLE SECTIONS AND A NEW
DOCUMENT — AND THE SESSION'S BEST FINDING WAS THAT AN ARTEFACT PRICED AS EXPENSIVE WAS NOT.**

`Lesson_01.html` **v03.31.2 → v03.31.3** (minor; §8's A-Star slash) ·
`Lesson_02.html` **v03.22.0 → v03.22.1** · `Lesson_03.html` **v03.43.2 → v03.44.3** (moderate) ·
`Lesson_04.html` **v04.29.2 → v04.29.3** · `Lesson_05.html` **v04.29.1 → v04.29.2** ·
`Lesson_06.html` **v04.32.4 → v04.32.5** · `Lesson_07.html` **v04.31.5 → v04.31.6** ·
`newproject.html` **v2.62 → v2.63** · `L02_GRAPHIC_2-09_folder_structure.svg` ·
`ZUMO_SUPER_BIBLE.md` **v8.171 → v8.173** (§16.48 and §16.49 NEW, both seated with numbered
bodies; §11's two stale path lines corrected) ·
`ZUMO_GPT_REVIEW_WORKLIST.md` **v1.2 → v1.3** (PART 0 — CLOSED ROWS) ·
`ZUMO_BENCH_TESTS.md` **v1.0.0 → v1.1** · `ZUMO_FLAGGED_CHECKS.md` **NEW** ·
`GPT_WORKLIST.md` regenerated, **stamp line only** (the Bible session bump obligation, S175).

**AND THE CLOSE SWEEP FOUND THE DELETED SIX-FEET FIGURE ALIVE IN A LIVE INSTRUCTION OUTSIDE
EVERY GATE.** `ZUMO_S110_SHOT_BRIEF_WEEK1.md` **v1.0 → v1.2** still said *Rig once: smooth floor,
6+ feet clear* and *Show the full 6 feet*, staging `IMAGE 3.2` — L03's still-unshot testing-setup
photograph. **A photo taken to that brief would have put the deleted figure back into the book as
a picture, and nothing here reads inside an image.** Amended with the reason in place. **A
document that instructs a human carries a retired figure as easily as one that instructs a
compiler, and this repo gates only the second kind.** A free finding came with it: the brief's
*a figure saved under any other name reads as no asset* is stricter than `image_audit`, where
only the PREFIX is load-bearing — tested against the matcher rather than read.

**TWENTY-ONE BANK BUMPS ACROSS FIFTEEN BANKS.** `QUIZ_L01` **1.1.6** · `QUIZ_L02` **1.0.10** ·
`QUIZ_L03` **1.0.10** · `QUIZ_L04` **1.1.12** · `QUIZ_L05` **1.0.4** · `QUIZ_L06` **1.0.6** ·
`QUIZ_L07` **1.0.5** · `QUIZ_L08` **1.0.6** · `QUIZ_L09` **1.0.6** · `QUIZ_L10` **1.0.8** ·
`QUIZ_L11` **1.0.13** · `QUIZ_L12` **1.0.9** · `QUIZ_L13` **1.0.15** · `QUIZ_L15` **1.0.8** ·
`QUIZ_L16` **1.0.17**. **ONE question changed** — `L03_B35`, rekeyed off the deleted ±10%
figure. **Every other bump is a PIN**, and the path sweep's pins were **earned by a CLOSED DIFF
rather than a re-read** (rule 37, S167's method): each bumped lesson diffed against HEAD under a
transform normalising the path and the version line, with L02/L04/L05/L06/L07 showing **zero
residue**. The assertion that makes those pins safe was measured book-wide: **no question in any
bank asserts a project path or a floor-space figure**, in a stem, an option or a rationale.

**CENSUS 41,006 → 41,010.** The +4 is DERIVED, not drift: L03's Step 3 gained one paragraph and
its checkpoint gained one item. Every other edit this session landed inside an existing line.

---

## WHAT SHIPPED IN S178
**ONE LESSON, FOUR BANKS, ONE INSTRUMENT AND ONE DOCUMENT — AND THE SESSION'S BEST FINDING WAS
THAT THE PRICED DESIGN'S OWN RATIONALE IS WHAT BROKE IT.**
`Lesson_02.html` **v03.21.5 → v03.22.0** (moderate, both §5b homes; 10 edits) ·
`Lesson_01.html` **v03.31.1 → v03.31.2** (minor, hidden home only; `L01-13`) ·
`QUIZ_L01` **1.1.4 → 1.1.5** · `QUIZ_L02` **1.0.6 → 1.0.8** · `QUIZ_L03` **1.0.4 → 1.0.5** ·
`QUIZ_L04` **1.1.6 → 1.1.7** · `QUIZ_L16` **1.0.13 → 1.0.15** ·
`book_gates` **v1.72.8 → v1.72.9** (gate 78 predicate widened; §27.11 digest moved) ·
`ZUMO_SUPER_BIBLE.md` **v8.170 → v8.171** (§16.47 NEW, seated with a numbered body) ·
`GPT_WORKLIST.md` regenerated, **stamp line only** · `css/book.css` regenerated.

**§16.47 — A WIDENING CLOSES THE CASE IT WAS AIMED AT, NOT THE PROPERTY.** Gate 78 under-reached
twice in one arm for two different reasons, and the second under-reach survived the fix for the
first. **Seven controls, one per invocation, planted into LIVE.md's current session region:** the
bare figure still FAILS · the bold-wrapped figure now FAILS *(SILENT on build 1)* · a backticked
figure is SILENT · bold AROUND inline code is SILENT *(the recorded trap, proven absent)* · the
clean tree is 78/78 · a figure broken across a line wrap FAILS *(new reach)* · and the TRUE figure
bold-wrapped is SILENT *(no false positive)*.

**THE TEN L02 EDITS:** `L02-01` Build 3's reveal is a compile error, measured · `L02-02`+`L02-03`
§8A's two blocks converted to `ledYellow` and K&R · `L02-08` §4 continuity, L01-07's wording ·
`L02-10` Step 3, *a green build is evidence, not proof* · `L02-11`+`L02-18` closed by ONE sentence ·
`L02-14` Step 7 · `L02-16` *The Seven Sections* is the RoboLore standard, not a C++ rule.

**AND LESSON 1's LAST OPEN ROW CLOSED, WHICH MAKES ALL FIFTEEN L01 FINDINGS ACCOUNTED FOR.**
`L01-13`: three sites disagreed about where a project lives, and **step 1 was broken independent
of which path was right** — it told the student to copy the Lesson 1 project folder *while in*
`Documents/PlatformIO`, one level above where the turn-in callout says that folder sits.
**§16.46's shape a third time: read in order, the lesson disproved its own instruction.** The
contradiction convicted it; the DIRECTION of the fix needed the room, because **PlatformIO is not
installed in the container** — DJ confirmed `Documents/PlatformIO/Projects/<name>` against a live
wizard (§24.17 carve-out 1, used exactly as intended). The other four L01 rows were already dead
and an INSTRUMENT says so rather than a memory: **gate 76** retires the *brain of your Zumo*
claim book-wide (`L01-02`, `L01-10`), **gate 73** asserts every printed figure has an index row
(`L01-14`), **gate 71** asserts the strip's catalog name (`L01-15`). **No question in any bank
asserts a filesystem path**, so all three `lesson_01` pins were earned. **L01 now owes only a
bench pass.**

**CENSUS 41,009 → 41,006, AND THE −3 IS DERIVED:** §8A.2's block went 6 printed lines to 5 and
§8A.3's went 10 to 8 when both were converted from Allman to K&R. No content was lost.

**§27.8b's CYCLE WAS NOT OWED, SECOND SESSION RUNNING**, established by S177's scratch-copy control
before the tree was touched: **582 rules both ends, 2,065 declarations, ZERO names added, dropped
or repointed**, the entire diff being one `<span>` frequency count. A plain regeneration sufficed.

**S175's COUPLING FIRED A FOURTH TIME, ON SCHEDULE.** `build_worklist --check` green at open, red
immediately after the Bible bump; **the entire diff is the stamp line**, S177 → S178, 38 files and
9 findings UNCHANGED.

---
---

## WHAT SHIPPED IN S177
**ONE LESSON, THREE BANKS, ONE INSTRUMENT AND TWO DOCUMENTS — AND THE SESSION'S BEST FINDING WAS
A QUIZ BANK THAT HAD BEEN RIGHT WHILE ITS LESSON WAS WRONG.**
`Lesson_01.html` **v03.30.1 → v03.31.0** (moderate, both §5b homes; 21 edits) ·
`QUIZ_L01` **1.1.2 → 1.1.3** · `QUIZ_L02` **1.0.4 → 1.0.5** · `QUIZ_L16` **1.0.11 → 1.0.12** ·
`book_gates` **v1.72.7 → v1.72.8** (§27.11 digest only) ·
`ZUMO_SUPER_BIBLE.md` **v8.169 → v8.170** (§16.46 NEW, seated with a numbered body) ·
`GPT_WORKLIST.md` regenerated, **stamp line only** · `css/book.css` regenerated.

**§16.46 — THE BOOK CONTRADICTED ITSELF INSIDE ONE SECTION.** §6 said switch power OFF and watch
the upload fail; WARNING 1.46 six thousand characters earlier says the cable powers the chip and
the display and only the motors need the switch. The vendor citation agrees, but the internal
contradiction is what convicts it — **read in order, the section disproves its own exercise.**

**THE TEN FINDINGS:** `L01-01` unplug the cable (four sites) · `L01-03` PlatformIO does not fetch
libraries over Git, and **dropping the step would have broken every Mac in the room** · `L01-07`
five things, not three, table NOT enlarged · `L01-08` B had a `printf` · `L01-09` not the first
programmers in history · `L01-11` a thermostat closes a loop too · `L01-12` the USB chime is
Windows-only · `L01-04` Challenge 9 props up rather than going to the floor · `L01-05` Challenge
4's reveal corrected · `L01-06` Challenge 11 prints to the screen its scaffold promised and
defends 4500 against 4200. Plus the "Still stuck?" checklist, which called §7's own instruction a
bug.

**THE FLOOR TEST IS A NAMED RITUAL, SEATED ONCE IN CHALLENGE 4** — upload tethered, unplug, switch
power ON, set down, hands clear — **as prose and not a callout**, because a callout moves
`callout_id`, the family map and §5.1's geometry baseline for a paragraph needing none of them
(rule 46).

**FOUR TIMES NOW GPT HAS BEEN RIGHT ABOUT THE PLACE AND WRONG ABOUT THE DIAGNOSIS**, and twice the
handoff's own summary overstated a finding. **Verify before editing. Every time.**

**S175's COUPLING FIRED A THIRD TIME, ON SCHEDULE:** `build_worklist --check` green at open, red
immediately after the Bible bump, **the entire diff being the stamp line** with 38 files and 9
findings unchanged.

---

## WHAT SHIPPED IN S176
**ONE INSTRUMENT, ONE DOCUMENT AND ONE GENERATED FILE — AND THE SESSION'S BEST FINDING WAS A
CONTROL THAT DISAGREED WITH THE AUTHOR AND WAS RIGHT.**
`gate_payload_match` **v1.8.0 → v1.9.0** (`PAYLOAD_CENSUS`, 216 pins, `--update-census`) ·
`ZUMO_SUPER_BIBLE.md` **v8.168.3 → v8.169** (§16.45 NEW, seated with a numbered body) ·
`GPT_WORKLIST.md` regenerated, **stamp line only**.

**§16.45 — A SUBSET TEST CANNOT SEE A DELETION.** The derivation predicate asks whether every
payload line comes FROM the lesson; a payload that has LOST a line is still a subset, so loss was
invisible by construction. Measured: one deleted guard, gate PASS, advisory unmoved at 635.
The truth is PINNED because there is nothing to import it from — the payload IS the artefact under
test — which is exactly why `BOXED_FP` exists, one layer along.

**WHAT DERIVATION CANNOT SEE AND THE CENSUS NOW CAN:** a DELETED executable line, and two
executable lines REORDERED. Addition, a changed value and a reworded trailing comment also fire but
ALREADY fail derivation, so no new obligation is created there. **Reindentation is SILENT** — the
one genuinely free edit stays free.

**CONTROL 4 CORRECTED A PUBLISHED PREDICTION.** It was written expecting silence on a comment
reword; a TRAILING comment is part of the stripped line, so the arm fires. The code comment
carrying the wrong prediction was corrected to match the artefact.

**STATED SCOPE LIMIT (rule 78):** a pin asserts content has not MOVED since a human blessed it,
never that it is CORRECT. Declared, not hidden.

---

## WHAT SHIPPED IN S175
**ONE DOCUMENT AND ONE GENERATED FILE — AND THE SESSION'S BEST FINDING WAS THAT FILING THE
CORRECTION INVALIDATED AN ARTEFACT NOBODY HAD COUPLED TO IT.**
`ZUMO_SUPER_BIBLE.md` **v8.168 → v8.168.3** (§16.41's figure corrected in both homes; §16.44 NEW) ·
`book_gates` **v1.72.5 → v1.72.7** (78 gates) · `GPT_WORKLIST.md` regenerated, **stamp line only**.
**v8.168.1 and v8.168.2 were DELIVERED mid-session** with the gate recorded as owed-not-built; DJ ruled BUILD IT
(§5, a unique version per delivery).

**GATE 78 / §16.44 — A STATED DISCARD FIGURE IS THE ONE ARM 9 MEASURES.** Recorded first as
unbuildable, because the pair appears in three registers with one spelling — **and the double
check's first arm proved it by firing LOUD on a CLEAN tree.** The scope was the defect, not the
predicate: **history is excludable BY PROPERTY (rule 20)**, so the Bible's changelog is not read
at all (§16.37), LIVE.md is read only in its current session region, and the handoff whole. The
predicate is a **claim form, not a spelling** (rule 19) — assertive register only, blind to the
narrative one. Truth **imported** from `byte_audit.DISCARD_BASELINE`, never typed (rules 83/84),
**zero compile cost**. Eight controls, one per invocation, md5-exact restores; the
narrative-register plant, a wrong figure in backticks, and the blinding reword all **SILENT**; a removed handoff fires §12.2
AND §16.44, complementarity not a fault. **Stated scope limit (rule 78): another claim form is
not reached — this closes ONE shape, not the class.**

**THE LEAD, SETTLED.** `byte_audit --discards` measures **15 discards over 7 of 105 payloads,
7 adjudicated, 0 unexplained.** Two genuinely independent routes agree: the compile, and the
arithmetic of v8.165's own pre-fix figures (23 − 8 = 15 over 9 − 2 = 7). **A third was
available and was DECLINED** — `DISCARD_BASELINE` sums to 15 across 7 entries, but ARM 9
asserts that baseline against the compile, so it is one method read twice and counting it
would be rule 79's own defect.

**THE DISTINCTION THAT LICENSED THE EDIT.** §16.37 protects a provenance record. The clause
beside the figure — *both payloads re-measured with the attribute injected, 4 discards → 0
each* — **IS** provenance, an account of an S172 measurement, and was left untouched. *The
book stands at NINE over SEVEN* is not an account of an event; it is a present-tense claim
about the artefact. **A record of what was read is history; a claim about what the book IS is
a fact with a comparator.** S174's site was a DIFFERENT one and is closed — LIVE.md's struck
S173 block, where rule 50's other fate took the number with the sentence that carried it.

**AND THE COUPLING.** `build_worklist --check` was green at open and red after the bump.
S174 moved the worklist's session stamp off a pinned literal onto `current_session()`, so the
Bible saying *S175 ran* made the committed file stale by one line. **The fix that stopped a
FRESH copy looking stale is what makes every COMMITTED copy go stale one session later.**
Population measured and it is **one**: `build_worklist.py` is the only reader of
`current_session()` outside `session_versions`, and `GPT_WORKLIST.md` the only generated
artefact carrying a derived session stamp — `css/book.css` and `IMAGE_WORKLIST.md` stamp
their GENERATOR's version, which is S174's coupling and not this one. **It now belongs in the
close ritual beside the stylesheet.**

**THE CLOSE WAS PROVEN ON A SCRATCH TREE BEFORE THE REAL ONE WAS TOUCHED.** The bump left
`session_versions --check` with four disagreements and `--selftest` FAILING, where the
pristine HEAD tree passed — so the failure was mine, not pre-existing, and that was verified
by cloning HEAD rather than assumed. Simulating the close in a copy (handoff renamed to S176,
LIVE.md's session and Bible token moved) returned both to green before anything real moved.
...
L13 **v02.35.0** (two challenge templates and two solutions guarded) · Maker **v2.62** ·
`session_versions` **v1.29.1** with the NEW `--currency` arm · `byte_audit` **v1.8** (ARM 7
narrowed, two blind spots stated) · `book_gates` **v1.72.5** · banks `QUIZ_L13` **1.0.10**,
`QUIZ_L14` **1.0.9**, `QUIZ_L16` **1.0.11**.

**THE UNBUMPED MAKER REACHED DJ'S HANDS BEFORE ANY INSTRUMENT LOOKED AT IT**, and the arm that
now catches it was written in the same pass. **`gate_payload_match`'s one-directionality is
measured and left open on purpose** — a design change with its own controls does not belong three
weeks from a launch, and it goes forward with a reproduction rather than a note.

**AND `site_parity` IS v1.2.** It flapped once in 19 runs post-push and the mismatched asset never named itself. Measured first and it did NOT reproduce — 163 assets over three passes all status 0, then 59 clean runs — **so v1.2 is not offered as that flap's proven cause.** What was wrong on its own terms: a socket error counted as unreachable NOISE, but ANY `HTTPError` code came back as a hard MISMATCH reading *LIVE 404*, **so a CDN 503 accused the repo of not publishing a file it publishes.** Transient statuses are now unreachable, retried once, and NOTED on stderr with the asset and the code **even when the retry succeeds** — an instrument whose noise leaves no trace is one nobody can ever diagnose. CONTROL H NEW, synthetic and network-free, **asserting the retry by COUNTING calls**, because a retry that never happens looks identical to one that succeeded. The original flap remains unexplained.
...
`case SWEEP_DONE:` in the `loop()` dispatch across 48 payloads (Maker **v2.61**), printed in L13 with a
paragraph tying it to `LINE_LOST` three cases up. Every figure re-derived against a compile: 7 Maker
labels, 17 L15/L16 step figures, 15 headroom claims, 15 L13/L14 prose figures, 19 bank figures.

**THE DECISION WAS THE WORK, NOT THE PATCH.** The patch is +2 bytes in L13. The price was 62 stated
figures across four lessons, two banks, the Maker and an SVG — and **that price was measured on a
scratch tree and reported before the repo was touched**, which is what made it a decision rather than
a discovery. §24.19's worked example: the cheap fix that restores the book's own pattern beat the
semantically richer one that would have moved what two lessons teach.

**WHAT WAS DELIBERATELY NOT DONE, AND IT IS THE ONE THING S172 SHOULD READ FIRST:** the L13 and L14
banks carry figures that look stale from BEFORE this session — `20,516` in both (retired at S166),
plus `25,942`, `25,886`, `25,906`, `25,816` and `25,202` in L14. **Some are certainly deliberate
wrong-answer distractors**, and those banks were not read this session. **That is a pin arc — READ →
FIX → QUIZ — not a find-and-replace**, and bulk-rewriting risks turning a correct distractor into a
wrong one.

## WHAT SHIPPED IN S170
**TWO FILES, NO LESSON, NO BANK, NO GATE — AND THE VALUE IS ENTIRELY IN WHAT THE INSTRUMENT CAN NOW SAY.**
`pio_harness.sh` v3.1 turns warnings on, keeps core stderr, gives each build its own error file, and
appends `warn=` to the PASS/OVER line — APPENDED, never inserted, because `byte_audit` reads that line
with `startswith("PASS")` and a `flash=(\d+)` search and both survive a trailing field where neither
survives a reordering. `byte_audit.py` v1.6 records the signatures and adds ARM 8.

**THE READ IS WHERE THE VALUE WAS, THIRD SESSION RUNNING.** The flag change is two characters; the
population it exposed took the session, and 109 of 113 warnings turned out to be the book working
correctly. **A measurement that mostly confirms the design is still worth taking, because until you
take it you cannot tell it from one that does not.**

**WHAT ARM 8 DOES NOT DO, STATED SO NOBODY DISCOVERS IT LATER:** it takes the build-up model on trust
in intermediate steps — an explanation, not a proof — it reads only what the compiler chose to say,
it cannot tell a moved signature from a fixed-and-new one, and it counts signatures rather than lines.

**WHOLE-TREE md5 AT CLOSE: 5 changed, 1 new, 1 deleted across 448 tracked files, every one
attributable — AND THE FIRST DRAFT OF THAT SENTENCE SAID *2 changed, 0 new, 0 deleted* ABOVE A LIST
NAMING A NEW FILE AND A DELETED ONE.** It also omitted `ZUMO_AFTER_LAUNCH.md`, a changed file, which
DJ would then not have pushed. **S163's defect verbatim** (v8.155.1: *the handoff's file count said
six above a list of seven*) — the count was carried in the head while the list was typed. Caught by
the double check's whole-tree arm and by nothing else, because **no gate reads a manifest.**

## WHAT SHIPPED IN S169
**ONE PAYLOAD, THREE STALE FIGURES, AND AN ARM THAT HAS NOW BEEN EARNED THREE TIMES AND FINALLY SHIPS.**
`16/step_5_zn_traded` gives L16's second trade checkpoint a door and a compile. Its label carries the
figure, so the number that no arm could reach is now asserted on every run. The buzzer-cut state is
measured but deliberately gets NO payload — it is what §7.4 hands to the student, a hypothetical the
book offers rather than a build it ships, and ARM 7 skips it on the subjunctive rather than on a name.

**THE READ IS WHERE THE VALUE WAS.** A catch-up door is additive and moved no figure; the three
defects it surfaced were all pre-existing survivors of S168's rebaseline, and two of them sat in a
lesson and a bank that contradicted themselves in the same breath.

**WHAT ARM 7 DOES NOT DO, STATED SO NOBODY DISCOVERS IT LATER:** it is blind to a claim naming the
WRONG build — a stale figure that still equals SOME build's headroom is silent, exactly as ARM 6 is —
and it reads only headroom, not figures. A wrong FIGURE in prose remains ARM 2's blind spot and L13's
`25,198` sites remain the unreachable class.

**ARM 3 AT CLOSE: 6 changed, 0 new, 0 deleted, every one attributable.**

## WHAT SHIPPED IN S167
**ONE RULING, 477 SITES, AND A SPLIT INTO TWO ACTS OF DIFFERENT KINDS.** Act 1 is text — 55 lesson sites plus
110 bank strings across `centre`/`centred`, `colour`, `behaviour`, `labelled`, `metre`, `neighbour` and
`recognise`, zero Maker, zero bytes. Act 2 is `travelled`, a VARIABLE, at 89 payloads. DJ ruled both. **Four
of the lesson sites spelled one phrase BOTH WAYS inside a single file** — L04's two sibling challenge cards,
L09's §3 table row against its §5 prose, and L06 line 308's *a colored sticker … and the colour is the gear
ratio* in one sentence. Those were defects under either ruling.

**HELD, MEASURED, NOT SWEPT.** `BookComponentStandard.md` carries **44** British forms and
**`gen_component.py` pins the literal section anchor `'### 5.2 Colour is never'`** — sweeping it edits a
generator's boundary string (rule 56), so both would have to move in one commit and neither did.
`ROBOCUP_RESCUE_LINE_2026.md` is held as the rulebook extract that must match its source. Bible changelog
narration is history and is not rewritten (rule 37).

**THE TWO L13 CONTENT EDITS AND THE ARC THAT CLOSED THEM.** §3's *four lines* deleted; §5 gains a paragraph
naming the three things that must be true about the room for `lastLegCm < lastRowCm - VICTIM_SHORT_CM` to
mean anything — a rectangular zone, parallel rows, and a previous row that actually REACHED a wall rather
than ending on `MAX_ROW_CM`. **§27.16 caught my own edit** on an `&deg;` written where the literal ° is
visible in source. `QUIZ_L13` **A17b** asks which of four candidates is NOT one of the three.

**APPLIED S167:** fifteen lessons swept, all MINOR except **L13 v02.32.0 MODERATE** (it gains a paragraph, so
both §5b homes move and the census moves +1) · Maker **v2.58.5** · all sixteen banks bumped · Bible **v8.160**.
Census 40,889 → **40,890**.

## WHAT SHIPPED IN S166
**SIX PIN ARCS, FIFTEEN EDITS, AND EVERY ONE OF THEM A SENTENCE.** `QUIZ_L12` owed two (`B06` keyed the retired
*INTENT, not RESULT* as its CORRECT answer; `A18` graded B4's pre-S162 symptom). `QUIZ_L11` owed **zero** — the C1
pass left L11's own prose calling the gap drive an open loop in three places, which reads like residue and is not,
because S162 read those and ruled them moment-descriptions (§16.15). `QUIZ_L13` owed three, all §16.27: `A09` asked
*why does `RobotMotion.h` need an include it never needed before* where the live Step 4 says the opposite, so the
question's PREMISE was what the lesson denies. `QUIZ_L14` owed six, five of them one claim — S159 gave
`COMPETITION_MODE` a second job and the retired *changes theater, never behavior* survived in **three distractor
rationales** (§16.30, fourth firing). `QUIZ_L15` owed one, `B47`'s unscoped superlative (§16.16). `QUIZ_L16` owed
four, all the two-trades-versus-three story.

**THE HARNESS AND ITS TWO SCRIPTS.** Rebuilt from scratch, all eight standing controls reproduced with L11
`after_step_1` **20,592** first (rule 30). `harness_setup.sh` **v1.0.0 NEW** — pinned SHAs, `LIBDIRS` derived from
`pio_harness.sh` and cross-checked (controlled by planting a fake library name). **Its own first draft took a target
directory that never reached `pio_harness.sh`'s hardcoded `H`, and printed `objects: 41` against a directory the
caller never named** — a script whose argument does not reach the thing it configures is worse than one with no
argument. `pio_harness.sh` registered at its existing **v3.0**; `session_versions` **v1.28.0**, 42 artefacts, and
**Control G caught the registration landing without the emission.**

**THE DOUBLE AND TRIPLE CHECKS RAN FOURTEEN ARMS ACROSS FOURTEEN PREDICATES AND FOUND NOTHING AGAINST THE TREE —
AND TWO THINGS AGAINST MY OWN ARMS.** Every `source:` pin resolved against REAL git history (**57, 0 phantom**);
1,241 questions structurally sane; the suite re-run in an independent copy; every retired claim swept from parsed
bank bodies and all sixteen lessons. **A line-level authorship arm reported 0 unclassified out of 279 changed lines
and was BLIND** — seeded with an undeclared `points: 2` it stayed silent, because its `continuation` category was a
catch-all matching every indented line. Rebuilt and re-blinded, it fires. Its first draft had also reported 50
findings that were all source pins: **the classifier was broken, not the tree.** And one lesson hit looked exactly
like the L13 defect I had just fixed — L14's *"`RobotHelpers.cpp` needs one include it never needed before"* — until
the L13 payload was extracted and checked: different file, true claim (§16.15).


**POST-PUSH VERIFICATION FOUND TWO THINGS THE PRE-PUSH SUITE COULD NOT SEE, BOTH ABOUT THE PUSH PATH
RATHER THAN THE BOOK.** The clone came back byte-identical on all 448 tracked files with the deletion
landed — then: **the executable bit does not survive GitHub Desktop.** `harness_setup.sh` arrives
tracked **100644**, so `./harness_setup.sh` fails on a fresh clone, which is the only kind of tree a
session may verify in. Every instruction now says **`sh harness_setup.sh`**, which works whether or not
the bit is ever set, and the script's own header records why so nobody restores `./` later thinking it
tidier (**v1.0.1**; re-tested from scratch, `objects: 41`, standing control **20,592**). And
**`site_parity` is not trustworthy on its first run after a push** — first run 1 MISMATCH, three runs
after it PARITY, tree untouched, Pages mid-rebuild. **Both directions are unsafe alone: a first-run
MISMATCH is a phantom, and a first-run PARITY can be the OLD site agreeing with itself before the new
content deploys.** The second case is the dangerous one because it passes. Run it twice; believe the
repeat.
...
measured: a header can claim a read at a version the pin never reached. `QUIZ_L08` claimed v04.31.4 with its
pin at v04.31.3, `QUIZ_L07` claimed v04.31.4 with its pin at v04.31.2, `QUIZ_L06` claimed v04.32.2 with its pin
at v04.32.1. **Every bump this session was earned by a read performed this session**, never on the header's
authority — because in all three cases the evidence sentence beside the claim described a different file or a
different section than the diff actually touched.

**THE POPULATION: EIGHT header-newer pairs across seven banks**, and the sweep separated them from **three
pin-newer pairs** (L02/l01, L04/l04, L08/l08) which are not defects. Seven remain open, all in the backlog.
**They are not authority to bump.**

**NO NEW GATE, AND THAT WAS CONTROLLED RATHER THAN ARGUED.** The recurrence — a Verified-against line written
at the live version with the pin left behind — puts the pin below live with no backlog entry, which gate 75's
drift arm already catches. Planted in `QUIZ_L05` it FAILS §24.18 ALONE naming *NEW drift*. The eight survive
only because the S162 backlog absorbed them at creation, so the condition is self-extinguishing.

**THE TWO CONTENT EDITS ARE BOTH `why` FIELDS.** QUIZ_SPEC §4 makes `cite:` a promise about where to re-read,
and nothing gates a rationale against its section — the §16.30 finding that distractor rationales are where
claims survive longest, reached from a third direction.

**GATE 74 FIRED ON THIS SESSION'S OWN PROVENANCE AND WAS NOT WIDENED.** Writing the retired C1 wording into two
bank headers to record what changed made §16.31 FAIL. S163 gave gate 76 a structural exemption for exactly this
shape; the disposition here is opposite and the reason is the population — `L01_B14`'s distractor IS the
retired claim and cannot be reworded, while narration can be, losslessly (rule 20).

**`ZUMO_AFTER_LAUNCH.md`'s FOOTER NAMED A FILE THAT NO LONGER EXISTS**, `ZUMO_S163_HANDOFF.md`, deleted at that
session's own push. A pointer at a document rewritten every session is stale by construction; it now says so.

---
## WHAT SHIPPED IN S163
**S163 FOLLOW-UP (post-push, `2899729`): GATE 76's COVERAGE ARM WAS BLIND AND THE HANDOFF'S FILE COUNT SAID SIX ABOVE A LIST OF SEVEN.** The arm compared SCANNED against FOUND, both from its own globs, so an empty population passed — with every bank moved aside gate 76 printed **PASS** while §24.2 and §24.18 fired. `book_gates` **v1.70.1** adds a POPULATION arm whose denominator is the suite's own lesson list (rule 29), controlled three ways. The count defect was the one hand-typed number in a delivery whose generator output was right at every stage. **Both were found by verifying the PUSHED CLONE rather than the tree that authored it** — 447 tracked, all eight digests byte-identical, S163's handoff removed, no `__pycache__`, 76/76 green in the clone. Bible **v8.155.1**.


**TWO UNREAD-PIN ARCS, AND THE SECOND ONE FOUND A BOOK DEFECT.** S162 named 48 stale `source:` pins and
ruled the honest cost a READ → FIX → QUIZ arc per bank. `QUIZ_L02` (two pins, both resolving to
`e27d50b`) owed **zero** content edits: L02 moved two content lines, both §3.3, and B14/B15 are stated
verbatim in the live text; the third changed line is a `data-nobuild` attribute on a Maker link.
`QUIZ_L01` (one pin) owed **three**, all of them grading claims §16.25 retired.

**§16.25's RESIDUE: TWO SENTENCES THAT NEVER SAY "A-STAR".** L01 §3.3's NOTE — *"brain,"
"microcontroller," or "controller board" … the same component* — collapsed board into chip two lines
under the KEY TERM that separates them, and §5.0's `board` bullet read *"the exact board, so the compiler
knows its memory size and pin layout"*, which §3.3 and §5.5 contradict in the same file. Both corrected.
`board` now reads *the build profile the compiler builds for* and points at 3.3; the NOTE keeps
"brain" and "microcontroller" for the chip and says what the board is.

**THE BANK EDITS.** `L01_B14`'s correct answer is now the build profile, **with the retired claim as its
strongest distractor** and a `why` naming 3.3 — so the trap now costs a student instead of rewarding one
(the `L08_B21` move from S161). `L01_B55`'s `board` pair corrected. `L01_B56` re-stemmed to grade the
board/chip distinction directly, answer FALSE. `L01_B52`'s glossary matching was **read and left**: four
pairs, no retired term.

**GATE 76 NEW (§16.25).** Six retired forms forbidden outright in all sixteen lessons and the Maker;
in the banks, forbidden only where a question ASSERTS them. **The exemptions are structural, not a name
list:** a `#` comment is provenance and a `correct: false` option is the trap being taught. Five
controls, one per invocation, listed in the Bible entry.

**A HANDOFF NUMBER AND A HANDOFF LABEL WERE BOTH WRONG.** `quiz_bank --selftest` names and passes **24**
controls, not 14. And the Ziegler–Nichols trade is **156 B**; WEAVE is **162 B**. Both chains close
either way — only the labels were wrong.

**L16's CEILING ITEM IS CLOSED.** `16/finished` = **28,564**, 108 spare; the **28,726** in Step 5 is the
mid-trade waypoint the lesson itself calls *"Still over. By 54"*. Carried as open since S158.
...
**L06 v04.32.3 · L08 v04.32.0 · L10 v02.30.2 · `quiz_bank` v1.1.0 · `book_gates` v1.68.8 ·
Bible v8.151 · banks L06/L08/L09/L10/L11/L12/L13/L14/L16 1.0.2, L07/L15 1.0.1 · 73/73.
Census 40,889 unchanged — every edit landed inside an existing line.**

See Bible **§16.31** (C1 canon, six ruled phrasings) and **§24.18** (version homes).
C1 residue, derived not tagged: L11 ×6 · L03 ×2 · L12 ×1 · `QUIZ_L11` ×2 · the two
Maker-coupled comment blocks with their lesson twins · L15-08 under a predicate not yet written.

---
## WHAT SHIPPED IN S160

*S160 closed with no WHAT SHIPPED block, so its narrative lived ONLY in the Date/Status header
that S161 overwrote — the S151 shape recorded in v8.143, committed again here and caught by the
S161 double check. Recovered VERBATIM from the pushed clone at `4a9e1d1`, never reconstructed.*

**Date:** August 19, 2026 (Session 176 — **A SUBSET TEST CANNOT SEE A DELETION, AND THE GATE HAD BEEN BLIND IN THAT DIRECTION FOR ITS WHOLE LIFE.** S173 handed `gate_payload_match`'s one-directionality forward as *a ruling, then a design*, with a reproduction. **The reproduction was run first and it UNDER-STATED the case:** deleting ONE executable line — the S172 kill-switch guard `if (turnDegreesGyro(90.0 * sweepDir) == STOP_KILL) break;` — from `13/challenge_9_1_keep_sweeping`, leaving `Lesson_13.html` untouched, left the gate printing **PASS with the advisory count unmoved at 635**. Not one number changed, so **the gate could not see the removal of the thing S172 shipped.** **RULED A DEFECT CLASS AND NOT A STATED SCOPE LIMIT**, because a stated limit is a shape a gate declines to reach while this was a gate passing on a corruption of the artefact it certifies. **THE SYMMETRIC PREDICATE WAS REJECTED RATHER THAN OVERLOOKED (rule 20):** *every lesson line appears in some payload* convicts wrong-code examples, "before" versions and the Serial-cut demos, which lessons print on purpose and no payload carries. **SHIPPED INSTEAD: `PAYLOAD_CENSUS`, 216 pins, `[count, md5]` over STRIPPED executable lines**, DERIVED with `--update-census` and never typed (rules 83/84), reading the WHOLE registry rather than the lessons passed in — count AND hash, because a count alone lets a loss in one file cancel against a gain in a sibling file of the same payload (rule 79's shape). **TEN CONTROLS, ONE MUTATION PER INVOCATION, EVERY RESTORE md5-EXACT — AND TWO ARE THE ARM'S ENTIRE REASON TO EXIST, BECAUSE DERIVATION PASSES ON BOTH: a DELETED line, and two executable lines REORDERED**, the second found only because a control was written for it. **AND A CONTROL CORRECTED THE AUTHOR, WHICH IS THE REUSABLE HALF:** control 4 was written expecting SILENCE on a comment reword, from a probe that counted LINES only — but a **TRAILING** comment is part of the stripped executable line, so the md5 sees it. **The wrong prediction had already been published in the code comment; the comment was corrected to match the artefact rather than the control re-aimed to match the comment.** Three further controls hold the pin itself: an EMPTIED table fires rather than passing on no truth, an ORPHAN pin fires (S138's shape), and a pin whose COUNT is corrupted while its md5 stays correct fires, proving both fields are asserted. **STATED SCOPE LIMIT (rule 78): a pin asserts content has not MOVED since a human blessed it, never that it is CORRECT** — `BOXED_FP` carries the identical limit, declared rather than hidden. **MAINTENANCE COST MEASURED, NOT FEARED:** every edit moving executable content already needs a paired lesson edit to keep derivation green, so the census adds **no obligation except on deletion and reordering**, and reindentation stays silent. **AND S175's COUPLING FIRED AGAIN ON SCHEDULE:** `build_worklist --check` was green at open and red immediately after the Bible bump, with **the entire diff being the stamp line** and 38 files and 9 findings UNCHANGED — the close-ritual item works. **`site_parity` RAN TWICE AND BOTH RUNS SAY PARITY**, closing S175's one open item, which had needed a published tree. Bible **v8.169**, §16.45 NEW and seated with a numbered body rather than added to the §16.32–§16.44 changelog-only queue. `gate_payload_match` **v1.9.0**.)

**Status:** ✅ **S176 — THE PAYLOAD GATE IS TWO-DIRECTIONAL.** **78/78 gates**, `gate_payload_match` **PASS with the census armed**, `quiz_bank` 16 banks at **1,246** questions, `build_css --check` current at 574 rules, `image_audit --check` current, `build_worklist --check` current **after regeneration**, `callout_id` 1127/0, `next_pointer` clean, census **40,993**, `site_parity` **PARITY on two consecutive runs**. **`byte_audit` DID NOT RUN AND DID NOT NEED TO** — no payload, lesson or bank moved; its eight standing controls stand verified as of S175. **NO lesson, bank or payload changed; no Maker change.**
...
**`byte_audit.py` v1.1 NEW — the only instrument in this repo that compiles. §16.22 and §16.23
NEW. Five banks gain a `Verified against:` line. Two lessons gain a `data-nobuild` declaration.**

### 1. THE MEASUREMENT THAT RULED THE SPLIT ALSO CAUGHT ME MAKING THE S27 ERROR

67 step catch-up rows, derived from the `KINDS` table rather than from any handoff — the handoff
had carried *64 rows, clean*. **27 genuine IDENTITY · 28 OFFSET · 8 coincidental · 4 special.**

Every lesson's `step_1` resolves to `after_step_1`, and in **eight** of ten that payload is
source-identical to the previous lesson's `finished`, because Step 1 is *copy your previous
project* and contains no work. **Those eight agree under both conventions and are evidence for
neither.** I first reported all ten that way, on compiled size. **L13 and L14 are different
source:** L13's Step 1 adds five states plus `SILVER_RAW_MAX`, `WALL_STOP_COUNT`, `ROW_STEP_CM`,
`VICTIM_SHORT_CM` and the `StopReason` enum; L14's adds `COMPETITION_MODE` and three self-test
limits. Both cost **zero bytes**. The lessons say so — L12 Step 1 is *Copy Your Lesson 11
Project*, L13's is *The Numbers, the Names, the Reasons*, L14's is *One Switch, Three Limits*.

**This is a standing limit on ARM 1, not a one-off: a payload can ship an entirely wrong
configuration block and the compiler will never see it.**

### 2. §16.22 — THE SPLIT IS LEGITIMATE, AND A GATE MAY ONLY ASSERT IT PER RANGE

DJ ruled it. L07–L10 are the lessons where the *doing* is the content, so handing over a finished
step deletes the lesson; L11–L16 are integration lessons where being one step behind blocks the
chain. **Rule 32.** Any gate must exclude the eight coincidental rows — they satisfy either
convention. That gate is **not written**.

### 3. §16.23 — THE DECLARATION IS IN MARKUP AND NAMES ITS REASON

`L02 broken_code` and `L10 step_4_RED` do not link, both by design, both declared in prose only.
`data-nobuild` now carries the reason on the catch-up link; a bare boolean was rejected because a
label is not the thing (rule 31). Control G is loud on a missing attribute **and on an empty one**.
...
**Nothing. That is the entry.** No file in the repo changed, no version moved, no bank was
written, nothing was pushed. The tree stands at `0e62713`, exactly where it opened. What the
session produced is a complete, measured specification for the L14 fix and a resolved answer to
a citation defect that had been rotting quietly for two rulebook editions — all of it living in
`ZUMO_S148_HANDOFF.md`, none of it in the book.

### 1. DJ RULED THE L14–L16 ARC PROCEEDS

Fall teaching scope remains **L01–L13**. At S147 open DJ ruled that the read arc continues into
L14 anyway — *"so we keep the pattern the same until the end. No risk of drift."* The ruling is
about the book's internal consistency, not about September.

### 2. L14 READ END TO END, TWELVE COMPILES, AND THE SHIFT IS NOT UNIFORM

Control first: **L11 `after_step_1` reproduced 20,516** from a fourth independent clone before
any figure was read.

| site | book said | measured |
|---|---|---|
| §6 lead-in (L13 finished) | 24,902 | **25,072** |
| Step 1 · Step 2 | 24,902 | **25,072** |
| Step 3 | 25,640 · up 738 | **25,816** · **up 744** |
| Step 4 / finished | 25,640 | **25,816** |
| 7C match mode | 25,604 · 36 fewer | **25,780** · **36 fewer** |
| B1 | 24,906 · 734 smaller | **25,076** · **740 smaller** |
| B2 / B3 / B4 | byte-identical | **all 25,816** |
| B4 in match mode | 8 bytes smaller | **25,780 → 25,772** |

**+170 at the top of the chain, +176 at the bottom**, so **two of the three deltas change**.
This is L13's lesson repeating, not L12's — and it is the third consecutive session in which
recomputing the deltas from the compiles was the thing that mattered. The *36 fewer* at 7C, the
three byte-identical sabotages, and B4's *8 bytes smaller* all survive untouched.

### 3. SEVEN DEFECTS INDEPENDENT OF THE BASELINE

Two paragraphs each price the same quantity twice, with different numbers. Step 3 says *up 738*
and then *the report card costs 830 bytes*; B1 asks about *734 bytes SMALLER* and its reveal
answers *the 820 missing bytes*. **`830` appears in exactly one sentence in the whole tree** and
nothing measures it — `selfTest()` has **no standalone symbol** in the Step 3 binary, LTO having
inlined it into `setup()`. Unlike L13's 120/26, an honest number exists in both cases (744 and
740), so the ruling is *replace*, not *delete*.

Also: Step 2's *"260 lines of code"* against a derived **108**; a dangling cross-reference to a
**"Section 6.3" that does not exist** (Section 6 is Steps 1–4, and every other cross-ref in the
lesson resolves); §4.3's *"add this to your code:"* introducing **no code**, above a callout that
says *zero new code*; a **draft self-correction left in shipped prose** in §7E — *"secretly
flatten a tire… no — have them cover one line sensor"*; and the Figures table's
*"twelve-item Competition Day Kit"* against a derived **11**.

### 4. THE CITATIONS WERE A WHOLE EDITION BEHIND — THE FINDING OF THE SESSION

L14 cites **§4.2.8** for the LoP procedure in §4.1 and **§4.3.7** for the same rule in its
Engineer's Log. It reads like one is a typo. It is not: RCJ 2025's changes list carries
**Added "Terms and Definitions"** — a new §4.1 that pushed all of Section 4 down by one.
`4.2.7 → 4.3.6`, `4.2.8 → 4.3.7`. **Every §4.x citation in the book is a 2024 number**, and the
answer was not in either lesson section but in the rulebook's own changelog. RCAP U19 v2026.0, a
separately derived ruleset, numbers both rules identically — two independent editions agreeing.

The same pass established that **no edition carries a robot size, height, or weight limit**. The
25 cm is **bridge geometry** (RCJ §3.2.3), split into two wrong inspection bullets, and *"must
fit under obstacles"* is wrong twice: obstacles are **≥ 15 cm high** and the robot is expected to
go **around** them. RCAP drops the bridge clause entirely.

**`ROBOCUP_RESCUE_LINE_2026.md` carries the stale §4.2.7/§4.2.8 and is the source of L14's
error. It is also misnamed — its content is the 2025 edition throughout.**

### 5. ONE ITEM BLOCKED ON DJ, DELIBERATELY NOT GUESSED

**RCAP Junior Rescue Line U19 v2026.0** is RoboCup **Asia-Pacific**, derived from RCJ 2025, and
it is not a reprint: it **deletes the black victim, the red evacuation point, and ramps**, caps
the multiplier at **1.96**, and lets zone walls be any colour but red/green/black.

**The black victim is load-bearing in L13 — in scope, read, fixed and banked** — appearing as a
learning objective twice, as Reflection Question 5, in `IMAGE 13.1`'s caption (an unshot photo on
the September critical path), and as a banked question. Mercersburg is in Pennsylvania, so RCAP
almost certainly does not govern, **but the question was put to DJ and left unanswered at close.**
Only L14 §3.2's scoring table is blocked behind it.

A third upload, **RoboCup Rescue Robot League 2026D**, is a different competition entirely —
80 kg teleoperated robots, tethers, stepfields, GeoTIFF maps. Nothing in it applies, and it is
recorded here so nobody mines it again.

### 6. FOUR NEW RULES (63–66)

**63.** A citation is a claim about an **edition**, not about a number — two citations of one
rule that disagree are two editions in one document; go to the changes list, do not pick the more
common one. **64.** A file named for a year is not evidence of that year. **65.** Check the
**league** before the edition. **66.** A regional variant is a different game, not a reprint.
...
across 13 lessons take `KEY TERM: <strong>Term</strong>` with the definition on the next line - 65
canon gained the prefix, L04's five `Key Term:` took the caps spelling, L14/L16's five gained the
`<strong>`, L10's five plain terms gained both, and L15's three inline `<b>` heads became the head
div every other block already had. The glossary's 151 are **untouched**: they were already exactly
the word and the definition.

**The prefix sits OUTSIDE the `<strong>`, and that is structural rather than cosmetic.** 19 body head
anchors sit there — 18 spelled `id="term-*"` and one, L03 `3.44`, spelled `id="glossary-trim"` — as the targets of the ruled body-to-glossary link direction §27.14
enforces, and that span is what the end-of-book glossary harvest extracts as the term. It renders
identically either way, because the head div is already bold.

**The pin fell 125 to 55 and the gate ruled it, not the session.** `canon_of` now names 70
previously-unnameable blocks from CONTENT, and gate 62's coverage arm reported those holds expired
without being told to - S132's behaviour and rule 20 for the second time. Retired **by the property**,
the gate's own `_need` derivation, never a hand list, and the count reconciles: 83 converted, less
the 13 that already named themselves and were therefore never pinned, leaves 70. **The coupling is
recorded rather than hidden:** those 70 now depend on the prefix staying, so removing one fires
§24.14a, §24.14c and §24.14d together, which is three gates agreeing.

**An inline head becoming its own line took the prose with it.** L15's three definitions were
lowercase because they were grammatically continuing `<b>KEY TERM - Term:</b>`, so the converter's
span extends past the head to capitalise, asserting both that a letter follows and that it was lower
case. **This is the only prose S134 touched.**

**Four body blocks are held BY NAME (§25.2a) and the question they raise is unruled.** `3.31` is a
provenance question, `3.101` an operator announcement, `6.24` a formula box, `14.28` a procedural
list - none a term followed by a definition. DJ held constrain explicitly; the other three are the
same shape. Two wear another family's canon ground, so whether they belong to KEY TERM at all is a
FAMILY question for a later session.

New instrument `keyterm_prefix.py` **v1.0**, registered and emitted; `book_gates` reaches **v1.63**
at 68 gates; `session_versions` reaches **v1.24.0**, its Control G naming the new tool as
registered-but-not-emitted on the first run for the fifth consecutive tool-adding session.
Applied S134, all thirteen MODERATE (the pages render differently, so the §5b homes move).
L11, L12 and L13 carry no body KEY TERM blocks and are unchanged.


---
## WHAT SHIPPED IN S133

**The em dash leaves the body KEY TERMs — 24 blocks, not the 41 first reported.** S132 made all
151 glossary entries the canon term card, no em dash; the body half of the same family was never
touched. The first count measured a PROXY - blocks missing `<strong>` - which conflated four
shapes. Against the separator itself: 41 already correct, **24 em-dash**, 10 carrying a literal
`KEY TERM:` prefix, 9 plain, 3 outliers. The 24 converted, L02 x11, L03 x7, L09 x6, and L03's
seven `<span id="term-*">` became `<strong id="term-*">` so the anchors survived. The other 22
shapes are UNRULED and parked, and the 3 L15 outliers carry real em dashes inside their
definition prose - a dash rule would eat punctuation.

**Every back-to-top link in the book is one spelling.** 237 links, `<p class="p-ta-right">`,
where five wrappers had been in use and L15 and L16's 28 rendered flush LEFT with no
`text-align` at all. Verified three ways that share no code: a markup census, a DOM ancestor
walk, and OCR of a render putting "Back" at x=890 on L14 against x=122 on L15. Three rules died
carrying two declarations each and **§27.15b did not fire, because all three were used by
back-to-top links and nothing else** - checked before deletion, not after.

**§1 has a rule and a gate (66).** Eight of sixteen lessons opened the hook with a real figure,
three had a placeholder, five had nothing. Five placeholders authored; the gate accepts a
placeholder by design, because a gate demanding a landed asset is a to-do list wearing a gate.
**Its predicate is the authored name `L##_IMAGE_`/`L##_GRAPHIC_`, not the directory** - the
first draft keyed on the path, counted every decorative mark as a figure and reported a false
16 of 16. Control-run three ways, including one where a mark is substituted for a placeholder.

**§2 has a rule and a gate (67).** Objectives exist, each is a box, and the list does not also
draw a bullet. L01 and L15 carried 13 objectives with no box at all. The lead-in normalised to
the ten-lesson majority - 15 of 16 now carry the comma, L16 keeps its own wording by ruling.

**`Learning Objectives` cost a taxonomy fix, and the fix found three copies of one rule.**
Renaming L02's heading re-familied it LEARN, because CANON matched a bare prefix and `LEARNING`
starts with `LEARN`. The prefix must now end on a WORD BOUNDARY - **measured inert first: across
all 1,119 live labels the result changes for zero of them.** The matcher was written out three
times, twice in `book_gates` and once in `family_tag`; all three now import the exported
`canon_of()`. The OBJECTIVES rule names the NOUN rather than the string start.
...
hold exactly one link and nothing else** - twelve words or fewer, zero code, zero prose. DJ:
*"they don't really help anyone catch up, they are just links to the project builder."* The
label promised the missing work and delivered a download button. All 77 now read
**Catch Up with Project Maker - <step>**, which names the tool and keeps the step so the
boxes stay tellable apart - L10 carries fifteen on one page, the S65 shape where six
identically-labelled TRY IT boxes became unusable. The body was NOT changed: it already
reads *Open Step 3 in Project Maker*. Parentheticals preserved, including L16's
*(will not build - that is the lesson)*.

**Reveal-label normalisation (post-push).** The fifteen challenge-card reveals under
THE TEMPLATE in L07-L09 took the canon wording "Check your work - open after you've tried",
retiring "Click to reveal a worked version" (9) and "Show answer" (6 of 12).
**Scoped by the nearest preceding heading, not by the string** - L08's six Mystery 5 blocks
say *Show answer* too and are a §4.5 sabotage construct, so a bare replace would have taken
them; they are untouched. The 21 *Compare with...* reveals are also untouched: each names its
file and eight say whether it is the complete file or only the new block, which the
eight-file lessons depend on. Canon wording 31 -> 46.
...
**`image_audit` v1.2 — the neighbour arm stops counting decorations.** The arm exists for a real case:
L10 prints *"this is the same photo you met in Lesson 5"* and wires L05's file directly, so a filename
mismatch there is not a missing figure. S130's 884 `<img data-mark>` marks landed in that same prose.
**Six figures were satisfied by `marks/` alone and by no figure asset at all**, verified per tag rather
than inferred from the count: L03 3.2 / 3.5 / 3.6, L12 12.1, L14 14.1, L16 16.1.

**THE PREDICATE IS THE ATTRIBUTE, NOT THE DIRECTORY.** Keyed on `images/marks/` the audit would read a
SPELLING and silently revert the day the files move. `data-mark` is authored — the whole reason S130
authored it. The two sets coincide at **884 today and that is measured**, and CONTROL F2 fails if the
predicate ever starts reading the path. Spans are computed over the WHOLE file, never over the window,
because an attribute can sit outside a window that contains the tag's `src=`.

**GATE 63 (§10) SHARES NO CODE WITH THE FIX.** Its independent arm re-derives landing from filenames on
disk with its own regexes and never asks `image_audit`, so it fails on whatever a future arc decorates
the prose with. Disagreements are a NAMED set with a reason each; arm 4 fails a name that has stopped
needing its exemption. Control-run four ways from a snapshot of the FIXED state, each firing gate 63
**ALONE** at 62 of 62 others green, untouched tree passing at both ends.

**RECORDED UNRULED:** L07's `[IMAGE 7.3]` is landed by `L07_GRAPHIC_7-15_platformio_file_tree.svg` — a
GRAPHIC standing in for an IMAGE across the two number spaces (§10), with no prose declaring the
substitution where L10's case says so in the lesson. Held by name in `REUSE`, marked UNRULED.

**THE GLOSSARY ARC: MEASURED, RULED, NOT STARTED.** The glossary is **151 entries in FIVE schemas** —
KEY TERM card (97, eleven lessons), bare div (14, L04), `<dl>` (15, L13/L14), table (25, L11/L15). The
97 cards plus **87 body KEY TERM callouts reconcile exactly to the family map's 184**. **The other 54
carry no `data-family` and no `data-callout`** and are invisible to `build_family_map`, `family_tag`,
`callout_id`, the pin and every gate reading through them — which directly threatens S128's ruled
additive glossary, since a harvest keyed on `data-family` would miss a third of the terms with nothing
firing. L12's ten cards additionally wear a blue `#2e86ab` callout with no key mark while still tagged
KEY TERM. **DJ ruled A** (normalize all 151 to the canon term card) **and no em dash**. The canon needs
restating first: its inline `style=`, its `<span>🔑</span>` and its `<strong id="term-...">` are
superseded by §27, S130 and the live tree respectively. **Measured for the conversion:** every callout
inside a glossary region is a KEY TERM, **97 of 97**, so a structural tier would resolve all 151 with
**zero new pins** — against 54 new pins if the conversion runs first. 87 of the pin's 159 KEY TERM rows
are glossary-side and would become redundant, which is a separate ruling and deliberately not folded in.

**A SETTLED QUESTION WAS RE-OPENED, AND THAT IS THE SESSION'S OTHER FINDING.** The per-lesson dual — a
§3 teaching KEY TERM callout and a §10 glossary restatement of the same term — is DESIGN, ruled at S128,
where the drift hypothesis was tested and failed. This session re-derived the duplication as a defect and
priced a ruling on it before DJ named the prior decision. **Re-derive a measurement; read the record for
a design.**
...
## WHAT SHIPPED IN S121

**THE FORWARD-POINTER ITEM DJ OPENED THIS SESSION IS CLOSED, AND THE MEASUREMENT THAT SETTLED IT
INVERTED THE QUESTION.** S120 recorded the census by CONSTRUCT and it was accurate: seven lessons
carry neither a *What's Next* heading nor a `Next:` paragraph. Re-measured by CONTENT, thirteen of
fifteen already named the next lesson in their final quarter. The practice was the norm; two lessons
were missing it. DJ ruled accordingly — *"keep the 2 new ones like the other 13"* — so this was two
authored pointers, not seven.

**WHAT WAS ACTUALLY BROKEN WAS SOMETHING NEITHER CENSUS ASKED ABOUT.** Not one lesson in the book
linked forward. Zero `<a href>` to a lesson file in any lesson tail, in sixteen files. Students have
been reading a bolded lesson name that looks exactly like a link for the life of the book.

**THE PLACEMENT DECISION IS THE PART WORTH KEEPING.** The obvious move — link the pointer prose
where it already sits — puts the door at 84% of the page, above Engineer's Log, above the bonus
block, above the quick reference. Engineer's Log feeds the TDP and the syllabus weights the TDP at
25%. A link there is an invitation to leave before the graded work, which is exactly §3.1's failure
scaled down from the book to the lesson. The block sits above the §5b footer instead.

**L11 AND L12 GOT REAL CONTENT, NOT FILLER.** Both hooks were written from the target lesson's own
§1: L12 opens with a robot on a slick floor whose encoders honestly report a 90° turn that never
happened, and L13 opens by counting the three jobs the line has done since Lesson 8 before taking it
away. Neither sentence was invented.

**AND THE CONTROL HARNESS FAILED BEFORE THE GATE DID.** Recorded in full above; the short version is
that `git checkout --` restores to the committed tree, the work was uncommitted, and three of four
controls therefore proved nothing while looking exactly like proof.

---

## WHAT SHIPPED IN S120

**THE FLOW ITEM DJ QUEUED AT S115 IS CLOSED, AND IT PRODUCED A §3 RULING RATHER THAN A PROSE
PATCH.** Three fixes were priced. Two were applied; the third — a Fall-terminal close in L13, where
the grid actually stops most students — was ruled OUT by DJ, and that ruling is the durable half.
The book has one ending, at 16, for anyone who reads it.

**L14 — *What's Next?* rewritten, and only that.** The *Final Word* and the 🏆 *Go Get 'Em!* box
were read in full and left untouched: a student at Lesson 14 has genuinely come that distance, and
a competition send-off is not a course close. What made L14 terminal was the menu beneath it —
*"After competition, you have options"* with the capstone listed third-of-three beside sumo and
maze solving. It now reads *"Two lessons remain"*, names L16 as the capstone, and demotes the
other-competitions line to a closing italic.

**L15 — the pointer it never had.** L15 was the only lesson of the three with no forward reference
of any kind: L13 names L14, L14 names L15, L15 named nothing, L16 ends the book. The new
`<p><strong>Next:</strong>` is seated on **L13's live placement** — after the *One sentence to
carry out of here* block, before Engineer's Log, unclassed, same markup — rather than invented.
Both of its numbers were verified against the files: L15's finished build is 28,034 bytes, L16
§4.2 audits that same figure, §16's ceiling is 28,672.

**THE GATE FINDING IS THE ONE TO CARRY.** §24's promise gate failed the rewrite —
`L14 -> L16: promises ['PID']` — because the new bullets had no terminal punctuation, so *full
PID* in the Lesson 15 bullet ran into the Lesson 16 sentence. **The original bullets had the
identical run-on.** They passed for their whole life because the third bullet, the one the ruling
required deleting, pushed the next period past the gate's 110-character window. The gate was never
protecting L14; a piece of prose that happened to sit there was. Fixed by punctuating the bullets,
never by widening the window.

**AND THE CALENDAR IS DELIBERATELY NOT IN THE BOOK.** `ZUMO_Teacher_Daily_Grid_WORKING.md` — newer
in the repo than the copy carried in the project files — already runs 23 periods, ends Fall at L13
Step 3, assigns only L13 §1–§4, and opens Winter mid-lesson at Step 4. It states the split is *"a
calendar seam, not a book seam."* §3.1 now says the same thing from the other side.

---

## WHAT SHIPPED IN S119

**L15 IS THE FOURTEENTH AND LAST CONVERSION.** BC01 had no ancestor of any kind — the first
reveal in the file is a §6 `catchup` — so five items were authored and §-ordered §3.2 → §3.3 →
§3.4 → §4.1 → §5.1, each citation read and verified to CONTAIN its answer. BC02 migrated §2's
seven objectives character-exact with the equality ASSERTED, supplying the literal ☐ because
L15's §2 carries no box glyph at all. BC03 migrated five and authored one. BC04 authored three.
The *What you built* narrative, the 🏆 one-sentence line, Engineer's Log #15 and the FINISHED
EARLY pointer all stayed OUTSIDE the family and reseated after BC04, on L10's arrangement.

**§7B WAS READ BEFORE IT WAS CITED** and it defines both K_u and T_u — including that the robot
hands over T_u = 2 ÷ WEAVE, because WEAVE counts sign-flips per second and one full oscillation
is two flips. A citation is verified by checking the cited section contains the answer.

**A CONTROL ASSERT WAS WRONG AND THE EDIT WAS RIGHT — S118's misfire 2, one session later.**
Seating the shared Brain Check column, the assert `back.count('brain-check-col')==1` failed. The
column carries the id twice by design, in the `<div>` and in the script's `getElementById` — and
L13 counts two as well. The belief was wrong, not the edit. Caught by checking L13 rather than by
editing the injection until the assert agreed.

---

## WHAT SHIPPED IN S117

**L12 IS THE TWELFTH CONVERSION, AND ITS EXIT TICKET PAID FOR THREE DIFFERENT BLOCKS.** Ten rungs
went four ways: one parked as a duplicate of §2's Objective 1, five folded into BC02's **I have…**
group, and **four reshaped into BC03 questions** per DJ's ruling. BC01 and BC04 had no ancestor at
all, so five items and three prompts were authored from the lesson's own §3 and §8A prose.

**THE RULING IS §25.10j AND IT NAMES A TEST, NOT A CASE.** *"A = the student ticks a box claiming
they know. B = the student is asked, and finds out whether they actually do."* DJ ruled **B**. The
generalisation is the **verb**: a capability claim belongs in BC02's *I can…* list, an observation
claim in its *I have…* group, and an *understand why* claim in BC03 — because a checkbox cannot
tell a student who knows from a student who is willing to tick. It is the first item ever moved
**into** BC03; L07 and L09 both moved one out.

**THE SCORER RULED ONE PAIR AND WAS NOT ARGUED WITH.** All 10 × 6 combinations scored: rung 1
against Objective 1 at **0.78**, next-best pair anywhere in the matrix **0.42**. Reading says
Objective 6's content lands on rungs 4 and 5, which score 0.27 and 0.21 — the instrument ranks
rung 2 there instead, on shared words. **The normaliser was not tuned to fix that**, the
disagreement is written down in `ZUMO_PARKED_EXIT_ITEMS.md`, and only the pair above the
separation was acted on.

**ACHIEVABILITY WAS MEASURED AGAINST §7's OWN PROSE, NOT ASSUMED.** §7's opening TIP already
supplies the fallback — *"No delrin? Hold the robot. Grip the chassis firmly and let the wheels
fight you."* — which covers 7A, 7B and 7C because all three are motors-off or turn-in-place.
**It cannot cover 7E**, which drives four 30 cm sides and four corners. That rung now sits behind
the Mark-done lock, its wording generalised from *on the delrin* to *on a slick floor*, which does
not remove the dependency. **The question is about floor access in the room, and it is unanswered.**

**TWO PROCESS FAILURES, BOTH MINE, BOTH KEPT.** The §24.14 control misfired three times before it
landed, twice printing `exit=0` against an untouched tree; the fix was a **disk read-back assert
before the gate runs**, not a resolution to be more careful. And the first Bible write truncated
the file to **0 bytes** when an encode error hit a `open(p,'w')` that had already opened —
recovered from the clone, and the durable half of the rule is now stated: **encode first, then
open, then `os.replace`.**
...
**GATE 43 — THE STYLESHEET IS NOW A SINGLE POINT OF FAILURE, AND NOTHING WAS GUARDING IT.** Found
on the last check of the session, three pushes in. A declaration used to live in 25,036 places
across sixteen lessons; it now lives once. **`build_css --check` cannot validate `css/book.css`**
— it rebuilds from lessons read through `expand_classes`, which reads the stylesheet, so damage
propagates into the comparison and the check reports *current*, exit 0. Measured: deleting one
`color: white;` left **all 42 gates green and `--check` clean** while the lesson strip's links
went dark-on-dark in all sixteen lessons. Gate 43 baselines 664 rules / 2,434 declarations / a
body digest, control-run against a dropped declaration, a changed hex and an injected rule.
**The baseline is meant to move** — §26's repaint moves it, as §21's moved 218→223.
**The general lesson: consolidation creates a single point of failure, and the instrument that
built the artefact cannot be the instrument that guards it.** `book_gates` **v1.37**.
...
depth from the page and was control-run against both regression shapes. `book_gates` **v1.36**, and NEW **gate 42** fails on the domain appearing anywhere in any page in any syntax — it exists because reverting one image `src` to absolute passed all 41 preceding gates, proved by seeding it.
...
converted book-wide in one pass after the push. Lesson bytes **2,638,947 → 2,582,947**; total
**3,534,934 → 2,582,947, 27% smaller**. Census unchanged at 39,994. Independent parser over the
whole population: 25,036 elements, **zero mismatches**, visible text identical in all sixteen.

**The hold was released by measurement, not by assumption.** Those blocks are compared byte-exact
ACROSS lessons and read through `expand_classes`, so converting one lesson at a time is exactly
what §6.5a/§25.6/§6.8 exist to catch. Two measured facts made a single pass safe: the 624
attributes carry only **16 distinct strings**, each appearing an exact multiple of 16 — they were
already uniform book-wide — and **all 16 round-trip byte-exact** through the stylesheet, which is
only true because §27.8c fixed declaration order and §27.8d fixed colon spacing. §4.5's bonus
banner was never held and its byte-exact gate passes for the same reason: the precedent was
already in the tree.

**`strip_inline` v1.1 — `--include-held`, gated on a precondition rather than a promise.** It runs
`roundtrips()` first and REFUSES, naming offenders, if any held string cannot be handed back
unchanged. CONTROL I proves both directions. `css/book.css` **unchanged** — every held string
already had a rule.


**The sweep. All sixteen lessons.** 24,412 inline attributes converted, **624 held** — 39 per
lesson, every lesson, no exceptions — and 24,412 + 624 = **25,036**, the §27 census exactly. Zero
unmapped, zero dead classes. Lesson bytes **3,534,934 → 2,638,947** (25%; 23% counting the 81,806 B
stylesheet once). `css/book.css` **167 → 664 rules**.

**Render identity proved twice, the second time without the gates.** By construction, as at S104:
every class carries declarations canonically equal to the string it replaced. Then independently:
**25,036 styled elements compared in document order** against the pre-conversion tree — declaration
sets identical, visible text identical apart from one `<link>` line per lesson. Census 39,979 →
**39,994**, which is exactly the fifteen new links.

**§27.8a — widening `SOURCES` renames rules, and the rename is mostly invisible.** Naming is
frequency-ranked across the corpus, so adding fifteen lessons reshuffled it: **57 of L01's 167
names changed meaning and 46 KEPT THEIR SPELLING.** `.link-c-2e86ab` and `.link-c-2e86ab-2`
literally swapped bold for non-bold. Gate 41 sees only the 11 that vanished — a name that still
resolves repaints the page with every gate green. Every converted lesson must be re-stripped
whenever `SOURCES` changes.

**§27.8b — the order is forced, because the wrong one destroys data.** `expand_classes` reads the
stylesheet from disk and, by design, leaves an unresolvable class **in place** rather than failing.
Regenerate before restoring and 74 L01 elements are stranded permanently, silent everywhere but
gate 41. **restore → regenerate → apply.** Learned by throwing away a sandbox that had already
alphabetised its own source.

**§27.8c — `canon()` sorts; the gates assert authored order.** §4.5 and §4.5a failed on the first
attempt, and the S104 hold list turned out to name four block types where there were **five** — the
bonus banner (cap, panel, nav pill). The fix was one line in the generator, `build_css.preferred()`
emitting authored order, not an open-ended hold list. **23,364 of 23,886** attributes now round-trip
byte-exact through the expander; the other 522 converge on a sibling spelling `canon()` had already
proved render-identical. Holding blocks until the gates go green is §24.8 wearing a hard hat.

**`strip_inline.py` v1.0 — the tool S104 did not commit.** Eight controls. It never invents a
class: an attribute with no rule is left alone and **reported**, exit 1. Held blocks located **by
marker, never by offset**. CONTROL H is load-bearing — marker-derived locators independently
reproduce S104's hand-picked 39 held attributes in L01, two unrelated processes at the same number.

**§27.8d — one colon spelling, ruled by DJ.** `preferred()` returns an authored string, and the
book authored 62,602 declarations as `prop: value` against 1,139 as `prop:value`, so a faithful
emitter shipped both into one generated file. DJ: *"I don't care either way, I just want it to be
consistent."* **Spaced** — 98% of the source, and the only spelling the gates accept: emitting
unspaced broke **five** (§22, §25.2, §25.10h, §4.5, §4.5a). Separator only; order still comes from
`preferred()`. The sixteen lessons were **byte-unchanged**, since class names derive from the
spacing-insensitive canonical form.

**A third control found expiring on success.** `session_versions`'s CONTROL A seeded its corruption
with the literal `Lesson version: v03.20.0`; the moment L03 bumped, nothing was seeded and nothing
surfaced. It **failed loudly rather than passing silently** — right direction, wrong reason. Seeds
are patterns now, with an assert that something was actually seeded. Its CONTROL E and G then caught
`strip_inline` being unregistered and unemitted, in that order.
...
**Nothing student-visible.** One file changed: `ZUMO_SUPER_BIBLE.md` **v8.86 → v8.87** — §26 un-parked and re-headed, new **§26.8** recording all nine rulings plus a seven-item still-open list, §18.2's retired gold-gradient star spec corrected to §9's bronze/parchment. The repaint itself is NOT started; §26.8's open list is the queue.
...
**7. Bible v8.82 → v8.83 — §17.3 NEW: PHOTOGRAPHS ARE NOT DRAWINGS.** §17 governed drawn graphics
only, which is *why* gate 37 was written in S97 forbidding embedded rasters outright: the book had
no written rule for the other kind of asset. The new section records the IMAGE/GRAPHIC split **by
subject**, the Photoshop → Illustrator route (DJ owns both; nobody should be hand-editing XML or
asking an AI to redraw a photograph), and the two export settings that **fail silently in
production** — **Embed, never Link**, because an `<img>`-loaded SVG runs in secure static mode and
cannot fetch an external file, so a linked photo is a **blank graphic on the site** while looking
perfect on the machine that exported it; and **fonts**, where live `<text>` renders with the
*viewer's* fonts and an uninstalled face shifts the layout — the same trap §18.2 answered with
...
**23,066 B → 1,148,110 B, a 50× growth, +1.13 MB on the published site, and all four passed 38/38**,
...
**1. `ZUMO_Syllabus_WORKING.md` — the seven calendar-independent sections** (`7436d17`). Battery
safety from Bible §13 plus the motor card's stall rule; the academic-honesty AI section from
`TEACHER_NOTE_AI_tools.md`'s documented injections (`setMotorPower()`, `set motorSpeed()`,
`@^1.3.0`) and L01 §3.1; the day-one checklist from the grid's Pd 1 and L01's own preflight;
notebook and submission from `ZUMO_TDP_Template_v3.md`. **`In the Lab` is the one section with no
repo source** — written from course structure and shop practice, and flagged as a proposal. The
file also carries the project's first `.md` version line (v1.0, two homes: HTML comment line 3 and
footer), which is a NEW convention for a root `.md` that nothing governs.

**2. `build_family_map.py` v1.0.1 → v1.1.1** (`1b86d2c`, `80721d7`). **v1.0.2 — the apostrophe root
cause:** `norm()` folds `\u2019` on the INPUT side and CANON is written straight, but the
glyph+scheme fallback map hardcoded its OUTPUT values curly, and map values never pass back
through `norm()`. The one header-less block resolving by scheme therefore landed in a bucket of
its own — 16 + 1 instead of 17, 31 families instead of 30. Control-run: only the two ENGINEER rows
and the family count moved. That block was L07:3645. **v1.1.0 — the 15 rulings encoded**, matched
on label PREFIX and never line number, so an edit above a block cannot silently break its ruling.
**v1.1.1 — reference renamed.**

**3. `ZUMO_S94_FAMILY_MAP.md` → `ZUMO_FAMILY_MAP.md`, regenerated** (`4693c2f`, `80721d7`). Records
all 15 rulings block-by-block with their basis, how the LEARN/INSIGHT trio was settled, the
struck-through paint items (item 1 resolved by S95's teal; item 3 down from 12 one-offs to 6), and
the instrument corrections.

### The verification worth copying

Five checks ran. **Only three did any work.** That the edit landed; that every family row matched
the document; and that only the nine ruled families moved, by exactly the ruled amounts. The
fourth — `assigned 1048/1048` — proved nothing, as the control run demonstrated. The fifth was the
control run itself, and without it the fourth would have looked like evidence.

### Phantoms, now six in three sessions

**L03 Coach's Tips closed with zero edits** — *setup() fires once at power-on* is live as a 💡 TIP
in L03, and *AI autocomplete injects wrong code* is live as a ⚠️ WARNING with the Command Palette
fix in L01. **The queue item named a retired component:** "Coach" appears four times book-wide,
none of them a callout family, and in L02 an HTML comment records that an INSIGHT callout replaced
the old "Coach's Note." **Standing rule: before scheduling a queue item that names a component,
verify the component is live.** The queue is now the least reliable document in the project.
...
**1. Three instrument carry-overs closed.** `build_mark_index.py` v1.0.0 → **v1.0.1** — default
output moved out of repo root, and `len(html)` was labelled "bytes" against a 34,617-byte file
(now `len(html.encode())`, and the two agree). `build_family_map.py` made self-contained and given
its first version string, then **v1.0.1**. `.gitignore` gained `__pycache__/`, `*.pyc`, and
`ZUMO_MARK_INDEX.html`.

**2. Six one-off callout schemes retired**, 51 → 46 distinct schemes. L03:1535 · L05:1339 ·
L01:1159 · L08:124 · L01:413 · L03:937. All byte-neutral hex-for-hex swaps.

**3. L01:413 pulled a gate with it.** Snapping `#ffb300` → `#ffc107` brought the block into gate
35's scope, so `seen` went 250 → 251 and the coverage assert fired — **control-run before the
bump, so the number is doing work.** Its merged label split into the canonical `⚠️ WARNING` label
plus a separate title line, copied from the L01:847/970 pattern.

**4. TEAL — INSIGHT left the shared blue. DJ ruling.** Hue 174 chosen from the four unoccupied hue
arcs in the live palette: the only candidate whose border-against-panel contrast (**2.62**) sits
inside the book's existing register of 2.46–3.57, and its derived mark fill `#1f7a71` clears
**4.68:1** against its own panel, so INSIGHT's mark can ship as a plain `<img src>` under the S94
derived-fill route. Bible **v8.81 → v8.82**: type 7 split into **7 Learn** / **7a Insight**, and
the *second* type-7 row — the title-colour table at line 1082, nearly missed — split the same way.
`build_family_map` v1.0.1 with the 🔍-on-blue key **replaced, not kept**: after the split, a
magnifier on LEARN's blue is a defect to surface, not a block to classify silently. Re-ran from a
clean clone: INSIGHT still 59, LEARN still 46, assigned still 1,033 — **identical totals, so no
block was lost in the move.**

**5. L14:159 fixed** — the line split so the callout opens its own line, and the `<strong>` title
replaced with the canonical block form **byte-copied from the L13 sibling**, with an assert that
the L13 form exists before using it. `book_gates` **v1.26.3**. Callouts not opening their own
line: **0**.

### The finding worth carrying forward

Where the Bible and `BookComponentStandard` describe the same thing, **they have not been diffed.**
The Bible's KEY TERM row names `#f3e5f5`/`#9c27b0`, live on **33** blocks, while the dominant KEY
TERM scheme is `#e7d4ff`/`#9b59b6` at **136**. A systematic diff of the two documents is
unglamorous and is probably the highest-value paint work left.

---

## WHAT SHIPPED IN S94 (naming finished; paint deliberately untouched)
...
**2. `lesson_inventory.py` v1.1.0 → v1.1.1.** Banner expectation corrected **2 → 1**, stale since
S89, while §5b and the gate have required exactly ONE the whole time. Sixteen identical false leads
had trained the eye to skip the block that also held the two real S93 regressions.
**Control-run first**: a real second banner was seeded into a throwaway copy and the check fired
(`2 visible banner(s), expected 1`) before the fix was applied to the live tree. The anomalies list
now holds only the Brain Check family-norm line — **so any new entry in it is a real lead.**

**3. `ZUMO_TDP_Template` v2 → v3, adding A5 Lab Log.** The syllabus grades **Outside Work at 5%** on
a log the template gave students nowhere to record — a real gap in a graded deliverable. A5 is
date · in · out · what I worked on, eight seeded rows, running total, plus an assembly-checklist
line. Four assert-guarded edits, all `count==1`, diff **+24/−3** with all three removals intentional,
all 8 Part B sections and A1–A4 untouched. **`ZUMO_TDP_Template_v2.md` deleted** — the filename
carries the version here, so v3 did not overwrite v2 and two student-facing templates briefly
coexisted, which is the `ZUMO_SUPER_BIBLE (1).md` shape.

**4. `ZUMO_S94_FAMILY_RULINGS.md`** — every ruling, who made it, and why.

### The entity lesson, this time against the analysis rather than the book

**`ENGINEER'S LOG` was reported as ZERO blocks. It has 17.** The labels carry `&rsquo;`, so the
matcher missed all of them. **§24.11 verbatim — an entity is not the character it encodes** —
committed eleven sessions after the same mistake shipped a live L12 regression.
**Any future label matching must `html.unescape()` twice and normalise `\u2019` → `'` before
comparing.** `build_family_map.py` does; anything new must too.

Corrections that follow from it: **the "5 collisions / 121 repaints" figures are VOID** and must be
recomputed · **`WHAT YOU NEED` is 13 blocks, not 6**, across four schemes and two glyphs ·
**"104 blocks remaining" understated the job** — 420 blocks lacked a family label; the 104 were only
those on schemes where *nothing at all* matched.
...
1. **L03/L08/L09/L10 `finished`-payload debt.** All four lessons **have** a `finished` payload, every
   ...
   records **DJ's own S49 ruling that C01–C06 stay finished-preload.** Not a defect. The open design
   question — whole-template starters per §18.3 — means **authoring ~24 new payloads.**
2. **L03 "1000 ms = 1 second" explainer** — already written and live.
3. **L03 modulo explainer** — already written and live.

**Before doing any queue item, verify it still exists. Three of them did not.**

### One anomaly logged, not chased

**L14 line 159's `THE ONE IDEA` block extracted empty** while its L13 sibling read fine. Either the
block is malformed or the extractor lost it — worth a look either way.

---

## WHAT SHIPPED IN THE S93 SECOND BATCH (the triple-check found two live regressions)

**A push-verification pass on `b0489a0` confirmed the first batch landed byte-identical and all 35
gates passing — and then found two real defects that every gate had passed over for two sessions.**
Both were printed by `lesson_inventory --anomalies` the whole time.

**1. L12 — unescaped `<Wire.h>`, a regression caused by S92's own sweep.** Lines 594 and 774 read
`<Wire.h>` raw, so the browser tokenised it as an unknown element and the sentence rendered **"The
#include  goes at the TOP of the file."** with the filename gone. Provenance: **escaped at S87, S89,
S91 and `03d1e85`; raw from `514588e`, the S92 Option C push.** This is v8.67's defect — same lesson,
same sentence — re-introduced eleven sessions later, and caused by that sweep's own approved
principle of *authoring the string literally*. **An entity is not the character it encodes.**

**2. L04 line 1363 — crossed tags from a different sweep.** The block-form title conversion at
`53a44b6` (S91) replaced the **inner** `</strong>` with `</div>` instead of the outer one:
`<div …>&lt;/&gt; The <strong>deadband</div> — a band where the robot does nothing</strong>`. The
subtitle rendered outside its own title element and unbolded. Restored to
`<strong>deadband</strong> … </div>`, keeping the S92 block form.

**The class was swept, not the instance.** Rather than grepping for `<Wire.h>`, every page was parsed
and every start-tag whose name is not a real HTML element was reported. Across all 21 pages: **2 hits
before, 0 after.** That method is now canon as **§24.11**.

**The stale expectation that hid them.** The same anomalies list prints `1 visible banner(s),
expected 2` for **all sixteen** lessons, because `lesson_inventory` still carries the pre-S89
two-banner rule while §5b and the gate have required exactly ONE since the build banner was deleted.
Sixteen identical false leads trained the eye to skip the block that also held the two real ones.
**A uniform anomaly across every file is a lead about the instrument, not the book.** Logged for
correction, deliberately not corrected mid-verification.

**Also re-verified in this pass, since it had been asserted with grep:** *"zero references to
`images/icons/` or `images/marks/` in any page"* was re-established by parsing every attribute of
every element across all 21 pages, with a control proving the parser sees references at all — it
found **196 `<img src>`** and **0** to either directory. The claim holds, now on evidence that could
have failed. The contrast arithmetic (0.02 / 6.00 / 13.37) and every measured block count (1,048
callouts · 🔬 7 · 🛑 19 · WALL 18 · red 21 · SEE-scheme 51 · 👀 28) were recomputed against the
pushed tree and all match.

**Applied, both MODERATE** (rendering changes, so the one visible §5b banner moves with the hidden
comment): **L04 v04.13.0 → v04.14.0** · **L12 v01.13.0 → v01.14.0**. Bible **v8.80 → v8.81** (§24.11).
Census **unchanged at 39,970** — both fixes add characters, not lines, which is itself the check that
they were surgical.

---

## WHAT SHIPPED THIS BATCH (S93 — one family, by promotion; and a conflict written down instead of ruled on)

**1. `BookComponentStandard` v01.9.0 → v01.10.0 — GOING DEEPER is the 26th family, bronze.**
Seven edits, diff-audited against a control copy with nothing else moved: §7's headline count
(`Twenty-five` → `Twenty-six`, mark count unchanged at 48), the table row
`| GOING DEEPER | file-earmark-plus | bronze |` seated in the bronze group after INSIGHT, removal of
`file-earmark-plus going deeper` from §7.2's Systems list (8 → 7), a §7.2 paragraph recording the
promotion, §7.3's inventory (`26 for the 25 families … 22 supporting` → `27 for the 26 families … 21
supporting`), and both version homes (line 3 full, footer stamp MAJOR.MINOR per §1).

**It is a promotion, not an invention.** §7.2 already ruled on this move for ENGINEER'S LOG: *"A
supporting mark promoted to a family leaves the supporting list… the nav entry CITES it under §4.4
rather than holding a second copy of the glyph."* The Going Deeper page and the seven 🔬 hooks name one
destination, so **zero new icon cost** — `file-earmark-plus` was already in `images/icons/`.

**Role is bronze by DJ ruling.** The seven blocks had been wearing NOTE's slate `#eceff1`/`#607d8b`,
borrowed; bronze is the depth role (KEY TERM · GLOSSARY · INSIGHT) and INSIGHT is the *door* that S90
contrasted with THE WALL. The generated mark carries **`#725637`**, byte-identical to `stars.svg`, its
bronze sibling — bronze's contrast-corrected title, not the palette hex, per §5.0.

**2. The count gate control-ran, failing first.** Because `gen_component.py` line 8 states *"Everything
below is PARSED from BookComponentStandard.md,"* the row change moved `file-earmark-plus` from deferred
to shipping. Before regeneration the selftest **FAILED** with `marks/ is byte-identical to a fresh
generation (1 would change)` and `ships` moved 40 → 41. `--emit-marks` then produced **exactly one**
file: `diff -rq` against the control reports `Only in images/marks: file-earmark-plus.svg`. §7.3
arithmetic now asserts `27 + 21 == 48`; §1 asserts every stamp agrees at v01.10.

**3. The S91 roster paragraph is a LIVE count, and saying otherwise was this session's own error.**
The amendment first written into §7 claimed the S91 record was *"left as written… it states what was
approved at S91."* Archaeology disproved it: **S91 approved twenty-six families and forty-nine marks**,
and when SAFETY was retired later the same day (`c7d6f08`) the paragraph was **edited in place** to
twenty-five. It had never held S91's number. Corrected in the same session it was written; the
paragraph now carries twenty-six and says plainly that it is maintained, not frozen. Recorded as
**§26.7 — a record that has been edited in place is not a record.**

**4. §26 NEW — the palette conflict, parked.** See the Status line above and Bible §26. The one-line
summary: two canon documents define Heritage Blue with five disagreeing hexes, four tests favour the
upstream RoboLore set, the standard's own locked §9 already uses it, `gen_component` cannot see §9, the
semantic three turns out to be **approved elsewhere without purple**, and **none of it renders**, so it
is recorded rather than decided. **Do not rule on it until RoboLore is committed and its approved
instructional standard sits at its own canonical path.**

**5. The Bible was recovered.** Commit `3664bf8` correctly deleted the stray `ZUMO_SUPER_BIBLE (1).md`
that S92 had logged as housekeeping — but the same commit overwrote the live Bible with that stray's
header, regressing the version line **v8.79.1 → v8.78** and deleting the entire **v8.79 (S92 Option C)**
changelog entry. Recovered byte-exact from `ae31126` before v8.80 was written, so the S92 record is
intact. **A duplicate of a canon file is not inert — deleting it and overwriting from it are one hand
movement apart.**
...
Harbor `#7E95B4` · Steel `#708BAF` (S91, was `#6985AB`), light to dark, Deep Navy text throughout, no gradients, no
