...
**Date:** August 7, 2026 (Session 130 — **THE MARKS ARC LANDS, AND THE SECOND DECORATION-KEYED TIER DIES.** 884 leading emoji become `<img data-mark>` marks; §21's coverage lands at **1,134**, derived exactly as 250 + 884. The arc then exposed a price nobody had priced: `build_family_map` returned 1069/1069 and looked clean, while `family_tag` — the AUDITOR, which resolves from CONTENT so it is not checking the attribute against itself — reported **208 unnamed**. **The S129 acceptance was run against the one consumer that structurally could not fail.** DJ pushed back on the first recommendation — *"Shouldn't we verify them then there is no chance of loss?"* — and was right: *re-derivable from content* and *protected from silent change* had been collapsed into one word, and only the first is gone. All 212 verified against the pre-swap glyph with **zero mismatches**, then pinned. **§20.5 and §24.14c NEW; gate 60 new; the GLYPH tier is DELETED.**)
**Status:** ✅ **60 GATES PASS · THREE PROCESS FAILURES, ALL MINE, EACH CAUGHT BY AN ASSERT AND NONE BY A GATE.** **THE ONE TO CARRY: a stopgap that resolves real blocks is not a stopgap, it is a dependency.** S112 deleted a COLOUR-keyed tier and replaced it with a GLYPH table; S130 deleted the GLYPH table. Same failure, one decoration later, and the second was caused by the fix for the first. **A library that exits is the worst shape here:** `load_pins()` first raised `SystemExit`, and because `build_family_map` is imported by the suite, a missing pin **killed the run mid-suite and exited 0** — stopping before two gates and reporting success by the very exit code the ritual says to trust. **A control corrupted the pin file with no restore path** — untracked, so `git checkout` silently did nothing. **The first pin generator read the POST-swap tree**, emptying the retired-glyph column that was the record's whole point. **Three DERIVED identity keys failed** (87/212, 199/212, 14/193) with zero family disagreements in all three — the families were never in doubt, the IDENTITY was, and that is why `data-callout` is authored. **S130 ADDENDUM — 61 GATES.** The carried mark debt was wrong in BOTH directions: a reconciliation scoped to §7.1 called it false and listed **14 orphans**, one ruling from DELETING eleven correctly-indexed files, because **the standard has two tables and the instrument knew one**. Re-run against both: zero unindexed, and the seven ARE absent — but they are §7.2's SYSTEMS group, which **§7.2's own Grounds table already rules *"in scope: no"***. Two sessions had carried a resolved question as an open one. `build_mark_index` **v1.1.0** parses §7.2 instead of restating it, empty-loud; **gate 61** holds the seven BY NAME so the hold expires when any one lands. `glyph_scan` **v1.1** — its post-run line named an arc that has now RUN, and its own detector then caught **gate 60 still reading the glyph**, accepted after a behavioural proof that blanking it changes 0 of 1,069 resolutions. **S130 ADDENDUM 2 — 62 GATES.** A double check re-derived five load-bearing numbers by methods sharing no code with the originals; all five reproduced. Two figures differed and both were DEFINITIONAL, not errors — and one was a real defect underneath: **the census computed `count(newline) + 1` unconditionally**, counting the empty string after a trailing newline as a line, so it ran **one high per file, 16 high book-wide**, and `wc -l` had disagreed for its whole life. DJ: *"Why do we need it. Can't we fix the problem?"* — **a number needing a footnote is a defect, not a convention.** L03 normalised so all sixteen end in a newline; census reaches **40,683**. **Gate 62** is the second method, counting newlines without importing the parser. **§24.6c gains part 5** on the session's own catalogue: **nine of nine wrong claims came from a one-off code path, zero from a versioned instrument**, and the only two that reached DJ were the two with no assert at all. **§12.6 now records the census definition change** (Bible **v8.122.1**): every figure before S130 runs high by exactly the LESSON COUNT, an offset that is derived rather than magic. Recorded instead of retro-edited, because a reader comparing 40,698 with 40,683 computes a phantom −15 and hunts fifteen lines that never existed — S69 lost a session to that exact shape. Seated in §12.6 because **gate 62 is labelled §12.6**, so the investigator lands there. **Gate 62's trailing-newline arm deliberately stays at the sixteen lessons** — all five non-lesson pages were measured and already comply, so extending it would assert something already true on files outside the census.

**Versions:** L01 v03.24.0 · L02 v03.17.0 · L03 v03.36.1 · L04 v04.24.0 · L05 v04.24.0 · L06 v04.28.0 · L07 v04.27.0 · L08 v04.26.0 · L09 v05.21.0 · L10 v02.22.0 · L11 v02.23.0 · L12 v01.26.0 · L13 v02.23.0 · L14 v02.28.0 · L15 v02.24.0 · L16 v02.17.0 · going_deeper v01.6.1 — census **40,683** · Bible **v8.122.1** · BookComponentStandard v01.12.1 · gen_component v1.6.1 · Maker v2.45.4 · **book_gates v1.57** · lesson_inventory v1.3.4 · pill_sweep v1.1 · gate_payload_match v1.7 · build_family_map v1.4.0 · callout_id v1.0 · build_mark_index v1.1.0 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.22.0 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.3.0 · image_audit v1.1 · strip_inline v1.2 · build_worklist v1.1 · regex_audit v1.0 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · entity_sweep v1.0 · font_stack_sweep v1.3.0 · next_pointer v1.2 · family_tag v1.1 · mark_wire v1.0.2 · glyph_scan v1.1 · title_feed v1.0 · Timer v1.3.2 · `ZUMO_Syllabus_WORKING.md` v1.2 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `fe1f2b5`.**
