...
**Date:** August 14, 2026 (Session 155 — **A TRIPLE CHECK THAT VARIES THE MECHANISM AND NOT THE PREDICATE IS A SINGLE CHECK RUN THREE TIMES.** §16.25 priced the A-Star fix at 13 / 8 / 1 / ~1 and every figure was `grep -c` output, which counts matching LINES. Corrected to 15 / 10 / 2 / 2. The re-derivation then failed a second time with three independent mechanisms agreeing on a wrong widened pattern, and reading the hits is what settled it.)
**Status:** ✅ **S155 AT 73/73 GATES — A DOC-ONLY CORRECTION SESSION.** No lesson file changed, no instrument changed, no gate changed. **§16.25's own scope figures were LINE counts reported as occurrence counts** — rule 50 committed inside the section that cites it, in a ruling whose subject is *measure it, do not re-litigate it*. Verified **15 `Lesson_01.html` · 10 `Lesson_03.html` · 2 `newproject.html` · 2 pre-existing Bible · ZERO quiz banks**. **THE SECOND ERROR IS THE REUSABLE ONE:** the triple check ran `grep -o | wc -l`, `re.finditer` over raw bytes and `len(re.split()) - 1` — three genuinely independent mechanisms — and **all three agreed at 15 / 16 / 54 and all three were wrong**, because the widened pattern `a[-_ ]?star` matches the phrase *"a start line"* 36 times in `newproject.html`. **Three methods sharing a predicate are one method** (rule 83; rule 79 applied to a regex). §24.13 is satisfied only when the PREDICATE varies. Reading the hits replaced the count with an **inventory**, which is what the fix pass actually needs: six of L01's fifteen are the CORRECT build target, two are element ids, and two of L03's ten are a photograph's filename — **a pass scoped by the total would have edited things that are right.** Two findings no count could surface: **the lessons and Maker ship `a-star32u4` with a LOWERCASE u in six places while the Bible writes `a-star32U4` twelve times and §16.25 declares uppercase canonical** — the book contradicts its own canon, and normalising is a RULING since PlatformIO tolerates both; and **L03's photograph is named `L03_IMAGE_3-14_astar_board.jpg`** for a board the robot does not contain. **AND §11 CARRIES ONE OF THE WRONG CLAIMS ITSELF** — *"A-Star32U4 capitalization for the microcontroller"*, where the microcontroller is the ATmega32U4: §16.25 contradicts a line in its own file. Bible **v8.145.1**; handoff renumbered **S155 → S156**, caught by `session_versions --check` rather than by hand. **S155 ADDENDUM — DJ RULING: NORMALISE THE PlatformIO BOARD ID TO UPPERCASE U.** All six shipped sites changed: 4 in `Lesson_01.html` (two code blocks, each carrying `[env:...]` and `board = ...`) and 2 in `newproject.html`'s `var INI` string, which writes the `platformio.ini` for **every project every student downloads**. The two agreed with each other before and agree after — **the disagreement was between what ships and what canon says**, since the Bible writes `a-star32U4` seventeen times, §16.8 cites `boards/a-star32U4.json`, `pio_harness.sh` agrees, and §16.25 declares uppercase canonical. **AND THEN MEASURED RATHER THAN ASSUMED (§24.15): PlatformIO's board ID is CASE-INSENSITIVE on DJ's machine, August 2026 — `a-star32u4` and `a-star32U4` BOTH BUILD.** Instrument: two real builds in VS Code, lowercase by screenshot and uppercase reported. One machine, one PlatformIO version — one instrument, not a universal property, but the one the class actually uses. **The test was worth running because nothing in the tree could have answered it:** PlatformIO is not installed on the authoring box, no `a-star32*.json` exists, `docs.platformio.org` is unreachable, and `pio_harness.sh` calls `avr-gcc` directly with `-mmcu=atmega32u4` — **so the 20,516-byte control cannot witness this and never could.** Pololu's PRODUCT NAME and PlatformIO's BOARD ID are different namespaces and the ruling reasoned from the first to the second; had it gone the other way it would have broken the first string every student pastes in L01 §4. **Cost of the test: one build. Cost of skipping it: a launch-day failure no instrument could have seen coming.** `Lesson_01` **v03.28.5** · Maker **v2.51**; 73/73 gates and `gate_payload_match` PASS.

**Versions:** L01 v03.28.5 · L02 v03.21.3 · L03 v03.41.1 · L04 v04.29.1 · L05 v04.29.0 · L06 v04.32.1 · L07 v04.31.4 · L08 v04.31.1 · L09 v05.27.0 · L10 v02.29.2 · L11 v02.30.0 · L12 v01.31.3 · L13 v02.29.0 · L14 v02.34.0 · L15 v02.31.2 · L16 v02.24.0 · going_deeper v01.6.1 — census **40,668** · Bible **v8.145.1** · BookComponentStandard v01.13.0 · gen_component v1.6.1 · Maker v2.51 · **book_gates v1.68.1** · lesson_inventory v1.3.5 · pill_sweep v1.1 · gate_payload_match v1.8.0 · build_family_map v1.6.4 · callout_id v1.0 · keyterm_prefix v1.0.1 · build_mark_index v1.1.0 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.26.0 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.3.0 · image_audit v1.2 · strip_inline v1.2 · build_worklist v1.1 · regex_audit v1.0 · byte_audit v1.2 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · entity_sweep v1.0 · font_stack_sweep v1.3.0 · next_pointer v1.2 · family_tag v1.2.1 · glossary_convert v1.0 · mark_wire v1.0.2 · glyph_scan v1.1 · title_feed v1.0 · quiz_bank v1.0.1 · Timer v1.3.2 · `ZUMO_Syllabus_WORKING.md` v1.2 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `d99c7df`.**
