...
**Date:** August 4, 2026 (Session 117 — **A CONTROL THAT DIES BEFORE IT WRITES IS A CONTROL THAT NEVER RAN, AND KNOWING THAT DOES NOT STOP IT HAPPENING.** S116 canonized exactly this rule and S117 broke it **three more times in one sitting, on the same gate**: the §24.14 deletion control used a regex that matched nothing, then a class absent from L04, then a class the family map does not count as a callout at all — and on two of the three the script raised, the shell moved on, and `book_gates` printed **`exit=0` against a completely untouched tree**, a zero indistinguishable from a real pass. **What fixed it was not remembering the rule but a line of code:** read the file back off disk and assert the count moved *before* the gate is allowed to run. Attempt four did that and fired correctly at 1060/1061. **AND A SECOND FAILURE THAT DESTROYED A FILE: an encode error inside `open(p,'w')` truncates to zero.** The first draft of this session's Bible entry carried an unpaired surrogate escape; `open()` truncated `ZUMO_SUPER_BIBLE.md`, *then* `.write()` raised, and the Bible was **0 bytes**. Recovered in one command from the clone — which is the only reason it was a footnote and not a session. §12 has required atomic temp-file + `os.replace` for a long time; the missing half is the order *inside* the write: **encode the whole blob first, then open**, and prose carrying emoji goes in a heredoc file, never a Python string literal. **L12 IS THE TWELFTH CONVERSION, AND §25.10j IS NEW: A CHECKLIST RUNG THAT ASKS A *WHY* IS A KNOWLEDGE-CHECK ITEM WEARING A CHECKBOX.** L12's ten-rung *Exit Ticket* held four rungs no §2 objective covers and that are not bench observations either, each a knowledge claim carrying its own §-citation and each with nowhere legal to go — §25.5 makes BC02's **I can…** group §2's objectives character-exact, and a third labelled group would be a construct invented to hold four items. **DJ ruled B of two options** — *A, the student ticks a box claiming they know; B, the student is asked, and finds out whether they actually do* — so the four became BC03 questions, the rung supplying the stem and the citation and the session authoring the interrogative form and the answer. **The test is the VERB, not the tense:** capability → BC02 *I can…*, observation → BC02 *I have…*, *understand why* → BC03. It is the first move **into** BC03 where L07 and L09 both went the other way, and it matters because L12's BC03 had **no ancestor at all** — four items authored from nothing became four authored from four stems the lesson wrote itself. **§25.10i APPLIED AND RULED ONLY ONE PAIR:** a scored diff of all **10 × 6** combinations separates on exactly one, rung 1 at **0.78** against Objective 1 with the next-best pair in the whole matrix at **0.42**. **Unlike L11, none of L12's past-tense rungs is a duplicate** — rungs 2, 3, 4, 6 and 10 are bench observations and outcome claims that folded into **I have…** with no ruling needed, so S116's open question stays open and now depends entirely on L13/L15. **AN INSTRUMENT-VS-READING DISAGREEMENT IS RECORDED AND DELIBERATELY NOT RESOLVED:** the scorer ranks rung 2 against Objective 6 highest of the keepers at 0.42 on shared words, while reading says Objective 6's content lands on rungs 4 and 5 at 0.27 and 0.21 — **the normaliser was not tuned to produce that pairing**, and only the pair above the separation was acted on. BC01 had no ancestor either (0 pre-§6 `check`, 0 `quiz`, 0 TRY IT; all thirteen pre-§9 reveals are `catchup`), so five items authored and §-ordered §3.1→§3.2→§3.3→§3.4→§3.5 with every citation read and verified to CONTAIN its answer; BC02 migrates §2's six objectives character-exact with the literal ☐; BC04 is three authored prompts with no reveals, the Engineer's Log surviving as its own block and the 🏆 *one sentence* plus *Next lesson* prose kept OUTSIDE the family on L10's arrangement. **THREE BASELINES MOVED, EACH CONTROLLED BOTH WAYS:** §27.11 to **643 rules / 2,357 declarations**, diffed by SELECTOR — exactly one gone (`.div-ddd-3`, the consumed checklist box, 5 declarations, and the −5 IS that rule), **zero born, zero altered, so no class RENAME**; §21 coverage 235 → **240**, sole delta `BrainGear_Incomplete.png` 55 → 60; family map 1057 → **1061**, exactly ONE family moving, BRAIN CHECK 44 → 48, the other 29 byte-identical. After the moves a deleted `color: white;` still FAILS, a deleted callout FAILS naming **1060/1061**, a broken image ref FAILS, and leaving `'12'` in `BC_PENDING` FAILS **naming L12**. **Only two files differ from the pushed clone** — `Lesson_12.html` and `css/book.css` — the other fifteen lessons returning byte-identical through restore → regenerate → apply, measured and not assumed. **OPEN FOR DJ, and it is a fact about the room rather than the book:** BC02's folded rung *"Run the Lesson 6 square on a slick floor with both buttons"* sits behind the Mark-done lock, and §7E cannot use §7's no-materials fallback because you cannot hold the chassis while the robot drives four 30 cm sides. `book_gates` **v1.44.3**, `build_family_map` **v1.3.5**. Applied S117: L12 **v01.20.0** (moderate — the page renders differently, so both §5b homes move). Census **40,579**. Bible **v8.106**.)
**Status:** ✅ **48/48 GATES PASS · L12 IS THE TWELFTH CONVERSION, AND TWO LESSONS REMAIN (L13, L15).** **§25.10j is new canon** per DJ ruling: a checklist rung that claims to *understand why* is a Knowledge Check item wearing a checkbox, and the test is the **verb, not the tense**. **ANSWERED, AND IT OPENED SOMETHING LARGER:** DJ — *"We don't use delrin sheets. It's melamine (I think) and we have tons of tiles sitting in the lab."* Access is not rationed, so the folded 7E rung is earnable and **nothing comes out of the locked list**. **But the book names a material the lab does not stock** — 17 times in L12, twice more inside `newproject.html`'s 7D and 7E payloads, which `gate_payload_match` byte-compares. **DJ stopped the rename and was right to:** *"I think originally I said we could use delrin to get a slick surface, but it's prob not much slicker than the melamine."* Delrin was chosen **because it is slick**; if the lab tile is not meaningfully slicker than the floor, renaming the material only writes the wrong physics down more accurately. **The exposure is 7E and only 7E** — §7's hands fallback covers 7C and 7D, but nobody holds a robot through four 30 cm sides, and 7E's payoff, GRAPHIC 12.3, §8A.1 and the locked BC02 rung all rest on the surface genuinely slipping. **If it does not, 7E fails QUIETLY**, with both squares coming home and the climax reading as a claim the robot never demonstrated. **It is a BENCH item, not a decision:** one tile, one robot, load 7E, press A. Nothing renamed until that runs — a rename done first has to be done again. **ALSO OPEN, CARRIED FROM S116 AND NOW NEARLY MOOT:** the past-tense-rung question did **not** recur on L12, and **neither L13 nor L15 carries a checklist at all** — so decide it on those two or retire it deliberately. **BOTH REMAINING CONVERSIONS ARE ALL-AUTHORING:** no checklist ancestor, no Knowledge Check, narrative §10s, so expect all four Brain Check blocks written from nothing on each.

**Versions:** L01 v03.19.2 · L02 v03.11.2 · L03 v03.28.1 · L04 v04.19.2 · L05 v04.19.2 · L06 v04.23.2 · L07 v04.22.1 · L08 v04.19.2 · L09 v05.16.2 · L10 v02.17.0 · L11 v02.18.0 · L12 v01.20.0 · L13 v02.17.1 · L14 v02.22.0 · L15 v02.17.1 · L16 v02.12.1 · going_deeper v01.4.1 — census **40,579** · Bible **v8.106** · BookComponentStandard v01.12.1 · gen_component v1.6.1 · Maker v2.45.2 · **book_gates v1.44.3** · lesson_inventory v1.2.0 · pill_sweep v1.1 · gate_payload_match v1.7 · build_family_map v1.3.5 · build_mark_index v1.0.2 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.17.0 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.2.1 · image_audit v1.1 · strip_inline v1.1 · build_worklist v1.1 · regex_audit v1.0 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · font_stack_sweep v1.2.0 · `ZUMO_Syllabus_WORKING.md` v1.2 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `b307865`.**
