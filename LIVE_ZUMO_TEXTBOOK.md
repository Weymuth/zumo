...
**Date:** August 7, 2026 (Session 126, second push — **§27.15e: THE DARK CODE BLOCK GRADUATES, AND A BORDER 398 BLOCKS CARRIED FOR TWO YEARS TURNED OUT TO BE INVISIBLE.** 802 `<pre>` elements, fourteen class names, and the fourteen disagreed. Two declarations were a real split; **DJ's eye resolved the first one** — shown four rendered specimens he said they all looked the same, and the measurement agrees: `#333` on `#1e1e1e` is **1.32:1** against a 3:1 minimum. The 57/42 border split was never a preference, it was **the L09/L10 seam** (L02–L09 bordered 365 of 372; L10–L16 bordered 29 of 281) — the aggregate was a true number telling the wrong story. **THE FIRST VALUE PROPOSED WAS WRONG AND ITS OWN GATE CAUGHT IT:** `#686868` was computed by hand as "3:1" and is **2.992:1**. Gate 57 asserts the RATIO, not the spelling, so it failed before the push; **`#696969`** is the lowest grey that genuinely clears 3:1.)
**Status:** ✅ **57 GATES PASS · ONE DARK CODE BLOCK, ONE RULE, NO EXCEPTION LIST.** The 30 light blocks need no opt-out — their class beats the element rule on every property it sets, checked not assumed — and **the 109 blocks sitting inside a dark wrapper `<div>` in L02/L03 were UNWRAPPED rather than excepted**, because an opt-out is an exception someone must remember forever. All 109 held exactly one `<pre>`, verified across all **eight** wrapper spellings — a population first sampled at ONE and reported as two, §24.8 again. **§27.15b FIRED FOR REAL:** 176 `<pre>` joined `.p-m-0`'s 120 users, `<pre>` became dominant, the rule renamed to `.pre-m-0`, and **119 `<p>` plus one `<ul>` went dead** — caught by `strip_inline --verify`, invisible to every gate. Tree **REVERTED to the pushed clone and re-derived**, not patched forward, with survivors mapped by **identical declarations** rather than name arithmetic. **THE CONTROL HARNESS THEN COMMITTED S121's ERROR AGAIN:** it snapshotted the stylesheets but not the lesson file an injection touched, and the cleanup ran `git checkout --` on `Lesson_05.html` — which reverts to HEAD, and none of this was committed, so it silently destroyed that lesson's share of the build. **The untouched-tree control exposed it by failing** — the one control whose only job is to have nothing wrong with it. Redone from a full-tree snapshot, six ways, each firing gate 57 **ALONE** once the stylesheet regenerates, including on `#686868`. **§27.11 → 604 / 2,141 and reconciles exactly:** 23 rules die carrying 148 declarations, 3 born carrying 3, one altered gaining 4; −148 + 3 + 4 = −141. **PROOF NOTHING ELSE MOVED:** every lesson expanded against its own stylesheet before and after, and every changed line is a `<pre>`, a removed wrapper `<div>`, or whitespace — **ZERO others across all sixteen.** Census 41,128 → **40,700**, and the 428-line fall IS the unwrap.

**Versions:** L01 v03.23.0 · L02 v03.15.0 · L03 v03.33.0 · L04 v04.23.0 · L05 v04.22.0 · L06 v04.27.0 · L07 v04.26.0 · L08 v04.23.0 · L09 v05.20.0 · L10 v02.21.0 · L11 v02.22.0 · L12 v01.24.0 · L13 v02.22.0 · L14 v02.27.0 · L15 v02.23.0 · L16 v02.15.0 · going_deeper v01.6.0 — census **40,700** · Bible **v8.118** · BookComponentStandard v01.12.1 · gen_component v1.6.1 · Maker v2.45.3 · **book_gates v1.53** · lesson_inventory v1.3.0 · pill_sweep v1.1 · gate_payload_match v1.7 · build_family_map v1.3.7 · build_mark_index v1.0.2 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.19.0 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.3.0 · image_audit v1.1 · strip_inline v1.1 · build_worklist v1.1 · regex_audit v1.0 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · font_stack_sweep v1.3.0 · next_pointer v1.1 · title_feed v1.0 · Timer v1.3.1 · `ZUMO_Syllabus_WORKING.md` v1.2 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `791dd51`.**
