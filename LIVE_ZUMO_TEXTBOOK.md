...
**Date:** August 7, 2026 (Session 127 — **§27.16: ONE SPELLING PER CHARACTER, AND THE CENSUS DISCREPANCY WAS A MEASURING INSTRUMENT, NOT A DISAGREEMENT.** The book spelled the same character two ways — **5,935 non-ASCII characters literal, 4,827 as entities** — so every byte-reading instrument counted a fraction of each population. S126's *95 / 3,258* is reproducible at exactly one scope out of eight and the 15-occurrence gap **IS `≈` U+2248**; but that scope is the defect, and the true figure decoded is **110 distinct / 3,984**. **34 characters were written BOTH ways and 15 existed ONLY as entities** — 🏠 © 🕵 ⭐ 🎓 and the three medals — appearing in **no census ever taken of this book**, which is why every glyph count in the handoff was low (💡 114 is 118, 🔑 158 is 174). **The all-entity direction was considered and fails on its own terms:** entity form is three spellings, the book already carries 14 characters spelled two ways inside it, and 103 of 143 characters have no name at all. **The ruling is a property: write it however you can SEE it in the file** — literal for 4,789, an entity for the **239** whose literal form is invisible in source, and **U+2011 the non-breaking hyphen was missed on the first pass** and is the sharpest of the three.)
**Status:** ✅ **58 GATES PASS · THE PROOF IS RENDER-EQUIVALENCE, NOT THE DIFF.** The decoded text of all 21 files is **byte-identical before and after**, asserted twice — inside the tool and independently against a full-tree snapshot. **The coupling predicted as dangerous was the safe one:** 439 entities sit inside `<pre>` and `gate_payload_match` byte-derives Maker payloads from those blocks, but `decode_pres()` calls `H.unescape`, so it compares DECODED text and passed untouched. **What broke was S126's rule 18 arriving live: four predicates pinned a spelling**, and `book_gates.py:396` already carried the correct dual form **eleven lines from its broken twin at 701**. **The dangerous one was silent** — `strip_inline`'s locators are found by the same pinned strings, and blind they dropped L01's held attributes **39 → 32**, seven unprotected, caught by its own Control H and by no gate. An **AST scan of every instrument** then enumerated the rest rather than waiting for the next failure, and found `_LAND` listing `&larr;` but not `←` — **already blind to 33 of 36 left-arrows before this session**. **Gate 58 asserts the property and guards its own definition**; control-run seven ways, four defect shapes each firing it ALONE at 57 of 57 green, a `HOLD` drift firing alone, an entity inside `<script>` correctly silent, and the untouched tree at 58 of 58 at both ends with a full-tree diff proving the restore real.

**Versions:** L01 v03.23.1 · L02 v03.15.1 · L03 v03.33.1 · L04 v04.23.1 · L05 v04.22.1 · L06 v04.27.1 · L07 v04.26.1 · L08 v04.23.1 · L09 v05.20.1 · L10 v02.21.1 · L11 v02.22.1 · L12 v01.24.1 · L13 v02.22.1 · L14 v02.27.1 · L15 v02.23.1 · L16 v02.15.1 · going_deeper v01.6.1 — census **40,700** · Bible **v8.119** · BookComponentStandard v01.12.1 · gen_component v1.6.1 · Maker v2.45.4 · **book_gates v1.54** · lesson_inventory v1.3.0 · pill_sweep v1.1 · gate_payload_match v1.7 · build_family_map v1.3.7 · build_mark_index v1.0.2 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.20.0 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.3.0 · image_audit v1.1 · strip_inline v1.2 · build_worklist v1.1 · regex_audit v1.0 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · entity_sweep v1.0 · font_stack_sweep v1.3.0 · next_pointer v1.2 · title_feed v1.0 · Timer v1.3.2 · `ZUMO_Syllabus_WORKING.md` v1.2 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `4226af5`.**
