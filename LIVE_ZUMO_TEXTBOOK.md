...
**Date:** July 30, 2026 (Session 95 — **THE INSTRUMENTS WERE WHERE THE DEFECTS WERE.** Nothing a student opened was wrong at S95 open and 35 gates said so; **two of those gates were making claims they could not back.** `§5.1 callout title uses the block form` computed its anchor as `src.find('>', off)` from the start of the callout's line — and L14 line 159's `THE ONE IDEA` shares its line with the `</div>` that closes the block above it, so the gate landed on THAT tag, ran one element late, inspected the callout's own `<div` opener instead of the title inside it, and **a bare `<strong>` title passed unseen for sessions.** The identical idiom sat in gate 35 too; an assert pinned the count at **2** before patching, because fixing one would have left the other half-blind. **THE CONTROL-RUN TOOK TWO ATTEMPTS AND THE FIRST WAS WORTHLESS** — run from `/tmp` the old gate crashed on `import lesson_inventory` and printed no failure, which reads exactly like a pass; **an absent FAIL is not a PASS (§24.8).** Re-run in-tree against the same re-seeded defect: v1.26.2 → `PASS`, v1.26.3 → `FAIL 14 line 159`. **EVERY FINDING SHRANK WHEN MEASURED, three for three:** the L14 anomaly was real but **1 block, not a class**; "six callouts need normalising" was **one** (the other six are `<p>` and `<details>` callouts that start their own lines fine — the test asked "does this line start with `<div`?", conflating *isn't a div* with *shares a line*); and the "12 one-off schemes" were 12 real schemes of which **only 5 were typo-shaped.** **TEAL SHIPPED, DJ RULING:** INSIGHT left the blue it shared with LEARN for `#e9f7f5`/`#2da99d`, hue 174, **31 blocks across 10 lessons**, and **18 of them also carried a deep-blue title `#0d47a1`** moved to `#165a53` — a blue title on a teal panel would have undone the split. **AND THE COLLISION WAS CANON, NOT DRIFT:** the Bible declared *"Learn / Insight"* as ONE type with two glyphs while `BookComponentStandard`'s roster listed them as TWO families with different roles, and the two documents disagreed for sessions **because the roster renders on zero pixels.**)
**Status:** ✅ **35/35 GATES PASS, clone-verified at `9ba2630`. Nine pushes this session, every one verified by fresh clone and md5.** Census **39,972**. Callout schemes **51 → 46**, one-off schemes **12 → 6**, off-canon widths **115 → 114** (paid, not moved). **THE S95 RULING THAT BINDS FUTURE SESSIONS: a repaint landing on one of the off-canon blocks normalises the width in the same edit** — recorded in `book_gates.py`'s header so a session reads it from the file, not a handoff. It exists because the geometry gate baselines off-canon widths per `(lesson, px, border, bg)`, so repainting any of the 115 moves it to a tuple with baseline 0 and reads as NEW debt; L03:937 went 5px → canon 4px and its `GEOM_BASELINE` row was **deleted** because the gate's own shrinkage branch asked for it. **THREE INSTRUMENT CARRY-OVERS CLOSED:** `build_mark_index.py` landed and its default output moved OUT of repo root (root was the reintroduction path for the very stray that failed the gate at S95 open), `build_family_map.py` is self-contained (it read `/tmp/inv.json` and died on a fresh clone; proven by diffing old-with-JSON against new-without to an empty diff), and `.gitignore` covers `__pycache__/`. **STAGE TWO IS OPEN AND IS NOT A PHANTOM — it is recorded in the Bible changelog so it survives this handoff:** two live blocks are labelled `Learn/Insight` (L03:3636, L09:1342), Bible line 1033's Problem-Solving item names the shared hex pair by hand, and §18's data-type callout gives LEARN's blue a third job. **§26 STAYS PARKED and needed no ruling** — `BookComponentStandard` records **ZERO** live callout hexes, so teal was not a §5.0 change and invented no Heritage Blue role. **SCHEDULE BLOCKED UNTIL ~AUG 24** (DJ does not know his teaching weekdays), but the syllabus is **not** fully blocked: seven of its eight open items need no calendar, and with **five weeks to September 8** that is the most student-facing unblocked work on the queue.

**Versions:** L01 **v03.15.0** · L02 **v03.6.0** · L03 **v03.20.0** · L04 **v04.15.0** · L05 **v04.15.0** · L06 **v04.19.0** · L07 **v04.15.0** · L08 **v04.14.0** · L09 **v05.12.0** · L10 **v02.11.0** · L11 v02.12.0 · L12 v01.14.0 · L13 v02.12.0 · L14 **v02.15.0** · L15 v02.11.0 · L16 v02.7.0 · going_deeper v01.1.1 — **eleven lessons moved in S95; census 39,972** · Bible **v8.82** · BookComponentStandard v01.10.0 (26 families in the roster; the S94 map requires 9 more) · gen_component v1.5 (ALL CHECKS PASS) · Maker v2.45.1 · **book_gates v1.26.3 (35 gates, 35/35)** · lesson_inventory v1.1.1 · pill_sweep v1.0 · gate_payload_match v1.6 · **build_family_map v1.0.1** · **build_mark_index v1.0.1** · gen_bonus_banner v1.2 · gen_part_banners v1.0 · Harness v3.0 · `images/icons/` 49 incl. LICENSE · `images/marks/` **41** · teal `#e9f7f5`/`#2da99d` carries **31 blocks**, LEARN's blue **43**. **Verified by fresh clone at `9ba2630`.**
