...
**Date:** August 3, 2026 (Session 110 — IN PROGRESS. **THE VISUAL DIRECTION CHANGED AFTER BEING MEASURED.** DJ ruled sun-faded, then *“None of these really are blue heritage focused”* — and he was right: measured by CIELAB hue gap to canon, warm earth and every faded variant scored **1 of 6** bands within 20°, and E, which had been presented as reconciling book and brand, scored 3 of 6. **H2 is ruled** — sun-faded bands taken FROM Heritage canon, 5 of 6 within 20°, recorded in `ZUMO_S110_VISUAL_RULING.md` with `build_palette.py` as its generator so the table cannot drift from the derivation. The structural finding outlives the ruling: **Heritage is five colours but only TWO hue families**, so six groups separate by LIGHTNESS, which is §5.0.1's ramp principle arriving from the other side.)
**Status:** ✅ **45/45 GATES PASS · Census 40,025 · Bible v8.98.** **REGEX FAILED THREE TIMES IN ONE SESSION AND THE THIRD WAS INSIDE THE SWEEP WRITTEN TO CHECK THE FIRST TWO** — `\b` matched `div-ai-center` inside `div-ai-center-2` and produced five phantom findings; `startswith('#section-1')` matched `#section-10`; a stray `.html$` in a stray-file grep counted four legitimate pages. `-` and `#` are not word characters. **`class_sweep.py` v1.0 is the answer** — a parser-based class index; the corpus holds **292 prefix collisions among 657 class tokens**, so a substring audit miscounts 126 of them. All five regex-derived scope claims reproduced under the parser. Also: `pill_sweep`'s audit had reported *** MIXED *** on 15 of 16 lessons since S103 because its swept-detector looked for an inline string the class migration deleted — an alarm that fires on everything, printing unread in the session-open ritual; fixed and now migration-aware. `session_versions` **v1.15** gains ROSTER COVERAGE after `build_palette` and `class_sweep` sat unregistered and invisible to `--check`; its CONTROL E could not see them because CONTROL E lives in `--selftest`, which runs at session OPEN, before any new instrument exists. Six week-one figures briefed in `ZUMO_S110_SHOT_BRIEF_WEEK1.md`; `[IMAGE 3.14]` is NOT a shot — its only tag sits inside its own retirement row, so outstanding is **18, not 19**. **`font_stack_sweep` v1.1.0 → v1.2.0: its value parser could not read a quoted multi-face stack, so a `--write` pass would have CORRUPTED `css/book.css`** — `'Consolas','Monaco','Courier New',monospace` matched only the first quoted face and the rest was left dangling; nine declarations would have ended up with a family literally named *"Courier New, monospace"*. It had reported `8 x Consolas` as bare for seven sessions, contradicting a standing note that said zero were bare — **the note was right and the instrument was wrong.** Fixing the parser then exposed a second bug: the web-served exemption took a FACE and was being handed a whole VALUE, and only ever agreed because the old parser truncated stacks to one face. **`svg_layout_audit` v1.19 → v1.20 clears the last standing `regex_audit` lead, which was a real defect.** v1.18 guarded `<text>` against rotation and left `<image>` unguarded; **3 images sit under a rotation** and the two at 90° had their box measured on the wrong edge — 16% too wide, understating the resolution ratio by 14% and biasing toward a FALSE 'under the floor'. Silent today, so it needed measuring rather than waiting. Quarter turns are now measured exactly and anything off-axis is REFUSED OUT LOUD. **My first fix swapped the box for both checks and produced two new false findings** — the swap belongs to the resolution edge, not to the aspect comparison, which rotates with the photo; caught by diffing whole-corpus findings against the pushed version. `_ctm`'s docstring claimed rotate/matrix were 'reported' when the function drops them silently — corrected, and its regex narrowed to what it handles. **`gen_part_banners` v1.0 → v1.1: the tool had been DEAD since S103 and nothing said so.** Its block matcher is written against the inline-style form the migration deleted, so `process()` found zero PART blocks and the run ended in an `AssertionError` on Lesson_01 — for seven sessions, unnoticed, because nothing in the ritual ever ran it. `book_gates` gate 27 kept passing throughout because it reads through `expand_classes()` and this tool read raw bytes: **two readers of one construct, only one of them taught about the migration.** Now reads through the same expander. **`--apply` is REFUSED** — emitting these blocks would write inline `style=\"\"` into pages that link `css/book.css`, which §27.12 forbids and gate 41 catches; repairs go through restore → regenerate → apply. Six controls, including CROSS-TOOL AGREEMENT: this tool's PART colours and titles are asserted identical to gate 27's, read as TEXT because importing `book_gates` runs all 45 gates as a side effect. **A SYSTEMATIC SWEEP OF WHAT NOTHING RUNS FOUND A SECOND DEAD GENERATOR.** Of 25 root `.py` files, 16 are in the ritual and **6 were run by nothing at all** — the class that produced the day's defects. Running all six: `gen_bonus_banner` **v1.3.0 → v1.4.0** had died on `L02: expected exactly 1 nav pill, found 0` on every run since S103, because it matched the pill by an INLINE-STYLE signature the migration deleted. Same defect, same cause and same fix as `gen_part_banners`; `--write` likewise refused. `build_mark_index` and `sweep_option_c` run clean; `gate_payload_match` and `extract_project` are argument-taking tools, not defects. **`sweep_option_c` was nearly a WRONG FINDING** — its scheme table keys on hexes that raw files no longer carry, so blindness looked certain; reading it through `expand_classes` produced BYTE-IDENTICAL output, so it is not blind and the hypothesis was wrong. Cheap test, wrong theory, no finding published.

**Versions:** L01 v03.18.0 · L02 v03.10.2 · L03 v03.23.2 · L04 v04.18.1 · L05 v04.18.1 · L06 v04.22.1 · L07 v04.20.2 · L08 v04.18.1 · L09 v05.15.1 · L10 v02.15.0 · L11 v02.16.0 · L12 v01.18.0 · L13 v02.16.0 · L14 v02.20.0 · L15 v02.16.0 · L16 v02.11.0 · going_deeper v01.2.0 — census **40,025** · Bible **v8.98** · BookComponentStandard v01.12.1 · gen_component v1.6.1 · Maker v2.45.1 · **book_gates v1.39.2** · lesson_inventory v1.2.0 · pill_sweep v1.1 · gate_payload_match v1.6 · build_family_map v1.1.3 · build_mark_index v1.0.2 · gen_bonus_banner v1.4.0 · gen_part_banners v1.1 · session_versions v1.15 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.2.1 · image_audit v1.1 · strip_inline v1.1 · build_worklist v1.1 · regex_audit v1.0 · build_palette v1.0 · class_sweep v1.0 · font_stack_sweep v1.2.0 · `ZUMO_Syllabus_WORKING.md` v1.0 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `ed50e34`.**
