...
**Date:** August 7, 2026 (Session 126 — **A PAGE CAN STYLE ITSELF AND STILL BE PART OF THE BOOK, BUT ONLY IF THE RESOLVER CAN SEE IT.** Three jobs, each finding the next. DJ: *"Fix the typo, all should be consistent in the entire book"* — and the fix was NOT to pick a spelling. **The two pinks were the last two of FIVE classed inline `<code>` elements in the book, all in L03**, invisible to S124's strip because it enumerated the class names it knew (§24.8). §27.15a says there is no exception list, so all five classes went and the elements take the ruled wash. **THEN §27.15d, DJ RULING A: `expand_classes` knew exactly ONE stylesheet**, so a page styling itself from its own `<style>` block was opaque to every gate reading through it — which is why **`going_deeper.html`'s hero satisfied §25.6 ONLY BECAUSE IT WAS INLINE.** The seven attributes S125 recorded as a scope question were not leftovers; six were load-bearing for a gate. **AND THE REPAIR FOR A SUBSTRING-KEYED SCOPE INTRODUCED A WORSE BUG THAT ONLY A COVERAGE ARM COULD SEE** — a predicate demanding `href="css/book.css"` matched ZERO lessons, which link `../css/book.css` (§27.10), and §27 passed on an empty scope looking exactly like a gate that works.)
**Status:** ✅ **57 GATE CHECKS PASS · THE BOOK NOW HAS ZERO INLINE-CODE EXCEPTIONS AND ZERO INLINE `style` ATTRIBUTES ON ANY PAGE THAT CONSUMES THE STYLESHEET LAYER.** §27.11 moves to **624 / 2,282**; the −15 declarations IS the three dead rules at five each, **zero born, zero altered**, so §27.15b's rename trap does not fire and the stylesheet came back **byte-identical across two independent derivations**. **§27.15d NEW:** `lesson_inventory` **v1.3.0** gains `page_css()`, merged with the page winning any collision — the seating §27.15c already ruled — and it is **INERT for the sixteen lessons, measured not assumed: not one carries a `<style>` block.** going_deeper's hero and footer convert to classes whose declarations restate the lessons' byte-faithfully; **that second spelling is safe here and only here because §25.6 is the watcher**, comparing all seventeen heroes by RESOLVED styling — a `padding: 30px` → `31px` drift FAILS it alone. The page adopts **Inter**, closing S125's second find (it was loading the webfont and rendering Arial), and four dead `.hero` rules go, zero uses verified before deletion. **§27.12's SCOPE NOW DERIVES FROM EITHER STYLESHEET** (DJ: *"Yes watch pages that link"*), with `index.html` held **BY NAME** (§25.2a) because DJ ruled its one inline style out and **a fix with no gate is an unguarded fix**; tool pages stay out by §25.6a. **THREE SCOPE DEFECTS FOUND IN ONE PASS, EACH BY THE NEXT:** a COMMENT mentioning `css/book.css` pulled going_deeper into §27; the specific predicate that replaced it matched zero lessons; and **that was caught ONLY because gate 44 had a coverage arm and §27 did not** — S117/S118 a third time, and the first time the empty scope was one written this session. **COMPLEMENTARITY MEASURED (S106 shape):** an inline style in going_deeper's preamble — a span no other gate covers — FAILS §27.12 **alone** under the new scope and is **completely invisible** under the old. Control-run seven ways from a pristine snapshot with read-back asserts, untouched tree passing at both ends; `regex_audit` 0 leads.

**Versions:** L01 v03.22.0 · L02 v03.14.0 · L03 v03.32.0 · L04 v04.22.0 · L05 v04.21.0 · L06 v04.26.0 · L07 v04.25.0 · L08 v04.22.0 · L09 v05.19.0 · L10 v02.20.0 · L11 v02.21.0 · L12 v01.23.0 · L13 v02.21.0 · L14 v02.26.0 · L15 v02.22.0 · L16 v02.14.0 · going_deeper v01.6.0 — census **41,128** · Bible **v8.117** · BookComponentStandard v01.12.1 · gen_component v1.6.1 · Maker v2.45.3 · **book_gates v1.52** · lesson_inventory v1.3.0 · pill_sweep v1.1 · gate_payload_match v1.7 · build_family_map v1.3.7 · build_mark_index v1.0.2 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.19.0 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.3.0 · image_audit v1.1 · strip_inline v1.1 · build_worklist v1.1 · regex_audit v1.0 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · font_stack_sweep v1.3.0 · next_pointer v1.1 · title_feed v1.0 · Timer v1.3.1 · `ZUMO_Syllabus_WORKING.md` v1.2 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `ecee35e`.**
