...
**Date:** July 29, 2026 (Session 91 — **HERITAGE BLUE IS THE STUDY'S ROW C, AND THE PAGE MOVING MOVED EVERYTHING UNDER IT.** DJ approved the **26-family component table** — names, marks and role assignments — and it is now recorded on disk in §7 rather than living in a chat window; THE WALL's name was raised for rename at the same ruling and DJ kept it. `BookComponentStandard` **v01.5.0 → v01.6.0**, moderate per §2 ("approved addition"), costing two stamp edits both inside the standard because no other file carries the conformance stamp. **The RoboLore palette study settled Heritage Blue's actual values:** row C is the Heritage Blue Collection and the standard's five hexes were corrected to it — the same palette sampled slightly lighter, four slots within 2.5° of hue, Warm Brass within 0.2°, and **Parchment the one real divergence at 8.5° warmer**. Because every tint is 10% accent mixed into the page, moving Parchment moved all 8 tints, all 8 titles and all 8 contrast figures; each was **re-derived, not hand-patched**, and **the floor lands at 5.11 — exactly where the book already was**. **41 marks regenerated.** **A GATE WENT QUIET INSTEAD OF FIRING, AND THE PUSH PROVED IT.** The S91 marks landed at `images/images_marks_S91/` because they shipped as a zipped directory and the extractor named the folder after the archive; `images/marks/` was deleted in the same push. `gen_component --selftest` reported **ALL CHECKS PASS** — its marks check carried a `not yet generated` branch, so a MISSING directory and a correctly generated one were indistinguishable. §24.8's test applied and failed: if the answer were the opposite, the instrument would not look different. Fixed in `gen_component` **v1.3 → v1.4** — absence now fails once the standard names shipped marks — and control-run three ways, the decisive one being that the real S91 defect **FAILS under the new gate and PASSED under the old**. Still 22 checks; one was rewritten, none added. **§24.10 IS CANON (Bible v8.77 → v8.78):** the parser is the default instrument and grep reads one known line. DJ's ruling — *"Grep has caused most of the issues we have faced in the book"* — and the record is one-sided. Deliberately NOT a fourth restatement of §11/§24.6a/§24.6c: those already say a scan finding is a candidate and did not stop it, including in this session. **§24.10 GOT ITS TOOL THE SAME SESSION (§24.2):** `lesson_inventory.py` **v1.0.5 → v1.1.0** now enumerates every callout by the **(background, border, glyph) TRIPLE** — the family is carried by the triple, not by a name, because L11/L12 label theirs with a bold lead SENTENCE and no family name at all. New `--callouts` per-lesson view and `--schemes` book-wide roll-up. **It finds 1,048 blocks, matching the S90 census; the ad-hoc regex that preceded it found 1,005 and missed 43.** It answers the question S91 got wrong — THE WALL `#fdecea`/`#e74c3c`/🛑 is **18 blocks in L11 and L12 ONLY** — and reproduces S90's seam finding independently: `#eafaf1`/`#27ae60` carries 🏆 in L12 and ✅ in L11, one scheme with two glyphs. **A defect in the collector was caught by running it:** the glyph field came back EMPTY for exactly the lessons the tool was built to read, because L11/L12 write numeric entities (`&#128721;`) and `flat()` preserves entities by design — unescaped for glyph detection only. Geometry census: **933 × 4px · 112 × 5px · 3 × 3px, 115 off-canon**, and nothing gates it yet. **GATE 33 — CALLOUT GEOMETRY, AGAINST A FROZEN BASELINE (`book_gates` v1.21 → v1.22).** §5.1 fixes the rule at `border-left: 4px solid` and **115 live blocks are off it**, 83 of them in L11/L12's second design system. Shipping it ABSOLUTE would fail every run until a repaint that is itself blocked on an unapproved palette — a gate that cries wolf gets ignored (S90) and would drag the other 32 down with it. **The existing debt is therefore FROZEN as a 38-signature baseline and anything NEW fails.** This is NOT the S82 widen-the-matcher move DJ ruled against: widening accepts 5px forever and everywhere, whereas a baseline names the debt that exists, rejects the 116th block, and is built to reach ZERO at the repaint, when it becomes absolute. Signatures are `(lesson, px, border, bg)` so they survive line shifts, and geometry is read through `lesson_inventory`'s parser per §24.10, never a regex in the gate. **Control-run four ways:** untouched PASS · a 7px block injected in L06 FAILED naming lesson, width, border and bg · one 5px block RETIRED in L15 PASSED and reported `debt 114/115, 1 retired — tighten the baseline` · a lesson removed tripped `COVERAGE: 15 lessons parsed, expected 16`. **Recorded so nobody reads the 115 as pure drift:** `#1a5276`/`#f8f9fa` is one block per lesson in L01–L11 and `#6c757d`/`#f8f9fa` one per lesson in L12–L16 — uniform constructs that happen to sit at 5px. **THE SAFETY FAMILY IS RETIRED (DJ ruling: *"get rid of safety and make them all warning"*), AND SIX LESSONS CHANGED.** Reading the one amber scheme `#fff8e1`/`#ffc107` found it doing **six jobs across 87 blocks**, not the two the table names — the strongest single argument for the repaint. DJ's markup retired SAFETY, so **the red role empties: the semantic palette is a semantic THREE**, green/amber/purple, and `shield-exclamation` loses its family. Seven callouts carried SAFETY in their label — six as a family label and L01's as a topic name (`Battery Safety`) — and all seven now read WARNING. The L04 §1 jumper-promise block moved amber → NOTE per §6.6a, matching **L04's own** NOTE form rather than another lesson's, per §6.12c. **Recorded against myself: I printed a before-count of 123 that I had typed rather than measured** — the real figures are amber 87→86 and NOTE-scheme 126→127, exactly one block moved (§24.4). **DJ's markup also proposed two families the table does not have — REMINDER (27 blocks, the largest population in the amber scheme) and RULE/POLICY (4)** — and a usage census of all 105 triples found the repurposing DJ asked for: **WRITE IT and COMMON PITFALLS have ZERO live blocks each**, so the table can absorb both new families at 25 with no new icons. Note HINT/ANSWER/GLOSSARY also read zero but are alive as OTHER constructs (`<details>` reveals, a page section) and must not be retired. Applied S91, all six MODERATE (label text renders differently, both §5b homes moved): L01 **v03.11.0** · L04 **v04.10.0** · L06 **v04.15.0** · L07 **v04.11.0** · L08 **v04.10.0** · L09 **v05.8.0**. **Then DJ ruled the labels BARE** (*"get rid of the second part of the title because it makes us have more categories"*) — which turns out not to be a new preference but **§6.6a compliance, LOCKED since S61: "Labels are bare."** A topic suffix after the family word reads as another category, which is exactly the disease this session spent its length on. Applied to the four in hand — L01's `WARNING — Batteries`, L06's `— first autonomous drive`, L07's `— tuning runs` all now read bare `⚠️ WARNING`, and L04's jumper promise reads bare `📘 Note`. **The parser then found the scope: 72 labels book-wide carry a topic suffix against §6.6a — 28 NOTE, 27 TIP, 17 WARNING — of which **3 are now fixed and 69 remain** — a rule locked for thirty sessions and never swept. **Recorded against myself for the SECOND time this session:** I wrote "4 fixed, 68 remain" by subtracting my own edit count instead of re-running the parser. L04's label read `📘 One promise before we start` and carried no family word, so it was never one of the 72 — §24.4, twice in one session, both times caught only by measuring afterwards.** Patch bumps, visible banners already correct at MAJOR.MINOR: L01 **v03.11.1** · L04 **v04.10.1** · L06 **v04.15.1** · L07 **v04.11.1**.)
**Status:** **THE COMPLEMENT WAS ALREADY IN THE PALETTE.** Deep Navy sits at 214.3°; its exact 180° complement is 34.3°, and Antique Bronze measures 34.6° — building a candidate from scratch at the complement returns Antique Bronze. Nobody had written down why the warm metals sit easily against the navy. Of the two 90° positions, **−90° (124.3°) is unusable** — 21.6° from the existing green, under §5.0.1's 30° floor — and **+90° (304.3°, a plum) is the one open slot** for a fifth semantic role, clear of purple by 43.9° and the band ramp by 89.8°. **A ruled change broke a gate and the generator caught it independently:** row C's lighter navy dropped Deep Navy text on Steel Blue to **4.17**, under 4.5, matching a hand computation made before the tool ran. DJ ruled **option A — Steel Blue `#6985AB` → `#708BAF`** (4.51, ramp still monotonic at 56.3% against Harbor's 60.0%) over reverting the band text, because A changes one hex and B changes a rule. **Caught en route: brass's title contrast was published as 5.44, which is bronze's number** — brass borrows bronze's title colour and the figure had been copied with it; verified by reproducing 7 of 8 published tints byte-exact from the 10% mix model, and every row reproduced its quoted figure within 0.02 except brass. **STILL UNAPPROVED AND STILL HOLDING THE REPAINT: the semantic palette** — green, amber, red, purple. §7's approval entry says so explicitly so nobody later reads the roster ruling as covering it.

**Versions:** L01 **v03.11.1** · L02 **v03.2.1** · L03 **v03.16.1** · L04 **v04.10.1** · L05 **v04.11.2** · L06 **v04.15.1** · L07 **v04.11.1** · L08 **v04.10.0** · L09 **v05.8.0** · L10 **v02.7.1** · L11 **v02.9.1** · L12 **v01.10.1** · L13 **v02.9.1** · L14 **v02.11.1** · L15 **v02.8.1** · L16 **v02.5.5** · going_deeper **v01.1.1** — **six lessons changed in S91** · Bible **v8.78** · **BookComponentStandard v01.6.0** · **gen_component v1.4 (22 checks, ALL PASS)** · Maker **v2.45.1** · book_gates **v1.22 (33 gates, 33/33 PASS)** · pill_sweep v1.0 · lesson_inventory **v1.1.0** · gen_bonus_banner **v1.2** · gen_part_banners v1.0 · Gate v1.6 · Harness v3.0 · `images/icons/` **49 marks + LICENSE (generator input)** · `images/marks/` **41 generated marks, all regenerated S91** · **web tools: timer v1.3.0 · Maker v2.45.1 · tutor v1.1.0 · index v1.3.0**.
