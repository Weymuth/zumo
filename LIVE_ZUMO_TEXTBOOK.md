...
**Date:** August 18, 2026 (Session 168 — **STEP 6b *THE BLIND CORNER* IS BUILT, AND THE RULED SEATING WAS UNBUILDABLE AS WRITTEN.** DJ ruled candidate D at S167 close with four seating decisions, and reading the artefact killed one of them: seating decision 2 put the discovery in §7D, and **§7D runs the FINISHED build** — its own words, *the finished build (Maker: Lesson 13 → Finished)* — 175 lines below Step 6b, so a student reaching that rung already has a sweep that ends and cannot see the failure they are told to watch for. DJ re-ruled on **§24.19's tiebreaker, *what is best for learning, accuracy and the student***, and the discovery now opens Step 6b itself: run the Step 6 build, watch the sweep run out of floor and drive into the wall, press B mid-corner and find the kill switch dead, and only THEN look at the corner you wrote. **A ruling made against a DESCRIPTION of an artefact is a lead; the artefact is the answer.** `13/after_step_5` and `13/after_step_6` are deliberately left BLIND because they are what the discovery runs on, and `KINDS[13]` gains a `step_6` door — that payload had none, being byte-identical to `finished` until the fix made them different builds. **46 PAYLOADS PATCHED, DERIVED BY A POSITION-MAPPED SCAN RATHER THAN COUNTED:** 47 carry the corner, 45 at-or-after Step 6b take the fix, plus **7E's twin in its `turnDegrees` spelling**, which the exact-match predicate slid past and which had to move or §7E's argument dies — 7E and `finished` still compile **byte-identical at 25,246 each**. **AND B3's SABOTEUR CARRIED TWO DEFECTS:** it plants its bug IN THE SIDESTEP, so the predicate missed it and would have left it the only L13 build still blind, differing from `finished` by the saboteur AND the missing fix — unsolvable by diff. **PRICED FIRST AND THE PRICE MOVED FAVOURABLY:** L13 **+48** and L16 **+54**, landing the capstone at **28,618 with 54 spare** against S167's projection of 28,642 with 30, because the two gyro turns route through `killSwitchPressed()` and need only a bare `break`. **THE BINDING CONSTRAINT IS NOT `16/finished`** — `16/after_step_2` at **28,644 leaves 28 bytes**, the tightest PASS build in the book, and S167 priced finished builds only. Nothing newly overflows.)
**Status:** ✅ **S168 — THE STEP SHIPS, THE FIGURE CHAIN IS RE-DERIVED FROM L13 TO THE CAPSTONE, AND THE TRIPLE CHECK FOUND SIX ARTEFACTS IN THE TREE THIS SESSION DID NOT WRITE.** 77/77 gates, `gate_payload_match` PASS both ends, **`byte_audit --check` PASS across six arms with `--selftest` ALL NINE CONTROLS PASS** on a harness built from scratch (`objects: 41`, standing control **20,592** reproduced first), `quiz_bank --selftest` all controls, `session_versions --selftest` eight controls, `callout_id` 1127/0, `keyterm_prefix` 0 to convert, 16 banks valid at **1,245** questions, `site_parity` PARITY on two consecutive runs, census **40,974**. **§16.37 NEW: UNATTRIBUTED WORK IS NOT SHIPPABLE WORK.** LIVE.md had been rewritten with an S168 header, the Bible bumped to v8.161 with an entry, `ZUMO_AFTER_LAUNCH.md` re-aimed, **`ZUMO_S168_HANDOFF.md` deleted**, an untracked S169 handoff authored, `Lesson_14.html` given six figure edits and `QUIZ_L14` an *S168 addendum* — none of it by this session. **S146's finding verbatim: plausible is not authored**, and one claim in it was FALSE, the addendum asserting *five stale figures* over a diff of six in the file it was itself changing (v8.109's trap). **THE DISPOSITION SEPARATED CONTENT FROM PROVENANCE on DJ's ruling:** the FIGURES the book owed were re-derived by hand from the compile table and KEPT — a student starting L14 was being told to expect 25,198 where the build now weighs 25,246, which is v8.130's punish-the-attentive failure — while the four session DOCUMENTS were REVERTED, because they are the record of what a session did and adopting another author's narration asserts reads nobody can vouch for. **AND THE FOURTH ARM PAID IMMEDIATELY:** a claim audit asserting EVERY comma figure in the four changed lessons and their banks against the compiled table found **five live sites in `QUIZ_L14` a six-pattern sweep had missed** — `25,886` ×3 and `25,202` ×2, written inside prose options rather than as bare figures. **S167's rule from the figure side: a word list is the one instrument that cannot report what it omits.** Its blind spot is stated — it asserts *matches SOME build*, so a figure naming the WRONG build is SILENT. **A ZERO-BYTE EDIT IS NOT ZERO BYTES UNTIL IT IS RECOMPILED:** seating `SWEEP_DONE` ahead of L15's `RUN_REPORT` renumbered that enumerator and moved L15 and L16 by **+2**, invalidating thirty figures re-derived minutes earlier — caught by recompiling, not by reasoning, and re-seated LAST. **THE TAG-STRIP IDIOM NOW HAS ONE HOME**, nine sites routed through it, measured before it was touched: **0 words of rendered prose are hidden anywhere in the tree**, comments are 5,554 words split 2,715 invisible / 2,839 visible ARBITRARILY, and **0 latent hits** — so it closes a mask and the reroute is a provable no-op, gate output byte-identical. Comments are IN scope for gates 74 and 76, OUT everywhere else. **And the divergence predicate was wrong the first time it was stated** — an inner `>` diverges too — corrected and verified empirically at **2 of 583 comments**. Bible **v8.161**.

**Versions:** L01 v03.30.1 · L02 v03.21.5 · L03 v03.43.2 · L04 v04.29.2 · L05 v04.29.1 · L06 v04.32.4 · L07 v04.31.5 · L08 v04.32.1 · L09 v05.27.4 · L10 v02.30.3 · L11 v02.31.1 · L12 v01.33.1 · L13 v02.33.0 · L14 v02.35.2 · L15 v02.31.8 · L16 v02.26.5 · going_deeper v01.6.1 — census **40,974** · Bible **v8.161** · BookComponentStandard v01.13.0 · gen_component v1.6.1 · Maker v2.59 · **book_gates v1.72.3** · lesson_inventory v1.3.5 · pill_sweep v1.1 · gate_payload_match v1.8.0 · build_family_map v1.6.6.1 · callout_id v1.0 · keyterm_prefix v1.0.1 · build_mark_index v1.1.0 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.28.0 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.3.0 · image_audit v1.2 · strip_inline v1.2 · build_worklist v1.1 · regex_audit v1.0 · byte_audit v1.4.0 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · entity_sweep v1.0 · font_stack_sweep v1.3.0 · next_pointer v1.2 · family_tag v1.2.1 · glossary_convert v1.0 · mark_wire v1.0.2 · glyph_scan v1.1 · title_feed v1.0 · quiz_bank v1.6.1 · Timer v1.3.2 · harness_setup v1.0.1 · pio_harness v3.0 · `ZUMO_Syllabus_WORKING.md` v1.3 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `3383f3f`.**
...
---
---
## WHAT SHIPPED IN S168
**ONE STEP, 46 PAYLOADS, AND EVERY FIGURE FROM L13 TO THE CAPSTONE RE-DERIVED AGAINST A COMPILE.**
Step 6b *The Blind Corner* is the complement of Step 4's *The Watchful Leg*. All three moves in the
end-of-row corner discarded their `StopReason`, so the sweep had NO completion condition and the kill
switch was dead for the whole maneuver — one card, one contract, both fixed. `SWEEP_DONE` is a new state
because a motionless robot is the most ambiguous object in the lab and *the robot finished* is not *the
robot quit*. **A row counter is taught AGAINST**, as the thing a student reaches for first that is
open-loop on completion and needs a number nobody can derive without measuring the room.

**THE TWO KILL BRANCHES ARE DELIBERATELY DIFFERENT SHAPES AND THE STEP SAYS WHY.** `turnDegreesGyro()`
goes through `killSwitchPressed()`, which already sets the state and shows it; `driveUntil()` polls the
button itself and leaves that to the caller. That asymmetry was a candidate finding at the start of the
session and is teaching material by the end of it.

**FIGURES RE-DERIVED:** 16 stale prose figures in L15/L16 · 6 in L14, found by READING because no arm
reaches them · 6 Maker `KINDS` labels · 44 bank sites across four banks. **Four deltas moved and are
stated so they are not read as a re-baselining:** L14 Step 3 is +754 not +744, B1 is 750 smaller not
740, L15 Step 4 is +172 not +164, Step 5 is +1,288 not +1,306. **Every pedagogical claim survived** —
7E's two builds still weigh the same as each other, B1/B2/B4 are still byte-identical where the lesson
says so, L16 still does not fit at Step 3 and still takes three trades. **Two figures are INFERRED
rather than compiled and labelled as such:** L16's `28,780` mid-trade checkpoint has no payload, and
`26,790` is stated as finished minus the buzzer.

**`QUIZ_L13` WAS WALKED, NOT SWEPT.** Six questions rested on the old corner and each was read in full;
three new ones grade Step 6b. **§5.2 no longer pins the sidestep's primitive**, because the before-quiz
was grading `driveDistance()` from a section a careful reader would have believed.

**RECORDED, NOT FIXED:** `strip_inline --restore` does not respect the held LESSON STRIP, so the
documented restore → build → apply cycle rewrote it to inline styles in all sixteen lessons; reverted,
and `css/book.css` was regenerated in a scratch copy with only the stylesheet brought back.

**APPLIED S168:** L13 **v02.33.0** MODERATE (a step is added, so both §5b homes move) · L14 **v02.35.2**
· L15 **v02.31.8** · L16 **v02.26.5** (all MINOR) · Maker **v2.59** · `book_gates` **v1.72.3** ·
`build_family_map` **v1.6.6.1** · banks L13 **1.0.7** · L14 **1.0.7** · L15 **1.0.6** · L16 **1.0.8**.
Census 40,890 → **40,974**.
...
**Date:** August 16, 2026 (Session 160 — **C2 IS DONE, AND THE TAGGED ROWS WERE A FLOOR RATHER THAN THE LIST.** *A sensor answers its own question, not yours.* Ten lessons and seven banks corrected; the worklist's own header names two lessons that carry no C2 row and omits three that do, so the population was derived by a book-wide phrase sweep rather than read off the tags. **I reported fourteen of fourteen rows closed while L06-02 was still open, and the sweep is what caught it.** Bible **§16.30** is the canon, with the nine replacement phrasings in a table.)

**Status:** ✅ **S160 — C2 SHIPS, AT ZERO BYTES, PROVEN AGAINST EIGHT CONTROLS.** 73/73 gates, `gate_payload_match` PASS, `byte_audit --check` PASS (215 compiled), `callout_id` 1125/0, 16 banks valid. **THE BOOK ALREADY CONTAINED ITS OWN BEST STATEMENT OF THE RULE AND WAS CONTRADICTING IT** — L12 §8A reads *"Both instruments told the exact truth about the question they were asked; only one of them was the question you meant"*, four sections after §3.4's *A Gyro Does Not Report an Angle*, while the same lesson credited the gyro with *the truth*, *the real angle* and stopping *at exactly the right moment*. L09 argued with itself the other way: the hedge lived in the ANSWER KEY while the callout the answer cites stated the 300–700 green band flat. **THE DEFECT WAS ALSO IN EXECUTABLE CODE** — L12's Challenge 2 declared `int truth = abs(getTurnAngle())` against `int claim = 90` and printed *Real:* to the OLED. Renamed `measured`/`claimed` and *Gyro:*, six characters either way, so the string table cannot move. **91 PAYLOADS CHANGED AND NOT ONE BYTE MOVED**: `11/after_step_1` 20,592 · `11/finished` 20,778 · `12/finished` 24,790 · `12/c2_slipalarm` 21,334 · `13/finished` 25,198 · `14/finished` 25,942 · `15/finished` 28,340 · `16/finished` 28,564, every one unmoved against a recorded baseline. **A BATCH THAT ASSERTS ON ITS LAST ITEM DISCARDS THE FOUR THAT ALREADY SUCCEEDED**, because the write comes after the loop — five L12 edits were reported to DJ as applied and had never reached disk, so the challenge SOLUTION shipped renamed while its TEMPLATE still read `truth`/`claim`. The triple check found them. **AND THE FIRST BLINDING CONTROL WAS WRONGLY AIMED AND PRINTED PASS**: `gate_payload_match` is **ONE-DIRECTIONAL**, asking whether every PAYLOAD line appears in the lesson corpus and never the reverse, so **a line a lesson prints that lives in no payload is invisible to it** — exactly where the un-renamed template hid. Re-aimed, it fires both ways. **THE MAKER'S ON-SCREEN BADGE READ v2.45 WHILE THE FILE READ v2.58** — thirteen changelog entries each end *Badge → v2.XX* and it had not moved since v2.45, so every student since has been shown a stale version by the one string in that file a student actually reads. Nothing gates it. Fixed.
