...
**Date:** August 3, 2026 (Sessions 111–112 — **GOING DEEPER IS ON THE BOOK’S SETTINGS** — the only page in the book with a dark design system, and the S111 repaint had made it worse before better: Theory’s `#1f2a3d` on its hero measured **1.31 against `--bg`**, so the hero stopped being a distinguishable element. 11 custom properties remapped, `--purple` deleted because it styled nothing, four hard-coded dark panels each given a deliberate destination, zero dark values left. It still does NOT link `css/book.css` — sharing the stylesheet means widening `build_css.SOURCES`, which at S105 re-spelled 46 class names invisibly. Also: **§27.14 IS IN: every link and every id resolves**, 1,237 links and 705 ids across twenty pages that no gate had ever looked at, control-run on four defect shapes. Its own lesson is the one to carry: **where a gate sits in the file is part of the gate** — appended below the summary and its `sys.exit(1)` it printed PASS *after* `ALL GATES PASS` on a clean tree and never executed at all on a failing one. Prior, S111 — **THE REPAINT IS APPLIED BOOK-WIDE, AND THE THING THAT NEARLY BROKE IT WAS SCOPE, NOT COLOUR.** 2,460 elements across sixteen lessons plus `going_deeper.html`, all **134 gradients flattened**, the 87 challenge-card headers moved to the §9 band. **Callouts were protected BY DECLARATION STRING, never by hex** — four of the five old group colours are also used by nine callout rules, so a hex-level substitution would have repainted 63 callout elements in breach of v8.87's Scope C and **no gate would have said anything**, because gate 41 only asks whether a class resolves. A repaint is scoped by CONSTRUCT, and a colour is not a construct.)
**Status:** ✅ **47/47 GATES PASS · THE CASE STRAGGLERS ARE GONE AND `color_index --check` IS CLEAN.** Eight values were spelled in BOTH cases — CSS reads one colour, every raw-string comparison in `book_gates` reads two, which is the S111 trap that cost most of a session. **25 occurrences lowercased in the lesson SOURCE**, through the documented cycle restore → edit → `build_css` → `apply --include-held` (§27.8b). **SCOPE WAS THE RISK AND IT WAS CAUGHT:** the first pass lowercased **222 occurrences across 24 values**, because 16 more are uppercase-ONLY — inconsistent with house style but carrying no variance and ruled by nobody. Reverted to the eight. **§27.11's baseline MOVED BY DESIGN, 650 → 646 rules / 2,371 → 2,367 declarations, and the drop is PROVEN to be the merge:** four duplicate rule pairs collapsed and the four survivors absorbed exactly one element each — 2,576→2,577, 698→699, 49→50, 8→9. Four class names also SHORTENED (`.tok-569cd6-c-569cd6` → `.tok-569cd6`) because the collision that lengthened them is gone; the old spellings no longer resolve, so a stale reference **fails gate 41 instead of hiding** — the safe direction of the S105 hazard, and it was checked for explicitly (4 classes keep a name with changed declarations, all four case-only in the same value). All 16 lessons MINOR: nothing renders differently, so the visible banners stay put per §5b. `book_gates` **v1.42.0 → v1.42.1**. ✅ **47/47 GATES PASS · THE FAMILY MAP IS CLOSED AT 1048/1048 AND GATED.** L15:295 ruled **THINK ABOUT IT** (DJ) — it asks the reader a question immediately before the section that answers it, and had been filed STILL GREEN by COLOUR alone since the map existed, STILL GREEN being the byte-count-report family. THINK ABOUT IT 7 → **8**. **`book_gates` v1.41.0 → v1.42.0 adds §24.14 as gate 47.** Control-run three ways: the L15 rule removed **FAILED naming L15:295**; a crashed generator **FAILED on the exit code**, so a broken tool cannot read as a silent pass; and with an unrelated gate failing it **still executed** — **where a gate sits in the file is part of the gate**, the same lesson gate 46 recorded this session. It shells out deliberately rather than re-implementing the matcher, which would be the third regex §24 forbids. **THE CALLOUT COLOUR TABLE IS DELETED — FAMILY NOW COMES FROM CONTENT.** `build_family_map` **v1.1.3 → v1.3**. At session open the map reported **1047/1048** and no gate could say so, because the generator is not in `book_gates`; control-run against the pre-repaint tree `185e086` with the identical version gave **1048/1048**. The orphan was the small half. **252 of 1,048 blocks — a quarter of the map — were resolved by HEX.** A glyph-first tier took that to 39, controlled positionally with **0 blocks changed** and a sabotage arm moving 159 to prove the control could fire. DJ then read all 39 and ruled each by content, and the colour table is gone. **FIVE BLOCKS MOVED, AND PAINT WAS HIDING THEM:** thirteen L12 blocks wore one glyph and one green and were **three families** — eight conceptual payoffs, **three byte-count build reports** (*"21,342 → 24,534 bytes. +3,192"*) and **two observed-behaviour blocks** (*"Button C on delrin: the square closes"*). **S94 had already ruled a byte-count report is STILL GREEN and 16 elsewhere are filed that way; these three read as INSIGHT purely because they shared paint with their neighbours. The ruling was right and the colour overrode it.** INSIGHT 60 → **55**, STILL GREEN 16 → **19**, WHAT YOU SHOULD SEE 25 → **27**; exactly 5 changed and only those 5, and all 39 prefixes were verified to match exactly one block book-wide before being written. **THE ORDER IS THE POINT: family from CONTENT, then the mark and the colour are OUTPUTS of the family** — DJ: *"Glyphs are not going to be used in the same form as they are now."* 41 marks are generated and none are wired in, so a taxonomy resting on emoji rested on something already scheduled for replacement; the surviving glyph tier is labelled a stopgap in the file. **STILL OPEN:** L15:295 is outside the 39 and unruled, so the map stands at 1047/1048 and **gate 47 cannot be written yet** — a gate written to 1047 would be written to the sweep instead of the ruling. Also S112: `color_index.py` **v1.0** indexes the book BY VALUE (219 colours; 8 case stragglers open), `going_deeper` **v01.4.1**, gate 46 §27.14, and `GPT_BRIEF_FORMAT.md`. **THE FAMILY MAP WAS REPORTING 1047/1048 AND NO GATE COULD SAY SO.** `build_family_map.py` is not in `book_gates`, so the count drifted silently through the S111 repaint while 46/46 stayed green. Control-run against the pre-repaint tree `185e086` with the IDENTICAL generator version: **1048/1048**. The orphan is L15 line 295, a header-less block reading *"How long does one pass through your loop actually take?"*, whose accent moved `#3a7d5c` → `#433014`. **The repaint did not break the assignment; it revealed that the assignment was never earned** — the block reached a COLOUR-keyed fallback and STILL GREEN is the byte-count-report family, which a rhetorical question is not. **THE REAL FINDING WAS THE SIZE OF THE SURFACE: 252 of 1,048 blocks — a quarter of the map — were resolved by HEX.** `build_family_map` **v1.1.3 → v1.2** adds a GLYPH-FIRST tier ahead of the colour table, because thirteen glyphs each resolve to exactly one family across every block that reached the fallback, so for those the hex was corroboration and not evidence. **252 → 39.** Controlled POSITIONALLY across all 1,048 blocks — **0 changed family** — with a sabotage arm retyping 🔑 that moved 159, so the control can fire; a totals-only comparison would have passed a swap. **The remaining 39 are the fragile surface and need rulings:** 🏆 splits INSIGHT 13 / REAL-WORLD CONNECTION 1, ✅ splits INSIGHT 6 / CHECKPOINT 5, and 14 glyph-less blocks split KEY TERM 10 / LEARN 4. Ruling those retires the colour table and makes the map repaint-proof. `ZUMO_FAMILY_MAP.md` annotated in place per §26.7 — its *"All 1,048 assigned"* header is left standing and marked false, because the table below it did not change. Also S112: `GPT_BRIEF_FORMAT.md` records why the returned GPT package split three-good-one-unusable — **a drafting model cannot count the book**, and every future brief opens with a MEASURED FACTS block naming the instrument behind each number. **`color_index.py` v1.0 IS IN — the book is now indexed BY VALUE, not by spelling.** 219 distinct colours across 21 pages and one stylesheet. It reads colours out of DECLARATION VALUES only (§24.10), because a `#[0-9a-f]{6}` sweep matches href fragments and matches nothing at all in `.p-c-666 {` or `class="tok-569cd6"`, where the hex carries no `#`. REACH is computed on `expand_classes` output, so a value delivered through a class counts against the page it paints: **`#666666` reports 18 declarations reaching 17 pages** — 45 divs, 19 spans, 16 paragraphs — which is precisely the answer that was not available before the footer edit earlier this session. **FIRST RUN, A REAL FINDING: eight values still differ only by case, all eight inside the generated `css/book.css`**, including `.page` carrying `#F5F2E9` on all sixteen lesson bodies. S111 normalised 1,809 occurrences and these survived it. They render identically — CSS hex is case-insensitive — so the harm is entirely instrument-facing, which is exactly how S111's diagnosis went wrong in the first place. **`--check` fails on CASE variance only.** Format variance is reported and never failed: `white`×94 / `#ffffff`×13 / `#fff`×10 is a house-style ruling with a defensible answer either way, and a gate that fails on `white` is a gate that gets switched off. **The tool's own selftest caught the tool's own worst bug:** `variants()` first lowercased the spellings before comparing them, folding `#3A7D5C` into `#3a7d5c` and reporting NO variance on the one defect it was written for — **an instrument that normalises before it compares cannot see a normalisation defect.** Control B caught it; the fix is recorded in the function body. `session_versions` v1.15 → v1.15.1 registers it, and its CONTROL G fired correctly on the way in — registered in ARTEFACTS but absent from both emitted blocks, the exact trap its own header documents. **`going_deeper` v01.4.1 — ONE of DJ's two logged items was a defect and the other was canon.** The dead `.pp` rule is deleted: 0 uses, while its five sibling token classes (`.kw` `.cm` `.nu` `.st` `.ty`) are all live, so it was an orphan and not a dead group — the same defect class as `--purple`, deleted eleven lines away in the same S112 pass and missed. **The `#666` was NOT a stray and has been left alone.** It is the footer credits colour, shared by all seventeen pages: §25.6 compares footer skeletons on `LI.expand_classes` output, so the sixteen lessons' `<p class="p-c-666">` expands to the identical inline declaration, and the class name encodes the hex. Substituting `var(--text-dim)` in going_deeper alone fired §25.6 immediately — **caught by a gate, not by reading, and reverted.** Moving that value is a book-wide ruling that also renames eight `.p-c-666*` class families. Recorded because it is S111's lesson arriving from the other direction: **a colour is not a construct, and neither is an isolated instance of one.** Also: the literal `#666666` never appeared in the file — it was the shorthand `#666`, and a grep for the long form returns nothing and reads as already-fixed. ✅ **46/46 GATES PASS · Census 40,013 · Bible v8.100 · `build_css --check` current at 650 rules · 0 dead classes · `regex_audit` 0 leads.** **THE PALETTE WAS RE-RULED FROM RENDERED SPECIMENS** and recorded in `ZUMO_S111_VISUAL_RULING.md`, with `build_palette.py` **v1.1** as its generator: eight bands, the **+18° Wrap Up rotation DROPPED** (re-derived, it bought ΔE76 **0.33**), **Heritage Slate Blue OUT of the band set** — DJ found Theory and Testing by eye before any number said so, and leave-one-out over nine candidates showed only removing a navy helps (Slate Blue → 18.9, Theory → 17.5, any other band → 9.4, **Hardware → 5.3, worse**) — Testing on a new teal at 200°, **rose 337° and hunter green 148° added, amber MEASURED AND REJECTED** at 20.4° from WARNING and ΔE76 11.6 from Challenges. **Chroma damping 0.62 → 0.90**, a deliberate step back from S110's sun-faded look, which also *improved* separation 15.4 → 22.2 because damping had been flattening the bands toward each other; chroma is now **per band**, with Challenges at the gamut ceiling of 1.20 since **real Warm Brass is L\* 69 and white on it is 2.34, so a band carrying white cap text cannot be brass.** **SEVEN GATES FIRED, exactly the number S108 predicted, and they sorted into three kinds:** four generators hard-coding the old band colours (`BAND_END`, `gen_part_banners`, `gen_bonus_banner`, §25.10h), one real gap (**`going_deeper.html` is page 17, is not in `lessons/`, and does not link `css/book.css`** — §25.6 caught it as sixteen heroes agreeing and one not), and two baselines that move by design. **THE CASE TRAP COST THE MOST TIME:** `build_palette` emits UPPERCASE hexes and every value parsed out of the book is lowercase; the mismatch made five constructs read as NEW off-canon blocks, then *passed* §5.1 while failing four other gates once half-fixed, sending the first diagnosis in the wrong direction. 1,809 occurrences normalised. **`strip_inline` HAS NO DEFECT — one was reported and the report was wrong:** `--apply --include-held` already existed and round-trips **byte-identically across all sixteen** (controlled: 0 files changed). Also S111: **`[IMAGE 2.5]` RETIRED** into a live code block, because `image_audit` can ask whether a tag HAS an asset and never whether the tag SHOULD EXIST — planned 145 → 144, outstanding 19 → **18**; **THE ICON LEGEND IS GONE** from the ten lessons that had one, **L11–L16 never had it**, and §6.6 is annotated in place recording that it specified THIRTEEN icons while every live block carried TWELVE; a **link and id audit of all 20 pages** found 1,237 links, 705 ids, **0 duplicates and 0 broken**, control-run with three planted defects; and **41 marks sit generated in `images/marks/` with not one wired into a lesson**, against 2,016 emoji glyphs. **A CORRECTION, per §24.6c:** the S111 handoff listed *"gradients 134 instances, unruled"* as a canon debt. **v8.87 banned them absolutely, ten sessions ago.** The count reconciles exactly — 133 class-driven elements plus one inline in `going_deeper.html` — but an inherited handoff line was read as a finding and a decided question was reopened for most of a session. The debt list is not canon; the Bible is.

**Versions:** L01 v03.19.1 · L02 v03.11.1 · L03 v03.24.1 · L04 v04.19.1 · L05 v04.19.1 · L06 v04.23.1 · L07 v04.21.1 · L08 v04.19.1 · L09 v05.16.1 · L10 v02.16.1 · L11 v02.17.1 · L12 v01.19.1 · L13 v02.17.1 · L14 v02.21.1 · L15 v02.17.1 · L16 v02.12.1 · going_deeper v01.4.1 — census **40,013** · Bible **v8.100** · BookComponentStandard v01.12.1 · gen_component v1.6.1 · Maker v2.45.2 · **book_gates v1.42.1** · lesson_inventory v1.2.0 · pill_sweep v1.1 · gate_payload_match v1.7 · build_family_map v1.3.1 · build_mark_index v1.0.2 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.15.1 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.2.1 · image_audit v1.1 · strip_inline v1.1 · build_worklist v1.1 · regex_audit v1.0 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · font_stack_sweep v1.2.0 · `ZUMO_Syllabus_WORKING.md` v1.0 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `8ae3857`.**
