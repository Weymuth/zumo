...
**Date:** July 24, 2026 (Session 69 — L05 proximity prose reconciled to the S68 hardware truth · timer-coverage gaps closed · two audit false positives root-caused and canonized · all 12 Bonus Challenges tagged).
**Status:** **THE AUDIT ITSELF WAS THE DEFECT SOURCE THIS SESSION.** Two findings reported to DJ turned out to be false positives, both from the same mechanism: structure inferred from a proxy string and never checked against the thing itself. (1) The timer iframes carry `label=Step+2`, so the audit concluded L02 timed its **build steps** — it does not; every timer in the book sits on a `data-challenge` card and the label merely names the step the card belongs to. That produced "22 untimed build steps in L03/L04" and a proposal to insert 22 timers onto plain build prose, a device existing nowhere in the book; DJ's scope confirmation is the only thing that stopped it. (2) `grep -oE "Step [0-9]+"` was **case-sensitive** and matched only the mixed-case text inside card headings, while L02 writes its build steps as `STEP N:` — nine steps found where there are **eleven**, from which a label "drift" was manufactured that does not exist (all 11 labels correct; `STEP 7` legitimately carries two TRY IT cards, so the duplicate "Step 7" is the truth). Canonized as Bible **§24.6c AN AUDIT GREP IS AN UNGATED GATE — CONTROL-RUN IT TOO**, extending §24.6b from gates to ad-hoc audit greps and §11 from prose greps to structural ones. An attempt to give §24.6c its own gate per §24.2 was **written, control-run, and deliberately withdrawn** — the invariant is not uniform (main challenges wrap in `<div data-challenge>`, bonus challenges are a bare `<h3>` with no wrapper), and a third pass tuning a character-distance threshold was the very behaviour the new rule forbids; `book_gates.py` stays at **v1.2**. The withdrawn gate did surface one **verified** defect, checked per block rather than by pattern: **all 12 Bonus Challenges in L02 and L03 carried no `data-challenge` marker**, so twelve exercises were invisible to the AI Tutor picker (§20.2) — including all six that L03's timers point at. §4.2's S65 audit had reported book-wide completeness while never covering bonus challenges as a construct type, the same class of miss. All 12 now tagged `N.b1`–`N.b6` / `data-kind="bonus"` on their self-naming `<h3>` per §4.3, with a matching **Bonus Challenges** optgroup added to the tutor, following the L11 `mystery` precedent. Marker census **119**, zero duplicates — verified by injecting a duplicate and confirming the §4.2 gate FAILS. **All 16 book gates PASS on the delivered set.**

**Versions:** L01 v03.6.5 · L02 **v02.14.0** · L03 **v03.12.0** · L04 **v04.5.4** · L05 **v04.7.1** · L06 v04.10.1 · L07 v04.6.2 · L08 v04.5.1 · L09 v05.3.1 · L10 v02.4.2 · L11 v02.6.2 · L12 v01.6.2 · L13 v02.5.2 · L14 v02.7.2 · L15 v02.5.2 · L16 v02.4.1 · Bible **v8.51** · Maker v2.45 · Gate v1.6 · Harness v3.0 · pill_sweep v1.0 · book_gates v1.2 · going_deeper v01.0.0 · tutor.html updated (new Bonus Challenges optgroup).
