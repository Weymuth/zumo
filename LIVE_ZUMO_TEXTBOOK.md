...
**Date:** August 3, 2026 (Session 113 — **THE FAMILY MAP'S DENOMINATOR IS A FROZEN BASELINE, NOT A COUNT.** L03's `[IMAGE 3.4]` placeholder became a real WHAT YOU SHOULD SEE callout carrying a §22 terminal block, and the generator printed **`assigned 1049 / 1048`** — a line that reads like a count and is not one, because 1048 is a literal on line 196. Gate 47 failed correctly and for the wrong-shaped reason: it was comparing against a remembered number. The literal moved to 1049 and was **controlled before it was touched** — identical generator version, pre-edit tree against post-edit tree, and the only two lines that differ are WHAT YOU SHOULD SEE 27 → 28 and the total, with the other **29 family counts byte-identical**. Parsing the true total (`assigned + len(unk)`) is the obvious fix and is deliberately NOT taken — it changes what gate 47 MEANS, and until it is ruled **every callout added anywhere in the book fails gate 47 until that literal is hand-edited.** §27.11's digest also moved, and moved the safe way: **646 rules and 2,367 declarations are unchanged** — not one rule added, removed or altered — and only rule ORDER changed, because `build_css` sorts by usage count. Diffed in full before the baseline was touched, then control-run after: deleting one `color: white;` still FAILS, restored byte-identical. Prior, S111–S112 — GOING DEEPER IS ON THE BOOK'S SETTINGS, §27.14 every link and id resolves, and the callout family map closed at 1048/1048 and was gated as §24.14.)
**Status:** ✅ **47/47 GATES PASS · TWO FIGURE DEBTS CLEARED WITHOUT A CAMERA, AND A THIRD REFUSED.** Outstanding images **18 → 16**, planned **144 → 142** [`image_audit.py`]. **`[IMAGE 3.4]` became live content:** L03 Step 12 tells the student *"Look for SUCCESS in the terminal output"* and the lesson then never showed it — a dashed placeholder box sat where the answer belonged. It is now a `👀 WHAT YOU SHOULD SEE` callout carrying a §22 terminal block, copied in shape from L01's live build-result block, **with no numbers invented**. **`IMAGE 3.14`'s brackets dropped:** that row was already marked ❌ removed and `image_audit` still counted it outstanding, because the BRACKETED tag is what marks a figure PLANNED — S111's retired `IMAGE 2.5` row is the precedent and carries the bare tag. Its description text is deliberately UNTOUCHED: **the row records a replacement that did not happen** — it names GRAPHIC 3.18 as the replacement for an *"A-Star 32U4 board top view"*, but GRAPHIC 3.18 is the gear-train cutaway, a different subject, and the string *"Inside the can"* it cites appears nowhere else in L03. A guess written into canon is worse than a gap, so it is a question, not an edit. **`[IMAGE 3.6]` REFUSED:** the Serial Monitor TRIM log needs real numbers from a real run, and a fabricated log is the same defect as a fabricated screenshot. **`GPT_BRIEF_FORMAT.md` → Bible §24.15**, moved in full — it was a standing rule sitting at the repo root where nothing gated it, nothing linked it and no roster named it, the §12.2 shape exactly. **The file must now be DELETED (GitHub Desktop).** Bible **v8.100 → v8.101**. **Recorded, not fixed: §24.14 does not exist** — gate 47 ships printing `§24.14 every callout block resolves to a family` against a section number this Bible has never carried, the same debt shape as §21.1's thresholds living only in `book_gates.py`. **Also found: GPT Task 2 and Task 4 are not in the repo.** Only `GPT_BRIEF_FORMAT.md` references them and the syllabus's own *Still To Add* list still carries its four blocked items, so the S112 outputs were reviewed in chat and never staged. L03 **v03.24.1 → v03.25.0** (moderate — a planned figure became live content, so the page renders differently and both §5b homes move) · `build_family_map` **v1.3.1 → v1.3.2** · `book_gates` **v1.42.1 → v1.42.2**.

**Versions:** L01 v03.19.1 · L02 v03.11.1 · L03 v03.25.0 · L04 v04.19.1 · L05 v04.19.1 · L06 v04.23.1 · L07 v04.21.1 · L08 v04.19.1 · L09 v05.16.1 · L10 v02.16.1 · L11 v02.17.1 · L12 v01.19.1 · L13 v02.17.1 · L14 v02.21.1 · L15 v02.17.1 · L16 v02.12.1 · going_deeper v01.4.1 — census **40,019** · Bible **v8.101** · BookComponentStandard v01.12.1 · gen_component v1.6.1 · Maker v2.45.2 · **book_gates v1.42.2** · lesson_inventory v1.2.0 · pill_sweep v1.1 · gate_payload_match v1.7 · build_family_map v1.3.2 · build_mark_index v1.0.2 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.15.1 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.2.1 · image_audit v1.1 · strip_inline v1.1 · build_worklist v1.1 · regex_audit v1.0 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · font_stack_sweep v1.2.0 · `ZUMO_Syllabus_WORKING.md` v1.0 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `b4d5559`.**
