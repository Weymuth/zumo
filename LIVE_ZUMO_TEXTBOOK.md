...
**Date:** August 5, 2026 (Session 118 — **THE SESSION OPENED ON A DEFECT NOBODY HAD QUEUED: S117's L12 CONVERSION WAS PUSHED TO THE REPO ROOT INSTEAD OF `lessons/`.** `Lesson_12.html` sat at the root at v01.20.0, linked by nothing and reachable at a public 200, while `lessons/Lesson_12.html` was still **v01.19.1** — the pre-conversion file, and the one the published site was serving. `css/book.css` had gone up correctly, so the stylesheet was at the S117 state and the lesson consuming it was not. **Seven gates failed and all seven named the same cause**; `session_versions --check` said it in one line, `written=v01.20.0 files=v01.19.1`. The fix was a file move, controlled both ways, **41/48 → 48/48**, needing no version bump and no LIVE.md edit because the content was already v01.20.0 and both documents already claimed it. **The part worth keeping is the paperwork:** LIVE.md recorded *"Verified by fresh clone at `b307865`"* — a commit that PREDATES the one carrying the work, so the verification named the wrong object. **A push is verified against the tree you intended to push, not against the last sha you wrote down.** **L13 IS THE THIRTEENTH CONVERSION, AND §25.10k IS NEW: OBJECTIVES THAT ARE NOUN PHRASES ARE REWORDED IN SECTION 2, NOT IN BC02.** L13 is the first lesson whose §2 objectives could not migrate character-exact into BC02's **I can…** group, because they are noun phrases — *"The lawnmower sweep — covering an unknown room with rows"* reads, under that heading, as *"I can… ☐ The lawnmower sweep."* **Measured before it was ruled, across all sixteen lessons: L13 is the only one**; L02–L12, L14 and L16 open verb-first under *"By the end of this lesson you will be able to:"*, L13 carries neither the verbs nor the lead-in, and L01/L15 carry no ☐ list at all. **DJ ruled A of three priced options** — reword §2 and migrate that, so §25.5 holds by construction. B (migrate the nouns and relabel the group) dies on §25.10j, which had just ruled that an *understand* claim belongs in BC03 rather than behind a checkbox; C (reword only inside BC02) dies on principle, because the two lists drift while both still render correctly. **The general rule is that BC02 is a COPY, and editing a copy to read better is how two documents stop agreeing.** Only the opening verb moved on each of the six; the equality is ASSERTED in the builder, and the assert must normalise the box glyph away because §2 ships `&#9744;` and BC02 must ship the literal ☐ **by design**. **L13 HAD NO ANCESTOR OF ANY KIND** — zero pre-§6 reveals of any type, no checklist, no Knowledge Check, no Reflection Questions, its §10 being a narrative *Inventory* plus a *Next:* pointer — so BC01 (5), BC03 (5) and BC04 (3) were all authored from nothing. BC01 is §-ordered §3 → §4.1 → §4.2 → §4.3 → §5.1, because **L13's §3 is FLAT with no subsections**, unlike L11's and L12's five §3.x; every citation was read and verified to contain its answer, and BC01 and BC03 are deliberately non-overlapping — BC01 asks why the clamp destroys silver, BC03 asks how you decide which channel a question belongs to. **THIS BUILD IS A CLASS RENAME, S115's SHAPE AND NOT S117's, AND THAT IS THE SESSION'S REAL FINDING.** `.ul-ls-none-2` and `.ul-ls-none-3` **swapped declaration sets** — same spelling, different meaning — because `build_css` ranks by usage count and BC02's list moved the counts. So **TEN lessons differ from the pushed clone, not one**, and which nine came along is a MEASUREMENT: L05–L12 and L14 changed by NAME ONLY, proved two ways rather than asserted — a line diff showing all 44 changed lines touch only those two class names, and an expansion through each tree's own stylesheet returning byte-identical for all nine. **A rename that changes no pixel still changes every file that spells it.** **THREE BASELINES MOVED, EACH CONTROLLED BOTH WAYS:** §21 coverage 240 → **245**, sole delta `BrainGear_Incomplete.png` 60 → 65; family map 1061 → **1065**, at an identical generator version with exactly ONE family moving, BRAIN CHECK 48 → 52 and the other 29 byte-identical; §27.11 digest to `ee0c8ac2039bc90e` with rules and declarations UNCHANGED at 643/2,357, zero born and zero died. After the moves a deleted `color: white;` FAILS, a broken image ref FAILS, `'13'` left in `BC_PENDING` FAILS **naming L13**, and a deleted callout FAILS naming **1064/1065**. **THE CONTROL MISFIRED TWICE AND BOTH ARE KEPT, BECAUSE THEY ARE DIFFERENT FAILURES AND NEITHER IS THE ONE S117 CANONIZED.** The first deleted a `<details data-reveal="quiz">`: the injection LANDED, the read-back confirmed it, and §24.14 stayed silent — because a reveal is not a callout the family map counts, which is S117's third attempt repeating verbatim. **A read-back proves something moved; it cannot prove the thing that moved is the thing the gate measures** — ask the generator what it counts. The second is new and nastier: the landed-assert `'brain-check-04' not in back` is **ALWAYS FALSE**, because the Brain Check column carries nav links to that anchor, so a perfectly correct injection reports as a failed one and the natural next move is to edit the control until it agrees with a wrong belief. **AN ASSERT THAT CANNOT SUCCEED IS THE MIRROR OF AN ASSERT THAT CANNOT FAIL, AND BOTH READ AS EVIDENCE.** `book_gates` **v1.44.4**, `build_family_map` **v1.3.6**. Applied S118: L13 **v02.18.0** (moderate — the page renders differently, so both §5b homes move) plus nine MINOR bumps carrying the class rename with every visible banner asserted unmoved. Census **40,780**. Bible **v8.107**.)
**Status:** ✅ **48/48 GATES PASS · L13 IS THE THIRTEENTH CONVERSION, AND ONE LESSON REMAINS (L15).** **§25.10k is new canon** per DJ ruling: when a lesson's §2 objectives do not fit BC02's group label, the objectives are what change — never the copy. **THE SESSION OPENED BY REPAIRING S117's PUSH**, which had put the converted L12 at the repo root while the site kept serving the pre-conversion file from `lessons/`. **L15 IS THE LAST CONVERSION AND IT CARRIES A WARNING:** the §2 survey shows **zero ☐ objectives** in L15, the same reading L01 gives, so §25.5's migrate step may have nothing to migrate — measure it before planning BC02, because L13 proved that is exactly where the surprise lives. **S116's past-tense question did not recur on L12 or L13 and L13 had no checklist at all**; L15 is its last chance to recur, and if it does not, retire it deliberately rather than carrying it a fifth session.

**Versions:** L01 v03.19.2 · L02 v03.11.2 · L03 v03.28.1 · L04 v04.19.2 · L05 v04.19.3 · L06 v04.23.3 · L07 v04.22.2 · L08 v04.19.3 · L09 v05.16.3 · L10 v02.17.1 · L11 v02.18.1 · L12 v01.20.1 · L13 v02.18.0 · L14 v02.22.1 · L15 v02.17.1 · L16 v02.12.1 · going_deeper v01.4.1 — census **40,780** · Bible **v8.107** · BookComponentStandard v01.12.1 · gen_component v1.6.1 · Maker v2.45.2 · **book_gates v1.44.4** · lesson_inventory v1.2.0 · pill_sweep v1.1 · gate_payload_match v1.7 · build_family_map v1.3.6 · build_mark_index v1.0.2 · gen_bonus_banner v1.4.1 · gen_part_banners v1.2 · session_versions v1.17.0 · fit_raster_svg v1.2 · flatten_alpha v1.2 · svg_layout_audit v1.20 · site_parity v1.1 · build_css v1.2.1 · image_audit v1.1 · strip_inline v1.1 · build_worklist v1.1 · regex_audit v1.0 · build_palette v1.1 · class_sweep v1.0 · color_index v1.0 · font_stack_sweep v1.2.0 · `ZUMO_Syllabus_WORKING.md` v1.2 · `images/marks/` **41** · `images/icons/` 49 incl. LICENSE. **Verified by fresh clone at `UNKNOWN`.**
