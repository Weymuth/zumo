...
EXPECTED = sorted(
    ...
    # S194: syllabus.html is canonical at the root. It is GENERATED by
    # build_syllabus_html.py from ZUMO_Syllabus_WORKING.md and carries INLINE STYLES on
    # purpose — DJ pastes it into Canvas, which strips <style> and class=. It therefore
    # links neither stylesheet and is outside §27.12 by the §25.6a rule that exempts the
    # tool pages. Declared here rather than discovered: a new delivery path is a new
    # unguarded path, and this gate is where the declaration lives.
    ['going_deeper.html', 'index.html', 'newproject.html', 'syllabus.html',
     'timer.html', 'tutor/tutor.html'])
...
#   thirty-two of them, which is exactly 4 blocks x 4 moves x 2 keywords. NO rule and
#   NO declaration moved; the whole textual diff is the usage census (S160's reason).
    ...
    for _p76, _lab76 in _RETIRED76:
        ...
        if _n76:
            bad.append('%s carries the RETIRED identity claim "%s" x%d - §16.25: the '
                       'board is the Zumo 32U4 OLED Main Board, the chip is the ATmega32U4, '
