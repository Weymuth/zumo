...
| `L10-12` | ✅ SHIPPED | S192 | Same claim family, closed in the same pass. L10 carried **four** sites, two of them in the reference apparatus students actually consult — the glossary *Closed Loop* entry and the Quick Reference row whose TRIM column read *never*, with the reason given as the P-controller already correcting bias. **The literal status glyph is deliberately NOT quoted here: it is the structural marker `census.worklist()` reads as REFUTED, and quoting it inside a row silently moved `fixed` by one. Caught immediately because the tally is derived now.**. A fifth bank question, **`L10_B21`**, was found only by an assertive-register sweep after the first four were fixed. **S193 — RESIDUE, AND THE ROW STAYS CLOSED (DJ ruled): `L10_B21`'s stem and correct option were fixed here at S192; the `why:` on its third DISTRACTOR still asserted the retired mechanism — *the stated mechanism is two controllers correcting the same thing at once* — describing something the book no longer says anywhere. `retired_claims` read CLEAN over it: the structural exemption covers a declared-wrong OPTION (a taught trap), but a `why:` asserts what the book states, and all four S192 spellings missed it because all four were keyed on the PROSE that sweep had just read — four readings of one corpus, not four readings of the claim (rules 83/84). Fixed, then a fifth spelling registered. No new row; the total stays 245.** |
...
| `L08-15` | ✅ SHIPPED | S193 | **The C1 family INVERTED, and the smallest row in the file: 2 sites, 0 Maker payloads, 0 bytes.** C1 called feed-forward a rival of the loop; this called a feed-forward rule a loop. C6's GOAL said *two proportional controllers running on the same error* — but `speed = BASE_SPEED − KS×|error|` is proportional to the STEERING loop's error and **nothing measures the robot's actual speed**, so no loop closes on it. The GOAL now names the asymmetry and, per DJ's S193 ruling, **poses the forward question without answering it** — *what would you have to add? You already own the sensor* (L06's encoders), **and names where the answer lives: Lesson 15.** The first draft of that sentence stopped at *you already own the sensor* and pointed nowhere. **The completeness sweep is what corrected it: L15 §5.8 BUILDS THE REAL SPEED LOOP** — `measureSpeedCmPerSec()` reads `averageCounts()` and `COUNTS_PER_CM`, compares against `TARGET_SPEED_CMS`, and adds `speedTrim` to `BASE_SPEED`, under a comment that says *You already own this instrument*. **So the book both refutes the old claim and answers the new question, and a triple check that had stopped at three agreeing arms would have shipped a question with no address.** **`L08_A17` needed restructuring, not rewording** — its stem asserted the defect while its correct option stated the truth (the `L08_B25` / `L11_B44` shape); it now asks why it is a rule and NOT a loop. **GPT's second half does NOT reproduce:** *Challenge 5 Adaptive Kp is gain scheduling, not PID* — measured, **zero** sites call it PID; all 5 L08 `PID` hits are the nav bar, §1's Real-World Connection and *Looking Ahead: The Full PID*. **`L15` GRAPHIC 15.2's _two controllers_ (P vs PD) is TRUE and untouched — a hit is not a defect (rule 34), and it is now the registry's standing negative control.** |
...
**Derived at S193 by `census.worklist()`, not carried forward — and this table is the reason to derive it.** Every figure below comes from enumerating Part 2's ID rows
...
> **THIS TABLE WAS FIVE CLOSURES STALE FOR A SESSION AND EVERY GATE PASSED OVER IT.** S192 closed
> `L08-08`, `L10-12`, `L11-08`, `L12-18` and `L15-08`, moved the figure in the prose that states it,
> and left the table that tabulates it reading 95 / 89 / 148. **Gate 81 asserts a digit beside a
> STATUS WORD; a table cell is a bare digit between pipes, so the gate could not see it.** It is
> regenerated from `census.worklist()` now, per lesson, with `closed + parked + open == findings`
> asserted on every row — but nothing yet stops it going stale again.
