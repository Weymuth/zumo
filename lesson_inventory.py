#!/usr/bin/env python3
# lesson_inventory.py v1.0 (S81) — exhaustive structural ENUMERATION of the lesson files.
#
# Usage:
...
BANNER = ('lesson_inventory.py v1.0 — ENUMERATION, NOT A VERDICT (Bible §24.6a).\n'
...
def summary(invs):
    w(f'{"L":<4}{"lines":>7}{"heads":>7}{"h3":>5}{"h4":>5}{"fence":>7}{"cons":>6}'
    ...
    for inv in invs:
        ...
        w(f'{inv["lesson"]:<4}{inv["lines"]:>7}{len(inv["headings"]):>7}{h3:>5}{h4:>5}'
          f'{len(inv["sections"]):>7}{len(inv["constructs"]):>6}{myst:>6}'
    ...
    w(f'{"TOTAL":<4}{sum(i["lines"] for i in invs):>7}'
      ...
      f'{sum(len(i["sections"]) for i in invs):>7}'
